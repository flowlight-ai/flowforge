...
import { defineAcpSnapshotSuite, type Scenario, type SnapshotSuiteOptions } from '@flowforge/acp-snapshot'
import { resolvePwshPath } from '@flowforge/pwsh-local'
import { decodeStorageRecord } from '@flowforge/session'

/**
 * The acp-agent example's snapshot suite: the scenario table for
 * `flowforge-acp-snapshot`'s suite factory, which owns every compare/guard mechanic
...
// The flowforge-acp-demo bin (the demo:acp entry), this example's cordis.yml, and
...
function snapshotModeFromEnv(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    case 'refresh':
      return 'refresh'
    default:
      throw new Error(`unknown FF_SNAPSHOT mode: ${value}`)
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  // `false` command exits 1, rather than becoming SANDBOX_UNAVAILABLE.
  {
    ...
    env: { FF_PERMISSION_MODE: 'read-only' },
    posixOnly: true,
  },
  ...
  // foreground error and background job marker without a platform runner.
  {
    ...
    configPath: PARTIAL_LANDLOCK_CONFIG,
    env: {
      FF_PERMISSION_MODE: 'read-only',
      FF_SNAPSHOT_MISSING_SANDBOX_RUNNER: '1',
    },
    posixOnly: true,
  },
  ...
  // (XXX(concurrent-subagents) in flowforge-llm-replay).
  ...
  // published-handle disposal failure.
  {
    ...
    env: { FF_SUBAGENT_PUBLISHED_FAILURE: '1' },
    hasModelTurn: true,
    recorded: false,
    overridden: true,
    configPath: SUBAGENT_DURABILITY_FAILURE_CONFIG,
  ...
  // no session-scoped UI picker on the automation protocol.
  {
    ...
    systemPromptSource: 'text-turn',
    toolSchemasSource: 'text-turn',
    env: { FF_PERMISSION_MODE: 'workspace-write' },
  },
  {
...
defineAcpSnapshotSuite({
  ...
  scenarios: SCENARIOS,
  mode: snapshotModeFromEnv(process.env.FF_SNAPSHOT),
  hasPwsh,
})
