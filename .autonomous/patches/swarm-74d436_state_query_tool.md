...
class StateQueryTool:
    """
    状态查询工具基类
    优先读取内存状态数据进行检索；无状态数据或检索结果为空时自动降级网页搜索
    子类必须配置以下属性并实现 _do_search 方法：
