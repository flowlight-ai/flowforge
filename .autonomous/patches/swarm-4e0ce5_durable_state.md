...
class DurableState(BaseModel):
    state_id: str = Field(default_factory=lambda: hashlib.sha1(f"{datetime.now(timezone.utc).isoformat()}{id(object())}".encode()).hexdigest(), description="记录唯一 ID")
