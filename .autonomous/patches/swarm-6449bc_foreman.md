...
class ForemanConfig:
    """Foreman 调度器配置"""
    loop_interval_seconds: float = field(default_factory=lambda: 10.0)
    emergency_poll_interval_seconds: float = field(default_factory=lambda: 1.0)
    max_concurrent_tasks: int = field(default_factory=lambda: 5)
    task_scan_limit: int = field(default_factory=lambda: 8)
