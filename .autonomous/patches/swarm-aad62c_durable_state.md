...
# 文件路径：flowforge/persistence/durable_state.py
...
class DurableState(BaseModel):
    state_id: str = Field(
        default_factory=lambda: f"state-{uuid4().hex[:16]}",
        description="记录唯一 ID",
    )
