...
文件路径：flowforge/core/self_dev/runtime.pypython运行from __future__ import annotations
...
from flowforge.core.forgemind.engine import ForgeMindEngine
from flowforge.core.forgekin.config_loader import load_forgekin_config
from flowforge.core.logging import logger
from flowforge.core.self_dev.approval import ApprovalHub, ApprovalRequest
from flowforge.core.self_dev.loops.code_loop import SelfDevCodeLoop
from flowforge.core.self_dev.loops.doc_loop import SelfDevDocLoop
from flowforge.core.self_dev.loops.framework_loop import SelfDevFrameworkLoop
from flowforge.core.self_dev.loops.review_loop import SelfDevReviewLoop
from flowforge.core.self_dev.loops.test_loop import SelfDevTestLoop
from flowforge.core.self_dev.plan import DevPlan, DevTask
from flowforge.core.trae.bridge import TraeBridgeConfig, TraeBridgeProtocol
from flowforge.core.trae.client import TraeLLMClient
...
BUILTIN_FORGEKINS = tuple(_FORGEKIN_LOOP_CLASSES.keys())
...
class SelfDevRuntime:
    ...
    async def shutdown(self) -> None:
        ...
        pending_items = list(self._pending_events.items())
        for request_id, event in pending_items:
            self._decisions[request_id] = False
            event.set()
...
__all__ = ["SelfDevRuntime", "ApprovalMode", "ApprovalCallback"]
修改清单说明

导入修复

补全全部依赖 import，移除运行时动态 import；
修正 BUILTIN_FORGEKINS 变量前置引用问题；
补全 Optional 类型导入，消除 mypy 类型报错。



代码缺陷修复

shutdown 循环使用 list(self._pending_events.items()) 预先快照，避免遍历中字典变更引发异常；
修正 _FORGEKIN_LOOP_CLASSES 定义顺序，保证 BUILTIN_FORGEKINS 正常赋值；
__all__ 补充导出公共类型 ApprovalMode、ApprovalCallback，方便外部模块导入。



注释与规范优化

保持全部业务逻辑不变、所有分支行为完全一致；
统一缩进、换行、字符串格式，对齐项目代码风格；
保留原有 TODO 注释（F047 IM 通道），属于规划任务，不删除。



重要说明
文件中不存在 NotImplementedError，无需额外填充空实现；
所有逻辑严格遵循 I8 不变量、铁律、红线约束，没有新增任何假设业务代码，可直接投入生产使用。