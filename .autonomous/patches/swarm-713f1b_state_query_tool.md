...
class StateQueryTool:
    """状态查询工具基类
    基于内存持久状态进行查询，当本地状态缺失/查询结果为空时自动降级到网页搜索兜底。
    子类必须实现 _do_search 方法；可按需重写 _is_empty_result 自定义空结果判定。
    子类需要定义类属性：
        state_key_template: str  # memory 读取key模板，支持 {entity_id} 占位符
        state_scope_field: Optional[str]
        state_merge_fields: List[str]  # 需要合并更新的字典字段
        state_list_fields: List[str]   # 需要扩展追加的列表字段
    """
    # 子类覆写这些类成员
            ...
            for entry_key, entry_data in raw_data.items():
                ...
                if self.state_scope_field and scope is not None:
                    try:
                        ...
                        # non-numeric keys are always included
                        pass
                entries_processed += 1
    ...
    async def _do_search(self, query: str, entity_id: str, scope: int, state_data: dict) -> ToolOutput:
        ...
        raise NotImplementedError(f"{self.__class__.__name__} must implement _do_search()")
