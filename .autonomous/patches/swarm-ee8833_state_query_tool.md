...
# 项目基础模型（工程中已存在，直接引用）
from flowforge.core.tool_model import ToolInput, ToolOutput
...
class StateQueryTool:
    """
    状态查询基础工具类
    支持从内存加载实体状态数据进行查询，无状态数据或查询为空时自动降级到网页搜索
    子类必须实现：state_key_template, state_scope_field, state_merge_fields, state_list_fields, _do_search
    """
    # 子类需要重写的属性
