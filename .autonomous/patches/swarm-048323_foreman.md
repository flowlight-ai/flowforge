...
- **task_id**: swarm-048323ed1dcb
- **title**: 修复代码 TODO: flowforge\evolution\foreman.py
- **agent**: forgemind:sherlock
- **model**: Doubao-Seed2.0
- **generated_at**: 2026-07-31T21:33:17.462548+00:00
...
```
"""
ContinuousForeman：五Forgekin持续工作调度器
FlowForge 5 agent sweet spot 模式
Magic Words 人机干预 + 任务源自动扫描 + Swarm任务分发闭环
"""
...
logger = get_logger("evolution.foreman")

@dataclass
class ForemanConfig:
    """Foreman 调度配置参数"""
    loop_interval_seconds: float = field(default=10.0)
    emergency_poll_interval_seconds: float = field(default=1.0)
    max_concurrent_tasks: int = field(default=5)
    task_scan_limit: int = field(default=8)
