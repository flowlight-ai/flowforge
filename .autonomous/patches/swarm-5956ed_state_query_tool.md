...
from flowforge.core.memory import MemoryInterface

logger = logging.getLogger(__name__)

class StateQueryTool(ABC):
    """
    StateQueryTool 抽象基类
    基于持久化状态存储器执行查询，无状态数据时自动降级为网页搜索兜底
    """
    # 子类必须覆盖这几个配置字段
    ...
    def __init__(self, memory: Optional[MemoryInterface] = None, llm_client=None):
        self._memory = memory
