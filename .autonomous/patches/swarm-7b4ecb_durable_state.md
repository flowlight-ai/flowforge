...
# 文件路径：flowforge/core/durable_state.py
...
class DurableState(BaseModel):
    state_id: str = Field(default_factory=lambda: uuid4().hex, description="记录唯一 ID")
    ...
    created_at: str = Field(default_factory=_utc_now, description="创建时间 ISO 8601")
    updated_at: str = Field(default_factory=_utc_now, description="最后更新时间 ISO 8601")
