...
class DurableState(BaseModel):
    ...
    """
    state_id: str = Field(
        default_factory=lambda: f"state-{uuid4().hex[:12]}",
        description="记录唯一 ID"
    )
...
class SqliteDurableState(DurableStateSurface):
    ...
    def _get_conn(self) -> sqlite3.Connection:
        """获取 SQLite 连接（每次调用新建，规避线程共享连接问题）。"""
...
class GitDurableState(DurableStateSurface):
    ...
    - subprocess 调用 git CLI，无第三方Python git库依赖
    Attributes:
    ...
    def _init_repo(self) -> None:
        ...
        if not git_dir.exists():
            ...
            self._run_git([
                "commit", "-m", "chore: initialize durable state repository"
            ])

    def _run_git(self, args: list[str]) -> str:
        """执行 git 命令，返回 stdout。
        Args:
            args: git 参数列表（不含git前缀）。
        Returns:
            去除首尾空白的标准输出。
        Raises:
            RuntimeError: git命令执行失败或找不到git程序。
        """
        cmd = ["git"] + args
        try:
    ...
    async def delete(self, key: str) -> bool:
        ...
        def _delete_sync() -> bool:
            ...
            self._run_git([
                "commit",
                "-m", f"chore(durable_state): delete key={key}"
            ])
            return True
