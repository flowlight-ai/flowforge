...
今日のコーディング・エージェント——**OpenCode、Codex CLI、Gemini CLI、Qoder、iFlow、WorkBuddy、Claude Code**——が得意なのはただ一つ。*1 つのセッション内でタスクを実行しきること* だ。しかし彼らは使い捨ての請負人だ。セッションが終われば忘れ、サーバーを再起動すれば白紙に戻り、モデルをアップグレードすれば苦労して蓄積した経験は消え去る。先月リリースした成果に責任を持つ者もいない。

FlowForge が答えるのは、より難しく、取り組む者が少ない問いだ。

> **エージェントはいかにして数ヶ月・複数のモデル世代をまたいで、永続的なアイデンティティを保ち、能力を蓄積し、検証可能性を維持し、ガバナンスのもとで進化し続けるのか？**

FlowForge はこれらのエージェントと競合しない——**雇用する。** 既存のコーディング・エージェントを能力拡張として束ね、職業的な **Forgekin** へと育て上げる**ハーネス・レイヤー**だ。Soul Imprint（永続的アイデンティティ）、成長の軌跡、そしてガバナンスを備えている。

| 次元 | 主流コーディング・エージェント（OpenCode · Codex · Gemini CLI · Qoder · iFlow · Claude Code） | FlowForge |
|------|--------------------------------------------------|-----------|
| **カテゴリ** | 使い捨てのタスク実行者（"優秀な手"） | 永続的アイデンティティを持つ Forgekin のためのハーネス・レイヤー |
| **アイデンティティ** | セッションごとに新しいインスタンス。"自分は誰か"がない | Soul Imprint + 形態の系譜。数ヶ月・モデル世代をまたいで存続 |
| **成長** | 能力はモデルの重みに固定。アップグレード = 交換 | 自己進化ループ：自らのドキュメント・コード・フレームワーク・レビュー・テストを自律進化 |
| **レビュー** | 同じモデルが自らの成果を承認 | クロスベンダーの独立レビュー——自らの成果を承認できるエージェントはいない |
| **自律性** | 全自動か人間参加かの二択 | 覚醒段階 6 レベルの漸進的自律 + 6 層ガードレール + マジックワード緊急停止 |
| **記憶** | セッション・コンテキスト / 平坦なメモリファイル | EchoStore（エピソード記憶）× MindCodex（蒸留知識）のドメイン横断連邦 |
| **形態** | 1 つの抽象的エージェント / 単一 CLI | 5 つの進化可能な形態——生物 / 組織 / 物品 / 仮想 / ハイブリッド |

> **あなたがすでに使っているコーディング・エージェントは労働力だ。FlowForge は彼らに名前と記憶、そしてマネージャーを与える。**
