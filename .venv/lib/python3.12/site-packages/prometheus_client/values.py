...
def MultiProcessValue(process_identifier=os.getpid):
    ...
    class MmapedValue:
        ...
        def set_exemplar(self, exemplar):
            # Exemplars are held in-memory per value object. The mmap file
            # format only stores (value, timestamp), so exemplars are not
            # shared across processes; each process observes its own exemplar.
            with lock:
                self.__check_for_pid_change()
                self._exemplar = exemplar
