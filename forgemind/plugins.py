"""ForgeMind 应用层 Plugin — 注册万物灵智体到 FlowForge 核心。

:class:`ForgeMindPlugin` 实现 Plugin V3 协议的四个 v7.0 育灵体系钩子:

    - :meth:`register_forgekins`           — 注册通用灵智体模板
    - :meth:`register_forge_skills`        — 注册通用锻造技能
    - :meth:`register_council_channels`    — 注册灵议渠道
...
_DEFAULT_FORGEKIN_TEMPLATES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "requirement": "智能灯具，节能+用户舒适愿景，与家电灵智体组队",
        "iot_protocol": "matter",
    ...
    },
    {
        ...
        "requirement": "组织灵智体，业务决策+组织健康+合规治理",
...
class ForgeMindPlugin(FlowForgePlugin):
    ...
    通过 Plugin V3 协议四钩子注册万物灵智体相关能力，单向依赖核心层
    （编程红线第 10 / 12 条）。

    注册内容:
        - 通用灵智体模板（孙悟空 / 家猫橘子 / 客厅吊灯 / 某科技公司）
    ...
    manifest = PluginManifest(
        name="forgemind",
        version="0.1.0",
        description="万物灵智体应用层 — 锻造万事万物灵智（v7.0 育灵体系）",
        author="FlowForge Team",
        # V3 ForgeMind / Forgekin fields
        forgekin_species="virtual",  # 默认形态: 虚拟灵智体
        ...
        council_dir="forgemind/council",   # Phase 6 灵议
        auto_forge_dir="forgemind/config", # 自我进化配置
    )

    def __init__(self) -> None:
        super().__init__()
        # 育灵流水线实例（延迟初始化，避免在 import 时加载 YAML）
    ...
    # ── Plugin V3 四钩子实现（v7.0 育灵体系）──────────────────────

    def register_forgekins(self, forgekin_registry: Any) -> None:
        """注册通用灵智体模板（猫 / 公司 / 桌椅 / 孙悟空等）。

        forgemind 应用层养"通用灵智体"——区别于 *Forge 的"垂直业务灵智体"。
        本方法将预定义的灵智体模板注册到灵智体注册表，供 operator 通过
    ...
    def register_forge_skills(self, skill_registry: Any) -> None:
        ...
        forge_skills = [
            ...
            },
            {
                ...
                "skill_type": "native",
                "description": "锻造新灵智体（仅 E6 灵智阶可用，达成 operator 养万物愿景）",
        ...
        for skill in forge_skills:
            skill_registry.register(
                name=skill["name"],
                forgekin_id="forgemind:*",  # 通用技能，所有 forgemind 灵智体可加载
    ...
    def register_council_channels(self, council_registry: Any) -> None:
        ...
        灵议是多灵智体议事机制，用于解决跨灵智体冲突、复杂决策、愿景方向
        校准。本方法注册 forgemind 应用层的默认灵议渠道。
        ...
        council_channels = [
            {
                ...
                "description": "万物灵智体愿景对齐灵议——校准 VISION §7 七条锚点",
            ...
            },
            {
                ...
                "description": "跨灵族协作灵议——BioForgekin 与 OrgForgekin 等",
                "participants": ["forgemind:template:*"],
                "readonly_paths": [],
            },
        ]
    ...
    def register_auto_forge_config(self, auto_forge_engine: Any) -> None:
        ...
        新锻造灵智体默认仅启用 Mode A（范围守卫），Mode B/C 需觉醒阶 ≥ E4
        后由 operator 显式授权。
    ...
    async def forge_from_template(
        ...
        if template is None:
            ...
            raise KeyError(
                f"未找到灵智体模板: {template_name!r}（可用模板: {available}）"
            )
