...
The public package family belongs to the `@flowforge` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@flowforge/node-addon-landlock-run
@flowforge/node-addon-landlock-run-<platform>
```
