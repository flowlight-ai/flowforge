...
INVALID_RESPONSE_PATTERNS = (
    ...
    "请输入你需要处理的问题",
    # 2026-08-12修复: qianwen-web UI界面文本静默失败(HTTP 200但返回页面提示而非LLM生成内容)
    # 实测: topic选题时Qwen3.6-Plus返回"已按照要求输出文章，你可以基于当前结果进一步编辑润色～无标题创建于 08-12 12:43改为对话直接回答"(53字符)
    # 若不拦截会当作正常选题内容,导致 topic_strategist 解析失败返回空选题列表
    "已按照要求输出文章",
    "无标题创建于",
    "改为对话直接回答",
)
