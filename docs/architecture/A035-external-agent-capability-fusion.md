...
> **对应 arch.md**: [doc:../arch.md#§3.10] + [doc:../arch.md#§3.14]（灵锻 SpiritForge + 灵议 Mind Council）
...
ExternalAgentAdapter 抽象层（A031）需要实现"用完即走 -> 用完即学"的能力增长机制，让灵智体（Forgekin）多次调用三方 Agent 后"学到"能力。但 v7.0 无三方 Agent 能力融合机制，与 clowder-ai 最大差距是 clowder-ai 的猫会从调用工具中学习，v7.0 的 Forgekin 不会。本架构在 `core/external_agent/capability_fusion.py` 建立三方 Agent 能力融合层，解决以下架构层问题：
...
4. **Eval Ledger 净增益未硬门**：合入锻典前无前后测对比净增益 > 0 硬门，低质量条目可入典。
5. **能力画像融合缺失**：成熟度 L3+ 锻典条目未合入 F001 CapabilityProfile 作为 SkillPackage。
6. **锻典（Mind Codex）未对接**：F039 灵典可检索知识库无三方 Agent 调用经验条目来源。
7. **Knowledge Object Contract 字段未校验**：CL-005 Knowledge Object Contract 字段完整性未校验，蒸馏候选字段可能缺失。

### 1.2 架构约束

- **单向依赖约束**：CapabilityDistiller 必须单向依赖 F001 CapabilityProfile + F014 Memory Collection + F018 Eval Contract + F032 能力画像 + F033 共享状态 + F039 灵典，禁止反向依赖 *Forge。
- **DI 容器约束**：FusionSourceCollector / CapabilityDistiller / CapabilityFusionApplier 实例必须通过 DI 容器注入到 ExternalAgentBridge。
- **Repository 层约束**：FusionSource / CapabilityDistillationCandidate / 锻典条目写入必须通过 Repository 层，禁止直接操作数据库。
...
- **CL-004 Eval Ledger 约束**：合入锻典前必须前后测对比，净增益 > 0 才允许合入。
...
- **对 F001 能力画像的影响**：成熟度 L3+ 锻典条目作为 SkillPackage 合入 CapabilityProfile。
...
- **决策 3：CL-003 五级成熟度阶梯严格不可跳级**
  L0 Episode（单次调用） -> L1 Pattern（3+ 相似） -> L2 Draft（灵锻主动抽象） -> L3 Validated（Eval A/B 验证） -> L4 Standard（operator 批准）。每级晋升必须满足前级条件，禁止跳级。L4 必须 operator 显式批准（require_operator_approval_for_L4=true）。

- **决策 4：CL-004 Eval Ledger 净增益硬门**
  合入锻典前必须前后测对比：使用候选能力前后的任务完成质量分对比，净增益 > 0 才允许合入。这避免低质量能力条目污染锻典。

- **决策 5：CL-005 Knowledge Object Contract 字段完整性校验**
  CapabilityDistillationCandidate 必须含 trigger_pattern（何时使用）/ procedure（怎么用）/ precondition（前置条件）/ postcondition（预期效果）/ anti_pattern（反模式）/ provenance（来源 Episode ID）六字段。缺一即校验失败，禁止提交到锻典。

- **决策 6：L3+ 锻典条目作为 SkillPackage 合入 F001 CapabilityProfile**
  成熟度 L3 Validated 及以上的锻典条目作为 SkillPackage 合入 F001 CapabilityProfile。下次任务可路由到此灵智体（无需调用三方 Agent）。这实现"用完即学"闭环。

- **决策 7：与灵锻（SpiritForge）联动蒸馏**
  CapabilityDistiller.distill() 与灵锻（SpiritForge，F014 + arch.md §3.14）联动，由灵锻主动抽象出能力候选。灵锻作为蒸馏引擎，能力融合层提供原料与目标画像。
...
- CL-004 Eval Ledger 净增益 > 0 才允许合入锻典，净增益 <= 0 时合入被拒绝。
- CL-005 Knowledge Object Contract 六字段必须完整，缺一即校验失败。
- 成熟度 L3+ 锻典条目必须作为 SkillPackage 合入 F001 CapabilityProfile。
- FusionSource / CapabilityDistillationCandidate / 锻典条目写入必须通过 Repository 层。
...
| CapabilityDistiller | `core/external_agent/capability_fusion.py` | 能力蒸馏器（与灵锻联动） |
...
class CapabilityDistiller(ABC):
    ...
    async def distill(
        ...
        """
        蒸馏能力候选：
        1. 灵锻主动抽象出 distilled_capability
...
[合入能力画像阶段（F001 CapabilityProfile）]
    CapabilityFusionApplier.apply_to_profile(forgekin_id, codex_entry_id)
        |
        v
    将 L3+ 锻典条目作为 SkillPackage 合入 CapabilityProfile
        |
        v
    下次任务可路由到此灵智体（无需调用三方 Agent）
        `--> 实现"用完即学"闭环

[失败经验蒸馏阶段（与 F034 联动）]
    F034 FallbackExecutionRecord 写入 F014 灵忆
        |
        v
    作为 FusionSource 的补充原料
        `--> 灵锻可蒸馏出"何时不应调用某厂商"的反模式知识
```
...
- **依赖 F001 CapabilityProfile**：成熟度 L3+ 锻典条目合入目标画像。
- **依赖 F014 Memory Collection**：FusionSource + FallbackExecutionRecord 写入灵忆作为蒸馏原料。
...
- **依赖 F039 Mind Codex（灵典可检索知识库）**：锻典条目提交目标。
- **依赖灵锻 SpiritForge（arch.md §3.14）**：蒸馏引擎。
- **依赖 core/interfaces**：Repository / DI 容器抽象。

### 4.2 下游影响

- **影响 A031 ExternalAgentBridge**：Bridge 在调用成功后调用 FusionSourceCollector.collect() 采集融合来源。
- **影响 F001 CapabilityProfile**：L3+ 锻典条目作为 SkillPackage 合入，灵智体能力画像动态增长。
- **影响 F039 灵典可检索知识库**：锻典条目作为灵典知识库条目来源。
- **影响灵锻 SpiritForge**：能力融合层提供蒸馏原料与目标画像。
...
- CL-004 Eval Ledger 净增益 > 0 才允许合入锻典。
- CL-005 Knowledge Object Contract 六字段必须完整，缺一即校验失败。
- 成熟度 L3+ 锻典条目必须作为 SkillPackage 合入 F001 CapabilityProfile。
- FusionSource / CapabilityDistillationCandidate / 锻典条目写入必须通过 Repository 层。
...
- [ ] AC-3: Repository 层通过 —— FusionSource / CapabilityDistillationCandidate / 锻典条目通过 Repository 写入，无直接数据库操作。
...
- [ ] AC-11: CL-005 字段完整性不变量通过 —— 缺任一字段的 CapabilityDistillationCandidate 提交到锻典被拒绝。
- [ ] AC-12: L3+ 合入不变量通过 —— 仅 L3_VALIDATED / L4_STANDARD 锻典条目可合入 F001 CapabilityProfile。
- [ ] AC-13: 用完即学闭环不变量通过 —— L3+ 锻典条目合入后，下次类似任务灵智体可自主完成（不调三方 Agent）。
...
- [doc:../design/naming-contract.md]（灵锻 SpiritForge + 锻典 Mind Codex + 能力画像 Capability Profile）
...
| 日期 | 版本 | 变更 | 变更者 |
|------|:----:|------|--------|
| 2026-07-19 | v0.1 | 初始创建（FusionSource 采集 + 相似聚类 + CL-003 五级阶梯 + CL-004 Eval Ledger + CL-005 Knowledge Object Contract + L3+ 合入 CapabilityProfile 架构） | 架构师灵智体（猫头鹰·鲁班） |
