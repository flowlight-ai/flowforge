...
本 Feature 在架构层解决以下问题：FlowForge 三层架构中，forgemind 作为 Layer 2 应用层承载通用灵智体（Forgekin，社区社交称"灵智体"），*Forge 作为 Layer 3 垂直业务层承载垂直领域灵智体，二者之间的**边界划分**与**双向流通协作模式**如何工程化落地。

具体子问题：
- **边界划分**：forgemind 与 *Forge 各自承载什么类型的灵智体? 通用能力与垂直能力的分界线在哪?
- **协作协议**：*Forge 通过什么协议向 forgemind 注册垂直灵智体? forgemind 通用灵智体如何"进化"为 *Forge 垂直灵智体? 垂直灵智体如何"回炉"沉淀为通用能力?
...
- **DI 容器约束**：*Forge 注册的灵智体必须通过 DI 容器管理，禁止绕过 DI 容器直接实例化（编程红线第 12 条）
...
- **Plugin V3 协议约束**：*Forge 必须通过 Plugin V3 四钩子注册灵智体到 forgemind，禁止绕过 Plugin 协议直接实例化（铁律 §0.5）
- **ForgekinEngine 装饰器约束**：灵智体执行入口必须是 ForgekinEngine 装饰 HybridExecutor + HarnessOrchestrator，禁止绕过 Harness 护栏（arch.md §2.2 决策 2）
...
- **对 *Forge 垂直业务层（Layer 3）的影响**：每个 *Forge 项目需在 `plugins.py` 实现 Plugin V3 四钩子，注册垂直灵智体到 forgemind
- **对 F028 ForgePipeline 锻造流水线的影响**：流水线新增"通用→垂直"分支与"垂直→通用"回炉分支
- **对 F037 灵智体市场的影响**：市场上架条目新增 `layer` 字段区分通用 / 垂直承载层
...
│  │  ├─ register_forgekins()        接收 *Forge 注册的垂直灵智体      │  │
│  │  ├─ register_forge_skills()      接收 *Forge 注册的垂直技能包      │  │
│  │  ├─ register_council_channels() 接收 *Forge 注册的灵议通道        │  │
│  │  └─ register_auto_forge_config() 接收 *Forge 注册的灵锻配置       │  │
│  └────────────────────────┬─────────────────────────────────────────┘  │
│                           │                                            │
│  ┌────────────────────────▼─────────────────────────────────────────┐  │
...
│                                                                        │
│  ┌──────────────────────────────────────────────────────────────────┐  │
│  │ 通用灵智体承载：forgekins/*.yaml（鲁班/夏洛克/梵高/平头哥/文心） │  │
│  └──────────────────────────────────────────────────────────────────┘  │
└────────────────────────────────┬───────────────────────────────────────┘
                                 │ 单向依赖（forgemind → 核心）
                                 ▼
...
- **决策 1：forgemind 定位为 Layer 2 应用层（通用灵智体承载）**
  - 理由：operator 第 8/9 条指令明确"flowforge 中需要新增一个 forgemind 模块，其是 flowforge 的应用层项目"。forgemind 承载跨领域复用的通用灵智体（如架构师猫头鹰·鲁班、开发者猎犬·夏洛克），避免在核心框架层污染业务语义
  - 替代方案：将通用灵智体放在核心框架层 → 违反编程红线第 10 条"禁止在 flowforge 中写死业务领域代码"
...
- **决策 3：跨层迁移（进化 / 回炉）必须 operator 批准**
  - 理由：跨层迁移涉及能力画像复制 / 蒸馏，存在能力丢失或污染风险。operator（CVO）是愿景锚点的最终裁决者，拉闸权不可被灵智体代理（arch.md §2.6 不变量 4）
  - 替代方案：自动迁移 → 与 VISION.md §7 operator 拉闸权锚点冲突
- **决策 4：回炉仅蒸馏通用能力，垂直能力保留原层**
  - 理由：垂直能力（如 contentforge 的 seo_writing）是 *Forge 的领域资产，回炉到通用层会污染 forgemind 的通用性。仅蒸馏可跨领域复用的通用能力（如写作风格、协作习惯）
  - 替代方案：全量回炉 → forgemind 沦为 *Forge 的能力聚合层，违反"通用 vs 垂直"分界
- **决策 5：跨层迁移记录写入 F038 进化谱系**
  - 理由：迁移是灵智体生命周期的重要事件，必须可追溯。F038 ForgekinLineage 已定义 `LAYER_TRANSITION` 关系类型，本 Feature 复用其谱系存储
  - 替代方案：独立存储迁移记录 → 谱系断裂，无法回答"这个垂直灵智体的祖先是哪个通用灵智体"

### 2.3 架构不变量

- forgemind（Layer 2）必须单向依赖 FlowForge 核心框架层（Layer 1），绝对禁止反向调用
- forgemind 必须不含任何 *Forge 垂直业务领域代码（编程红线第 10 条）
- *Forge 必须通过 Plugin V3 四钩子注册灵智体到 forgemind，禁止绕过 Plugin 协议直接实例化（编程红线第 12 条）
- 通用灵智体进化为垂直灵智体必须满足 Eval ≥ 0.85 + 5+ 任务前置条件
- 跨层迁移（进化 / 回炉）必须 operator 显式批准，禁止灵智体自动迁移
...
- **ForgeMindPlugin（`flowforge/forgemind/plugins.py`）**：Plugin V3 四钩子入口，接收 *Forge 注册的垂直灵智体 / 技能 / 议事通道 / 灵锻配置
- **ForgeRelationshipManager（`flowforge/forgemind/relationship/manager.py`）**：关系管理器，维护灵智体与承载层的当前关系、原始层、迁移历史
...
- **ReclaimExecutor（`flowforge/forgemind/relationship/reclaim.py`）**：垂直→通用回炉执行器，调用灵锻 SpiritForge 蒸馏通用能力到 forgemind 层
...
class ForgeLayer(str, Enum):
    """灵智体承载层（AI 术语优先：Forge Layer）"""
    FORGEMIND = "forgemind"          # 通用灵智体承载层（Layer 2）
    CONTENTFORGE = "contentforge"    # 内容创作垂直灵智体承载层（Layer 3）
    DEVFORGE = "devforge"            # 开发垂直灵智体承载层（Layer 3）
    NOVELFORGE = "novelforge"        # 小说创作垂直灵智体承载层（Layer 3）
    MALLFORGE = "mallforge"          # 电商运营垂直灵智体承载层（Layer 3)
...
进化流（通用 → 垂直）:
  ...
  │  任务>=5)      │
  └────────┬───────┘
           │ 1. request_evolve_to_vertical(target=contentforge)
           ▼
  ┌────────────────────────────────────────────┐
...
- **F037 灵智体市场**：MarketplaceListing 新增 `layer: ForgeLayer` 字段，市场查询支持按承载层过滤（通用 / 垂直）
- **F038 进化谱系**：LineageRelation 新增 `LAYER_TRANSITION` 关系类型，所有跨层迁移写入谱系边
- **F039 灵典可检索知识库**：回炉蒸馏产出的通用 SkillPackage 写入 Mind Codex，可被其他灵智体检索消费
- **F027 多形态智能体形态分类**：跨层迁移不改形态（BioForgekin 迁移后仍是 BioForgekin），但形态进化与层迁移可串联
- **所有 *Forge 项目（ContentForge / DevForge / NovelForge / MallForge）**：每个 *Forge 的 `plugins.py` 必须实现 Plugin V3 四钩子，注册垂直灵智体到 forgemind

### 4.3 跨模块不变量

- 通用灵智体进化为垂直灵智体前必须通过 F018 Eval Contract 校验 Eval ≥ 0.85
- 跨层迁移必须同时写入 ForgeRelationshipRepository 与 F038 LineageStore，二者保持一致
- 回炉蒸馏产出的 SkillPackage 必须通过 F039 Mind Codex 的 CL-005 七字段契约校验才能入库
- *Forge 注册的垂直灵智体必须继承 ForgekinBase，实现 observe / act / verify 三方法契约
- 通用能力沉淀必须通过灵锻 SpiritForge 蒸馏，禁止直接复制原始经验日志
...
- [ ] AC-6: ForgeLayer 枚举完整——包含 forgemind + 4 个 *Forge（contentforge / devforge / novelforge / mallforge）
