...
forgemind 应用层需要为 BIO/ORG/OBJ/HYBRID 形态灵智体（Forgekin）提供物理世界感知通道，对标业界 Embodied AI（具身智能）工程实现路径。但 v7.0 仅有数字工具（web_search/file_rw/git 等），无物理传感器适配层。本架构在 forgemind 内部建立物理 AI 传感器接入层，解决以下架构层问题：

1. **物理感知通道缺失**：摄像头/麦克风/IoT 传感器/可穿戴设备四类物理感知设备无统一适配抽象。
2. **形态门控未编码**：仅 BIO/OBJ/HYBRID 形态灵智体可绑定物理传感器，VIRTUAL 形态应被拒绝，但 v7.0 无门控机制。
3. **传感器事件未对接灵忆**：传感器采集的事件无统一写入 EchoStore（灵忆）的入口，物理感知无法成为灵智体经验记忆。
4. **采样率适配缺失**：高采样率（摄像头 30Hz）与低采样率（温度 0.1Hz）传感器无差异化的入灵忆策略。
5. **设备故障降级缺失**：传感器离线时灵智体无降级机制，可能阻塞决策回路。
...
- **对 F023 liveness 规范读模型的影响**：传感器离线时灵智体进入 liveness degraded 状态，不阻塞决策回路。
...
- **决策 2：形态门控由 SpeciesRegistry 集中校验**
  SensorRegistry.bind() 调用 SpeciesRegistry.assert_sensor_allowed(species, channel) 校验形态合法性。VIRTUAL 形态灵智体绑定物理传感器被拒绝。这避免形态门控逻辑分散，保证与 F027 一致。

- **决策 3：传感器事件统一写入 F014 EchoStore 情景记忆**
  PhysicalEventIngestor.ingest(event) 将 SensorEvent 转换为 EchoStore 条目，成为灵智体经验记忆一部分。灵智体通过 ForgekinBase.observe() 读取 EchoStore 中的近期传感器事件作为 Observation。

- **决策 4：采样率差异化适配（特征提取 vs 直存）**
  高采样率传感器（如摄像头 30Hz）做特征提取后入灵忆（避免灵忆爆炸）；低采样率传感器（如温度 0.1Hz）直接入灵忆。采样率适配策略 YAML 外置，避免 .py 硬编码。

- **决策 5：设备故障触发 F023 liveness degraded 状态**
  传感器离线时灵智体进入 liveness degraded 状态，observe() 返回最近一次有效快照，不阻塞决策回路。这避免单点传感器故障导致灵智体完全停摆。
...
- 高采样率传感器必须做特征提取后入灵忆，禁止原始数据直接写入 EchoStore（避免灵忆爆炸）。
...
| SensorBinding | `forgemind/sensors/base.py` | 传感器绑定数据模型（灵智体 <-> 物理设备） |
...
- **依赖 F023 liveness 规范读模型**：传感器故障时灵智体进入 degraded 状态。
...
- **影响 F030 虚拟世界设定层**：HYBRID 形态灵智体同时接入物理传感器（F029）与虚拟世界设定（F030），二者通过灵智体决策回路融合。
...
- SensorEvent 必须通过 PhysicalEventIngestor 写入 F014 EchoStore，禁止直接操作数据库。
- 高采样率传感器（>= 1Hz）必须做特征提取后入灵忆，禁止原始数据直存。
- 传感器离线时灵智体必须进入 F023 liveness degraded 状态，禁止阻塞决策回路。
...
- [ ] AC-5: 形态门控通过 —— VIRTUAL 形态灵智体绑定物理传感器被拒绝。

### 5.2 架构不变量验收

- [ ] AC-6: 8 通道枚举不变量通过 —— SensorChannel 仅含 8 通道，运行时无法新增。
- [ ] AC-7: 采样率适配不变量通过 —— 摄像头（30Hz）数据经特征提取后入灵忆，温度（0.1Hz）数据直存。
- [ ] AC-8: 故障降级不变量通过 —— 传感器离线后灵智体进入 liveness degraded 状态，observe() 返回最近有效快照。
...
| 日期 | 版本 | 变更 | 变更者 |
|------|:----:|------|--------|
| 2026-07-19 | v0.1 | 初始创建（8 通道 + 4 类 Adapter + 形态门控 + Tier 0 保护架构） | 架构师灵智体（猫头鹰·鲁班） |
