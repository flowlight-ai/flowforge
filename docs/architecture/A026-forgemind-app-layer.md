...
1. **多形态智能体（Multi-Form Agent）实践场缺失**：BIO/ORG/OBJ/VIRTUAL/HYBRID 五种形态灵智体（Forgekin）无统一承载目录，散落在 *Forge 业务项目中。
2. **FlowForge 自我演进练兵场缺失**：FlowForge 无法"自己开发自己"，缺少一个承载育灵（Forge Nurturing）全流程代码的应用层。
3. **通用灵智体与垂直业务灵智体边界模糊**：未明确 forgemind（养通用灵智体）与 *Forge（养垂直业务灵智体）的架构边界，存在反向依赖风险。
...
- **Repository 层约束**：forgemind 的灵印（MindImprint）、灵忆（EchoStore）写入必须通过 Repository 层，禁止 `cursor.execute()` 直接操作数据库（架构红线第 4 条）。
- **配置驱动约束**：5 种形态定义、锻造流水线阶段、预置灵智体描述必须 YAML 外置到 `forgemind/config/*.yaml`，禁止 .py 文件硬编码（架构红线第 5 条 + P16）。
...
- **对核心框架层的影响**：要求核心框架层暴露 Plugin V3 四钩子注册点（`register_forgekins / register_forge_skills / register_council_channels / register_auto_forge_config`），并保证 ForgekinEngine 可加载 forgemind 注册的灵智体。
...
- **对持久状态层的影响**：forgemind 的灵印、形态进化记录、锻造清单必须写入 F008 Durable State Surfaces，作为 Forgekin 不可变身份锚点。
...
- **决策 1：forgemind 作为 flowforge 子目录而非独立项目**
  operator 指示（2026-07-17）明确 forgemind 是 FlowForge 应用层，承载多形态智能体育灵代码。作为子目录可使 FlowForge 自我演进直接复用核心框架层能力，避免通用灵智体与 *Forge 垂直业务混淆。未选"独立项目"是因为会失去 FlowForge 自我演进滋养；未选"分散到 *Forge"是因为违反"通用 vs 垂直"边界。

- **决策 2：双轨命名策略（产品层 ForgeMind / 代码层 Forgekin）**
  产品层对外宣称"灵智（ForgeMind）"作为体系名用于社区社交沟通，代码层与对外技术文档统一使用 AI 业界术语 Forgekin / ForgeMind / CapabilityProfile 等。这与 9 大点名称修订第 5 条"AI 术语优先"一致，避免"万物"虚幻用语，使用"多形态智能体（Multi-Form Agent）"。

- **决策 3：ForgekinBase 三方法契约（observe/act/verify）对应 Harness 七层**
  observe 对应 Evidence & Sensors (L3)；act 对应 Tool Mediation (L2)；verify 对应 Governance Boundary (L4)。三方法契约使任何形态灵智体必须建立现实闭环，避免"有 persona 无闭环"的虚幻智能体。
...
- **决策 5：forgemind 不含业务领域代码**
  forgemind 仅承载通用灵智体抽象 + 5 种形态灵智体 + 锻造流水线 + 传感器/世界设定层抽象。垂直业务灵智体（如 ContentForge 的选题灵智体）必须放到 *Forge 子项目并通过 Plugin V3 注册。这避免 FlowForge 反向依赖业务模块（架构红线第 12 条 + 编程红线第 10 条）。
...
- forgemind 的灵印（MindImprint）必须通过 Repository 层写入 F008 Durable State Surfaces，禁止直接操作数据库。
- forgemind 必须不含任何垂直业务领域代码（内容创作 / 小说 / 电商 / 开发等），违反即编程红线第 10 条。
- forgemind 灵智体必须建立现实闭环（observe -> act -> verify），缺一即架构契约违反。
...
| ForgekinBase | `forgemind/base.py` | 灵智体抽象基类（observe/act/verify 三方法契约） | -> core/interfaces |
...
| ForgekinEngine | `forgemind/engine.py` | 灵智体执行引擎宿主（与 HarnessOrchestrator 对接） | -> core/harness |
| ForgekinMarketplace | `forgemind/marketplace/` | 灵智体市场（注册/查询/上架） | -> F037 |
...
| MindCodex | `forgemind/codex/` | 灵典（Mind Codex）可检索知识库 | -> F039 |
| MindCouncil | `forgemind/council/` | 灵议（Mind Council）多 Forgekin 协作 | -> F002 |
| ForgekinsConfig | `forgemind/forgekins/*.yaml` | 5 个预置灵智体 YAML 配置 | -> core/config |
...
class ForgekinSpecies(str, Enum):
    ...
    BIO = "bio"               # BioForgekin 生物灵智体（对应 Embodied AI 路径）
    ORG = "org"               # OrgForgekin 组织灵智体
    OBJ = "obj"               # ObjForgekin 物品灵智体（对应 Embodied AI 路径）
    VIRTUAL = "virtual"       # VirtualForgekin 虚拟灵智体（对应 Character AI 路径）
...
class MindImprint(BaseModel):
    ...
    imprint_id: str
    soul_imprint_hash: str         # 灵印哈希（不可变）
...
```
[1] operator 提交育灵请求（YAML ForgingManifest）
            |
            v
...
[8] ForgekinLineage.append(record) -> 写入进化谱系（F038）
            |
            v
[9] ForgekinMarketplace.publish(forgekin) -> 上架到灵智体市场（F037）
```
...
- **依赖 F008 Durable State Surfaces**：灵印、形态进化记录、锻造清单写入持久状态层。
- **依赖 F014 Memory Collection**：育灵第 ③ 步记忆初始化 + 灵议过程中产生的灵忆条目。
...
- **影响 F036 forgemind 与 *Forge 关系**：forgemind 明确"通用灵智体 vs 垂直业务灵智体"边界，F036 进一步约束 *Forge 通过 Plugin V3 复用 forgemind 抽象。
- **影响 F037 灵智体市场 + F038 进化谱系**：forgemind 提供市场发布接口与谱系追踪接口。
- **影响 F039 灵典可检索知识库**：forgemind 提供 codex/ 目录承载灵典（Mind Codex）。

### 4.3 跨模块不变量

- forgemind 必须在 FlowForge 启动时通过 Plugin V3 完成四钩子注册，未注册时 ForgekinEngine 拒绝加载任何 Forgekin。
- forgemind 写入的灵印哈希（soul_imprint_hash）必须全局唯一且不可变，违反即身份漂移。
...
- forgemind 注册的预置灵智体（猫头鹰·鲁班 / 猎犬·夏洛克 / 孔雀·梵高 / 蜜獾·平头哥 / 钢笔·文心）必须 YAML 外置到 `forgemind/forgekins/*.yaml`，禁止 .py 硬编码。
...
- [ ] AC-3: Repository 层通过 —— 灵印/灵忆/进化谱系写入均通过 Repository 抽象，无 `cursor.execute()` 调用。
- [ ] AC-4: 配置驱动通过 —— 5 形态定义、6 步锻造清单、5 预置灵智体均 YAML 外置到 `forgemind/config/*.yaml`。
...
- [ ] AC-9: 灵印不可变不变量通过 —— 同一 forgekin_id 的 soul_imprint_hash 在整个生命周期内不变。
- [ ] AC-10: 现实闭环不变量通过 —— 5 种形态灵智体均实现 observe/act/verify 三方法且可端到端调用。
