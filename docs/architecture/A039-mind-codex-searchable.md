...
本 Feature 在架构层解决以下问题：forgemind 应用层需要一个灵典可检索知识库（Mind Codex Searchable），作为所有灵智体（Forgekin，社区社交称"灵智体"）的共享知识库，存储灵锻（SpiritForge）产出的蒸馏知识。该知识库需支持三检索入口（语义检索 + 全文检索 + 图谱检索），并满足 CL-005 Knowledge Object Contract 七字段契约。

具体子问题：
- **与灵忆（EchoStore）区分**：灵典存储结构化蒸馏产物，灵忆存储原始任务日志，二者如何工程化隔离?
...
- **谱系追踪**：锻典条目如何关联 F038 进化谱系，支持按灵印查询某灵智体家族的知识资产?

### 1.2 架构约束

- **单向依赖约束**：灵典模块属于 forgemind 应用层（Layer 2），单向依赖 FlowForge 核心框架层（Layer 1）
- **DI 容器约束**：MindCodexStore / MindCodexSearcher / MindCodexConsumer 必须通过 DI 容器注入
- **Repository 层约束**：锻典条目必须通过 Repository 层持久化，禁止直接操作数据库（编程红线第 13 条）
- **配置驱动约束**：contract_validation / search / consumption_ranking 规则必须外置 YAML 配置
- **OpenSieve 统一入口约束**：所有检索必须通过 OpenSieve 统一入口（铁律 §2.2），禁止绕过自建检索引擎
- **CL-005 七字段契约约束**：每个锻典条目必须包含七字段，缺字段拒绝入库
- **与灵忆区分约束**：锻典存储结构化蒸馏产物（含七字段契约），灵忆（F014）存储原始任务日志，二者存储隔离
...
- **对 F014 多域记忆 Collection 的影响**：锻典作为多域记忆联邦 L6 层（procedural memory），与 F014 的 L1-L5 层隔离存储
- **对 F015 三检索入口的影响**：锻典检索接入 F015 的 graph_resolve / list_recent / search_evidence 三入口，并提供 RRF 融合
- **对 F017 消费加权排序的影响**：锻典消费信号（consumption_count / last_eval_score / last_consumed_at）接入 F017 14 信号加权排序
- **对 F028 ForgePipeline 的影响**：流水线第 3 步（记忆初始化）可预加载相关锻典条目到灵智体工作记忆
- **对 F035 三方 Agent 能力融合的影响**：三方 Agent 蒸馏的能力可写入锻典，供灵智体检索消费
- **对 F038 进化谱系的影响**：锻典条目包含 soul_imprint 字段，可按谱系查询某灵智体家族的知识资产
- **对 F040 Harness Eval 控制面的影响**：锻典消费信号作为 Eval 信号源之一
...
```
┌──────────────────────────────────────────────────────────────────────┐
│ 蒸馏知识生产者（多个 Feature 写入锻典）                               │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐            │
│  │ SpiritForge│ │ F035     │  │ F036     │  │ F100     │            │
│  │ 灵锻      │  │ 三方Agent│  │ 回炉蒸馏 │  │ Mode C   │            │
│  │ 蒸馏      │  │ 能力融合 │  │ 通用能力 │  │ 知识进化 │            │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬─────┘            │
└───────┼─────────────┼─────────────┼─────────────┼──────────────────┘
        │             │             │             │
...
│  │                                                                │  │
│  │  ┌──────────────────────────────────────────────────────────┐ │  │
│  │  │ MindCodexStore（锻典存储，与 F014 灵忆隔离）             │ │  │
│  │  │  ├─ add_entry()         添加条目                         │ │  │
│  │  │  ├─ get_entry()         查询条目                         │ │  │
...
│  │                                                                │  │
│  │  ┌──────────────────────────────────────────────────────────┐ │  │
│  │  │ MindCodexSearcher（三检索入口，接入 OpenSieve）          │ │  │
...
│  │                                                                │  │
│  │  ┌──────────────────────────────────────────────────────────┐ │  │
│  │  │ MindCodexRepository（持久层，与 F014 灵忆存储隔离）      │ │  │
│  │  │  ├─ save_entry()                                       │ │  │
│  │  │  ├─ query_entries()                                    │ │  │
...
│  ├─ F008 Durable State Surfaces（持久表面，锻典存储后端）             │
│  ├─ F014 Memory Collection（灵忆，与锻典隔离）                        │
│  ├─ F015 三检索入口（graph_resolve / list_recent / search_evidence）  │
│  ├─ F017 消费加权排序（14 信号 + Wilson 收缩）                        │
│  └─ OpenSieve Client（语义向量 + 全文 + 图谱检索统一入口）            │
└──────────────────────────────────────────────────────────────────────┘
```

### 2.2 关键架构决策

- **决策 1：灵典与灵忆存储隔离（结构化 vs 原始日志）**
  - 理由：naming-contract.md §2.8 明确"锻典是结构化、可检索、可复用的蒸馏产物，区别于灵忆（原始日志）"。混用存储会导致检索时无法区分"已验证知识"与"原始经验"，且灵忆的体量远大于锻典，混合存储影响检索性能
  - 替代方案：统一存储 → 检索时需额外过滤"是否蒸馏"，性能差且语义模糊
- **决策 2：CL-005 七字段契约硬校验（缺字段拒绝入库）**
  - 理由：CL-005 规定每个锻典条目必须包含 trigger / procedure / precondition / postcondition / anti_pattern / provenance / confidence 七字段。缺字段意味着灵智体无法判断"这个知识是否适用于当前场景"，入库即垃圾数据
  - 替代方案：软校验（仅警告）→ 锻典质量不可控，灵智体可能消费不完整知识
- **决策 3：三检索入口通过 OpenSieve 统一入口 + RRF 融合**
  - 理由：铁律 §2.2 要求所有数据检索走 OpenSieve。OpenSieve 已提供语义向量（Milvus）+ 全文（Elasticsearch BM25）+ 图谱（Neo4j）三入口与 RRF 融合。锻典检索复用 OpenSieve 能力，避免重复造轮子
  - 替代方案：锻典自建检索引擎 → 违反铁律，且重复实现 RRF 融合逻辑
- **决策 4：消费加权排序接入 F017 14 信号**
  - 理由：F017 已定义 14 行为指标 + Wilson 收缩 + 中心化偏移 + 分数时效衰减。锻典消费排序复用 F017 算法，保证记忆联邦内排序一致性
  - 替代方案：锻典独立排序算法 → 与 F017 排序结果不一致，灵智体在锻典与灵忆间检索时排序语义割裂
- **决策 5：成熟度门控（仅 L3+ 可消费）**
  - 理由：CL-003 五级知识成熟度阶梯规定 L0-L2 是未验证知识，L3+ 是已验证知识。允许消费 L0-L2 会导致灵智体使用未验证知识做决策，可能引发错误。L0-L2 仅存储，待 Eval Ledger 验证后晋升到 L3 才可消费
  - 替代方案：所有成熟度均可消费 → 锻典质量不可控，灵智体可能消费错误知识
- **决策 6：锻典条目关联 F038 进化谱系（soul_imprint 字段）**
  - 理由：锻典条目归属于某个灵智体，灵智体的谱系关系（分裂 / 融合 / 跨层迁移）应可追溯其知识资产。通过 soul_imprint 字段关联，可查询"某灵智体家族累计的锻典条目"
  - 替代方案：锻典与谱系解耦 → 无法回答"这个知识来自哪个灵智体家族"，知识溯源断裂
- **决策 7：provenance 字段指向 F014 灵忆 Episode ID**
  - 理由：锻典是灵忆的蒸馏产物，provenance 字段记录来源 Episode ID，可追溯"这个知识是从哪次任务经验蒸馏来的"。这是"知识溯源链"的关键环节
  - 替代方案：provenance 留空 → 锻典与灵忆脱节，无法验证知识的经验来源

### 2.3 架构不变量

- 每个锻典条目必须包含 CL-005 七字段（trigger / procedure / precondition / postcondition / anti_pattern / provenance / confidence），缺字段拒绝入库
- 锻典存储必须与 F014 灵忆存储隔离（distinguish_from_echo_store=true）
- 锻典 provenance 字段必须指向 F014 灵忆的 Episode ID（至少 1 个来源）
- 锻典检索必须通过 OpenSieve 统一入口（语义 + 全文 + 图谱）
- 三检索入口必须通过 RRF 融合（与 F015 一致）
- 消费加权排序必须接入 F017 14 信号（consumption_count + last_eval_score + recency）
- 仅 L3+ 成熟度条目可被检索消费，L0-L2 条目仅存储不消费
- 锻典条目必须包含 soul_imprint 字段，关联 F038 进化谱系
- 所有锻典条目必须通过 Repository 层持久化
- 所有锻典规则必须外置 YAML 配置，禁止硬编码
...
- **MindCodexStore（`flowforge/forgemind/codex/store.py`）**：锻典存储抽象，提供条目 CRUD 与 CL-005 契约校验
- **MindCodexSearcher（`flowforge/forgemind/codex/searcher.py`）**：锻典检索器，接入 OpenSieve 三入口 + RRF 融合
...
- **MindCodexRepository（`flowforge/forgemind/codex/repository.py`）**：持久层，与 F014 灵忆存储隔离
- **SpiritForge（`flowforge/forgemind/codex/spirit_forge.py`）**：灵锻引擎，从灵忆蒸馏知识到锻典
...
class MindCodexEntry(BaseModel):
    """锻典条目（CL-005 Knowledge Object Contract 七字段）"""
    entry_id: str
...
检索消费流（锻典 → 灵智体）:
  ...
  ┌────────────────────────────────────────────┐
  │ MindCodexSearcher                          │
  │  ├─ search_semantic()  OpenSieve Milvus    │
  │  ├─ search_grep()      OpenSieve ES BM25   │
...
- **F008 Durable State Surfaces**：锻典条目持久化复用 F008 持久表面，与 F014 灵忆存储隔离
- **F014 Memory Collection**：锻典的 provenance 字段指向 F014 灵忆 Episode ID，是灵忆的蒸馏产物
- **F015 三检索入口**：锻典检索接入 F015 的 graph_resolve / list_recent / search_evidence 三入口
- **F017 消费加权排序**：锻典消费排序复用 F017 14 信号 + Wilson 收缩算法
- **F035 三方 Agent 能力融合**：三方 Agent 蒸馏的能力可通过 SpiritForge 写入锻典
- **F038 进化谱系**：锻典条目包含 soul_imprint 字段，关联谱系
- **OpenSieve**：语义向量（Milvus）+ 全文（ES BM25）+ 图谱（Neo4j）三入口检索统一入口
- **ADR 008 多域记忆联邦**：本 Feature 是 ADR 008 的具体落地

### 4.2 下游影响

- **F028 ForgePipeline**：流水线第 3 步（记忆初始化）可预加载相关锻典条目到灵智体工作记忆
- **F040 Harness Eval 控制面**：锻典消费信号（consumption_count / last_eval_score）作为 Eval 信号源
- **F100 Mode C Knowledge Evolution**：Mode C 知识进化的产物写入锻典，经 Eval Ledger 验证后晋升 L3+
- **operator 知识审计**：operator 可通过锻典审计灵智体的知识资产，验证知识质量

### 4.3 跨模块不变量

- 锻典存储必须与 F014 灵忆存储物理隔离（不同表 / 不同 collection）
- 锻典条目的 provenance 字段必须指向有效的 F014 Episode ID
- 锻典检索必须同时调用 OpenSieve 三入口并通过 RRF 融合
- 消费加权排序的权重配置必须与 F017 一致（0.3 / 0.4 / 0.3）
- L0-L2 条目禁止被检索消费（MaturityGatekeeper 强制过滤）
- L3→L4 晋升必须 operator 批准（CL-003 规则）
- 锻典条目的 soul_imprint 必须与 F038 谱系节点一致
...
- [ ] AC-3: Repository 层通过——MindCodexRepository 抽象存在且与 F014 灵忆存储隔离
...
- [ ] AC-9: 锻典存储与 F014 灵忆存储物理隔离（distinguish_from_echo_store=true）
- [ ] AC-10: 锻典检索通过 OpenSieve 统一入口（无绕过）
- [ ] AC-11: 锻典条目包含 soul_imprint 字段，关联 F038 进化谱系
