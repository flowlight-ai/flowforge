...
- FlowForge 核心服务（端口 8000）
- 可选：LLM 网关（如 OpenRoute，端口由配置）
- 可选：数据源适配器（PostgreSQL / 向量数据库等，端口由配置）
- 三方 Agent 通过本地 CLI 调用

### 5.2 集群生产模式

- FlowForge 核心层多副本
- LLM 网关独立集群（可选，可直连模型厂商 API）
- 数据源适配器集群（按需配置 PostgreSQL / 向量数据库）
