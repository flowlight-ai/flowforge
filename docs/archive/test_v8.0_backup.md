...
4. ✅ 如果web_search失败降级到LLM，该测试标记为FAIL（数据不可信）
5. ✅ trends 数组至少含 3 条热点
6. ✅ 热点数据可通过搜索引擎验证时效性（非 LLM 编造）
7. ✅ article_writing 引用了阶段 1 的趋势数据
...
7. ⚠️ B2: _execute_parallel存在数据竞争（context_data同一引用），需代码修复后验证并行输出独立性
...
### 17.5 IT-SOLO-05：翻译意图（Fast-path或Planning）

**用户输入A（简单翻译）**：`"请把'人工智能正在改变世界'翻译成英文"`
- 预期：`_is_simple_message()` 可能= True → Fast-path（1次LLM）

**用户输入B（复杂翻译）**：`"请将以下文章翻译成日文并分析其中的文化差异：[长文章]"`
- 预期：`_is_simple_message()` = False → Planning路径

**通过条件**：
1. ✅ 简单翻译：LLM ≤ 2次
2. ✅ 复杂翻译：LLM ≥ 2次，有Planning阶段
3. ✅ 输出包含翻译内容
...
| 测试ID | Agent | 测试内容 | 预期行为 |
|--------|-------|---------|---------|
| UT-GA-01 | Planner | 接收任务输入，输出执行计划 | plan包含≥1个步骤 |
| UT-GA-02 | Executor | 按计划执行工具调用 | 正确调用指定工具 |
| UT-GA-03 | Verifier | 验证执行结果 | 返回验证结果和评分 |
| UT-GA-04 | Reviewer | 审查内容质量 | 返回审查意见 |
| UT-GA-05 | Drafter | 生成初稿 | 输出非空文本 |
| UT-GA-06 | Critic | 批评初稿问题 | 返回问题列表 |
| UT-GA-07 | Refiner | 根据批评改进 | 输出改进版本 |
| UT-GA-08 | Analyst | 分析数据/信息 | 返回分析结论 |
| UT-GA-09 | Processor | 处理/转换数据 | 返回处理结果 |
| UT-GA-10 | Validator | 校验数据合规性 | 返回校验结果 |
| UT-GA-11 | Deliverer | 交付最终产物 | 返回交付物 |
| UT-GA-12 | Finalizer | 最终整理 | 返回最终输出 |
| UT-GA-13 | Approver | 审批通过/驳回 | 返回审批决定 |
| UT-GA-14 | Generator | 生成内容 | 输出非空内容 |
| UT-GA-15 | ReactThinker | ReAct思考步骤 | 输出Thought |
| UT-GA-16 | ReactActor | ReAct行动步骤 | 输出Action |
| UT-GA-17 | ReactObserver | ReAct观察步骤 | 输出Observation |
