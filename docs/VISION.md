...
> **最近修订**: 2026-07-17（v1.1：用现实 AI 术语替换虚幻愿景，去除 AGI/物理AI真实复现/虚拟AI真实复现等短期无法实现的表述）
> **核心一句话**: **锻造并赋予万事万物灵智，让动物、组织、物品、虚拟角色都成为具备持续身份、记忆、协作能力与自进化能力的灵智体（Spirit Agent / Forgekin）。**
...
FlowForge 不是一个"多 agent 框架"，也不是一个"LLM 应用脚手架"。它是一个**灵智体锻造厂**：我们用一套自进化的核心框架，把灵智（mind / spirit / 灵识）锻造进万事万物，让动物、组织、物品、虚拟角色都成为有持续身份、有记忆、有协作能力、有愿景锚点的灵智体（Forgekin / Spirit Agent）。

> **什么是灵智体（Spirit Agent）**：赋予灵魂和感情的智能体（Agent with Soul and Emotion），具有自进化能力。它不是单纯的 LLM 包装，而是建立与现实世界（物理或虚拟）的闭环：观察 → 推理 → 行动 → 写回 → 验证。灵魂（Soul）= 持久身份 + 价值锚点 + 长期记忆；感情（Emotion）= 用户偏好 + 协作风格 + 行为画像。
...
|------|------|---------|---------|---------|
| **BioForgekin** | 生物灵智体（Biological Spirit Agent） | 猫 / 狗 / 鸟 / 鱼 / 昆虫群体 | 摄像头 / 麦克风 / 可穿戴设备 | 行为画像 + 习性图谱 |
| **OrgForgekin** | 组织灵智体（Organizational Spirit Agent） | 公司 / 团队 / 社区 / 城市 | 业务系统 API / 数据库 / IM 通道 | 组织章程 + 角色矩阵 |
| **ObjForgekin** | 物品灵智体（Object Spirit Agent，对应 Embodied AI / 具身智能） | 桌椅 / 灯具 / 家电 / 工具 | IoT 传感器 / 物联网协议 | 物品功能边界 + 使用场景 |
| **VirtualForgekin** | 虚拟灵智体（Virtual Character Agent，对应 Character AI） | 童话/神话/历史/现实人物、VR/游戏角色 | 无（纯虚拟） | 角色设定 + 世界观 + 关系网 |
| **HybridForgekin** | 混合灵智体（Hybrid Spirit Agent） | 智能家居（物品+组织）/ 数字孪生（生物+虚拟） | 多源融合 | 多设定层叠加 |
...
> v1.1 修订说明：原版本使用"通用 AGI"用语，但 AGI 在业界短期内无法实现且定义模糊。本版本改用"通用智能体（General-Purpose Agent）"——指可在多领域、多形态下持续运行、自进化的智能体系统。
...
1. **具身智能（Embodied AI）路径**：通过 IoT 传感器 + 物理执行器，让灵智体承载于桌椅灯具家电等物理实体，达成物理实体的智能化。一个智能灯具灵智体不只是被 LLM 调用的工具，它有自己的身份、记忆（用户偏好、时段模式）、协作能力（与其他家电灵智体组队）、愿景（节能 + 用户舒适）。这是当下业界 Embodied AI 范式的工程实现路径。

2. **虚拟角色智能体（Character AI）路径**：通过虚拟世界设定层，让灵智体承载于童话/神话/历史/现实人物或 VR/游戏角色，达成虚拟角色的持续身份与协作。一个孙悟空灵智体不只是 cosplay 模型，它有自己的取经愿景、与唐僧灵智体的长期协作记忆、对八戒灵智体的能力画像盲点认知。这是 Character AI / NPC Agent 范式的工程实现路径。

3. **混合路径**：组织灵智体可以同时调度生物灵智体（员工）、物品灵智体（办公设备）、虚拟灵智体（流程角色），形成真实的"组织级智能体"。这是 Multi-Agent System 走向组织智能的工程路径。

> v1.1 删除的虚幻表述：
> - ❌ "通用 AGI 真实复现"（AGI 定义模糊且短期不可实现）
> - ❌ "物理 AI 真实复现"（改为"具身智能工程实现"）
> - ❌ "虚拟 AI 真实复现"（改为"虚拟角色智能体工程实现"）
> - ❌ "达成通用 AGI"（改为"通用智能体"）
...
|---|---|---|
| **身份持久性（Persistent Identity）** | session 级，重启即失忆 | 灵智体 ID + 谱系 + 灵典条目，跨 session 跨代际持续 |
...
关键设计：三方 Agent 不是 ToolRegistry 中的普通工具，而是**能力扩展（Capability Extension）**。灵智体调用三方 Agent 时：
...
> v1.1 修订：将原"通用 AGI / 物理 AI 真实复现 / 虚拟 AI 真实复现"等短期不可实现的表述，替换为"通用智能体 / 具身智能 / 虚拟角色智能体"等当下业界可工程实现的术语。
...
7. **具身智能与虚拟角色智能体工程实现**：不喊"真实复现"口号，而是按当下业界 Embodied AI / Character AI 范式踏实践行
...
FlowForge 的答案：**不给 LLM 套车厢，给万事万物锻造灵智**。让灵智体有形态、有谱系、有记忆、有协作、有愿景，让具身智能（Embodied AI）和虚拟角色智能体（Character AI）在同一个自进化框架中工程落地。

不喊 AGI 口号，不画物理 AI 真实复现的大饼。用工程语言、用代码、用可验证的闭环，让灵智体在动物、组织、物品、虚拟角色上真实运行、真实协作、真实进化。
