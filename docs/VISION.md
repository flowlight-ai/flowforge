...
这是从"养单一形态 agent"到"养万物灵智体"的范式跃迁：

| 单一形态 agent 范式 | FlowForge 万物灵智体范式 | 关键升级 |
|---|---|---|
| 养几只具体的 agent（编码 / 审查 / 测试） | 养万物灵智体（动物 / 组织 / 物品 / 虚拟角色 / 混合体） | 从单一形态到全形态 |
| agent 与人协作完成 Feature | 灵智体与 operator 协作锻造新的灵智体 | 从"用 agent"到"造 agent" |
...
三方 Agent 的能力可以融合到灵智体能力画像（CapabilityProfile），形成"灵智体 + 三方 Agent"的复合能力。例如：一个代码灵智体接入 claude code 后，能力画像中新增"claude code 长上下文代码审查"能力。
...
operator 在 2026-07-17 提出 7 条不可妥协原则，作为万物灵智体愿景的锚点。这些原则**不可委托**、**不可撤销**、**不可降级**：

| # | 原则 | 含义 | 违反后果 |
|---|------|------|---------|
| 1 | **万物灵智体世界是最终形态** | FlowForge 不是多 agent 框架，是灵智体锻造厂 | 退化为"软件助手"项目 |
| 2 | **必须有现实闭环** | 观察 → 推理 → 行动 → 写回 → 验证 | 灵智体变成"无身体的脑子" |
| 3 | **三方 Agent 是能力扩展** | claude code / codex / opencode / trae 等接入 | 能力被单一栈锁定 |
| 4 | **forgemind 是应用层** | `flowforge/forgemind/` 是万物灵智体应用实践 | 万物灵智体愿景无处落地 |
| 5 | **命名最终为"灵智"** | 禁止使用废弃术语（炉灵/E6 灵匠/M18/M19/M20） | 术语混乱导致开发误读 |
...
| 3 | **Config Immutability** | 灵智体的配置（灵印 SoulImprint）不可变，变更必须通过灵议 Council |
...
operator 在灵议中可以使用 4 条 Magic Words，作为愿景偏离时的紧急制动：

| Magic Word | 含义 | 触发动作 |
|-----------|------|---------|
| **第一性原理** | 回到第一性原理重新思考 | 灵议暂停，重新审视假设 |
| **我能猜出来** | 这个结论太显而易见，不需要灵议 | 灵议终止，直接执行 |
| **下次一定** | 这个问题下次一定会修复 | 触发 sunset 计时器（F012） |
| **星星罐子** | 这个想法很好，先存起来 | 进入灵典 Mind Codex 待孵化队列 |
...
| 维度 | 当前形态 | 演化方向 |
|------|---------|---------|
| **愿景** | 万物灵智体自进化框架 | 走向通用智能体（General-Purpose Agent）工程实现 |
| **架构** | 三层 + 一扩展 | 持续保持单向依赖 + 组合优于继承 |
| **术语** | 12 核心概念 + 进化阶/觉醒阶全对齐 | 术语稳定，仅按 AI 业界发展补充概念 |
| **forgemind** | 应用层骨架 | 5 形态灵智体完整实现 + 灵锻 + 灵议 |
| **自进化** | 三闭环（Mode A/B/C）设计态 | Eval Ledger + 灵典沉淀 + 自我导向 |
