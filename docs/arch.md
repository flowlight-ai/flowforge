...
> **日期**：2026-07-18
> **定位**：从 Agent 编排框架进化为 **万物灵智体锻造平台（ForgeMind Platform）**——为 AI Agent 提供约束、反馈、上下文管理与熵控制的完整控制论系统，并支持锻造万物灵智体（动物/组织/物品/虚拟角色），走向具身智能（Embodied AI）和虚拟角色智能体（Character AI）的工程实现。
> **哲学**：让架构成为配置，让扩展成为插件，让 Harness 负责约束、验证和进化；让 FlowForge 自身成为万物灵智体的锻造厂。
> **关系声明**：FlowForge 是独立开源项目（MIT），三层架构 = 核心框架层（flowforge/）+ forgemind 应用层（flowforge/forgemind/）+ *Forge 垂直业务层（contentforge/devforge/novelforge/mallforge 等）。
...
> **审核状态**：✅ operator 已审核通过命名方案 + 体系设计；已通过 7 方 16 份审核（review.md 终稿 v1.4，含第十三章/第十四章 clowder-ai 深度补审 41 项 CL-001~CL-041），按命名融合方案 + roleagent 工程路径补审 + forgemind/三方 Agent 补审 + clowder-ai 深度补审 全部修订
> **拆分声明**：详细架构决策已拆分到 docs/decisions/ 子目录（每 ADR 一文件），详细 Feature 规格已拆分到 docs/features/ 子目录；v6.0/v7.0 历史章节仅作背景资料，不作为开发依据。

***

# v7.1 增补章节（架构层灵智体定义 + 术语修订 + 工程路径强化）
...
## v7.1-§A0 灵智体（Forgekin / Spirit Agent）架构定义
...
在架构层面，灵智体是 ForgekinBase 抽象基类的具体实现，建立与现实世界（物理或虚拟）的闭环：
...
**架构契约**：所有灵智体继承 `ForgekinBase`（位于 `flowforge/forgemind/base.py`），实现 `observe` / `act` / `verify` 三个抽象方法。灵智体通过 Plugin V3 四钩子注册到 forgemind 应用层，可调用 FlowForge 核心框架能力 + 任何三方 Agent（claude code/codex/opencode/trae）。

## v7.1-§A1 术语修订表（架构文档虚幻用语替换）

> **来源**：operator 第 1 条指令——去除虚幻用语，用当下业界可工程实现的 AI 术语描述

| v7.0 旧术语（虚幻） | v7.1 新术语（可工程实现） | AI 业界概念 |
|------|------|------------|
| 物理 AI 真实复现 | 具身智能工程实现（Embodied AI Engineering） | Embodied AI / Physical Agent |
| 虚拟 AI 真实复现 | 虚拟角色智能体工程实现（Character AI Engineering） | Character AI / NPC Agent |
| 混合 AI 真实复现 | 混合智能体工程实现（Hybrid Agent Engineering） | Hybrid Agent / Cyber-Physical Agent |
| 通用 AGI | 通用智能体（General-Purpose Agent） | General-Purpose Agent |
| 物理 AI 设备 | 具身智能（Embodied AI）设备 | Embodied AI Device |
| 物理 AI 传感器接入 | 物理 AI 传感器接入（具身智能路径，Embodied AI） | Embodied AI Sensor Integration |
| 虚拟世界设定层 | 虚拟世界设定层（虚拟角色智能体路径，Character AI） | Character AI World Setting |
| 万物有灵（玄学化）| 万物灵智体（Forgekin，工程化） | Agent Morphology |

**废弃命名清单**：详见 `design/naming-contract.md#5`。本增补章节之后的所有 v7.0 章节，凡出现上述虚幻用语的，均按本表自动替换理解。

## v7.1-§A2 三层架构（v7.1 强化版）

> **来源**：`decisions/005-forgemind-application-layer.md` + operator 第 8/9 条指令
> **v7.1 强化点**：forgemind 应用层明确为"养万物灵智体"场所，承载 operator 万物灵智体愿景；灵智体可调用三方 Agent 作为能力扩展
...
└─────────────────────────────────────────────────────────────┘
                            ↑ Plugin V3
┌─────────────────────────────────────────────────────────────┐
│ Layer 2: forgemind 应用层（万物灵智体养灵场所）             │
...
- **forgemind 位置**：forgemind 是 Layer 2 应用层，介于 FlowForge 核心框架与 *Forge 垂直业务之间，是万物灵智体养灵所有代码存放的地方。
- **三方 Agent 位置**：三方 Agent 是 Layer 0 能力扩展层，灵智体通过 ExternalAgentAdapter 调用，不是工具而是能力扩展（详见 v7.1-§A4）。

## v7.1-§A3 万物灵智体形态分类（5 种形态 + AI 业界概念）
...
## v7.1-§A4 三方 Agent 集成架构（能力扩展层）

> **来源**：operator 第 10 条指令——"灵智体除了可以调用 flowforge 核心框架的能力外，还可以接入和使用任何三方的 Agent（claude code、codex、opencode、trae），目前你这块的设计感觉也比较弱，请加强。"
> **详细规格**：详见 `features/F031-external-agent-adapter.md` + `decisions/006-external-agent-integration.md` + `review/review.md#第十三章` CL-027（A2AAgentService + L2 协议适配层）

**架构定位**：三方 Agent 是灵智体的**能力扩展（Capability Extension）**，不是工具（Tool）。三方 Agent 的能力画像被纳入灵智体的能力画像融合，灵智体可按需调用。

```
┌──────────────────────────────────────────────────────────┐
│ 灵智体（Forgekin）                                       │
│  - 能力画像（Capability Profile）                        │
│  - native_abilities（原生能力）                          │
│  - external_abilities（三方 Agent 扩展能力，融合）       │
└─────────────────┬────────────────────────────────────────┘
                  │ ExternalAgentAdapter
                  ▼
┌──────────────────────────────────────────────────────────┐
│ ExternalAgentAdapter 抽象层                              │
│  - invoke(prompt, options) → AsyncIterable[AgentMessage] │
│  - capability_profile() → CapabilityProfile              │
│  - state_share(state_dict) → None                        │
│  - fallback_chain(errors) → RecoveryPlan                 │
└─────────────────┬────────────────────────────────────────┘
                  │ EAC v1 七契约
        ┌─────────┼─────────┬─────────┬─────────┐
        ▼         ▼         ▼         ▼
┌───────────┐ ┌────────┐ ┌──────────┐ ┌─────────┐
│ ClaudeCode│ │ Codex  │ │OpenCode  │ │  Trae   │
│ Adapter   │ │Adapter │ │Adapter   │ │Adapter  │
│ (CLI/SDK) │ │(CLI/API)│ │(CLI)    │ │(IDE/API)│
└───────────┘ └────────┘ └──────────┘ └─────────┘
```

**EAC v1 七契约**（参考 clowder-ai F050）：
- **A. Invocation**：调用协议（CLI 命令 / API endpoint / IDE 桥接）
- **B. Stream**：流式响应（AsyncIterable[AgentMessage]）
- **C. Session**：会话管理（session_id / context 保持）
- **D. Capability**：能力声明（capability_profile）
- **E. Collaboration**：协作协议（state_share / fallback）
- **F. Safety**：安全约束（六层 Guardrails）
- **G. Avatar Sync**：形象同步（如灵智体有虚拟形象）
- **H. System Prompt Configuration Map**：系统提示词配置映射（双层同步）
...
## v7.1-§A5 自我演进架构（支持自己开发自己）

> **来源**：operator 第 7/11 条指令——"按 roleagent.md 中描述的自我演进代码开发和文档开发（要求支持自己开发自己），clowder-ai 可以自己开发自己我相信你也可以的。"
> **详细规格**：详见 `review/review.md#第十二章` 12.5-12.6 节 + `review/review.md#第十三章` CL-022~CL-041

FlowForge 必须支持"自己开发自己"——文档 / 代码 / 框架三层自我演进闭环：

```
            ┌─────────────────────────────────────────────────────┐
            │              FlowForge Self-Dev Triad                │
            │                                                      │
            │  ┌─────────────┐  ┌─────────────┐  ┌──────────────┐ │
            │  │ SelfDevDoc  │  │ SelfDevCode │  │SelfDevFrame  │ │
            │  │    Loop     │  │    Loop     │  │    work      │ │
            │  │             │  │             │  │              │ │
            │  │ F100 Mode C │  │ F100 Mode B │  │ F100 Mode A  │ │
            │  │ Knowledge   │  │ Process     │  │ Scope Guard  │ │
            │  │ Evolution   │  │ Evolution   │  │              │ │
            │  └──────┬──────┘  └──────┬──────┘  └──────┬───────┘ │
            │         │                │                 │         │
            │         ▼                ▼                 ▼         │
            │  ┌─────────────────────────────────────────────────┐ │
            │  │           Eval Ledger (Replay A/B)              │ │
            │  │   净增益验证 → L0→L4 五级阶梯晋升                │ │
            │  └─────────────────────────────────────────────────┘ │
            └─────────────────────────────────────────────────────┘
```
...
**三闭环职责**：
- **SelfDevDocLoop**：自动从开发过程对话/commit/review 提取 Episode → Pattern → Draft → Validated → Standard 五级知识沉淀，自动更新 `docs/` 和 `prompts.md`。对应 F100 Mode C Knowledge Evolution。
- **SelfDevCodeLoop**：自动识别流程改进机会（5 槽提案模板），4 硬护栏下产出代码 PR，自动跑回归测试，验证净增益后合并。对应 F100 Mode B Process Evolution。
- **SelfDevFrameworkLoop**：自动判断架构演进是否在框架边界内（ScopeGuard 4 信号 + 频率限制），产出架构变更提案，跨 family review 后合并。对应 F100 Mode A Scope Guard。
...
## v7.1-§A6 12 核心概念 + 进化阶/觉醒阶 索引（架构层引用）

> **权威定义**：详见 `design/naming-contract.md`（v1.0 命名契约）

架构文档中所有出现以下中文术语的，均按 `design/naming-contract.md` 中英文加 AI 业界概念为准：

| # | 中文名 | 英文名 | AI 业界概念 |
|---|--------|--------|------------|
| 1 | 灵智（ForgeMind） | ForgeMind | Persistent Identity Agent / General-Purpose Agent |
| 2 | 灵智体（Forgekin / Spirit Agent） | Forgekin | Agent with Soul and Emotion / Autonomous Agent with Persistent Identity |
...
**进化阶 E1-E6**：E1 萌芽阶（Sprout）/ E2 萌芽阶·稳（Sprout-Stable）/ E3 成长阶（Growth）/ E4 成长阶·深（Growth-Deep）/ E5 觉醒阶（Awakened）/ E6 灵智阶（ForgeMind）

**觉醒阶 E1-E6**：E1 全导阶（Full-Human）/ E2 建议阶（Suggest）/ E3 受限自主阶（Bounded-Autonomous）/ E4 Evolving 阶（Evolving）/ E5 共创阶（Co-Creative）/ E6 灵智主导阶（ForgeMind-Led）

## v7.1-§A7 review.md 41 条 CL 同步矩阵（架构层收尾章）

> **来源**：`review/review.md` v1.4 第十三章（CL-001~CL-021）+ 第十四章（CL-022~CL-041）
> **完整矩阵**：详见 `spec.md` v7.1-§9.2（41 条 CL 完整同步矩阵——本章节不重复表格，仅从架构层视角补充架构变更影响分析）
> **设计规范层子章节占位索引**：详见 `design.md` v7.1-§D9.1（22 个新增/补全子章节 + 4 个 ADR/naming-contract 补全）
> **同步状态**：✅ 已同步 16 项（39.0%）/ 🟡 部分同步 6 项（14.6%）/ ❌ 未同步 19 项（46.4%）
> **架构层责任**：识别 41 条 CL 中引发**架构层变更**的子集，确保 v7.1 三层架构（核心框架层 / forgemind 应用层 / *Forge 垂直业务层）边界不被破坏
> **版本合并声明**：v7.1 增补章节是 v7.1 重构在架构层的权威更新，**已吸收合并 v7.0 增补章节及任何历史章节**（v7.0 不再作为独立版本存在）。三大主文档 v7.0 章节仅作背景资料保留，**不作为开发依据**；开发依据以 v7.1 增补章节（§A0~§A7）+ ADR/Feature 子目录为准。

### v7.1-§A7.1 架构层变更影响分析（10 项关键 CL）

> **筛选标准**：以下 10 项 CL 引发架构层变更（新增模块/重构现有模块/调整层间边界），需在架构层先做影响评估再落地设计规范。

| CL | 主题 | 架构层变更 | 影响层级 | 关联 design 子章节 |
|----|------|-----------|---------|------------------|
| CL-002 | Scope Guard | 新增 `flowforge/evolution/scope_guard.py` 模块（核心框架层）| Layer 1 核心框架层 | design §D7.5 |
| CL-014 | ProviderTransportRegistry | 新增 `flowforge/core/external_agent/registry.py` 模块（核心框架层）| Layer 1 核心框架层 | design §D6.4 |
| CL-022 | Plugin V3 manifest 完整契约 | 重构 `flowforge/core/plugin/` 增加 `PluginManifestValidator` + `ResourceOwnershipRegistry` + `PluginSecurityGuard` | Layer 1 核心框架层 | design §D5.4 |
| CL-023 | Schedule Factory Whitelist | 新增 `flowforge/core/plugin/schedule_registry.py` 模块（核心框架层）| Layer 1 核心框架层 | design §D5.5 |
| CL-027 | TeamAct Queue Steer | 重构 `flowforge/core/teamact/` 增加 `SteerCommand` + `RouteIntentStore` + `TaskProgressStore` | Layer 1 核心框架层 | design §D13 |
| CL-028 | Restart Recovery sweep | 新增 `flowforge/core/reliability/restart_recovery.py` 模块（核心框架层）| Layer 1 核心框架层 | ADR-010 §Restart Recovery Pipeline |
| CL-029 | Event Memory | 新增 `flowforge/core/memory/event_memory.py` 独立子模块（核心框架层，不混入 EchoStore）| Layer 1 核心框架层 | design §D14 |
| CL-008 | 9 个一等公民 | 新增虚拟世界一等公民建模（forgemind 应用层）| Layer 2 forgemind 应用层 | design §D10 |
| CL-012 | Bridge Layer 三协议 | 新增 Bridge Layer 三协议 + runtime coordinator（forgemind 应用层）| Layer 2 forgemind 应用层 | design §D12 |
| CL-032 | Agent Swarm 协同 | 重构 `flowforge/forgemind/` Mind Council 从"议事"升维到"协同执行"| Layer 2 forgemind 应用层 | design §D5.6 |

### v7.1-§A7.2 架构层不变性约束（铁律守护）

> **铁律**：以下 41 条 CL 的修复路径**必须遵守 v7.1 三层架构单向依赖铁律**——上层可以依赖下层，下层绝对禁止导入上层模块。FlowForge 反向依赖零容忍（flowforge 中禁止 import 任何 *Forge 模块）。

**架构边界守护清单**：

| 守护项 | 适用 CL | 守护规则 |
|--------|---------|---------|
| Layer 1 ← Layer 2 ← Layer 3 单向依赖 | 全部 41 项 | design 子章节落地时，新增模块必须放在正确层级；forgemind（Layer 2）可调用 flowforge/core（Layer 1），反之禁止 |
| FlowForge 反向依赖零容忍 | 全部 41 项 | flowforge/core/ 新增模块（scope_guard.py / event_memory.py / restart_recovery.py / registry.py / schedule_registry.py）禁止 import forgemind 或 *Forge |
| 核心框架层不写业务领域代码 | CL-022/023/027/029 | Plugin V3 manifest / Schedule Factory / TeamAct Queue / Event Memory 必须保持通用，禁止硬编码 contentforge/devforge/novelforge/mallforge 业务逻辑 |
| 配置驱动 > 代码继承 > 独立实现 | CL-022/023/037/040 | Plugin Manifest / Schedule Factory / MCP 治理 / docs front-matter 优先 YAML 配置驱动 |
| 组合优于继承 | CL-032 | Agent Swarm 协同模式必须用组合/插件实现，禁止用继承替代 |
| 提示词外置 | CL-037 | MCP 治理规范中 prompt 瘦身 50% 必须外置到 YAML 配置，禁止硬编码 |
| DI 容器注入 | CL-014/022/029 | ProviderTransportRegistry / PluginManifestValidator / EventMemoryStore 必须通过构造函数注入，禁止绕过 DI 容器直接实例化 |

### v7.1-§A7.3 架构层里程碑与依赖关系

| 里程碑 | 架构层关键变更 | 前置依赖 | 后续解锁 |
|--------|--------------|---------|---------|
| M1（P0 必修）| 新增 5 个核心框架层模块（scope_guard / registry / schedule_registry / restart_recovery / event_memory）+ 重构 plugin/ 与 teamact/ | ADR-010 §Restart Recovery Pipeline 先行 | M2 自我演进闭环可启用 |
| M2（P1 应修）| forgemind Agent Swarm 重构 + 三方 Agent MCP 治理 + Close Gate Validator | M1 完成 | M3 文档治理与品牌边界 |
| M3（P2 建议）| ADR-007 Hyperfocus Brake + ADR-010 CI/CD Tracking + naming-contract §7 内外品牌边界 | M2 完成 | v7.1 进入生态成熟阶段 |

> **架构层核心结论**：M1 是 v7.1 架构落地的关键里程碑——5 个新增核心框架层模块 + 2 个重构模块（plugin/teamact）共同构成 v7.1 走向工程实现的架构底座。M1 完成前，自我演进闭环（M2）和文档治理（M3）都无法稳定运行。

***

# 第零章：v7.0 万物灵智体世界愿景架构增补（⚠️ v7.0 历史背景资料，不作为开发依据）[术语按 v7.1 增补章节替换]

> **⚠️ 历史背景资料声明**：本章节为 v7.0 历史背景资料，**不作为开发依据**；开发依据以 v7.1 增补章节（§A0~§A7）+ ADR/Feature 子目录为准。本章节仅用于理解决策演化路径，**任何术语冲突以 v7.1 增补章节 + `design/naming-contract.md` v1.1 为唯一准绳**。
> **来源**：review.md 终稿 v1.2（第六章命名融合 + 第八章 roleagent 补审 + 第九章 forgemind/三方 Agent 补审 + 第十章决策框架 + 第十二章文档拆分）；v1.4 终稿增补第十三章/第十四章 41 条 CL 已落到 v7.1-§A7 同步矩阵
> **决策者**：operator（不可委托）
> **审核状态**：✅ operator 已审核通过命名方案 + 体系设计；7 方 16 份审核全部修订完成
> **不可变**：v7.0 增补章节中的架构决策与命名映射变更需 operator 直接决策，不能由灵智体自我演进修改
> **依赖 ADR**：ADR-004（能力画像）/ ADR-005（forgemind 应用层）/ ADR-006（三方 Agent）/ ADR-007（Harness 工程路径）/ ADR-008（多域记忆联邦）/ ADR-009（Eval 自代谢）/ ADR-010（分布式可靠性）/ ADR-011（伙伴系统数学）/ ADR-012（命名融合）/ ADR-013（万物灵智体愿景）

### §0.1 差异化优势对比表

> **来源**：review.md 第九章 9.4 节 + operator 指令第 6 条"灵智体优势务必要体现"
> **核心结论**：FlowForge v7.0 不是普通 multi-agent 框架，而是万物灵智体锻造平台，最大差异化优势在于"万物形态 + roleagent 工程路径 + 三方 Agent 集成 + 自我演进闭环 + 育灵体系命名"五维度同时具备。

| 维度 | LangGraph | AutoGen | CrewAI | clowder-ai 养猫 | v6.0 FlowForge | **v7.0 FlowForge ForgeMind** |
|------|-----------|---------|--------|-----------------|----------------|------------------------------|
| **万物灵智体形态** | 仅数字 agent（图节点） | 仅数字 agent（对话者） | 仅数字 agent（角色槽位） | 仅猫（数字+物理） | 仅数字 agent（业务 *Forge） | **万物（动物/组织/物品/虚拟角色/混合）5 形态可进化** |
| **roleagent 工程路径** | 无（仅图编排） | 部分（角色对话） | 部分（角色任务） | 部分（猫画像） | 无（仅 Harness 层） | **七大路径完整实现（能力画像/TeamAct/Harness 七层/多域记忆联邦/Eval 自代谢/分布式可靠性/伙伴系统数学）** |
| **三方 Agent 集成** | 无（仅自建节点） | 弱（仅 group chat） | 无（仅角色工具） | 无 | 弱（ExternalToolBridge 当工具） | **能力扩展级集成（claude code/codex/opencode/trae 四 Adapter + 能力画像融合 + fallback 链 + 六层 Guardrails）** |
| **自我演进闭环** | 无 | 无 | 无 | 检索驱动适配 | SelfEvolutionEngine 自创术语 | **灵锻 SpiritForge + 灵典 Mind Codex 蒸馏 + Evoling 自主进化 + 文档/代码/框架自我演进（自己开发自己）** |
| **育灵体系命名** | 无体系 | 无体系 | 无体系 | ad-hoc 命名 | "炉灵/养灵/魂忆/魂印/自锻/锻典/灵议/升华阶/火种等级" 内部混乱 | **ADR-012 双轨命名（ForgeMind 产品层 / Forgekin 代码层）+ 12 核心概念 + 进化阶 E-L0~L4 + 觉醒阶 E1-E6 最终形态 E6 灵智 ForgeMind** |

### §0.2 三层架构重构

> **来源**：review.md 第十章决策 1（已采纳 Harness v2.0 升级）+ ADR-005 + ADR-007
> **决策**：v6.0 七层架构中第 7 层"自进化层"取消独立层级，改为 Harness v2.0 升级（避免循环依赖 D-003）；ForgekinEngine 是装饰器不是独立入口（避免绕过护栏 D-004/D-005/D-020）

#### Layer 1：FlowForge 核心框架层（Harness v2.0）

```
flowforge/（不含 forgemind）
├── core/capability/         # 能力画像（ADR-004，F001）
├── core/teamact/            # TeamAct 六步循环（ADR-002，F002-F007）
├── core/harness/            # Harness 七层现实表面（ADR-007，F008-F013）
├── core/memory/federation/  # 多域记忆联邦六层（ADR-008，F014-F017, F039）
├── core/eval/               # Eval 自代谢系统（ADR-009，F018-F020, F040）
├── core/reliability/        # 分布式可靠性 Tier 1-4（ADR-010，F021-F025）
├── core/partnership/        # 伙伴系统数学（ADR-011）
├── core/external_agent/     # 三方 Agent 集成（ADR-006，F031-F035）
├── core/evolution/          # 自进化基础核心（灵锻 SpiritForge 引擎骨架）
├── core/plugin/             # Plugin V3 四钩子协议
└── ...其他 v6.0 模块（compiler/agents/loop/tools/events 等）
```

#### Layer 2：forgemind 应用层（万物灵智体）

```
flowforge/forgemind/         # ADR-005 决策的应用层
├── species/                 # 灵智体形态分类（5 种）
├── forging/                 # 锻造流水线（ForgePipeline 6 步）
├── sensors/                 # 物理传感器接入（具身智能路径，Embodied AI）
├── worlds/                  # 虚拟世界设定层（虚拟角色智能体路径，Character AI）
├── marketplace/             # 灵智体市场
├── lineage/                 # 灵智体进化谱系
├── codex/                   # 灵典 Mind Codex（可检索知识库）
├── council/                 # 灵议 Mind Council（多灵智体议事）
├── config/                  # forgemind 配置
└── tests/                   # 测试
```
...
```
contentforge/                # 内容创作灵智体
devforge/                    # 开发工厂灵智体
novelforge/                  # 小说创作灵智体
mallforge/                   # 电商运营灵智体
stockforge/                  # 股票分析灵智体（v7.0 新增）
...其他垂直业务灵智体
```

#### 与 v6.0 六层架构的映射表

| v6.0 六层架构 | v7.0 三层架构 | 说明 |
|--------------|--------------|------|
| 1. 基础设施层 | → Layer 1 基础部分 | SQLite/Redis/Qdrant/LangGraph/LLM API 不变 |
| 2. 能力层 | → Layer 1 核心框架层 | Tool/Skill/Agent/Memory 归入 core/ |
| 3. 执行引擎层 | → Layer 1 核心框架层 | HybridExecutor 保留，被 ForgekinEngine 装饰 |
| 4. Harness 驾驭层 | → Layer 1 核心框架层（Harness v2.0 升级） | 七层现实表面融入 core/harness/ |
| 5. 接入层 | → Layer 1 核心框架层 | FastAPI/WebSocket/Web UI 不变 |
| 6. 应用层 | → Layer 3 垂直业务层 | *Forge 重新定位为垂直业务灵智体 |
| 7. 自进化层（v6.0 新增） | → Layer 1 core/evolution/ + Layer 2 forgemind/ | **取消独立层级，避免循环依赖 D-003** |

#### 关键决策

- **决策 1（已采纳）**：第 7 层"自进化层"取消独立层级，改为 Harness v2.0 升级——避免 v6.0 自进化层↔应用层循环依赖（D-003）
- **决策 2（已采纳）**：ForgekinEngine 是装饰器不是独立入口——避免绕过 Harness 护栏（D-004/D-005/D-020）
- **决策 3（已采纳）**：forgemind 是 FlowForge 应用层（Layer 2），承载万物灵智体——避免通用灵智体与垂直业务灵智体混淆（FM-005）

### §0.3 ForgekinEngine 装饰器模式

> **来源**：review.md 第十章决策 2（已采纳）+ 修复 D-004/D-005/D-020/D-046
> **核心修复**：v6.0 ForgekinEngine 作为独立入口绕过 Harness 护栏（D-004）；v6.0 `__init__` 11 个依赖违反 DI 容器（D-046）

#### 装饰器设计原则

ForgekinEngine 装饰 `HybridExecutor` + `HarnessOrchestrator`，**不是独立入口**：
- 不再绕过 Harness 护栏（修复 D-004/D-005/D-020）
- 所有执行路径仍经 Harness 七层现实表面校验
- DI 容器注入（修复 D-046：v6.0 `__init__` 11 个依赖违反 DI）

#### 类图（ASCII）

```
┌─────────────────────────────────────────────────────────────────┐
│  Layer 1: core/evolution/forgekin_engine.py                     │
│                                                                  │
│  class ForgekinEngine(HybridExecutor):                          │
│      """灵智体引擎——装饰 HybridExecutor，叠加自进化闭环"""       │
│                                                                  │
│      def __init__(                                                │
│          self,                                                   │
│          hybrid_executor: HybridExecutor,   # 被装饰对象         │
│          mind_store: MindStore,             # DI 注入            │
│          echo_store: EchoStore,             # DI 注入            │
│          imprint_store: ImprintStore,       # DI 注入            │
│          mind_codex: MindCodex,             # DI 注入            │
│          spirit_forge_engine: SpiritForgeEngine,  # DI 注入      │
│          di_container: DIContainer,         # DI 容器            │
│      ):                                                          │
│          super().__init__(hybrid_executor._...)                  │
│          self._wrapped = hybrid_executor                         │
│          ...                                                     │
│                                                                  │
│      async def execute(self, input, context):                   │
│          # 1. observe: 加载灵忆/灵印                             │
│          # 2. act: 调用 wrapped.execute()（经 Harness 护栏）     │
│          # 3. verify: 记录 Episode + 灵印 proposal + 灵典蒸馏    │
│          # ↑ 复用 HybridExecutor 的 Harness 调用链，不绕过       │
│          ...                                                     │
└─────────────────────────────────────────────────────────────────┘
...
│  Layer 1: core/evolution/forgekin_engine.py                     │
│                                                                  │
│  class ForgekinEngine(HarnessOrchestrator):                     │
│      """灵智体引擎——装饰 HarnessOrchestrator，叠加自进化闭环"""  │
│                                                                  │
│      # 同样装饰 HarnessOrchestrator，确保七层护栏不被绕过        │
│      # 适用于 mode 执行路径                                      │
└─────────────────────────────────────────────────────────────────┘
```
...
- ForgekinEngine **不持有 HybridExecutor 的内部状态**，仅装饰其 `execute()` 方法
- 所有副作用（Episode 记录/灵印更新/灵典蒸馏）在 `execute()` 后置阶段完成
- 不可逆操作必须经 Harness L5 操作确认层（修复 D-005）
- ForgekinEngine 实例化必须经 DI 容器（修复 D-046）

### §0.4 育灵体系命名融合

> **来源**：ADR-012（命名融合）+ review.md 第六章
> **不可变**：命名变更需 operator 直接决策
...
| # | 中文 | 英文 | 含义 | 用途 |
|---|------|------|------|------|
| 1 | 灵智 | ForgeMind | 最终形态主名 | 文档/对外 |
| 2 | 灵智体 | Forgekin | 代码层主名 | 代码/API |
| 3 | 灵群 | ForgeKinship | 灵智体形态分类（5 种） | forgemind |
| 4 | 育灵 | Forge Nurturing | 灵智体锻造过程 | forgemind |
| 5 | 灵忆 | Mind Echo | 灵智体经验记忆 | 代码 |
| 6 | 灵印 | Mind Imprint | 灵智体认知画像 | 代码 |
| 7 | 灵锻 | SpiritForge | 经验蒸馏到灵典 | 代码 |
| 8 | 灵典 | Mind Codex | 蒸馏经验知识库 | 代码 |
| 9 | 灵议 | Mind Council | 多灵智体议事 | 代码 |
| 10 | 进化阶 | Evolution Hierarchy | E-L0~L4 知识阶梯 | forgemind |
| 11 | 觉醒阶 | Awakening Stages | E1-E6 成长阶段 | forgemind |
| 12 | 灵启 | Mind Initiation | 入门训练 E1 | forgemind |

#### 进化阶 E-L0~L4（知识阶梯）

| 阶 | 中文 | 英文 | 旧名 | 说明 |
|----|------|------|------|------|
| E-L0 | 萌芽 | Seed | 火种 Spark | 知识阶梯第 0 级 |
| E-L1 | 萌发 | Sprout | 余烬 Ember | 知识阶梯第 1 级 |
| E-L2 | 绽放 | Bloom | 火焰 Flame | 知识阶梯第 2 级 |
| E-L3 | 繁茂 | Thrive | 烈焰 Blaze | 知识阶梯第 3 级 |
| E-L4 | 进化 | Evolve | 锻火 Forge Fire | 知识阶梯第 4 级 |

#### 觉醒阶 E1-E6（成长阶段，最终形态 E6 已修订）

| 阶 | 中文 | 英文 | 说明 |
|----|------|------|------|
| E1 | 灵启 | Mind Initiation | 入门训练 |
| E2 | 觉醒 | Awakening | 自我认知建立 |
| E3 | 精通 | Mastery | 领域熟练 |
| E4 | 进化 | Evoling | 自主进化（涌现式起点） |
| E5 | 卓越 | Excellence | 跨域融合 |
| E6 | 灵智 | ForgeMind | **最终形态（operator 已修订，废弃"E6 灵匠 Mind Artisan"过渡命名）** |

#### 术语全局替换映射表（精简版，详见 ADR-012 §6.9）

| 原术语 | 新术语 | 备注 |
|--------|--------|------|
| 炉灵 Forgekin | 灵智 ForgeMind | 产品层 |
| 养灵 | 育灵 | 养成过程 |
| 魂忆 Soul Echo | 灵忆 Mind Echo | 记忆 |
| 魂印 Soul Imprint | 灵印 Mind Imprint | 画像 |
| 自锻 Auto-Forge | 灵锻 SpiritForge | 自主思考 |
| 锻典 Forge Codex | 灵典 Mind Codex | 技能库 |
| 灵议 Forgekin Council | 灵议 Mind Council | IM 协作 |
| 火种等级 Ember Hierarchy | 进化阶 Evolution Hierarchy | 知识阶梯 |
| 升华阶 Ascension Stages | 觉醒阶 Awakening Stages | 成长阶段 |
| SoulProfile | MindProfile | 代码类名 |
| SoulStore | MindStore | 代码类名 |
| AutoForgeEngine | SpiritForgeEngine | 代码类名 |
| auto_forge.yaml | spirit_forge.yaml | 配置文件名 |
| M18 SelfEvolutionEngine | ForgeMindEngine | v4.0 自创术语废弃 |
| M19 MemoryGovernanceManager | ForgeMindEngine | v4.0 自创术语废弃 |
| M20 FirstTouchRouter | ForgeMindEngine | v4.0 自创术语废弃 |
| HelixRAG | OpenSieve | 聚合检索中台 |
| ForgekinEngine | ForgekinEngine（保留） | 代码类名不变（双轨） |
| forgekin_id | forgekin_id（保留） | 变量名不变 |
| /api/v7/forgekins | /api/v7/forgekins（保留） | API 路径不变 |
| EchoStore | EchoStore（保留） | 代码类名不变 |
| ImprintStore | ImprintStore（保留） | 代码类名不变 |

> **保留说明**：Spark/Ember/Flame/Blaze 单独出现时仅在 v7.0 增补章节命名表中作为旧名标注，不替换；forgekin 小写不替换（变量名）；Forgekin 大写不替换（代码类名，双轨策略）。

### §0.5 roleagent 七大工程路径架构落地

> **来源**：review.md 第八章 47 项补审（RA-001~RA-047）+ roleagent.md 镜像
> **铁律**：七大工程路径是 Build to Persist 复利型基础设施，不可简化

| 路径 # | 名称 | ADR | Feature | 代码位置 | 架构层级 |
|--------|------|-----|---------|---------|---------|
| 1 | 能力画像 × Harness 契合度 | ADR-004 | F001 | flowforge/core/capability/ | Layer 1 |
| 2 | TeamAct 六步循环 + 五项终止 | ADR-002 | F002-F007 | flowforge/core/teamact/ | Layer 1 |
| 3 | Harness 七层现实表面 | ADR-007 | F008-F013 | flowforge/core/harness/ | Layer 1 |
| 4 | 多域记忆联邦（六层 + 三入口） | ADR-008 | F014-F017, F039 | flowforge/core/memory/federation/ | Layer 1 |
| 5 | Eval 自代谢系统 | ADR-009 | F018-F020, F040 | flowforge/core/eval/ | Layer 1 |
| 6 | 分布式可靠性（Tier 1-4 恢复） | ADR-010 | F021-F025 | flowforge/core/reliability/ | Layer 1 |
| 7 | 伙伴系统数学（上限/下限/波动吸收） | ADR-011 | — | flowforge/core/partnership/ | Layer 1 |

#### 七大路径概要

1. **能力画像（CapabilityProfile）**：六维能力描述 + 盲点冲突检测，灵智体调用三方 Agent 前先做 gap_analysis
2. **TeamAct 六步循环**：发起 → 接球 → 推回 → 转交 → 收尾 → 复盘；五项终止条件（任务完成/超时/熔断/退路/共识）
3. **Harness 七层现实表面**：Durable State / Evidence & Sensors / Governance Boundary / Magic Words / Entropy Control / Harnessability / Eval Control Plane
4. **多域记忆联邦**：六层（L1 工作/L2 情景/L3 语义/L4 联邦/L5 灵典/L6 谱系）+ 三检索入口（关键词/语义/时间衰减）+ 消费加权排序
5. **Eval 自代谢**：Eval Contract 五问（What/Why/How/Who/When）+ 三方信号交叉（任务/伙伴/Eval）+ 七类归因矩阵
6. **分布式可靠性**：Tier 1-4 恢复分级 + liveness 规范读模型 + 弱状态机 vs 强 workflow + 跨 provider 宿主抽象
7. **伙伴系统数学**：上限公式（取最大）+ 下限公式（连乘）+ 波动吸收为内部成本 + Token 账本
...
> 任何一个环节为 0，用户可见错误即为 0；任何一个环节失控，用户可见错误即放大。这是七大工程路径必须完整实现的数学基础。

### §0.6 forgemind 应用层架构

> **来源**：review.md 第九章 9.1 节 FM-001~FM-012 + ADR-005
> **核心问题修复**：v6.0 缺少应用层，万物灵智体愿景无处落地（FM-001）

#### forgemind 在三层架构中的位置

forgemind 是 Layer 2（应用层），位于 `flowforge/forgemind/` 子目录，不是独立项目。承载万物灵智体愿景的实践代码。
...
flowforge/forgemind/
├── __init__.py
├── plugins.py                    # ForgeMindPlugin 注册（Plugin V3）
├── species/                      # 灵智体形态分类（5 种）
...
class ForgekinBase(ABC):
    ...
    @abstractmethod
    async def observe(self, ctx: ForgekinContext) -> Observation:
        """观察：从物理传感器/虚拟世界/数字工具采集证据"""

    @abstractmethod
    async def act(self, plan: Plan, ctx: ForgekinContext) -> Action:
        """行动：经 Harness 七层护栏执行"""

    @abstractmethod
    async def verify(self, action: Action, ctx: ForgekinContext) -> Verdict:
        """验证：Eval Contract 五问 + 三方信号交叉"""
```
...
| 步骤 | 名称 | 说明 |
|------|------|------|
| 1 | 形态定义 | What to forge（5 形态枚举） |
| 2 | 能力注入 | Capability injection（CapabilityProfile 六维） |
| 3 | 记忆初始化 | Memory seeding（MindStore + EchoStore） |
| 4 | 价值观对齐 | Value alignment（operator 愿景锚点） |
| 5 | 能力验证 | Capability verification（Eval Contract） |
| 6 | 觉醒晋升 | Awakening promotion（E1→E2→...→E6） |

#### 万物灵智体 5 种形态分类

| 形态 | 中文 | 锻造示例 | 物理接入 | 虚拟设定 |
|------|------|---------|---------|---------|
| BioForgekin | 生物灵智体 | 猫/狗/植物 | 摄像头/麦克风/可穿戴 | 行为画像 + 习性图谱 |
| OrgForgekin | 组织灵智体 | 公司/团队/社区 | 业务系统 API/IM 通道 | 组织章程 + 角色矩阵 |
| ObjForgekin | 物品灵智体 | 桌椅/灯具/家电 | IoT 传感器 | 物品功能边界 + 使用场景 |
| VirtualForgekin | 虚拟灵智体 | 童话/神话/历史/游戏角色 | 无（纯虚拟） | 角色设定 + 世界观 + 关系网 |
| HybridForgekin | 混合灵智体 | 智能家居/数字孪生 | 多源融合 | 多设定层叠加 |

**形态可进化**：一只 BioForgekin 猫可通过积累组织协作经验进化为 HybridForgekin（既是宠物又是社区吉祥物）。

### §0.7 三方 Agent 集成架构

> **来源**：review.md 第九章 9.2 节 EX-001~EX-010 + ADR-006
> **核心问题修复**：v6.0 三方 Agent 当作工具调用，未当作能力扩展（EX-001）

#### ExternalAgentAdapter 抽象层目录结构
...
| 机制 | 用途 | Feature | 修复 |
|------|------|---------|------|
| ExternalAgentProfile | 三方 Agent 能力画像 | F032 | EX-002 |
| ExternalAgentSharedState | 执行状态写入灵智体共享状态 | F033 | EX-004 |
| ExternalAgentFallback | 失败回退链（跨厂商） | F034 | EX-007 |
| ExternalAgentCapabilityFusion | 能力画像融合到灵智体画像 | F035 | EX-010 |

#### 4 个首批 Adapter

| 三方 Agent | 厂商 | 接入方式 | 擅长 | 盲点 | fallback 优先级 |
|-----------|------|---------|------|------|:---:|
| Claude Code | Anthropic | CLI / SDK | 复杂重构 | 长上下文易漂移 | 1 |
| Codex | OpenAI | CLI / API | 推理 | 工具调用弱 | 2 |
| OpenCode | 开源 | CLI | 开源协作 | 企业场景弱 | 3 |
| Trae | ByteDance | IDE / API | IDE 集成 | 命令行长任务弱 | 4 |

#### 六层 Guardrails

| 治理层 | 机制 | 修复 |
|--------|------|------|
| L1 输入验证 | 三方 Agent 调用前必须通过 Schema 校验 | EX-005 |
| L2 系统提示约束 | 灵智体 system role 注入"禁止绕过审计" | EX-005 |
| L3 工具白名单 | 三方 Agent 只能调用 allow-list 内工具 | EX-005 |
| L4 输出验证 | 三方 Agent 输出必须通过 lint + 测试 | EX-005 |
| L5 操作确认 | 不可逆操作（merge/release）需 operator 确认 | EX-005 |
| L6 成本上限 | 每个灵智体有三方 Agent 调用配额 | EX-006 |
...
> **来源**：S-08 / X-017 修复 + ADR-005
> **核心修复**：v6.0 PluginProtocol 仅有 V2 钩子，无法注册万物灵智体

#### V3 四钩子

| 钩子 | 用途 | 注册方 |
|------|------|--------|
| `register_forgekins` | 注册通用灵智体形态 | forgemind / *Forge |
| `register_forge_skills` | 注册锻造技能 | forgemind / *Forge |
| `register_council_channels` | 注册灵议通道 | forgemind / *Forge |
| `register_auto_forge_config` | 注册灵锻造配置（灵锻 SpiritForge 触发条件） | forgemind / *Forge |

#### 与 V2 钩子的关系

- **V2 钩子保留**：`register_agents` / `register_tools` / `register_workflows` / `register_skills` 等 V2 钩子继续支持，不破坏向后兼容
- **V3 新增**：上述四钩子为 v7.0 新增，专为万物灵智体体系设计
- **协议版本识别**：PluginProtocol 增加 `protocol_version: int` 字段，DI 容器根据版本号选择注入路径

### §0.9 v6.0 模块映射到 v7.0

> **来源**：修复 D-016/D-015/D-002 + F-019
> **核心问题修复**：v6.0 与 v7.0 模块映射关系不清晰，缺少映射表

| v6.0 模块 | v7.0 模块 | 说明 |
|----------|----------|------|
| evolution/（v6.0 自进化层） | flowforge/core/evolution/ + flowforge/forgemind/codex/ | 取消独立层级，拆为基础核心（core/evolution/）与应用层灵典（forgemind/codex/） |
| MemoryManager 五层 | 多域记忆联邦六层（core/memory/federation/） | L1 工作/L2 情景/L3 语义/L4 联邦/L5 灵典/L6 谱系；三检索入口；消费加权排序 |
| HybridExecutor | ForgekinEngine 装饰 | ForgekinEngine 装饰 HybridExecutor + HarnessOrchestrator，不绕过护栏 |
| ExternalToolBridge | ExternalAgentAdapter | 从"工具调用"升级为"能力扩展"，4 大机制 + 4 Adapter + 六层 Guardrails |
| Soul Echo | Mind Echo（复用 MemoryManager） | 修复 D-017：v6.0 Soul Echo 与 MemoryManager 功能重叠，v7.0 Mind Echo 复用 MemoryManager 实现 |
| Forge Codex | Mind Codex（复用 Skill 系统） | 修复 D-018：v6.0 Forge Codex 与 Skill 系统重叠，v7.0 Mind Codex 复用 Skill 系统实现 |
| A2A Protocol | 复用 EventBus + Handoff | 修复 D-019：v6.0 A2A 单独实现，v7.0 复用 EventBus + TeamAct Handoff 胶囊 |
| Forgekin Council | Mind Council | 灵议通道，复用 IM 多渠道（Web Chat/飞书/微信/Slack/Discord） |
| SoulProfile | MindProfile | 代码类名替换（按 ADR-012 §6.9） |
| SoulStore | MindStore | 代码类名替换（按 ADR-012 §6.9） |
| AutoForgeEngine | SpiritForgeEngine | 代码类名替换（按 ADR-012 §6.9） |
| auto_forge.yaml | spirit_forge.yaml | 配置文件名替换（按 ADR-012 §6.9） |
| AscensionManager / AscensionStage | AwakeningManager / AwakeningStage | 觉醒阶管理器（替换"升华"为"觉醒"） |
| 第 7 层自进化层 | 取消独立层级（Harness v2.0 升级） | 修复 D-003 循环依赖 |
| M18 SelfEvolutionEngine | ForgeMindEngine | v4.0 自创术语废弃（按 ADR-012） |
| M19 MemoryGovernanceManager | ForgeMindEngine（映射到 M1-M17） | v4.0 自创术语废弃 |
| M20 FirstTouchRouter | ForgeMindEngine（映射到 M1-M17） | v4.0 自创术语废弃 |
| HelixRAG | OpenSieve | 聚合检索中台统一命名 |

### §0.10 v7.0 设计态声明 + 文档导航

> **来源**：review.md 第十章决策 7（标注"设计态"）+ 第十二章文档拆分
> **可证伪性原则**：避免 AGI 修饰词，所有"已实现"必须有代码可证伪，所有"设计态"必须明确标注

#### 已实现 vs 设计态清单

| 模块 | 状态 | 验证方式 |
|------|------|---------|
| FlowForge v6.0 Harness 层骨架 | ✅ 已实现 | 代码可证伪（core/harness/） |
| LoopExecutor + 9 大模式 | ✅ 已实现 | 代码可证伪（loop/ modes/） |
| Skill 系统 | ✅ 已实现 | 代码可证伪（core/skill/） |
| MCP 模块 | ✅ 已实现 | 代码可证伪（tools/mcp/） |
| CapabilityProfile 六维 | 🟡 设计态 | F001 Feature 规格 + ADR-004，待代码落地 |
| TeamAct 六步循环 | 🟡 设计态 | F002-F007 Feature 规格 + ADR-002，待代码落地 |
| Harness 七层现实表面 | 🟡 部分实现（Durable State/Evidence 已有；Magic Words/Entropy 待落地） | F008-F013 |
| 多域记忆联邦六层 | 🟡 设计态 | F014-F017, F039，待代码落地 |
| Eval 自代谢 | 🟡 设计态 | F018-F020, F040，待代码落地 |
| 分布式可靠性 Tier 1-4 | 🟡 设计态 | F021-F025，待代码落地 |
| 伙伴系统数学 | 🟡 设计态 | ADR-011，待代码落地 |
| forgemind 应用层 | 🟡 设计态 | ADR-005 + F026-F030 + F036-F038，待代码落地 |
| 万物灵智体 5 形态 | 🟡 设计态 | F027，待代码落地 |
| ForgePipeline 6 步 | 🟡 设计态 | F028，待代码落地 |
| 物理传感器接入 | 🟡 设计态（Phase 6+） | F029，待硬件支持 |
| 虚拟世界设定层 | 🟡 设计态 | F030，待代码落地 |
| 三方 Agent Adapter（claude code/codex/opencode/trae） | 🟡 设计态 | F031-F035 + ADR-006，待代码落地 |
| Plugin V3 四钩子 | 🟡 设计态 | 待 core/plugin/ 改造 |
| ForgekinEngine 装饰器模式 | 🟡 设计态 | §0.3 架构设计，待代码落地 |
| 灵锻 SpiritForge 引擎 | 🟡 设计态（v7.0 MVP 不含） | 安全护栏需先完善 |
| 灵议 Mind Council | 🟡 设计态（v7.0 MVP 不含） | 需先解决脑裂和身份认证 |
| Evoling 进化体状态（E4+） | 🟡 设计态（v7.0 MVP 不含） | 需先验证 E1-E3 引导式进化 |

#### 可证伪性原则

- 所有"已实现"必须有代码路径可证伪（违反 T1-T8 测试铁律的"已实现"声明视为虚假承诺）
- 所有"设计态"必须明确标注，避免对外宣传为"已具备"
- 禁止使用"通用 AGI""意识""灵魂"等不可证伪的修饰词；"灵智"仅作为产品命名，不作为能力声明
- "万物灵智体世界愿景"是 operator 战略方向，不是当前能力声明

#### 13 份依赖引用文档清单

| # | 文档 | 路径 |
|---|------|------|
| 1 | VISION.md | docs/VISION.md |
| 2 | roleagent.md | docs/roleagent.md |
| 3 | spec.md | docs/spec.md |
| 4 | design.md | docs/design.md |
| 5 | ADR-004 能力画像 | docs/decisions/004-capability-profile-routing.md |
| 6 | ADR-005 forgemind 应用层 | docs/decisions/005-forgemind-application-layer.md |
| 7 | ADR-006 三方 Agent 集成 | docs/decisions/006-external-agent-integration.md |
| 8 | ADR-012 命名融合 | docs/decisions/012-naming-fusion.md |
| 9 | ADR-013 万物灵智体愿景 | docs/decisions/013-all-things-spirit-mind-vision.md |
| 10 | review.md 终稿 | docs/review/review.md |
| 11 | F026 forgemind 应用层 Feature | docs/features/F026-forgemind-app-layer.md |
| 12 | F027 万物灵智体形态分类 | docs/features/F027-all-things-spirit-species.md |
| 13 | F031 三方 Agent 适配层 | docs/features/F031-external-agent-adapter.md |

#### 16 份审核文件清单

| # | 审核文件 | 路径 |
|---|---------|------|
| 1 | 终稿审核 | docs/review/review.md |
| 2 | 终稿审核 v1 | docs/review/review1.md |
| 3 | 终稿审核 d | docs/review/reviewd.md |
| 4 | 终稿审核 d1 | docs/review/reviewd1.md |
| 5 | deepseek 审核 | docs/review/deepseek.md |
| 6 | deepseek1 审核 | docs/review/deepseek1.md |
| 7 | doubao 审核 | docs/review/doubao.md |
| 8 | doubao1 审核 | docs/review/doubao1.md |
| 9 | glm 审核 | docs/review/glm.md |
| 10 | glm1 审核 | docs/review/glm1.md |
| 11 | kimi 审核 | docs/review/kimi.md |
| 12 | kimi1 审核 | docs/review/kimi1.md |
| 13 | minimax 审核 | docs/review/minimax.md |
| 14 | minimax1 审核 | docs/review/minimax1.md |
| 15 | qianwen 审核 | docs/review/qianwen.md |
| 16 | qianwen1 审核 | docs/review/qianwen1.md |

---

> **以下为 v6.0 历史内容，保留作为背景资料。术语已按 ADR-012 §6.9 映射表全局替换为 v7.0 命名（如炉灵→灵智、养灵→育灵、魂忆→灵忆、魂印→灵印、自锻→灵锻、锻典→灵典、火种等级→进化阶、升华阶→觉醒阶、SoulProfile→MindProfile、SoulStore→MindStore、AutoForgeEngine→SpiritForgeEngine、Forgekin Council→Mind Council、Forge Codex→Mind Codex、HelixRAG→OpenSieve 等）。v6.0 章节结构未破坏，仅做术语替换。**
...
> **核心变化**：新增第 7 层「自进化层（Evolution Layer）」，承载灵智养成体系
...
│     Forgekin Engine | SpiritForge | Mind Codex | Mind Echo/Imprint  │
...
│                                                                              │
│  ┌──────────────────┐  ┌──────────────────┐  ┌──────────────────────────┐  │
│  │  Soul Profile    │  │  Mind Echo       │  │  Mind Imprint            │  │
│  │  (身份与人格)     │  │  (灵忆-三层记忆) │  │  (灵印-认知画像)         │  │
│  │                  │  │                  │  │                          │  │
...
│                                                                              │
│  ┌──────────────────┐  ┌──────────────────────────────────────────────┐   │
│  │  Mind Codex     │  │       External Tool Bridge                   │   │
│  │  (灵典-技能库)    │  │  ┌──────────────┐  ┌─────────────────────┐  │   │
...
│                                                                              │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
│  │       Mind Council (灵议-IM多渠道) + A2A Protocol                │   │
│  │  ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ ┌──────────┐ │   │
...
│                     应用层 (*Forge 项目群)                                   │
│  ContentForge / DevForge / NovelForge / MallForge / StockForge              │
│  每个 *Forge 通过 PluginProtocol 注册灵智角色，获得自进化能力                │
└─────────────────────────────────────────────────────────────────────────────┘
                    │ 依赖
                    ▼
┌─────────────────────────────────────────────────────────────────────────────┐
...
class ForgekinEngine:
    """灵智引擎——自进化的统一入口，包装 HybridExecutor"""
    ...
    def __init__(
        ...
        codex: MindCodex,
        spirit_forge_engine: SpiritForgeEngine,
        external_tool_bridge: ExternalToolBridge,
        a2a_manager: A2AManager,
        awakening_manager: AwakeningManager,
    ):
        ...
        self._codex = codex
        self._spirit_forge = spirit_forge_engine
        self._tools = external_tool_bridge
        self._a2a = a2a_manager
        self._awakening = awakening_manager
    ...
    async def execute(
        ...
        # 10. 检查升华条件
        await self._awakening.check_promotion(forgekin_id)
...
CREATE TABLE forgekin_souls (
    ...
    awakening_stage TEXT NOT NULL DEFAULT 'E1',
...
### 16.4 Mind Imprint 画像

```python
class ImprintStore:
    """灵印存储——认知画像（双层结构）"""
    
    async def load(self, forgekin_id: str) -> MindImprint:
        """加载画像"""
...
### 17.3 灵锻群（Group Forge）

对标 clowder-ai 做梦群——多灵智协作灵锻：
...
```
┌─────────────────────────────────────────────────────────────────────┐
│              Web Chat 灵议（Mind Council UI）                    │
├──────────────┬────────────────────────┬────────────────────────────┤
│  灵智列表    │  议事大厅（多灵智对话）│  日记与状态                │
│  ─────────  │  ──────────────────   │  ──────────────────       │
│  ● Architect │  [Architect]: 我建议  │  📔 Architect 的日记       │
│    E3 Flame  │   采用微服务架构...    │   2026-07-14 灵锻          │
│              │                        │   "今天分析了3个需求..."   │
│  ● Writer    │  [Writer]: 从内容角度  │                            │
...
│    E1 Spark  │   Writer 的方案        │                            │
│              │                        │  📊 状态概览               │
│  + 新建灵智  │  [Coder]: 我来编码     │   活跃: 3 / 休眠: 1        │
│              │                        │   E3: 1 / E2: 1 / E1: 1    │
├──────────────┴────────────────────────┴────────────────────────────┤
│  💬 输入消息... @Architect 请审查  | [发送] [发起协作] [灵锻]      │
└─────────────────────────────────────────────────────────────────────┘
```
...
class DevForgePlugin(FlowForgePlugin):
    ...
    def register_forgekins(self, forgekin_registry) -> None:
        # 注册 DevForge 的灵智角色
        forgekin_registry.register(MindProfile(
            forgekin_id="fk_devforge_architect",
            name="Architect",
            kind="devforge:architect",
            soul= SoulSpec(
                persona="我是 DevForge 的架构师灵智...",
                worldview="配置驱动 > 代码继承",
                values=["单向依赖", "不过度工程化"],
                voice="直接、技术性",
            ),
...
| SpiritForge | 跳过（不灵锻） | spirit_forge 未启用 |
...
class ForgekinSecurityGuard:
    ...
    async def check_creation(
        self, creator_id: str, new_profile: MindProfile
    ) -> bool:
        """检查灵智创建权限（SR-05）"""
        creator = await self._soul.load(creator_id)
        if creator.awakening_stage != "E6":
            raise PermissionError("仅 E6 锻师可创建新灵智")
    ...
    async def check_external_tool(
        ...
        if tool not in soul.capabilities.external_tools_can_use:
            raise PermissionError(f"灵智无权使用 {tool}")
...
**理由**：不是所有任务都需要自我进化，流水线型任务用静态智能体更高效；需要成长的复杂任务用灵智。
...
### ADR-007-04: Mind Imprint no-classifier 红线

**决策**：Mind Imprint 禁止后台 classifier，必须基于显式行为。
...
**理由**：防止灵智过度打扰 operator；对标 clowder-ai Provoke 设计。
