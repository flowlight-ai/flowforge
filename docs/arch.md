...
> **拆分声明**：详细架构决策已拆分到 docs/decisions/ 子目录（每 ADR 一文件），详细 Feature 规格已拆分到 docs/features/ 子目录；v6.0 历史章节仅作背景资料，不作为开发依据。
...
### v7.1-§A5.1 v7.0 自我进化架构遗产归档说明

> **归档说明**：v7.0 是过程文档（未写过代码），其架构设计契约已合并到 v7.1 §A0-§A7 增补章节与 design.md 对应章节。以下 v7.0 章节已归档到 `_archive/` 目录：
>
> 1. **第零章：v7.0 万物灵智体世界愿景架构增补**（原 line 300-860，共 561 行）→ 归档到 `_archive/arch_v70_ch0_vision.md`
>    - §0.1 差异化优势对比表 → v7.1 §A0
>    - §0.2 三层架构重构 → v7.1 §A2
>    - §0.3 ForgekinEngine 装饰器模式 → v7.1 §A2/§A5
>    - §0.4 育灵体系命名融合 → v7.1 §A1/§A6
>    - §0.5 roleagent 七大工程路径架构落地 → v7.1 §A5
>    - §0.6 forgemind 应用层架构 → v7.1 §A2/§A3
>    - §0.7 三方 Agent 集成架构 → v7.1 §A4
>    - §0.8 Plugin V3 四钩子 → design.md §0.8
>    - §0.9 v6.0 模块映射到 v7.0 → design.md §0.7
>    - §0.10 v7.0 设计态声明 + 文档导航 → v7.1 §A7 + design.md §D8
>
> 2. **第十七章：v7.0 新增核心模块**（原 line 2453-2660，共 208 行）→ 归档到 `_archive/arch_v70_ch17_core_modules.md`
>    - 8 个核心模块（FlowForgeSDK / ModelCapabilityProvider / @tool 装饰器 / Guardrails / Agent Handoff / MCP Integration / Declarative Agent / Marketplace）→ 设计契约需根据 v7.1 §A2 三层架构重新评估，部分模块在 v7.1 Layer 1 核心框架层中仍有参考价值。
>
> 3. **v7.0 自我进化 Agent Harness 架构升级**（原 line 6131-7330，共 1200 行）→ 归档到 `_archive/arch_v70_self_evolution.md`
>    - §15 七层架构模型（自进化层作为第 7 层）→ **已被 v7.1 §A2 决策 1 取消**：第 7 层"自进化层"取消独立层级，改为 Harness v2.0 升级（融合到 Layer 1 核心框架层），理由：避免自进化层 ↔ 应用层循环依赖（D-003）。
>    - §16 ForgekinEngine 架构设计 → 设计契约已合并到 `design.md` §D5.8 ForgekinEngine 7 步自进化闭环契约。
>    - §17 SpiritForge Engine 架构 → 设计契约已合并到 `design.md` §D7 自我演进闭环设计规范。
>    - §19 灵议与 A2A 架构 → 设计契约已合并到 `design.md` §D5.11 forgemind API 端点契约。
>    - §21 配置驱动与 Feature Flag → 设计契约已合并到 `design.md` §D5.12 forgemind 配置文件契约。
>    - §22 安全与治理架构 → 设计契约已合并到 `design.md` §D5.9 forgemind 数据模型契约。
>    - §23 ADR-007-01~05 → ADR-007-02（自进化层作为第 7 层）已被 v7.1 §A2 决策 1 取消；其余 ADR（两类智能体分离、Trae Bridge、no-classifier 红线、Provoke 频率硬限）作为历史决策保留在归档文件中。
...
> **版本合并声明**：v7.1 增补章节是 v7.1 重构在架构层的权威更新，**已吸收合并 v7.0 增补章节及任何历史章节**（v7.0 不再作为独立版本存在）。三大主文档 v6.0 历史章节仅作背景资料保留，**不作为开发依据**；开发依据以 v7.1 增补章节（§A0~§A7）+ ADR/Feature 子目录为准。
