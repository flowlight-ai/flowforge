...
| 指标 | 数值 |
|------|------|
| **缺陷总数（DI count）** | **392**（历史 379 + 2026-08-15 第十二轮浏览器实测 +13）｜ ⚠️ 历史仪表盘 Fixed/Closed 口径与 2026-08-11 回归备注矛盾，建议全量重算（见 §五） |
| **加权缺陷指数（DI）** | **1159** ＝ S1×10 + S2×5 + S3×2 + S4×1（历史 1123 + 本轮 +36） |
| 状态：Open | 382（历史 369 + 本轮 13 单全部 Open）｜ ⚠️ 历史 Fixed(待回归)/Closed 口径待核对（见 §五） |
| 状态：Fixed（待回归） | 10（口径待核对，详见 §五） |
| 状态：Closed | 0（口径待核对，详见 §五） |> 更新：2026-08-09「实跑复测轮次」在 HEAD `5144892` 真实运行测试套件后追加 P-08…P-19（12 单，均运行时复现，状态 Open）。旧 7 单（P-01…P-07）字段本轮未改（P-04…P-07 由开发侧转 Fixed 待回归）；仅在 P-02/P-03 追加 `【2026-08-09 复测·实跑】` 观察，未作正式回归判定。
>
...
### P-471 — 提案系统（Proposal System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无提案系统。clowder-ai 实现了完整的线程创建提案机制，支持用户提交提案、批准/拒绝、待处理列表、猫侧提案行为等全生命周期管理。
- **影响**：缺少 AI 智能体主动发起变更的协作通道，用户无法审阅和批准 AI 的提议。

### P-472 — 反思系统（Reflection System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无 AI 反思能力。clowder-ai 实现了 POST /api/reflect 端点，允许 AI 基于存储的记忆通过 LLM 进行反思，可降级到备用模式。
- **影响**：AI 无法基于历史经验进行自我改进和知识沉淀。

### P-473 — 自动梦境系统（Auto-Dream System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无自动梦境机制。clowder-ai 实现了多端点梦境日记管理系统，支持列出日记、获取详情、记录交互、获取 AI 状态等。
- **影响**：缺少 AI 智能体在空闲时段的自主学习和模拟演练能力。

### P-474 — 礼宾/前台服务（Concierge Service）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无前台引导服务。clowder-ai 实现了完整的前台猫管理系统（985 行），支持前台配置、消息路由、线程创建、Triage 计划、调查任务管理等。
- **影响**：新用户缺乏自动引导和智能路由，用户体验差。

### P-475 — 猫咪角色/人格系统（Cat Persona System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无 AI 角色人格系统。clowder-ai 实现了完整的猫咪角色管理系统（1147 行），支持创建、更新、删除角色，管理所有猫咪信息、状态、CLI 配置。
- **影响**：AI 智能体缺乏个性化和角色一致性，交互体验单一。

### P-476 — 跨线程协调机制（Cross-Thread Coordination）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无跨线程协调能力。clowder-ai 实现了跨线程消息传递协调逻辑，支持 active/terminal 阶段管理和内容去重。
- **影响**：多个并发 AI 线程间无法协调，可能导致重复工作或冲突决策。

### P-477 — 会话链管理（Session Chain Management）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无会话链系统。clowder-ai 实现了会话链管理（381 行），支持列出会话、获取详情、手动解封、绑定 CLI 会话 ID 等。
- **影响**：无法管理 AI 会话的生命周期，长会话无法有效组织。

### P-478 — 浏览器推送通知（Push Notification System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无推送通知系统。clowder-ai 实现了浏览器推送通知（355 行），支持注册/取消订阅、测试推送、VAPID 密钥配置。
- **影响**：用户无法在后台接收 AI 任务完成通知，需手动刷新页面。

### P-479 — 积压工作管理（Backlog Management）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无积压工作系统。clowder-ai 实现了完整的积压工作管理（922 行），支持创建积压项、导入特性、自我认领、建议认领、决策处理、租约管理。
- **影响**：缺少 AI 智能体的任务优先级管理和自主工作分配能力。

### P-480 — 门控系统（Gate-Keeping System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无门控系统。clowder-ai 实现了门控持有者所有权验证（gate-keeping-cross-store），确保 issue 跟踪唯一性和跨存储策略决策。
- **影响**：并发工作可能导致冲突和重复，缺少工作项的唯一性保障。

### P-481 — 会话救援机制（Session Rescue Mechanism）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无会话救援机制。clowder-ai 实现了 claude-rescue（77 行），可扫描破损的会话并尝试修复，恢复因错误停滞的会话。
- **影响**：AI 会话崩溃后无法自动恢复，用户需手动重启整个会话。

### P-482 — 知识证据系统（Evidence System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无知识证据系统。clowder-ai 实现了完整的证据管理（800 行），支持语义搜索、批量重新索引、重建证据库、健康检查。
- **影响**：AI 知识检索缺乏可靠性和可验证性，无法确保证据来源可信。

### P-483 — 触感反馈/情绪系统（PawFeel Disposition）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无 AI 情绪反馈系统。clowder-ai 实现了 PawFeel 触感反馈系统（326 行），管理 AI 情绪状态、收件箱、职责配置、动作执行。
- **影响**：AI 智能体缺乏情绪感知和表达，交互体验机械化。

### P-484 — 持球机制（Hold Ball Mechanism）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无"持球"机制。clowder-ai 实现了持球任务管理（265 行），支持基于用户信号或事件触发取消任务，管理任务持有状态。
- **影响**：缺少 AI 任务暂挂和释放的控制机制，任务管理灵活性不足。

### P-485 — 知识流/涌现喂养（Knowledge Feed）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无知识流系统。clowder-ai 实现了知识涌现喂养机制（216 行），支持列出待审核/已处理/已拒绝的知识项，批准/拒绝候选知识，撤销已批准项。
- **影响**：缺少 AI 自主知识积累和审核机制，知识库无法自动生长。

### P-486 — 标签管理系统（Labels System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无标签管理系统。clowder-ai 实现了完整的标签 CRUD（103 行），支持创建、读取、更新、删除线程标签，删除时自动清理引用。
- **影响**：线程和内容缺乏分类和筛选能力，查找效率低。

### P-487 — 知识提炼系统（Distillation System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无知识提炼系统。clowder-ai 实现了蒸馏机制（111 行），支持标记证据项为通用可推广、提名候选项、审批/拒绝，将特定知识抽象为通用经验。
- **影响**：AI 无法从具体经验中提炼通用知识，知识复用率低。

### P-488 — 值班简报系统（Duty Briefing）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无值班简报系统。clowder-ai 实现了值班简报（85 行），支持设置/获取值班简报绑定线程，手动触发生成值班简报。
- **影响**：缺少 AI 智能体的值班交接和状态通报机制。

### P-489 — 视角分析系统（Perspectives System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无视角分析系统。clowder-ai 实现了视角分析（187 行），支持使用预定义的视角计划分析问题，通过 featureId 和 slug 运行分析。
- **影响**：无法从多角度分析问题，AI 决策缺乏全面性。

### P-490 — 配置包/集合系统（Packs System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无包/集合系统。clowder-ai 实现了 Packs 系统（155 行），支持安装、列出、移除和导出配置包，基于给定配置打包 AI 配置和相关数据。
- **影响**：AI 配置无法打包分发，环境迁移和共享困难。

### P-491 — 信号学习系统（Signal Study）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无信号学习系统。clowder-ai 实现了信号学习 API（309 行），支持获取文章研究元数据、读取笔记内容、链接/取消链接线程、批量更新文章状态。
- **影响**：AI 无法从外部信号中系统性地学习和积累知识。

### P-492 — 决议系统（Resolution System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无决议系统。clowder-ai 实现了决议管理（resolution-routes.ts），管理决议相关的生命周期。
- **影响**：缺少 AI 智能体的决策记录和决议追踪能力。

### P-493 — 回流系统（Reflux System）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无回流系统。clowder-ai 实现了回流机制（reflux-routes.ts），管理数据回流和处理流程。
- **影响**：体验数据无法反馈到系统优化中，闭环学习能力缺失。

### P-494 — 服务生命周期管理（Service Lifecycle）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
- **现象**：FlowForge 无服务生命周期管理。clowder-ai 实现了完整的服务生命周期系统（services-lifecycle-*），包括端口管理、锁机制、审计路由、帮助函数等。
- **影响**：缺少 AI 服务的注册、发现和健康管理能力。

### P-495 — 锚定遥测系统（Anchor Telemetry）
- **严重度**：S3 ｜ **分类**：功能缺失（clowder-ai 对比） ｜ **状态**：Open
...
## 五、第十二轮 浏览器端到端实测缺陷索引（2026-08-15）

> 测试人员（QA）：真实浏览器 E2E，遵循 T8 铁律（真实 Chromium + 真实 DOM 验证）。
> 环境：后端 `uvicorn app.main:app --port 8000`（已启动）、前端 `next dev --port 5174`（已启动）。
> 复现脚本（仓库内可复现）：`tests/e2e/_manual_browser_e2e.py` → 证据落盘 `/tmp/ff_e2e_report.json`。
> 明细（全字段 + 真实输出）：[`bugs/regression-browser-e2e-2026-08-15.md`](bugs/regression-browser-e2e-2026-08-15.md)
> 编号区间：P-528 … P-540（全仓连续，不与历史重复）。全部 13 单状态 `Open`，`T7/T8 = 是（T8）`。
...
**本轮严重度分布**：S2×4、S3×7、S4×2 ｜ **分类分布**：代码缺陷×9、功能缺失（浏览器实测）×4
