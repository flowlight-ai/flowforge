...
**症状**：v7.0 自进化层在第 7 层（应用层之上），但应用层又通过 PluginProtocol 注册可进化智能体角色，构成循环依赖（D-003）。

**规避**：自进化层作为"Harness v2.0 升级"嵌入到第 6 层，而非独立第 7 层。forgemind 应用层通过 Plugin 协议注册可进化智能体，单向依赖核心框架层。
...
**规避**：CapabilityProfile（能力画像，社区社交称"能力画像"）必须同时写"必杀技"和"致命弱点"。盲点决定了谁该 review 谁、谁和谁组队会翻车。
...
**症状**：把"光秃秃的 LLM 包装"当作可进化智能体。

**规避**：可进化智能体必须建立与现实之间的闭环（观察 → 推理 → 行动 → 写回 → 验证）。物理形态可进化智能体必须接入传感器，虚拟形态可进化智能体必须有世界设定层。
...
**规避**：三方 Agent 的能力画像被纳入可进化智能体能力画像融合，执行状态可写入共享状态，失败有 fallback 链，执行轨迹纳入 Eval 信号。
...
**症状**：可进化智能体自我演进修改了 VISION.md §7。

**规避**：VISION.md §7 的 7 条原则只能由 operator 修改，可进化智能体不能触碰。
...
**症状**：调用三方 Agent 后可进化智能体能力画像未更新。

**规避**：三方 Agent 执行后，能力画像通过 ExternalAgentCapabilityFusion 融合到可进化智能体主画像。
...
**规避**：M1-M17 模块映射到 v7.0 FR-EVO 系统组件，不新增 M18/M19/M20。详见 `[doc:design/naming-contract.md#6]` 废弃清单。
...
本文件由可进化智能体在每个 Bug 修复后自动追加教训条目，格式：
...
- 简短清晰（每条 < 200 字）
- 严格遵循 `[doc:design/naming-contract.md]` v2.0 命名规范
...
| 版本 | 日期 | 变更 | 作者 |
|------|------|------|------|
| v1.0 | 2026-07-17 | 初版：10 大类 38 个 TIP | Trae CN（agent） |
| v1.1 | 2026-07-19 | 按"官方名称优先"原则重构：将大量出现的 P2 体系别名替换为 P0 官方名称（可进化智能体 / CapabilityProfile / ForgeMind 等）；首次出现双标注；TIP-038 重写为指向命名契约 v2.0；TIP-036/TIP-037 简化历史变迁说明 | Trae CN（agent） |
