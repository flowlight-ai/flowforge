...
> **关联 VISION**: [doc:VISION.md#4]（灵智体相对其他 multi-agent 的核心优势）、[doc:VISION.md#6]（operator 原则第 5 条：命名最终为"灵智"）

## 1. 上下文

### 1.1 问题陈述

flowlight-ai/flowforge 新仓库当前通过 `default_llm_actors.py` 固定角色（如"你是内容创作者"），违反 `[doc:roleagent.md#第0章]` 核心主张："role-agent 是蒸汽马车式误判"——给灵智体分配岗位（PM/Dev/Test）是把新物种用旧坐标系测量。

需要实现 CapabilityProfile（能力画像）机制，让灵智体（Forgekin）不再被固定成岗位槽位，而是基于能力画像 × 任务画像的动态匹配路由。这是 operator 7 条原则中第 5 条（命名最终为"灵智"，禁止废弃术语）的落地基础——能力画像必须使用灵印 SoulImprint / 灵忆 EchoStore 等项目正式术语。

### 1.2 当前痛点

- `default_llm_actors.py` 硬编码"你是内容创作者"（违反编程红线第 10/11 条）
- 路由基于角色而非能力，无法动态匹配
- 跨厂商 review 不基于盲点画像（同厂商灵智体共享盲点）
- 无历史表现积累机制，灵智体重启即失忆
...
- **积累层**：historical_performance（历史表现，写入灵忆 EchoStore）
- **瞬时层**：current_state（当前状态）

CapabilityProfile 是灵智体的**长期主体**（灵印 SoulImprint），role 是**运行时标签**——每次任务可变，不复用 profile。
...
class CapabilityProfile(BaseModel):
    ...
    def has_blind_spot_conflict(self, other: "CapabilityProfile") -> bool:
        """检查与另一个灵智体的盲点是否冲突（用于跨厂商 review 配对）"""
        ...
```
...
- [ ] AC-A2: CapabilityRouter 基于能力匹配路由（不基于角色），路由算法延迟 < 100ms（10 个候选灵智体）
...
- [ ] AC-B3: E2E 测试 — 创建 5 个不同厂商灵智体（DeepSeek/Qwen/GLM/Kimi/HunYuan），给定任务画像，验证路由到最佳灵智体
...
| 风险 | 缓解 |
|------|------|
| 能力画像过时 | Eval 信号实时刷新，灵忆 EchoStore 持续积累 |
| 盲点识别不准 | 跨厂商 review 信号回流，七类归因矩阵定位 |
| 路由算法偏见 | 算法可解释性 + 灵议 MindCouncil 审查 |
| 实现复杂度高 | 分阶段：Phase 1 常量+变量层，Phase 4 补齐积累层 |
| 能力画像数据库维护成本 | 每灵智体一份画像，通过灵印 SoulImprint 持久化 |
...
| KD-3 | 跨厂商 review 基于盲点不重叠 | 同厂商灵智体共享训练偏差 | 2026-07-17 |
| KD-4 | 使用项目正式术语（灵印 SoulImprint / 灵忆 EchoStore） | operator 原则第 5 条 | 2026-07-17 |
...
- Phase A: 单元测试 + 集成测试通过，CapabilityProfile 模型 + CapabilityRouter 由架构师灵智体 review
- Phase B: E2E 测试由跨厂商 reviewer 灵智体 review，路由正确率与盲点检出率达标
...
| **VISION** | `docs/VISION.md#4` | 灵智体相对其他 multi-agent 的核心优势 |
| **VISION** | `docs/VISION.md#6` | operator 原则第 5 条（命名最终为"灵智"） |
