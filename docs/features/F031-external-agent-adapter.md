...
ExternalAgentAdapter 是 roleagent.md 第 1 章明确的"agent 交接协议与路由"扩展——编码灵智体与三方 Agent 的协作关系，不会因为单个模型更聪明而消失。
