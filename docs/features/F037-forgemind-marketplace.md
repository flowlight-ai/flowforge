...
Forgekin市场（ForgeMind Marketplace）是 forgemind 应用层的流通机制：用户可分享/订阅/交易自己锻造的Forgekin，如"我锻造的写作Forgekin"、"我锻造的代码审查Forgekin"、"我锻造的家猫Forgekin"。本 Feature 实现Forgekin上架/订阅/交易、能力画像摘要展示、与 F028 锻造产物联动、与 F036 forgemind/*Forge 关系联动，让Forgekin可跨用户共享。

这是 Build to Persist 基础设施——编码"Forgekin可流通"的工程规则，对标前期智能体共享生态。

## 2. 动机（Motivation）

`[doc:review/review.md#FM-007]` 指出：v7.0 有 plugin marketplace 但无 forgekin marketplace，Forgekin无法跨用户共享。可进化智能体需要市场机制：用户可分享/订阅/交易自己锻造的Forgekin。这是与动物养育范式（前期验证）愿景对标的生态差异——前期动物形态智能体可分享，FlowForge 的Forgekin也应可流通。
...
class MarketplaceListing(BaseModel):
    """市场上架条目"""
    listing_id: str
    seller_forgekin_id: str                      # 上架Forgekin ID
    ...
    soul_imprint_hash: str                       # SoulImprint哈希（身份锚点，不可篡改）
...
class MarketplaceSubscription(BaseModel):
    ...
    cloned_forgekin_id: str                      # 克隆到订阅者的Forgekin ID
...
class MarketplaceTrade(BaseModel):
    ...
    price_tokens: int
    traded_at: datetime
    soul_imprint_transferred: str                # SoulImprint转移记录
```
...
class ForgekinCloner(ABC):
    ...
    async def transfer_ownership(
        ...
        """交易时所有权转移（SoulImprint随之转移）"""
        ...
```

### 3.3 关键算法

- **克隆保留血缘**：订阅时克隆Forgekin，原SoulImprint作为"父SoulImprint"写入克隆体的血缘字段，克隆体生成新SoulImprint。血缘链写入 F038 进化谱系。
- **交易转移所有权**：交易时SoulImprint随之转移（原 operator 失去控制权），但原 operator 保留一份"已交易"记录供追溯。
- **能力画像摘要展示**：上架时自动从 F001 CapabilityProfile 生成摘要（擅长 + 盲点 + 历史表现 Wilson 下界），不暴露完整画像。
- **SoulImprint不可篡改**：上架时记录 soul_imprint_hash，交易/订阅时校验哈希一致，防止Forgekin被篡改后流通。
...
forgekin_marketplace:
  ...
  subscription:
    ...
    new_soul_imprint: true                       # 克隆体生成新SoulImprint
    write_to_lineage: true                       # 写入 F038
  trade:
    transfer_soul_imprint: true                  # SoulImprint转移
...
- [ ] AC-1: 上架条目字段完整（含SoulImprint哈希、能力摘要、进化阶/觉醒阶）
- [ ] AC-2: 订阅时克隆保留血缘，生成新SoulImprint
- [ ] AC-3: 交易时SoulImprint转移，原 operator 保留交易记录
- [ ] AC-4: SoulImprint哈希校验防止篡改
...
- 上架校验、克隆血缘、交易转移、SoulImprint哈希校验、搜索排序。
...
- 真实 operator A 锻造写作Forgekin（F028 完成六阶段），上架到市场。真实 operator B 订阅，验证克隆体血缘正确、新SoulImprint生成。再触发交易，验证SoulImprint转移、A 保留交易记录、B 获得控制权。**遵守 T1-T8**：真实 LLM、真实数据、真实工具调用。
