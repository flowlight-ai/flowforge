...
> **关联 VISION**: [doc:VISION.md#2]（万物灵智体形态分类）、[doc:VISION.md#6]（operator 原则第 1/2/4 条）

## 1. 上下文

### 1.1 问题陈述

flowlight-ai/flowforge 新仓库当前缺少应用层——万物灵智体愿景无处落地。operator 指示（2026-07-17）：

> flowforge 中需要新增一个 forgemind 模块，其是 flowforge 的应用层项目（用来实践万物锻造灵智体的应用）...forgemind 将是我们 flowforge 的育灵 ForgeNurturing 的所有代码存放的地方

这是 operator 7 条原则中第 1 条（万物灵智体世界是最终形态）、第 2 条（必须有现实闭环）、第 4 条（forgemind 是应用层）的落地。

### 1.2 当前痛点

- 万物灵智体愿景无处落地
- FlowForge 自我演进缺少练兵场（违反 operator 原则第 6 条）
- 通用灵智体（动物/物品/虚拟角色）与垂直业务灵智体（*Forge）混淆
...
forgemind 是 `flowforge/forgemind/` 子目录（不是独立项目），承载万物灵智体应用实践（育灵 ForgeNurturing）。通过 ForgeMindPlugin 注册到核心框架层（Plugin V3 协议四钩子）。

三层架构明确划分：
- **核心框架层**（`flowforge/` 除 forgemind）：提供 ForgeMindEngine 自进化核心
- **应用层**（`flowforge/forgemind/`）：育灵 ForgeNurturing，养公共通用灵智体
- **垂直业务层**（`contentforge/` 等）：各 *Forge 养垂直领域灵智体
...
class ForgekinSpecies(str, Enum):
    """灵智体形态分类（5 种，详见 F027）"""
    BIO = "bio"               # 生物灵智体 BioForgekin
    ORG = "org"               # 组织灵智体 OrgForgekin
    OBJ = "obj"               # 物品灵智体 ObjForgekin
    VIRTUAL = "virtual"       # 虚拟灵智体 VirtualForgekin
    HYBRID = "hybrid"         # 混合灵智体 HybridForgekin


class EvolutionStage(str, Enum):
    """觉醒阶 E1-E6（最终形态为灵智 ForgeMind）"""
    E1_DORMANT = "E1_dormant"
    E2_AWAKEN = "E2_awaken"
    E3_SENSE = "E3_sense"
    E4_ACT = "E4_act"
    E5_EVOLVING = "E5_evolving"
    E6_FORGEMIND = "E6_forgemind"   # 灵智（最终形态，非"E6 灵匠 Mind Artisan"）
...
class ForgekinBase(BaseModel):
    """灵智体基类（必须建立现实闭环：observe → act → verify）"""
    forgekin_id: str
    soul_imprint: str               # 灵印 SoulImprint（身份标识）
...
- [ ] AC-B1: 可创建 5 种形态灵智体（Bio/Org/Obj/Virtual/Hybrid）
- [ ] AC-B2: ForgePipeline 可完成锻造流程（E1 → E2）
- [ ] AC-B3: ForgekinMarketplace 可查询灵智体
- [ ] AC-B4: ForgekinLineage 可记录进化谱系
- [ ] AC-B5: 锻造流水线延迟 < 5s（不含 LLM 调用）
- [ ] AC-B6: E2E 测试 — 锻造猫灵智体（BioForgekin）+ 桌椅灵智体（ObjForgekin）+ 孙悟空灵智体（VirtualForgekin），3 个灵智体协作完成一个任务
- [ ] AC-B7: 灵智体必须建立现实闭环（observe → act → verify，operator 原则第 2 条）
- [ ] AC-B8: 遵守 T1-T8 测试铁律（真实 LLM 调用、真实场景数据、不跳过验证、不 Mock 工具、采集完整指标、LLM 生成内容经 LLM 审核、Web 功能操控浏览器验证 DOM）

## 4. 依赖

- **Evolved from**: 无
- **Blocked by**: F001（CapabilityProfile 灵智体能力画像）、F002（TeamAct 多灵智体协作）、Plugin V3 协议（核心框架层）
- **Related**: F027（万物灵智体形态分类）、F028（灵智体锻造流水线）、F029（物理 AI 传感器接入）、F030（虚拟世界设定层）、F036（forgemind 与 *Forge 关系）、F037（灵智体市场）、F038（灵智体进化谱系）

## 5. 风险

| 风险 | 缓解 |
|------|------|
| forgemind 与 *Forge 边界模糊 | 明确 forgemind 只养通用灵智体，*Forge 养垂直领域灵智体 |
| Plugin V3 协议需核心层重构 | Phase 1 优先实现 V3 协议四钩子 |
| 形态进化产生意外行为 | F027 流程 + Eval 把关 + 灵议 MindCouncil 审查 |
| 物理传感器接入需硬件支持 | Phase 6+ 才接入真实硬件，前期用模拟传感器 |
| forgemind 代码量大（5000+ 行） | 分阶段：Phase 2 骨架 + Phase 5 自我演进 + Phase 6 灵锻 |

## 6. Open Questions

| # | 问题 | 状态 |
|---|------|------|
| OQ-1 | 5 种形态灵智体的觉醒阶 E1-E6 是否统一标准，还是按形态差异化？ | ⬜ 未定 |
| OQ-2 | forgemind 灵智体的灵印 SoulImprint 是否需要与 *Forge 灵智体命名空间隔离？ | ⬜ 未定 |
...
| KD-3 | 灵智体必须建立现实闭环 | operator 原则第 2 条（observe → act → verify） | 2026-07-17 |
| KD-4 | 使用项目正式术语（育灵 ForgeNurturing / 灵议 MindCouncil / 灵锻 SpiritForge / ForgeMindEngine）