...
FlowForge 当前缺少应用层——可进化智能体（即多形态智能体 Multi-Form Agent）愿景无处落地。operator 指示（2026-07-17）：

> flowforge 中需要新增一个 forgemind 模块，其是 flowforge 的应用层项目（用来实践锻造可进化智能体的应用）...forgemind 将是我们 flowforge 的 Forge Nurturing（智能体入职与终身学习，原"养灵"）的所有代码存放的地方

### 1.2 当前痛点

- 可进化智能体愿景无处落地
- FlowForge 自我演进缺少练兵场
- 通用Forgekin与垂直业务Forgekin（*Forge）混淆
...
forgemind 是 `flowforge/forgemind/` 子目录，承载可进化智能体应用实践。通过 ForgeMindPlugin 注册到核心框架层（Plugin V3 协议）。
...
class ForgekinSpecies(str, Enum):
    ...
    ORG = "org"               # 组织Forgekin
    OBJ = "obj"               # 物品Forgekin
    VIRTUAL = "virtual"       # 虚拟Forgekin
    HYBRID = "hybrid"         # 混合Forgekin
...
class EvolutionStage(str, Enum):
    ...
    E6_FORGEMIND = "E6_forgemind"  # ForgeMind（最终形态）
...
- Forgekin必须建立现实闭环（observe → act → verify）
...
├── sensors/                      # 物理传感器
├── worlds/                       # 虚拟世界设定
├── marketplace/                  # Forgekin市场
├── lineage/                      # 进化谱系
├── codex/                        # MindCodex 蒸馏知识库
├── council/                      # MindCouncil 多智能体议事
├── config/                       # 配置
└── tests/
```
...
- 依赖 ADR 005 + ADR 013
- 依赖 F001 CapabilityProfile（Forgekin能力画像）
- 依赖 F002 TeamAct（多 Forgekin协作）
...
- [ ] AC-1: 可创建 5 种形态Forgekin（Bio/Org/Obj/Virtual/Hybrid）
- [ ] AC-2: ForgePipeline 可完成锻造流程（E1 → E2）
- [ ] AC-3: ForgeMindPlugin 通过 Plugin V3 四钩子注册
- [ ] AC-4: ForgekinMarketplace 可查询Forgekin
...
- [ ] AC-9: 5 种形态Forgekin全部通过 E2 觉醒
- [ ] AC-10: 可进化智能体 demo（猫 + 桌椅 + 孙悟空）端到端跑通
...
- 锻造一只猫Forgekin（BioForgekin）
- 锻造一个桌椅Forgekin（ObjForgekin）
- 锻造一个孙悟空Forgekin（VirtualForgekin）
- 3 个Forgekin协作完成一个任务
...
- trace 信号：锻造流水线日志
- 用户信号：Forgekin功能反馈
- 探针信号：5 种形态Forgekin可创建性

### 6.5 评估后做什么

- 通过 → Forgekin进入 E3 感知阶
- 失败 → 归因到七类矩阵
...
forgemind 是 operator 通用智能体（General-Purpose Agent）愿景的实践场，是 FlowForge 自我进化的练兵场。模型越强，Forgekin形态分类 + 锻造流水线越值钱。
...
- forgemind 可能与 *Forge 边界模糊（缓解：明确 forgemind 只养通用Forgekin）
...
### 9.1 方案 A: 可进化智能体放独立项目

- 优点：FlowForge 核心纯粹
- 缺点：forgemind 失去 FlowForge 自我演进滋养
- 未选择原因：operator 明确 forgemind 是应用层

### 9.2 方案 B: 可进化智能体分散到 *Forge

- 优点：复用 *Forge 框架
- 缺点：通用Forgekin与垂直业务Forgekin混淆
...
| 日期 | 版本 | 变更 | 变更者 |
|------|:----:|------|--------|
| 2026-07-17 | v0.1 | 初始创建 | operator + 架构师 Forgekin |
