...
forgemind 应用层（F026）需要将"养灵"过程从"配置 persona"升级为系统化育灵（Forge Nurturing）流水线。A028 已固化 6 阶段流水线架构，本详细设计在 `forgemind/forging/` 落地具体实现，解决以下工程层问题：
...
- **单向依赖约束**：`forgemind/forging/` 必须单向依赖 `flowforge/core/` 中的 F001/F008/F014/F018 + `forgemind/species/`（F027），禁止 `import contentforge`/`devforge`/`novelforge`/`mallforge`。
...
- **对 F014 多域记忆的影响**：③阶段 `MemorySeedHandler` 调用 `EchoStoreRepository.append()` 写入初始灵忆条目。
...
- **对 F037 灵智体市场的影响**：⑥阶段觉醒晋升后灵智体可由 `ForgekinMarketplace.publish()` 发布到市场。
...
class SpeciesDefineHandler(StageHandler):
    """① 形态定义：加载 SpeciesProfile + 创建灵印 MindImprint。"""
...
stages:
  ...
  # ③ 记忆初始化：自动执行，写入 F014 EchoStore 初始灵忆
  memory_seed:
    handler_class: forgemind.forging.handlers.memory_seed.MemorySeedHandler
    require_operator_approval: false
...
③阶段 `MemorySeedHandler` 调用 `EchoStoreRepository.append()` 写入初始灵忆。
...
#### 4.2.4 影响 F039 灵典可检索知识库

③阶段初始灵忆可作为灵典种子条目，通过 `MindCodex.index(echo_entry_id)` 索引。
...
- [ ] AC-E-06: Eval 信号回流到 F018，影响灵智体能力画像可靠性评估。
...
| IT-D028-015 | 完整流水线后 `ForgekinMarketplace.publish()` 调用 | 灵智体发布到市场（可选） |
...
- [doc:../design/naming-contract.md]（育灵 Forge Nurturing + 灵印 MindImprint + 觉醒阶 AwakeningStage）
...
| 日期 | 版本 | 变更 | 变更者 |
|------|:----:|------|--------|
| 2026-07-19 | v0.1 | 初始创建（6 阶段 StageHandler 实现 + DI 注入 + YAML 配置加载 + 审批门 + Eval 硬门 + 回滚机制详细设计） | 架构师灵智体（猫头鹰·鲁班） |
