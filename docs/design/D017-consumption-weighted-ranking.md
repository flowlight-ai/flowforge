...
Forgekin（Evolvable Agent，社区社交称"可进化智能体"）在记忆检索中需要判断"知识价值"。v7.0 靠向量相似度 + 时间衰减，导致两类问题：冷启动偏热点（新条目无消费数据被埋底）、LLM 自评失真（模型说自己好）。A017 架构设计已确认 L4 消费排序层实现 14 行为指标汇聚 + 调整后得分公式 + 贝叶斯收缩 + 中心化偏移 + 分数时效衰减。
