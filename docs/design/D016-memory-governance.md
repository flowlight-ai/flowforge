...
Forgekin（Evolvable Agent，社区社交称"可进化智能体"）在执行任务时检索记忆，旧记忆与新记忆一视同仁导致三类问题：权威倒挂（候选观察压过铁律）、触发失效（铁律未注入 system role）、僵尸知识（3 年前过时决策与今天最新决策同权重）。A016 架构设计已确认 L2 治理层形式化"权威性 / 触发方式 / 生命周期"三要素，让检索结果在 RRF 融合后经过权威硬序、触发过滤、生命周期衰减三步治理。
