...
| `packages/llm/llm` + `llm-deepseek` + `llm-pi-ai` + `llm-retry` + `token-meter` | `packages/llm/*` | vendor；追加 openroute provider（P: `forgemind/openroute_adapter.py`） | 🟦（D 系列 vendor ✅；openroute provider 批次50 交付） |
...
| `src/services/stores/*`（ports: ThreadStore/MessageStore/TaskStore/BacklogStore/MemoryStore/…） | `packages/cats/stores` + `packages/cats/stores-sqlite` | vendor；better-sqlite3 实现，双栈物理隔离（R18） | 🟦（ports 22 store + sqlite 核心 9 store 已交付；其余 13 个 sqlite store 批次52 补全） |
...
| `src/domains/cats/*`（档案路由/技能包/记忆发布/任务积压） | `packages/cats/routes` | vendor，路由按域插件挂载 | 🟦（批次55：先与已交付 chat-misc C22/chat 系服务做语义 diff，只翻译剩余挂载面） |
...
| `.cat-cafe/` 运行态 JSON（accounts.json / user-preferences.json / proxy-upstreams.json / provider-profiles 迁移） | `data/` + `~/.flowforge/` | 格式对齐（JSON），改名 ff2 域（R17） | 🟦（批次27 accounts/user-preferences 已随 cats-catalog 交付；批次46 cat-config-loader runtime-json 支持 5 kind；proxy-upstreams/provider-profiles 两 kind 批次52 补全） |
...
| `forgemind/*_to_openroute_proxy.py` + `anthropic_to_openroute_proxy.py` | `packages/llm/openroute` | 直接翻译为 provider | 🟦（批次50 交付） |
