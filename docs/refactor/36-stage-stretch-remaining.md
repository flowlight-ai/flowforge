...
| S5-2c | 完整引擎状态机（WerewolfLobby + GameEngine + GameOrchestrator 动作分派） | S5-2a/S5-2b | 待引擎移植 |
...
| 2 | S5（先 S5-1） | pure 层即时动工；引擎后续需 LLM | ✅ S5-1 已交付（2026-09-19）；**S5-2a seam 已交付 + S5-2b LLM runtime 已交付（2026-09-20）**；S5-2c 完整引擎依 S5-2b |
