...
> 内容与来源编号见 `review_code.md` §13.2；⚠ 项（Q1/Q3/Q5/Q6）需 EP1 前裁决，均已闭环（2026-09-16），EP1 无阻塞裁决项。

| 序 | 任务 | 来源 | 预估批次 | 状态 |
|---|---|---|---|---|
| 1 | `mcp-server` 整包（工具治理全家 + 6 toolsets + protocol-server）⚠Q3 | B1 | 2 | 🟩（EP1-1a 治理框架本体 + registry + snapshot + protocol-server 落 `packages/mcp/mcp-server`，PR #163；**EP1-1b 领域工具集迁移**落 `src/toolsets/`——B1 底座 + B2 范式确立已完成（2026-09-15）：注入式 `CallbackTransportPort` + authority 推导 + `defineMcpToolsetTools` 接入 EP1-1a 装配机制，collab 两小组（capability-evolution-change/entrusted-work-read）迁入确立 catalog 范式，`canonical-tool-sources.ts` 规模锚 `TOOLSET_GROUP_ANCHOR`，契约测试 3 文件 11/11 全绿、包级 tsc exit 0、oxlint 0、零 @cat-cafe 依赖；**EP1-1b 四家族全迁完（2026-09-16，B3-B6）**：collab 19 组 90 工具（含 callback 大组 49）/ memory 11 组 20 工具 / signals 2 组 12 工具 / limb 1 组 6 工具，合计 33 工具组 **128 工具**，`TOOLSET_GROUP_ANCHOR` 四家族规模锚齐备，契约测试 7 文件全绿（mcp-server 全包 13 文件 80/80）、包级 tsc exit 0、oxlint 0、零 @cat-cafe/@deepseek-ai/@clowder 运行时依赖；finance/audio 依 Q3 剔除） |
...
| 序 | 任务 | 状态 |
|---|---|---|
| 1 | Python 遗留目录日落与删除（`agents/`、`brain/`、`core/`、`llm/`、`loop/`、`forgemind/`、`evolution/`、`harness/`、`sop/`、`sdk.py` 等） | 🟩（**EP4 S11.1 冻结期完成 + S11.2 归档完成** 2026-09-16：`__main__.py`（`python -m flowforge`）S11.1 已补 DEPRECATED 冻结横幅；start.bat 阶段10 已切 TS 栈并标注 deprecated。**S11.2 归档已执行**：operator 2026-09-16 提前放行 P2（TS 默认入口稳定 ≥2 周未满仍执行「仅归档不删除」），Python 旧版运行时代码经 `git mv` 归档至 `python/legacy/`（agents/brain/core/llm/loop/forgemind/evolution/harness/sop/scheduler/session/memory/events/executor/services/observability/compiler/middleware/modes/mcp/a2a/review/security/app/cli/evaluators/workflows/skills/tools/vcs + 根 `sdk.py`/`__init__.py`/`__main__.py`/`pyproject.toml`/`requirements.txt`/`ruff.toml`/`py.typed` + `tests/`(pytest) + `scripts/*.py`；`config/`、`data/`、`web/`、`packages/`、`apps/` 保留原位）。pytest 基线快照落 `python/legacy-pytest-baseline-2026-09-16/`；根 `pyproject.toml` 已改指向 `python/legacy`。**S11.3 删除待归档后 ≥2 个发布迭代，本轮不执行**） |
| 2 | stretch 项按裁决排期（S1 凭据启用 / S2 TTS/邮件推送 / S4 desktop / S6 Python SDK 等） | ✅（**EP4-stretch 排期表** 2026-09-10：`31-stage11-sunset.md` §7 补 S1-S7 排期表——现阶段均不排期（缺凭据/服务或产品优先级不足），随 operator 新指令准入；S1 凭证接线后启用） *(2026-09-19 按批准计划 `35-stage-stretch-batch.md` 推进三张「待裁决 stretch」：**S-B21 assets 已实现**（`rest-controllers` 新增 `AvatarsController`——JSON/base64 上传 + 魔数嗅探 + 体积限制 `AVATAR_RAW_FILE_LIMIT_BYTES` + 静态头像清单；`AudioProxyController`——JSON 代理 + 显式 `audioServiceUrl` 配置门控否则 502 + lease 心跳；SSE `/api/audio/events` 明确 501 归 S2 平台路由），128 包级 vitest 全绿 / tsc exit 0 / oxlint 0，PR #202 已提交。S-S1（飞书真实 IImChannelAdapter）与 S-A13（code-runtime-python 内联移植）为后续批次）。 **S-S1 飞书真实通道已实现**（2026-09-19，决策 §4-2 配置探测→未配置降级）：`stretch-ports` 新增 `src/feishu/*` 三件套（`feishu-config.ts` 配置探测 / `feishu-im-channel.ts` `FeishuImChannelAdapter` 真实飞书 OpenAPI v2 + 未配置自身降级 / `feishu-connector-outbound.ts` 结构兼容 `IOutboundAdapter` 桥接入 connector gateway），`ChatStretchService` 增 `feishu` 选项 + `feishuChannel` getter 覆盖缺省 lark mock。26 包级 vitest 全绿 / tsc(host+client) exit 0 / oxlint 0。 **S-A13 code-runtime-python 已实现**（2026-09-19，决策 §4-1 内联最小必需契约）：`@flowforge/code-runtime-python` 落 `packages/code-runtime/code-runtime-python`——`PythonCodeRuntime extends CodeRuntime`（`language='python'/isolation='process'`），每次运行起全新 CPython 子进程，经 fd3（host→child 读）/fd4（child→host 写）分隔管道桥接 binding（规避 Windows 单 fd3 双向管道死锁），wall-clock/output-bytes 预算 + terminate-on-abort/timeout，`spawn` 清空 env，`py/bootstrap.py`+`py/protocol.py` 随包分发；36 包级 vitest 全绿（含真实 python 3.13 集成）/ tsc exit 0 / oxlint 0 / 无 forbidden 依赖；lib/ 已 gitignore。三张 stretch 批次全部收口）* **S4 desktop 契约层 S4-1 已交付**（2026-09-19，`36-stage-stretch-remaining.md` §1）：新包 `@flowforge/desktop`（`packages/apps/desktop`，clowder desktop 契约层、Electron 后置）——env 注入式 config / bridge 契约 / 包内不变式 / InMemory 总线 / 编排器；21 包级 vitest / tsc exit 0 / oxlint 0；随 PR 提交）* **S5 games 首子项 S5-1 已交付**（2026-09-19，`36-stage-stretch-remaining.md` §2/§5 顺序 S4→S5）：新包 `@flowforge/cats-games`（`packages/cats/games`）忠实移植 clowder `game-command-interceptor.ts`（`parseGameCommand`/`sanitizeCatIds`/`buildGameSeats` + 常量词汇 + `clampToPreset`）+ `src/schema.ts` 移植 `seatSchema`/`gameStartSchema` + `parsedGameCommandSchema`/`safeParseParsedGameCommand`（zod v4），复用 `@flowforge/cats-shared` `Seat`，零 LLM/传输依赖、注入式纯函数；30 包级 vitest 全绿 / tsc exit 0 / oxlint 0；S5-2 引擎状态机（WerewolfLobby/GameOrchestrator）后续依赖 LLM seam；随 PR 提交）** S5-2a LLM 注入 seam 已交付**（2026-09-20）：`@flowforge/cats-games` `src/llm/` 三件套 + 测试双例——`GameAIProvider` 端口（`generateAction`/`generateSpeech`）+ `assertAIActionResponse` 不变式守卫（`GameLlmInvariantViolation`）+ `buildWerewolfPrompt` 纯函数（移植 clowder werewolf-prompts 全角色）+ `GameWerewolfAIPlayer` 适配器（decideNightAction/decideSpeech/decideVote/decideSpeechWithFormat）+ `NoopGameAIProvider`/`ScriptedGameAIProvider` 测试双例；57 包级 vitest（新增 27）/ tsc exit 0 / oxlint 0；完整引擎 S5-2c 待 LLM runtime；随 PR 提交）* **S5-2b LLM runtime 具体注入已交付**（2026-09-20）：`@flowforge/cats-games` `src/llm/` 三件具体实现（移植 clowder `LlmAIProvider.ts` HTTP 传输、去除内部配置栈）——`llm-config.ts`（`LlmProviderKind`/`LlmConfigResolver` 注入端口/env 探测 `resolveLlmConfigFromEnv` + `assertLlmProviderKind` + `GameLlmConfigError` + `GAME_LLM_TIMEOUT_MS=10s`）+ `http-ai-provider.ts`（`HttpGameAIProvider`：注入 resolveConfig/fetch(timeoutMs，四协议分支 + AbortController 超时 + `parseActionResponse` markdown fence 容错 + assert 守卫）+ `llm-runtime.ts`（`GameLlmRuntime`：按 catId memo `providerFor`/`playerFor` 懒构造）；23 新增 vitest（config 8+http 11+runtime 4），update 后总 80 / tsc exit 0 / oxlint 0；完整引擎 S5-2c（WerewolfLobby+GameEngine+GameOrchestrator）依赖此 runtime；随 PR 提交）* |
| 3 | P1/P2 遗漏项收尾（A5-A7、A11、A13、A15-A16、A18-A19、A21-A23、A25-A28、A30-A31、B2、B6、B8-B9、B11、B13-B17、B19-B21、A35） | 🟩（**EP4-P12 收尾批次完成** 2026-09-10：可立即实施项全量落包并提交（PR #171 21 新包 + 独立提交 17b3a2a1 B9/B11 收尾，tsc+oxlint+377 vitest 全绿。**diff 类闭环**：B8 `skill-meta`/`skill-query` 补建为 `forgekin/governance`（12 vitest），`skill-manage` 挂载/级联内联进 governance-bootstrap 与 capabilities；B9 剩余纯工具归位为 `packages/util/stdlib`（@flowforge/util-stdlib 7 工具 30 vitest），`ndjson-parser`→limb/terminal cli，`loopback-request`/`owner-gate`→forgekin/capabilities write-guards；B11 mcp 同步补建为 capabilities 原生（`mcp-drift-detector.ts`+`mcp-drift-resolver.ts`，11 vitest 全绿）。**剩余裁决项已按推荐默认收口**：A11 agent-team→暂缓 stretch、A13 →随 S6 stretch、A35 文档站→暂缓 stretch、B2 finance→不纳入（剔除 finance/audio）、B19+B8 技能内容→按命名契约分批迁移核心包（见 review_code §15 Q1/Q3/Q4/Q5）。**B19 wave 1 追加**（2026-09-11，实例 `b19-skill-content-wave1`）：deep-research/expert-panel/cross-cat-handoff/debugging 四核心包内容资产迁入 `packages/forgekin/governance/skills/`（SKILL.md 原样 + manifest.yaml 路由 + README 命名映射），契约测试 `b19-skill-content.spec.ts` 3/3 全绿、tsc exit 0、oxlint 0。**B19 wave 2 追加**（2026-09-11，实例 `b19-skill-content-wave2`）：开发流程域 tdd/writing-plans/quality-gate/merge-gate 四核心包迁入 `packages/forgekin/governance/skills/`（manifest.yaml 扩至 8 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave2.spec.ts` 3/3 全绿、tsc exit 0、oxlint 0（16 files）。**B19 wave 3 追加**（2026-09-11，实例 `b19-skill-content-wave3`）：评审与协作域 fresh-context-review/request-review/receive-review/receive-handoff-grounding 四包迁入同目录（manifest.yaml 扩至 12 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave3.spec.ts` 3/3 全绿、tsc exit 0、oxlint 0（17 files）。**B19 wave 4 追加**（2026-09-11，实例 `b19-skill-content-wave4`）：协作与编排域 thread-orchestration/cross-thread-sync/collaborative-thinking/custody-recognition 四包迁入同目录（manifest.yaml 扩至 16 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave4.spec.ts` 3/3、wave1-4 合计 12/12 全绿、tsc exit 0、oxlint 0（18 files）。**B19 wave 5 追加**（2026-09-11，实例 `b19-skill-content-wave5`）：工程生命周期与工具链开发流程域 feat-lifecycle/worktree/co-creation-docs/owner-friendly-plugin-development 四包迁入同目录（manifest.yaml 扩至 20 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave5.spec.ts` 3/3、wave1-5 合计 15/15 全绿（governance 全量 46/46）、tsc exit 0、oxlint 0（19 files）。**B19 wave 6 追加**（2026-09-13，实例 `b19-skill-content-wave6`）：开发/调试域与能力进化/会话护栏域 code-as-harness/writing-skills/self-evolution/context-self-management/knowledge-engineering 五包迁入同目录（manifest.yaml 扩至 25 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave6.spec.ts` 3/3、wave1-6 合计 18/18 全绿（governance 全量 49/49）、tsc exit 0、oxlint 0。期间修复 `readSkillMeta` frontmatter 正则对 CRLF 工作树的容错（core.autocrlf=true 在 Windows checkout 转 CRLF，正则升级为 `---\r?\n`）。**B19 wave 7 追加**（2026-09-13，实例 `b19-skill-content-wave7`）：工程审计/响应与能力进化/前端交付/企业协作域 source-audit/incident-response/capability-evolution/console-dev/enterprise-workflow 五包迁入同目录（manifest.yaml 扩至 26 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave7.spec.ts` 3/3、wave1-7 合计 18/18 全绿（governance 全量 49/49）、tsc exit 0、oxlint 0。含 `readSkillMeta` frontmatter 正则对 CRLF 工作树容错（core.autocrlf=true 在 Windows checkout 转 CRLF，正则升级为 `---\r?\n`）。**B19 wave 8 追加**（2026-09-13，实例 `b19-skill-content-wave8`）：技术写作/引导/评测/浏览器域 tech-writing/guide-authoring/guide-interaction/eval-design/browser-automation/browser-preview 六包迁入同目录（manifest.yaml 扩至 26 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave8.spec.ts` 3/3、wave1-8 合计 21/21 全绿（governance 全量 49/49）、tsc exit 0、oxlint 0。含 `readSkillMeta` frontmatter 正则对 CRLF 工作树容错（`---\r?\n`）。**B19 wave 9 追加**（2026-09-13，实例 `b19-skill-content-wave9`）：记忆/检索/组织域 memory-navigation/memory-search-best-practices/proactive-memory-judgment/organize-threads/schedule-tasks/workspace-navigator 六包迁入同目录（manifest.yaml 扩至 26 包路由、README 追加命名映射），契约测试 `b19-skill-content-wave9.spec.ts` 3/3、wave1-9 合计 24/24 全绿（governance 全量 49/49）、tsc exit 0、oxlint 0。含 `readSkillMeta` frontmatter 正则对 CRLF 工作树容错（`---\r?\n`）。**B19 wave 10 追加**（2026-09-13，实例 `b19-skill-content-wave10`）：协作与生产域 rich-messaging/convention-graph-discovery/concept-demo-design/open-source-teardown/opensource-ops/agent-product-promo-director/bootcamp-guide/hyperfocus-brake/vision-rescue 九包迁入同目录（manifest.yaml 扩至 29 包路由、README 追加命名映射、`readSkillMeta` CRLF 容错、concept-demo-design/agent-product-promo-director 补 triggers），契约测试 `b19-skill-content-wave10.spec.ts` 3/3、governance 全量 49/49 全绿、tsc exit 0、oxlint 0。**B19 wave 12 追加**（2026-09-13，实例 `b19-skill-content-wave12`）：技能正文 `../.cat-cafe-shared-refs/<file>` 共享资产目录收尾——clowder `refs/` 35 份参考资产迁入 `packages/forgekin/governance/skills/.cat-cafe-shared-refs/`（LF + 无 BOM），契约测试 `b19-shared-refs-wave12.spec.ts` 校验每个 SKILL.md 相对引用均落盘可解析，`skill-meta.ts` 增 CRLF 容错（`/^---\r?\n/`），governance 契约测试 8 文件 48/48 全绿、包级 tsc exit 0、oxlint 0。**B19 wave 11 追加**（2026-09-13，实例 `b19-skill-content-wave11`）：创意与生产工具域 anime-forge/video-forge/sprite-forge/ppt-forge/image-generation/pencil-design/ttfund-skills 七包迁入同目录（manifest.yaml 扩至 27 包路由、README 追加命名映射、`readSkillMeta` CRLF 容错、五包补 triggers），契约测试 `b19-skill-content-wave11.spec.ts` 3/3、governance 全量 49/49 全绿、tsc exit 0、oxlint 0。EP4 工作线程已收口） |
