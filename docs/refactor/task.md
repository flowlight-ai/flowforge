...
| 3 | P1/P2 遗漏项收尾（A5-A7、A11、A13、A15-A16、A18-A19、A21-A23、A25-A28、A30-A31、B2、B6、B8-B9、B11、B13-B17、B19-B21、A35） | 🟦（**EP4-P12 收尾批次进行中** 2026-09-10：可立即实施项全量落包并提交 PR #171（新 21 包，tsc+oxlint+377 vitest 全绿，见 review_code §4。**diff 类已清点**：B8 缺 `skill-meta`/`skill-query` 已补建为 `forgekin/governance` 的 `skill-meta.ts`/`skill-query.ts`（readSkillMeta/parseManifestSkillMeta/resolveSkillMcpStatuses/listSkills/querySkill，12 vitest），`skill-manage` 的挂载/级联能力已内联进 governance-bootstrap 与 capabilities、不强复制；B9 剩余纯工具已归位为 `packages/util/stdlib`（@flowforge/util-stdlib：readJsonlTail/normalizeJsonUnicode/normalizeErrorMessage/initRepoIdentity-isSameRepo/tcpProbe/tokenizeKeyword-scoreKeywordRelevance/validateExternalUrl-resolveExternalUrl-fetchExternalUrlPinned，30 vitest，tsc+oxlint 全绿；`ndjson-parser`→limb/terminal cli，`loopback-request`/`owner-gate`→forgekin/capabilities write-guards），B11 mcp 同步已补建为 capabilities 原生（`mcp-drift-detector.ts`+`mcp-drift-resolver.ts`：detect→resolve→recheck 零 drift round-trip，11 vitest 全绿）。需裁决项（A11/A13/B2/B19/A35）与 B8 skill 级 drift 待 operator 指令或门槛） |
