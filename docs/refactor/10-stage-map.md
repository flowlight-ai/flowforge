...
| C38 | .cat-cafe 运行态 JSON（accounts/user-preferences/proxy-upstreams/provider-profiles） | data/ + ~/.flowforge | 4 | ✅（accounts/user-preferences/cat-catalog 模型批次27 cats-catalog 已交付；批次46 补 runtime-json 存储：accounts/capabilities/user-preferences/mcp-resolved 四文档 zod 严格校验 + tmp+rename 原子写 + 注入式 fs（缺失 null/非法拒绝），`RuntimeJsonStore`） |
