...
本 ADR 是 P1 决策，术语对齐项目正式命名（详见 `[doc:decisions/012-naming-fusion.md]`）：智能体主体称为灵智体（Forgekin），跨厂商群体审议称为灵议（MindCouncil），伙伴系统数学是 Forgekin 协作的"算盘"。

---

## 2. 决策

伙伴系统数学由四个独立模块构成，分别承担 roleagent.md 第7章的四个工程直觉公式。所有模块位于 `flowforge.core.partnership` 包下，依赖仅限 `flowforge.core.errors` 与 `flowforge.core.tracing`，可单测、可被灵议 MindCouncil 在运行时调用。
...
- 关键边界：任一门 `pass_probability == 0` → 整条链路坍塌为 `lower_bound=0.0`，该门记入 `failed_gates`；其余通过的门仍记入 `passed_gates`，便于灵议 MindCouncil 归因到具体门
...
选路策略遵循 roleagent.md 第7章"这只 agent 有没有带来其他 agent 不会自然带来的视角"的判断——`UpperBoundCalculator` 不替模型思考"视角是否互补"，只把每个候选的净期望摆出来，由调用方（通常是被灵议 MindCouncil 包装的 `CapabilityRouter`，见 `[doc:decisions/004-capability-profile-routing.md]`）决定是否扩展候选集。
...
| 方案 | 优点 | 缺点 |
|------|------|------|
| **方案 A（选定）: 四模块独立 + 确定性公式 + LLM-free** | 可单测、可被灵议 MindCouncil 运行时调用、不依赖任何 provider；公式与 roleagent.md 第7章一一对应；模块互不耦合，可独立演进 | 公式偏简化（连乘门假设近似独立），不能完全建模盲点相关性；需要上层 eval 补齐 |
...
- 术语对齐 `[doc:decisions/012-naming-fusion.md]`：Forgekin 协作数学以灵议 MindCouncil 为仲裁面，而非人类路由器（人类作为方向锚点而非消息总线）
...
| 上限 `max` 在候选极少时退化为单点，与单 agent 无差别 | 由 `CapabilityRouter` 负责扩展候选集；`UpperBoundResult.explanation` 暴露候选数与最佳路径细节供灵议 MindCouncil 审查 |
...
| 四模块均不调用 LLM，无法处理语义模糊的"门"或"候选" | 设计如此：模糊判断留给灵智体 Forgekin，数学模块只做确定性的"算盘"——符合 roleagent.md"harness 给数据不给结论"原则 |
...
- **方案 D（数学塞进路由层）**：违反项目规则铁律3"禁止绕过 DI 容器直接实例化"与编程红线第9条"禁止用继承替代组合/插件"；数学模块需要被 eval 控制面与灵议 MindCouncil 独立调用，耦合路由层将阻断可观测性
...
- 架构师灵智体（方案设计 + 公式落地 + 术语对齐项目正式命名）
- 灵议 MindCouncil（运行时仲裁面，负责调用四模块并归因）
...
| 日期 | 修订 | 修订者 |
|------|------|--------|
| 2026-07-21 | 初始版本，确立伙伴系统数学四模块决策（UpperBoundCalculator / LowerBoundCalculator / VarianceAbsorber / TokenLedger），术语对齐项目正式命名（灵智体 Forgekin / 灵议 MindCouncil） | operator + 架构师灵智体 |
...
- `[doc:decisions/012-naming-fusion.md]` — 命名融合（灵智体 Forgekin / 灵议 MindCouncil）
