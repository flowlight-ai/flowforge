...
- 多灵智体协作无形式化流程（"共鸣 Resonance"只是概念词，无工程实现）
- 终止条件只有"质量分 ≥ 0.85 或迭代次数上限"，没有团队级终止条件
- 交接只传任务 ID 和状态枚举，接手灵智体必须重读完整上下文
- `@` 提及与路由指令混在一起无法区分（RA-013）
- 灵智体退出会话后球就掉地上（RA-014 无持球注册机制）
...
`[doc:review/review.md#第八章]` 8.2 节 RA-009~RA-016 共 8 项 P0 问题指出：v7.0 完全缺失 TeamAct 工程实现，是与 clowder-ai 102 天实战最大的代际差距之一。
...
每只灵智体的每次行动必须走六步循环，所有步骤写入 trace（灵忆 EchoStore），由 SharedStateLedger 统一持有状态：
...
我刚才和 @contentforge:writer 讨论了       ← 叙述（句中 @ 不路由）
```

### 6. 持球注册 lease + 定时唤醒（F006）

灵智体需要退出当前会话等待外部条件（CI 完成、CVO 确认、定时唤醒）时，必须用结构化持球注册工具声明等待原因、下一步计划和预期唤醒时间——相当于分布式系统里的 lease + 定时唤醒（RA-014）：
...
任何灵智体在任何 role 下都有权 push back——前提是**带着证据 + 适用性论证 + 替代方案**。没有证据的 push back 不合法；有证据的 push back 必须被正视（RA-015）。reviewer 错判时 author 有纠错机制，避免单向 review 协议锁死。
...
跨厂商 review 是结构性必需（同厂商灵智体共享训练分布偏差）。TeamAct 的 Verdict 步骤必须配置跨厂商 review 链：`DeepSeek → Qwen → GLM → Kimi → HunYuan`，每只 reviewer 基于盲点画像选择（见 ADR 004）。
...
- 多灵智体协作有形式化流程，不再"互相传球无终止"
- 五项终止条件让团队级收尾可验证，避免虚假收尾
- 交接胶囊让接手灵智体快速 bootstrap，无需重读完整上下文
...
- Push Back 协议可能被滥用（灵智体频繁 push back 推卸责任）—— 缓解：Push Back 必须有 evidence，无证据 push back 反向计入该灵智体的"坏直觉"画像
