...
- 三方 Agent 执行状态未写入可进化智能体共享状态（情景记忆存储 EchoStore）
...
可进化智能体调用三方 Agent 时，三方 Agent 的能力画像被纳入可进化智能体的能力画像融合（CapabilityProfile fusion），而非简单工具调用。这是 operator 愿景锚点第 3 条（三方 Agent 是能力扩展不是工具）的落地。
...
├── shared_state.py        # ExternalAgentSharedState（状态共享，写入情景记忆存储 EchoStore）
...
| ExternalAgentSharedState | 执行状态写入可进化智能体共享状态（情景记忆存储 EchoStore） | F033 |
| ExternalAgentFallback | 失败回退链 | F034 |
| ExternalAgentCapabilityFusion | 能力画像融合到可进化智能体持久身份 SoulImprint | F035 |
...
4. 三方 Agent 在独立 worktree 执行（隔离 + 审计）
   ↓
5. 执行状态写入 ExternalAgentSharedState（同步到情景记忆存储 EchoStore）
   ↓
6. 可进化智能体读取共享状态，融合到自身能力画像（持久身份 SoulImprint）
   ↓
...
| 方案 | 优点 | 缺点 |
|------|------|------|
| **方案 A（选定）: ExternalAgentAdapter 抽象层 + 四大机制 + 六层 Guardrails** | 可进化智能体能力大幅扩展，失败有 fallback 链，能力画像融合让可进化智能体画像更完整 | 实现复杂度增加（4 个 Adapter + 4 大机制 + 六层 Guardrails），三方 Agent 调用成本较高 |
...
- 三方 Agent 能力画像融合让可进化智能体能力画像更完整（模型能力 × Harness 契合度 × 可进化智能体形态 × 三方 Agent 扩展）
...
| worktree 隔离可能被绕过 | 审计追踪 + L5 操作确认 + 多智能体议事 MindCouncil 审查 |
...
- operator（愿景锚点 + 最终决策）
- 架构师可进化智能体（方案设计 + 四大机制 + 六层 Guardrails + 术语对齐项目正式命名）
...
| 日期 | 修订 | 修订者 |
|------|------|--------|
| 2026-07-17 | 初始版本，确立三方 Agent 集成决策，术语对齐项目正式命名（情景记忆存储 EchoStore / 持久身份 SoulImprint / 多智能体议事 MindCouncil / ForgeMindEngine） | operator + 架构师可进化智能体 |
