...
> **日期**: 2026-07-17
> **决策者**: operator（不可委托）
> **依赖**: `[doc:VISION.md]` + `[doc:roleagent.md]` + `[doc:clowder-ai/docs/VISION.md]`
...
### 1. 万物灵智体世界为 FlowForge 最终形态

FlowForge 不是一个"多 agent 框架"，也不是"LLM 应用脚手架"，而是一个**灵智体锻造厂**。
...
灵智体可通过积累经验进化形态（如 BioForgekin → HybridForgekin）。形态进化通过 F027 流程触发。

### 4. 通用 AGI 三条路径

1. **物理 AI 路径**：IoT 传感器 + 物理执行器 + 物理现实闭环
2. **虚拟 AI 路径**：虚拟世界设定层 + 角色关系图谱 + 世界观约束
3. **混合路径**：组织灵智体调度多种形态灵智体
...
- 灵智体形态分类让 agent 有持续身份、形态、谱系，区别于主流 multi-agent
...
- 优点：实现简单，复用 clowder-ai 已有经验
- 缺点：无法达成 operator 通用 AGI 愿景
- 未选择原因：operator 明确要求万物灵智体世界
...
- `[doc:clowder-ai/docs/VISION.md]` — Cat Café 愿景（养小猫 → 养万物范式迁移）
