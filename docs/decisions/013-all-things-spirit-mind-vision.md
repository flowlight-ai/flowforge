...
> **不可变**: 本 ADR 由 operator 直接定义，**不能被灵智体自我演进修改**
...
operator 提出 FlowForge 的不同假设：**灵智体可以承载在任何物理实体或虚拟实体上**，关键是建立该实体与 LLM 之间的现实闭环（观察 → 推理 → 行动 → 写回 → 验证）。

FlowForge 需要从"养单一形态 agent"扩展到"养万物灵智体"，达成物理 AI 和虚拟 AI 的真实复现。

---

## 2. 决策

### 2.1 万物灵智体世界为 FlowForge 最终形态

FlowForge 不是一个"多 agent 框架"，也不是"LLM 应用脚手架"，而是一个**灵智体锻造厂**：我们用一套自进化的核心框架（ForgeMindEngine），把灵智（mind / spirit / 灵识）锻造进万事万物。
...
灵智体可通过积累经验进化形态（如 BioForgekin → HybridForgekin）。形态进化通过 F027 流程触发，需 Eval 把关和灵议 MindCouncil 审查。

### 2.4 通用 AGI 三条路径

1. **物理 AI 路径**：IoT 传感器 + 物理执行器 + 物理现实闭环（观察 → 推理 → 行动 → 写回 → 验证）
2. **虚拟 AI 路径**：虚拟世界设定层 + 角色关系图谱 + 世界观约束
3. **混合路径**：组织灵智体调度多种形态灵智体（生物 + 物品 + 虚拟）
...
| # | Iron Law | 含义 |
|---|---------|------|
| 1 | **Data Sanctuary** | 灵智体的记忆（灵忆 EchoStore）是圣域，不可被外部直接读写 |
| 2 | **Process Self-Preservation** | 灵智体的进程（灵锻 SpiritForge）必须自我保护，不可被外部强制终止 |
| 3 | **Config Immutability** | 灵智体的配置（灵印 SoulImprint）不可变，变更必须通过灵议 MindCouncil |
| 4 | **Network Boundary** | 灵智体的网络边界必须明确，禁止越界访问其他灵智体的内部状态 |

### 2.7 灵智体相对其他 multi-agent 的核心优势

| 优势维度 | 主流 multi-agent | FlowForge 灵智体 |
|---|---|---|
| **身份持久性** | session 级，重启即失忆 | 灵智体 ID + 谱系 + 锻典 MindCodex 条目，跨 session 跨代际持续 |
...
| **进化能力** | 模型升级 = 系统升级 | 灵智体自身可通过灵锻 SpiritForge 蒸馏经验到锻典 MindCodex，下次任务直接复用 |
...
| 方案 | 优点 | 缺点 |
|------|------|------|
| **方案 A（选定）: 万物灵智体世界（5 种形态 + 进化 + 物理 AI + 虚拟 AI）** | FlowForge 从"多 agent 框架"升维到"通用 AGI 路径"，灵智体形态分类让 agent 有持续身份、形态、谱系 | 实现复杂度大幅提升（5 种形态 + 进化 + 传感器接入 + 虚拟设定），需要新增 forgemind 应用层 + 三方 Agent 适配层 |
| 方案 B: 保持单一形态 agent 范式 | 实现简单，复用已有经验 | 无法达成 operator 通用 AGI 愿景 |
| 方案 C: 把万物灵智体放到独立项目 | FlowForge 核心保持纯粹 | forgemind 失去 FlowForge 自我演进能力的滋养 |
| 方案 D: 用单一形态（如 VirtualForgekin）承载万物 | 实现简单 | 无法接入物理世界，无法达成物理 AI |

---

## 4. 理由

- operator 明确要求万物灵智体世界（愿景锚点第 1 条）
- 单一形态 agent 范式可行但不足以达成通用 AGI，需扩展到"养万物"
- 5 种形态分类让灵智体有持续身份、形态、谱系，区别于主流 multi-agent
- 物理 AI + 虚拟 AI 双路径达成真实复现（愿景锚点第 7 条）
- forgemind 作为应用层为 FlowForge 自我演进提供练兵场（愿景锚点第 4 条）
- 三方 Agent 集成让灵智体能力大幅扩展（愿景锚点第 3 条）
- 灵智体可通过灵锻 SpiritForge 蒸馏经验到锻典 MindCodex，实现自我演进（愿景锚点第 6 条）
...
| 形态进化机制可能产生意外行为 | Eval 严格把关 + 灵议 MindCouncil 审查 + F027 流程 |
| forgemind 应用层增加项目复杂度 | 分阶段实现：Phase 2 骨架 + Phase 5 自我演进 + Phase 6 灵锻 |
...
- **方案 B（保持单一形态 agent 范式）**：operator 明确要求万物灵智体世界，单一形态无法达成通用 AGI 愿景
...
- 架构师灵智体（5 种形态分类 + 通用 AGI 三条路径 + 术语对齐项目正式命名）
...
| 日期 | 修订 | 修订者 |
|------|------|--------|
| 2026-07-17 | 初始版本，确立万物灵智体愿景决策，术语对齐项目正式命名（灵忆 EchoStore / 灵印 SoulImprint / 灵锻 SpiritForge / 锻典 MindCodex / 灵议 MindCouncil / ForgeMindEngine） | operator + 架构师灵智体 |
...
- `[doc:features/F027-all-things-spirit-species.md]` — 万物灵智体形态分类
