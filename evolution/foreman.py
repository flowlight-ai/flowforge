...
任务源（按优先级）：
1. operator 提交的显式任务（通过 IM 议事通道）
2. SteerCommand 触发的紧急任务（priority=emergency）
3. Eval Ledger 失败信号（trace 信号 + 探针信号）
4. task.md 中 ⏳/🔄 状态的任务
...
- I2 单一 foreman：全局唯一（通过 create_foreman 工厂保证）
- I3 任务不丢失：所有任务通过 SwarmCoordinator.submit_task 落盘 trace
...
logger = get_logger("flowforge.evolution.foreman")


# ── 常量 ──────────────────────────────────────────────────────────
DEFAULT_LOOP_INTERVAL_SECONDS = 60.0  # 默认循环间隔
DEFAULT_TASK_SCAN_LIMIT = 5  # 单次扫描最多处理的任务数
EMERGENCY_POLL_INTERVAL_SECONDS = 5.0  # 紧急任务轮询间隔


@dataclass
class ForemanConfig:
    """Foreman 配置（从 YAML 加载，铁律 5）."""
...
class ContinuousForeman:
        ...
        try:
            while not self._stop_requested:
                try:
                    ...
                    text = await self._magic_words_callback("")  # type: ignore
                    if not text:
                        continue
    ...
    async def _start_task_execution(self, task_id: str) -> None:
        ...
        - doc → runtime.run_doc_loop
        - code → runtime.run_code_loop
        - framework → runtime.run_framework_loop（含 I8 approval）
        - review → runtime.run_review_loop
        - test → runtime.run_test_loop

        Args:
            task_id: SwarmTask.task_id
        """
        if self._swarm is None:
            return

        task = self._swarm.get_task(task_id)
        if task is None:
            self._logger.warning(f"任务不存在: {task_id}")
            return

        if task.assigned_agent_id is None:
            self._logger.warning(f"任务未分配 agent: {task_id}")
            return

        # 检查并发上限
        if len(self._running_tasks) >= self._config.max_concurrent_tasks:
            self._logger.info(
                f"达到并发上限 {self._config.max_concurrent_tasks}，"
                f"任务 {task_id} 将在下次循环重试"
            )
            return

        # 从 task.context 提取 loop_type（决定路由到哪个 SelfDev 闭环）
        loop_type = task.context.get("loop_type", "")
        if not loop_type:
            # 根据分配的 agent 推断 loop_type
            loop_type = self._infer_loop_type(task.assigned_agent_id)
        ...
        async def _execute():
            try:
                ...
                # 发送心跳（任务开始）
                if self._swarm is not None:
                    await self._swarm.heartbeat(
                        task.assigned_agent_id,  # type: ignore[arg-type]
                        task_id=task_id,
                        progress=0.1,
                    )
                ...
                context["forgekin_id"] = task.assigned_agent_id

                result = await self._route_to_loop(loop_type, context)
                ...
                self._logger.info(
                    f"任务完成: id={task_id}, loop_type={loop_type}, "
                    f"agent={task.assigned_agent_id}"
                )

            except Exception as e:
                self._logger.exception(
                    f"任务执行失败: id={task_id}, loop_type={loop_type}, error={e}"
                )
                self._stats.total_tasks_failed += 1

                # 更新任务状态为 failed
                if self._swarm is not None and task_id in self._swarm._tasks:
                    self._swarm._tasks[task_id].status = "failed"
                    self._swarm._tasks[task_id].failure_reason = str(e)
                    self._swarm._tasks[task_id].retry_count += 1

        # 启动执行 task
        asyncio_task = asyncio.create_task(
            _execute(), name=f"swarm-task-{task_id}"
        )
    ...
    async def _route_to_loop(
        ...
        """
        if loop_type == "doc":
            return await self._runtime.run_doc_loop(context)
        elif loop_type == "code":
            return await self._runtime.run_code_loop(context)
        elif loop_type == "framework":
            return await self._runtime.run_framework_loop(context)
        elif loop_type == "review":
            return await self._runtime.run_review_loop(context)
        elif loop_type == "test":
            return await self._runtime.run_test_loop(context)
        else:
            raise ValueError(f"未知 loop_type: {loop_type}")

    def _infer_loop_type(self, agent_id: str) -> str:
        ...
        """
        # 提取短名（forgemind:wenxin → wenxin）
        short_id = agent_id.split(":")[-1] if ":" in agent_id else agent_id
        ...
        return mapping.get(short_id, "doc")  # 默认 doc
    ...
    def _cleanup_completed_tasks(self) -> None:
        ...
        for task_id in completed:
            ...
            # 检查是否有异常（已在 _execute 中处理，这里只清理引用）
            if asyncio_task.exception() and not asyncio_task.cancelled():
                self._logger.debug(
                    f"清理异常完成的 task: {task_id}, "
                    f"exception={asyncio_task.exception()}"
                )
    ...
    async def submit_operator_task(
        self,
        title: str,
        description: str,
        loop_type: str,
        ...
        task_data = {
            ...
            "required_capabilities": [],  # 由 forgekin_id 推断
            "loop_type": loop_type,
        ...
        # 紧急任务走 emergency_queue（立即处理）
        if priority in ("critical", "emergency"):
            await self._emergency_queue.put(task_data)
            self._logger.warning(
                f"operator 提交紧急任务: title={title}, priority={priority}"
            )
        else:
            self._submit_to_swarm(task_data)
            self._logger.info(
                f"operator 提交任务: title={title}, priority={priority}"
            )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "total_emergencies": self._stats.total_emergencies,
            "last_loop_at": self._stats.last_loop_at.isoformat()
            if self._stats.last_loop_at
            else None,
            "last_task_at": self._stats.last_task_at.isoformat()
    ...
    def get_swarm_workload(self) -> dict[str, int]:
        ...
        if self._swarm is None:
            return {}
        return self._swarm.get_agent_workload()

    # ══════════════════════════════════════════════════════════════
    # §6 工具方法
    # ══════════════════════════════════════════════════════════════

    def _create_swarm_coordinator(self) -> Any:
        """创建 SwarmCoordinator 实例（懒加载，避免 import 时硬依赖）.
        ...
        try:
            ...
            for agent_id, agent_cfg in agents_config.items():
                coordinator.register_agent(
                    ...
                    vendor=agent_cfg.get("vendor", "unknown"),
                )

            self._logger.info(
                f"SwarmCoordinator 创建: agents={list(agents_config.keys())}"
            )
            return coordinator

        except ImportError as e:
            self._logger.error(f"无法导入 SwarmCoordinator: {e}")
    ...
    def _import_swarm_task(self) -> Any:
        """懒导入 SwarmTask（避免 import 时硬依赖）."""
        from flowforge.forgemind.swarm import SwarmTask

        return SwarmTask


class _MockSwarmCoordinator:
    """简易 SwarmCoordinator mock（仅用于 SwarmCoordinator 导入失败时降级）.

    不实现完整 Swarm 协议，仅保证 foreman 不崩溃.
    """
    ...
    def submit_task(self, task: Any) -> str:
        """提交任务（仅记录日志）."""
        task_id = getattr(task, "task_id", f"mock-{uuid.uuid4().hex[:8]}")
