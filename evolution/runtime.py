"""生产环境 SelfDev 启动入口模块 — F046 §3.1 Phase 5 + F045 桥接的生产装配点.

负责：
1. 加载 trae_bridge.yaml 配置创建 TraeLLMClient
2. 加载 5 个 forgekin YAML 配置（wenxin/sherlock/luban/vangogh/davinci）
3. 注入 approval_callback（基于 ApprovalHub + F047 IM 议事通道）
4. 创建 ForgeMindEngine + 5 个 SelfDev 闭环实例并注册

遵守铁律：
- 红线 11：禁止硬编码路径/密钥/端口，所有路径从 config 读取
- 红线 12：禁止绕过 DI 容器直接实例化，所有依赖通过构造函数注入
- 铁律 5：配置外置到 YAML，不在 .py 中硬编码
...
from flowforge.forgemind.forgekins.roster import BUILTIN_FORGEKINS, load_forgekin_config
...
logger = get_logger("flowforge.evolution.runtime")
