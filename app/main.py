...
app.add_middleware(
    ...
    # use wildcard origins with credentials disabled — this allows the
    # Forgekin Council chat UI WebSocket to connect from any origin.
    # Production deployments should set explicit origins + allow_credentials=True.
    allow_origins=["*"],
...
try:
    from flowforge.app.api.endpoints import council as council_endpoints
    app.include_router(council_endpoints.router)
except ImportError as _e:
    logger.warning(f"council router not loaded: {_e}")

# ForgeMind Council Chat — static web UI (index.html + app.js + style.css)
# Served from web_legacy_backup/static/ so the chat page is reachable at
# http://localhost:8000/ without running a separate frontend dev server.
try:
    from pathlib import Path as _Path
    from fastapi.staticfiles import StaticFiles as _StaticFiles
    from fastapi.responses import HTMLResponse as _HTMLResponse
    _web_static_dir = _Path(__file__).resolve().parent.parent / "web_legacy_backup" / "static"
    if _web_static_dir.exists():
        app.mount("/static", _StaticFiles(directory=str(_web_static_dir)), name="web_static")

        @app.get("/", response_class=_HTMLResponse, include_in_schema=False)
        async def _serve_chat_ui() -> str:
            """Serve the Forgekin Council chat page at /."""
...
# was written against the legacy web app which served API paths under /api/*
# (without the /api/v1/ prefix). To let those tests run unmodified against the
# modular app, we expose thin alias routes that forward to the council and
# verify handlers.
#
# Aliases provided:
#   GET  /api/agents       → council.list_agents
#   POST /api/chat         → council.send_message
#   POST /api/verify/t7    → verify.verify_t7  (also mounted under /api/v1/verify)
#   GET  /api/verify/t8    → verify.verify_t8
try:
    from flowforge.app.api.endpoints import council as _council_compat
    from flowforge.app.api.endpoints import verify as _verify_compat
    ...
    # Mount the verify router under /api as well (it already has prefix="/verify",
    # so this exposes /api/verify/t7 and /api/verify/t8 in addition to /api/v1/*).
