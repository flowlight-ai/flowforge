...
async def list_frontend_plugins():
    ...
    if not plugin_loader.frontend_registry:
        return {"plugins": []}
    return {"plugins": plugin_loader.frontend_registry.get_all_plugins()}
...
async def get_plugins_for_mount(mount_point: str):
    ...
    plugins = plugin_loader.frontend_registry.get_plugins_for_mount(mount_point)
...
async def get_frontend_plugin(plugin_name: str):
    ...
    plugin = plugin_loader.frontend_registry.get_plugin(plugin_name)
    if not plugin:
