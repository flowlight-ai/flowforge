...
async def get_plugin(plugin_name: str):
    ...
    return {
        ...
        "state": lifecycle_manager.get_state(plugin_name).value if lifecycle_manager.get_state(plugin_name) else "unknown",
...
async def reload_plugin(plugin_name: str):
    ...
    if not lifecycle_manager:
        raise HTTPException(503, "Plugin lifecycle manager not available")
    result = await lifecycle_manager.reload_plugin(plugin_name)
