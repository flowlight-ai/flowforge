...
class LLMClient:
    ...
    async def complete(
        ...
        **kwargs: Any,
        ...
        ``prefer_api`` in kwargs, inject it so OpenRouteProvider forwards
        ``prefer_api=true`` to the OpenRoute gateway — which then skips
        webchat backends and uses API backends directly (avoids multi-minute
        webchat timeouts).
        """
        if self.prefer_api and "prefer_api" not in kwargs:
            kwargs["prefer_api"] = True
