...
class WorkflowExecutor(BaseModeExecutor):
    ...
    async def _execute_intelligent_chat(self, ctx: TaskContext, context_data: dict, is_auto: bool = False) -> dict:
        ...
        is_simple = self._validator.is_simple_message(intent)
        ...
        plan = self._validator.parse_execution_plan(plan_content)
        steps = plan.get("plan", [])
        intent_type = plan.get("intent_type", "chat")

        # ── Fallback: if planner returned empty plan, infer intent from user input ──
        if not steps:
            inferred_type = self._validator.infer_intent_type_from_text(intent)
            ...
            if intent_type not in ("chat",):
                inferred_steps = self._validator.infer_steps_from_intent(intent_type, intent)
                if inferred_steps:
            ...
            if inferred_type != "chat" and inferred_type in ("creation", "research", "write", "code"):
                intent_type = inferred_type
                plan["intent_type"] = inferred_type
                inferred_steps = self._validator.infer_steps_from_intent(inferred_type, intent)
                if inferred_steps:
                    steps = inferred_steps
                    plan["plan"] = steps
        ...
        if not steps:
            ...
            for step in steps:
                ...
                if step_input and step_context:
                    ...
                    if agent_name == "multilingual" and "text" not in merged_input:
                        best_text = self._validator.find_best_text_in_context(step_context)
                        if best_text:
                ...
                if step_type == "tool" or (tool_name and not agent_name):
                    ...
                    if search_failed or search_empty:
                        ...
                        if isinstance(result, dict) and result.get("search_available") is False:
                            ...
                            if result.get("success"):
                                extracted = self._validator.extract_step_content({"step": step_name, "type": "tool", "result": result})
                    ...
                    if result.get("success"):
                        extracted = self._validator.extract_step_content({"step": step_name, "type": "agent", "result": result})
                        if extracted and len(extracted.strip()) > 10:
            ...
            for i, sr in enumerate(step_results):
                extracted = self._validator.extract_step_content(sr)
            ...
            for sr in step_results:
                ...
                if sr.get("agent") in _QUALITY_CHECK_AGENTS:
                    continue
                c = self._validator.extract_step_content(sr)
