...
import type { ToolSchema } from '@flowforge/llm'
import AgentRegistry from '@flowforge/agent'
import type { Agent } from '@flowforge/agent'
import { createScope } from '@flowforge/scope'
import SessionStore, { SessionId } from '@flowforge/session'
import SessionProjectionRegistry from '@flowforge/session-projection'
import SqliteSessionQueryEngine from '@flowforge/session-query-sqlite'
import GoalService from '@flowforge/goal'
import SystemPrompt from '@flowforge/system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@flowforge/tools'
import LocalBashExecutor from '@flowforge/bash-local'
import * as BashEnvPlugin from '@flowforge/shell-env'
import { PwshLocalExecutor } from '@flowforge/pwsh-local'
import LocalSubprocessRuntime from '@flowforge/subprocess-local'
import LocalFileSystem from '@flowforge/fs-local'
import { AttachmentStore } from '@flowforge/attachment'
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@flowforge/attachment'
import UserQuestionService from '@flowforge/user-questions'
import PlanModeController from '@flowforge/plan-mode'
import WebRuntime from '@flowforge/web'
import * as WebSearchExa from '@flowforge/web-search-exa'
import * as WebFetchLocal from '@flowforge/web-fetch-http'
import SubagentRuntime from '@flowforge/subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@flowforge/subagent'
import * as ToolSubagentControl from '@flowforge/tool-subagent-control'
import * as ToolSubagentListAgents from '@flowforge/tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@flowforge/tool-subagent-report'
import SkillRegistry from '@flowforge/skill'
import * as SkillFileSystem from '@flowforge/skill-filesystem'
import LocalJobRegistry from '@flowforge/jobs-local'
import * as ToolAskUser from '@flowforge/tool-ask-user'
import * as ToolBash from '@flowforge/tool-bash'
import * as ToolPwsh from '@flowforge/tool-pwsh'
import * as ToolBashPersistent from '@flowforge/tool-bash-persistent'
import CordisHostRunner from '@flowforge/cordis-host-runner'
import * as ToolCordis from '@flowforge/tool-cordis'
import * as ToolFs from '@flowforge/tool-fs'
import * as ToolFsSearch from '@flowforge/tool-fs-search'
import * as ToolStrReplaceEditor from '@flowforge/tool-str-replace-editor'
import TerminalSessionService from '@flowforge/terminal'
import * as ToolPty from '@flowforge/tool-terminal'
import * as ToolGoal from '@flowforge/tool-goal'
import * as ToolSchedule from '@flowforge/schedule'
import Lsp from '@flowforge/lsp'
import * as ToolLsp from '@flowforge/tool-lsp'
import * as ToolSkill from '@flowforge/tool-skill'
import * as ToolSessionQuery from '@flowforge/tool-session-query'
import * as ToolTasks from '@flowforge/tool-jobs'
import * as ToolTodo from '@flowforge/tool-todo'
import * as ToolSubagent from '@flowforge/tool-subagent'
import * as ToolWeb from '@flowforge/tool-web'
import VmWorkflowEngine from '@flowforge/workflow-worker-thread'
import * as ToolRalph from '@flowforge/tool-ralph'
import * as ToolWorkflow from '@flowforge/tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@flowforge/tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@flowforge/tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@flowforge/pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `FF_*` environment comes from `@flowforge/shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@flowforge/tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@flowforge/cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@flowforge/tool-bash-persistent',
  ...
  {
    pkg: '@flowforge/tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@flowforge/fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. `read_image` is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@flowforge/tool-fs-search',
    dir: 'tool-fs-search',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@flowforge/lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@flowforge/tool-ralph',
    dir: 'tool-ralph',
  ...
  },
  {
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        flowforgeHome: resolve(root, '.tmp/tool-catalog/.flowforge'),
  ...
  {
    pkg: '@flowforge/tool-session-query',
  ...
  {
    pkg: '@flowforge/tool-subagent-control',
  ...
  {
    pkg: '@flowforge/tool-subagent-report',
