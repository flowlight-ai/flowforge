...
const SESSION_PACKAGE = '@flowforge/session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@flowforge/session/types'
 ...
 * top-level declaration (in `@flowforge/session`) and any declaration
 * merge inside a `declare module '@flowforge/session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @flowforge/session/known-event-types',
    ' */',
    '',
    '/**',
