  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-elevated)]">
      ...
      <div className="px-4 py-3 border-b border-[var(--border)] flex items-center gap-2 flex-shrink-0">
        ...
        <span className="text-sm font-semibold text-[var(--text)]">MCP 服务器</span>
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
        ...
        {servers.map((server) => (
          <div
            key={server.id}
            className="px-4 py-3 border-b border-[color-mix(in_srgb,var(--border)_50%,transparent)] hover:bg-[var(--bg-hover)] transition-colors"
          >
            <div className="flex items-center gap-2.5">
              <StatusDot status={server.status} />
              <span className="text-sm font-medium text-[var(--text)] flex-1 truncate">{server.name}</span>
              <span className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--bg-hover)] text-[var(--muted)] font-mono uppercase">
                {server.transport}
              </span>
            ...
            <div className="mt-1.5 ml-[18px] text-[11px] text-[var(--muted)] font-mono truncate">
              {server.transport === "stdio" ? server.command : server.url}
            </div>
            {server.statusMessage && (
              <div className={`mt-1 ml-[18px] text-[11px] ${server.status === "error" ? "text-[var(--danger)]" : "text-[var(--muted)]"}`}>
                {server.statusMessage}
              </div>
            )}
            {server.env && Object.keys(server.env).length > 0 && (
              <div className="mt-1.5 ml-[18px] flex gap-1 flex-wrap">
                {Object.entries(server.env).map(([k]) => (
                  <span key={k} className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--bg-hover)] text-[var(--muted)] font-mono">
                    {k}
                  </span>
                ))}
              </div>
      ...
      {/* Add/Edit form */}
      {showForm && (
        <div className="border-t border-[var(--border)] bg-[var(--bg-elevated)] p-4 space-y-3 max-h-[60%] overflow-y-auto">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-[var(--text)]">
              {editingId ? "编辑服务器" : "添加服务器"}
            </span>
            <button onClick={resetForm} className="text-[var(--muted)] hover:text-[var(--text)] text-xs">✕</button>
          </div>

          <div>
            <label className="text-xs text-[var(--muted)] block mb-1">名称</label>
            <input
              type="text"
              value={form.name}
          ...
          <div>
            <label className="text-xs text-[var(--muted)] block mb-1">传输类型</label>
          ...
          {form.transport === "stdio" ? (
            <div>
              <label className="text-xs text-[var(--muted)] block mb-1">命令</label>
              <input
                type="text"
                value={form.command || ""}
          ...
          {/* Env vars */}
          <div>
            <label className="text-xs text-[var(--muted)] block mb-1">环境变量</label>
            {form.env && Object.entries(form.env).map(([k, v]) => (
              <div key={k} className="flex items-center gap-1.5 mb-1.5">
                <span className="text-xs font-mono text-[var(--text)] bg-[var(--bg-hover)] px-2 py-1 rounded flex-1 truncate">{k}</span>
                <span className="text-xs font-mono text-[var(--muted)] bg-[var(--bg-hover)] px-2 py-1 rounded flex-1 truncate">{v}</span>
                <button onClick={() => removeEnvVar(k)} className="text-[var(--muted)] hover:text-[var(--danger)] text-xs p-1">✕</button>
              </div>
            ))}
            <div className="flex gap-1.5">
              <input
                ...
                placeholder="KEY"
                className="flex-1 bg-[var(--bg-elevated)] border border-[var(--border)] rounded px-2 py-1 text-xs text-[var(--text)] font-mono focus:outline-none focus:border-[var(--cafe-accent)]"
              />
              <input
                type="text"
                value={envVal}
                ...
                className="flex-1 bg-[var(--bg-elevated)] border border-[var(--border)] rounded px-2 py-1 text-xs text-[var(--text)] font-mono focus:outline-none focus:border-[var(--cafe-accent)]"
              />
              <button onClick={addEnvVar} className="text-xs px-2 py-1 bg-[var(--bg-hover)] text-[var(--muted)] hover:text-[var(--text)] rounded transition-colors">+</button>
            </div>
          </div>
          ...
          <button
            ...
            className="w-full py-2 text-xs font-medium rounded-lg bg-[var(--cafe-accent)] hover:bg-[var(--cafe-accent-hover)] text-[var(--cafe-accent-foreground)] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
