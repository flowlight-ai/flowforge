...
import NewDiffViewer, { computeFileDiff, DiffFile } from "./DiffViewer";
...
// computeDiff has been replaced by computeFileDiff from ./DiffViewer
...
export default function MarkdownPanel({
  ...
  const [primaryTab, setPrimaryTab] = useState<"editor" | "diff" | "changes" | "settings" | "terminal">("editor");
  ...
  const handlePrimaryTabChange = useCallback((tab: "editor" | "diff" | "changes" | "settings" | "terminal") => {
    setPrimaryTab(tab);
  ...
  return (
    <div className="helm-artifact-panel">
      ...
      <div className="editor-primary-tabs">
        ...
        <div className={`editor-primary-tab-wrapper${primaryTab === "changes" ? " active" : ""}`}>
          <div className="editor-primary-tab" onClick={() => handlePrimaryTabChange("changes")} title="全局变更">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M4 2v20l6-4 6 4V2z" /><path d="M8 10h8" /><path d="M8 6h8" />
            </svg>
            {!collapsed && <span className="editor-primary-tab-label">全局变更</span>}
          </div>
          <button className="editor-primary-tab-close" onClick={() => handlePrimaryTabChange("editor")} title="关闭全局变更">✕</button>
        </div>
        ...
        <div className={`editor-primary-tab-wrapper${primaryTab === "terminal" ? " active" : ""}`}>
          <div className="editor-primary-tab" onClick={() => handlePrimaryTabChange("terminal")} title="终端">
            ...
            {!collapsed && <span className="editor-primary-tab-label">终端</span>}
          </div>
          <button className="editor-primary-tab-close" onClick={() => handlePrimaryTabChange("editor")} title="关闭终端">✕</button>
        </div>
        ...
        <div className="editor-primary-more" ref={moreMenuRef} style={{ position: "relative" }}>
          ...
          {moreMenuOpen && (
            <div className="editor-plus-dropdown">
              ...
              <button
                className="editor-plus-dropdown-item"
                onClick={() => { handlePrimaryTabChange("terminal"); setMoreMenuOpen(false); }}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          ...
          {primaryTab === "settings" ? (
            ...
            <div className="settings-in-editor" style={{ padding: 16, overflowY: "auto", height: "100%" }}>
              <div className="settings-section" style={{ marginBottom: 24 }}>
                ...
                <div className="terminal-output" style={{ background: "#1e1e2e", color: "#cdd6f4", fontFamily: "monospace", padding: 12, borderRadius: 6, maxHeight: 500, overflowY: "auto", fontSize: 12, minHeight: 200 }}>
                  {terminalHistory.length === 0 && (
                    <div style={{ color: "#6c7086", fontSize: 11 }}>在下方输入命令执行...</div>
                  )}
                  {terminalHistory.map((item, i) => (
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && terminalInput.trim()) {
                        ...
                        fetch("/api/v1/system/execute", {
                          ...
                          .catch(err => {
                            setTerminalHistory(prev => [...prev, { cmd, output: `执行失败: ${err}`, timestamp: Date.now() }]);
                            setTerminalRunning(false);
                          });
                      }
                    }}
                    disabled={terminalRunning}
                    placeholder={terminalRunning ? "执行中..." : "输入命令..."}
                    autoFocus
                  />
                </div>
              </div>
            ...
            <NewDiffViewer
              files={[computeFileDiff(activeTab.originalContent, activeTab.content, activeTab.filePath)]}
              onRevertAll={handleRevertToOriginal}
            />
          ) : primaryTab === "changes" ? (
            <NewDiffViewer
              files={diffFiles || []}
            ...
            editMode ? (
              ...
              <div className="artifact-draft-preview">
