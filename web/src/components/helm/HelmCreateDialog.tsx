...
export function HelmCreateDialog({ visible, loading, onSubmit }: Props) {
  const config = useShellConfig();
  const [intent, setIntent] = useState("");
  const [mode, setMode] = useState<string>("helm");
  ...
  return (
    <div className="helm-overlay">
      <div
        className="helm-dialog animate-rise"
        style={{ maxWidth: "560px", width: "90vw" }}
      >
        <div className="dialog-header">
          <span className="dialog-icon">🚀</span>
          <span className="dialog-title">Helm 执行</span>
        </div>
