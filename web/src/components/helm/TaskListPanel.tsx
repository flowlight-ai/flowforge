...
import { ChatMessage, TaskHistoryItem } from "./helm-types";
...
function fixStalePhase(phase: HelmTaskPhase, timestamp: number): HelmTaskPhase {
...
function serverStatusToPhase(status: string, timestamp?: number): HelmTaskPhase {
...
export default function TaskListPanel({
  ...
  phase: HelmTaskPhase; intent: string; taskId: string | null; elapsed: number; onNewTask: () => void;
  onRestoreChat: (messages: ChatMessage[]) => void;
  onSwitchTask: (taskId: string, intent: string, persona: string, phase: HelmTaskPhase) => void;
  ...
  const phaseLabel: Record<HelmTaskPhase, string> = {
  ...
  const statusIcon = (p: HelmTaskPhase) => {
  ...
  const handleTaskClick = useCallback(async (item: TaskHistoryItem) => {
    ...
    const effectivePhase: HelmTaskPhase = fixStalePhase(item.phase, item.timestamp);
  ...
  return (
    <div className="helm-task-sidebar">
