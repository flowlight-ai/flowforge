import { ChatMessage, StepGroupData, TaskHistoryItem } from "./helm-types";
...
export const MODE_STYLES: Record<string, { label: string; color: string; bg: string }> = {
  ...
  helm: { label: "Helm", color: "#f43f5e", bg: "rgba(244,63,94,0.12)" },
...
export function getModeStyle(mode?: string) {
  if (!mode) return MODE_STYLES.helm;
...
export function entryToChatMessages(entry: any): ChatMessage[] {
  ...
  switch (entry.type) {
    ...
    case "draft-update":
      ...
      return [];
    case "system":
      if (entry.data?._plan) return [{ ...base, role: "plan" as const, content: "", data: { ...entry.data } }];
...
export function groupMessagesIntoSteps(
  ...
  let stepCounter = 0;
  const isTerminal = (p: HelmTaskPhase): boolean => p === "completed" || p === "error" || p === "rejected" || p === "interrupted";
  const isNotActive = (p: HelmTaskPhase): boolean => isTerminal(p) || p === "waiting_review" || p === "paused";
...
export function loadTaskHistory(brand: string): TaskHistoryItem[] {
  ...
  try { const raw = localStorage.getItem(`${brand}_helm_history`); return raw ? JSON.parse(raw) : []; } catch { return []; }
}

export function saveTaskHistory(brand: string, items: TaskHistoryItem[]): void {
  if (typeof window === "undefined") return;
  try { localStorage.setItem(`${brand}_helm_history`, JSON.stringify(items.slice(0, 20))); } catch {}
}
...
export function loadDeletedIds(brand: string): Set<string> {
  ...
  try { const raw = localStorage.getItem(`${brand}_helm_deleted`); return raw ? new Set(JSON.parse(raw)) : new Set(); } catch { return new Set(); }
}

export function saveDeletedIds(brand: string, ids: Set<string>): void {
  if (typeof window === "undefined") return;
  try { localStorage.setItem(`${brand}_helm_deleted`, JSON.stringify([...ids])); } catch {}
}
