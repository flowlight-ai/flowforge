...
export default function SettingsPanel({ config, onSave, onReset }: SettingsPanelProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-elevated)]">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  className={`relative w-8 h-4 rounded-full transition-colors ${
                    draft.general.autoSave ? "bg-[var(--cafe-accent)]" : "bg-[var(--bg-muted)]"
                  }`}
                ...
                <span className="text-xs text-[var(--muted)]">编辑后自动保存</span>
              </label>
            </SettingField>
          </div>
        )}
        ...
        {activeTab === "apikeys" && (
          <div className="space-y-3">
              ...
              <SettingField key={provider} label={provider}>
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-[var(--bg-elevated)] border border-[var(--border)] rounded-lg px-3 py-1.5 text-sm text-[var(--text)] font-mono focus:outline-none focus:border-[var(--cafe-accent)] pr-8"
                  />
                  <button
            ...
            {Object.keys(draft.apiKeys).length === 0 && (
              <div className="text-xs text-[var(--muted)] text-center py-4">暂无 API 密钥配置</div>
            )}
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <SettingField label="详细日志">
              <label className="flex items-center gap-2 cursor-pointer">
                <div
                  className={`relative w-8 h-4 rounded-full transition-colors ${
                    draft.advanced.verbose ? "bg-[var(--cafe-accent)]" : "bg-[var(--bg-muted)]"
                  }`}
                ...
                <span className="text-xs text-[var(--muted)]">输出详细调试信息</span>
              </label>
            </SettingField>
          </div>
        )}
      ...
      {/* Footer actions */}
      <div className="px-4 py-3 border-t border-[var(--border)] flex justify-end gap-2 flex-shrink-0">
        <button
          onClick={handleReset}
...
function SettingField({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      <label className="text-xs text-[var(--muted)] block mb-1.5">{label}</label>
      {children}
    </div>
  );
}
