...
import { boot, loadOverlayPatches } from '@flowforge/app-boot'
import SystemPrompt from '@flowforge/system-prompt'
import ToolRuntime from '@flowforge/tools'
import * as McpClient from '@flowforge/mcp-client/src/index.ts'
...
describe('third-party memory MCP example overlays', () => {
  it.each(examples)('parses $file with the documented generic plugin fields', (contract) => {
    ...
    expect(row.name).toBe('@flowforge/mcp-client')
