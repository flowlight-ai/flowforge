...
import { parseFlowforgeArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseFlowforgeArgs(argv, '1.2.3')
...
describe('parseFlowforgeArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@flowforge/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@flowforge/cordis'] })
