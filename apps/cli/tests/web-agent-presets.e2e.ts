...
import { boot, healProfilesModuleFallback, loadOverlayPatches } from '@flowforge/app-boot'
import { provideCmdline } from '@flowforge/cmdline'
import { SessionId } from '@flowforge/session'
import type { Agent } from '@flowforge/agent'
import type { PatchOptions } from '@flowforge/cordis-plugin-include'
import { afterAll, beforeAll, describe, expect, it } from 'vitest'
import { settingsNamespace } from '@flowforge/settings'
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@flowforge/agent-presets'
import { applyChildComposition, childSessionMeta } from '@flowforge/subagent'
import { CallId } from '@flowforge/llm'
import type {} from '@flowforge/compaction-basic'
...
import type {} from '@flowforge/token-meter'

const CONFIG_DIR = fileURLToPath(new URL('../config/', import.meta.url))
const REPO_ROOT = fileURLToPath(new URL('../../..', import.meta.url))
/** The shipped Web surface: the @flowforge/base and @flowforge/web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const patches: PatchOptions[] = [
    ...loadOverlayPatches('ff-test', BASE_PATCH),
    ...loadOverlayPatches('ff-test', WEB_PATCH),
    // The settings row defaults to `$FF_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $FF_HOME. Unpinned, this
    // file writes the developer's own `~/.flowforge/storages/` — and then reads it
    ...
    { insert: [
      { id: 'directory-picker-browse', name: '@flowforge/host-directory-picker-browse' },
      { id: 'ui-directory-picker-browse', name: '@flowforge/client-ui-directory-picker-browse' },
    ] },
    // The roster AppCLIEntry would patch in; only the shipped root, so a
    // developer's own `~/.flowforge/.preset` cannot change this test's outcome.
  ...
  return await boot('ff-test', rootConfig, patches, (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'preset-skill-proj-'))
    await mkdir(join(proj, '.flowforge', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.flowforge', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['ff-badge'])
      ...
      expect(scoped).toContain('ff-badge')
      ...
      const loaded = await ctx.tools.execute({
        ...
        name: 'skill',
        arguments: { name: 'ff-badge' },
        signal: new AbortController().signal,
        agent: handle.agent,
      })
      expect(loaded.isError).toBe(false)
      expect(JSON.stringify(loaded.content)).toContain('powered by flowforge')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('ff-badge')
...
describe('product subagent rows in user presets', () => {
  ...
  beforeAll(async () => {
    const root = await mkdtemp(join(tmpdir(), 'product-presets-'))
    ...
    productCtx = await bootWeb(settingsFile, [
      { insert: [
        { id: 'subagent-codex', name: '@flowforge/subagent-codex' },
        { id: 'subagent-claude-code', name: '@flowforge/subagent-claude-code' },
      ] },
      {
        id: 'agent-presets',
        config: {
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<dshHome>/.agent-presets` itself. `$FF_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'preset-derived-'))
    previousHome = process.env.FF_HOME
    ...
    await writeFile(
      ...
      '- id: tool-todo\n  name: \'@flowforge/tool-todo\'\n  config:\n    allowParallelInProgress: true\n',
    )
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.FF_HOME
    else process.env.FF_HOME = previousHome
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
