/**
 * `flowforge plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `flowforge.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `flowforge.bundle` joins the layer stack; a
 ...
 * gained its `flowforge.bundle` declaration in a newer version.
 * @module @flowforge/cli/plugin
 */
 ...
 * @returns true when the package manifest declares `flowforge.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.flowforge?.bundle?.patch !== undefined
}

/**
 * Reconcile `flowforge.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `flowforge.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.flowforge?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no flowforge.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (@flowforge/base and friends) are not dependencies.
  ...
  if (!changed) return
  after.flowforge = { ...after.flowforge, profile: { ...after.flowforge?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `flowforge` was invoked from.
...
/**
 * Run one `flowforge plugin` invocation: init if needed, forward to pnpm, reconcile.
