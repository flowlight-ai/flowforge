/**
 * Shared profile boot for every `flowforge` surface: resolve the profile, stack its
 * patch layers (bundle layers in `flowforge.profile.bundles` order, the profile's
 ...
 * @module @flowforge/cli/profile-boot
 */

import { writeFileSync } from 'node:fs'
import { join, resolve } from 'node:path'
import { fileURLToPath } from 'node:url'
import { FiberState, type Context } from '@flowforge/cordis'
...
import {
  ...
  watchUserPatches,
  type Profile,
} from '@flowforge/app-boot'
import { resolveFlowforgeHome } from '@flowforge/home-paths'

/** Shipped agent-preset root: beside this app's own config, in both source and built layouts. */
const SHIPPED_PRESET_ROOT = fileURLToPath(new URL('../config/agent-presets/', import.meta.url))

import { FF_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@flowforge/launch-environment'
import { provideCmdline } from '@flowforge/cmdline'
import { createProcessShutdown, type ProcessShutdown } from './process-shutdown.ts'

const NAME = 'flowforge'

/**
 * The home-level user patch layer (`$FF_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$FF_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveFlowforgeHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this flowforge installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the FF_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# flowforge profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's flowforge.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `FF_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$FF_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `flowforge.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$FF_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  // The writable root the roster appends is `@flowforge/agent-presets`' own, so a
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.FF_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(FF_LAUNCH_ENVIRONMENT_KEY, options.environment)
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@flowforge/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@flowforge/cordis-plugin-hmr', config: { root: [] } })
      }
