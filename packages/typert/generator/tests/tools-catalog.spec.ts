...
import type { TypertContribution } from '@flowforge/typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@flowforge/tool-cordis/src/api-catalog.ts'
...
describe('model-driven flowforge-tools generation', () => {
  // Whole-workspace compiler analysis is slow on Windows dev machines
  // (upstream CI finishes within 30s); keep a generous ceiling here.
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 120_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@flowforge/tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('flowforge-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@flowforge/tools')
    ...
    const record = ctx.typert.getPackage('@flowforge/tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@flowforge/tools', 'host')).toBeUndefined()
  })
})
