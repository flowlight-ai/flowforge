...
} from '@flowforge/typert-registry'
import type {
  InvocationDescriptor,
  TypertContext,
  TypertLookup,
  TypertRemoteContribution,
} from '@flowforge/typert-protocol'
import { apply as applyClientRegistry, inject as clientRegistryInject } from '../src/client/index.ts'

declare module '@flowforge/typert-protocol' {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@flowforge/tools', 'ToolInput')).toBe('@flowforge/tools#ToolInput')
    expect(typertPackageKey('@flowforge/tools', 'host')).toBe('@flowforge/tools#host')
    expect(ctx.typert.get('@flowforge/tools#ToolInput')).toMatchObject({
      package: '@flowforge/tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@flowforge/tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@flowforge/tools', 'host')).toMatchObject({
      key: '@flowforge/tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@flowforge/tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@flowforge/tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@flowforge/tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@flowforge/tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@flowforge/tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@flowforge/tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@flowforge/tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@flowforge/tools#ToolInput')).not.toBe(projected)
  })
