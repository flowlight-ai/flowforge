...
import type { Session, SessionEvent, SurfaceEvent, SurfaceEventType } from '@flowforge/session'
import SqliteSessionPersistence, { SCHEMA_VERSION } from '@flowforge/session-persistence-sqlite'
import {
  openDatabase,
  rowToEvent,
  rowToMeta,
...
describe('SqliteSessionPersistence: durability and crash semantics', () => {
  ...
  // The same-file reopen identity check plants a symlink alias, which Windows
  // refuses without admin/developer-mode privileges (EPERM).
