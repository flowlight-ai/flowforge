/**
 * Duplicate-install-safe lossless JSON and immutable-value helpers.
 *
 * Ported from dsh dsh-util-values `src/index.ts` (the `snapshotJsonValue`,
 * `deepFreeze`, `deepEqualJson` core), self-contained so this package has no
 * dependency on any `@deepseek-ai/*` package. Host mechanisms (assertNever) are
 * omitted; only the pure JSON snapshot / deep-equal / deep-freeze logic remains.
 * @module @flowforge/session-format/pure/json-value
 */
