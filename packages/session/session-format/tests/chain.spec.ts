...
describe('Session format chain', () => {
  ...
  it('plans and runs a full cross-version chain through every adjacent edge', () => {
    const v0toV1 = defineSessionFormatMigration({
      ...
      migrate: artifact => ({
        ...
        events: artifact.events.map(event => ({ ...event, data: { ...(event.data as Record<string, unknown>), v: 1 } })),
      }),
      validateTarget: () => {},
      validateTargetHeader: () => {},
    })
    const v1toV2 = defineSessionFormatMigration({
      name: '@test/v1-to-v2', fromVersion: 1, toVersion: 2,
      migrateHeader: header => ({ ...header, version: 2 }),
      migrate: artifact => ({
        ...artifact,
        header: { ...artifact.header, version: 2 },
        events: artifact.events.map(event => ({ ...event, data: { ...(event.data as Record<string, unknown>), v: 2 } })),
      }),
      validateTarget: () => {},
      validateTargetHeader: () => {},
    })
    const chain = createSessionFormatChain({
      currentVersion: 2,
      migrations: [v0toV1, v1toV2],
      restoreCurrent: value => value,
      restoreCurrentHeader: value => value,
    })
    const source = { ...currentArtifact, header: { ...currentArtifact.header, version: 0 } }

    expect(chain.plan(0)).toEqual([v0toV1, v1toV2])
    expect(chain.plan(1)).toEqual([v1toV2])
    const migrated = chain.migrate(source)
    expect(migrated.header.version).toBe(2)
    expect(migrated.events[0]?.data).toMatchObject({ v: 2 })
    expect(chain.migrateHeader(source.header).version).toBe(2)
  })
