...
import LlmRuntime, { createUserMessage, CallId, isAgentLoopRequest, LlmAdapter  } from '@flowforge/llm'
import type { FinishReason, GenerateOptions, StreamChunk } from '@flowforge/llm'
import SessionStore, { SessionId } from '@flowforge/session'
import { SessionTitleProviderId } from '@flowforge/session-title'
import type { SessionTitleProviderRequest } from '@flowforge/session-title'
...
} from '@flowforge/session-title-llm'
import type { SessionTitleLlmConfig } from '@flowforge/session-title-llm'
  ...
  override async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    await new Promise<never>((_resolve, reject) => {
      const rejectAbort = (): void => {
        // exercise exact AbortSignal.reason propagation
