...
import { getOrCreateAnonymousUserId } from '@flowforge/anonymous-user-id'
import Loader from '@flowforge/cordis-plugin-loader'
import { recordFeedback } from '@flowforge/command-feedback'
...
// pin FF_HOME to a temp dir so the suite never touches the ambient ~/.flowforge.
let tempHome: string
let previousFlowforgeHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'flowforge-otel-home-'))
  previousFlowforgeHome = process.env.FF_HOME
  process.env.FF_HOME = tempHome
})
afterAll(() => {
  if (previousFlowforgeHome === undefined) delete process.env.FF_HOME
  else process.env.FF_HOME = previousFlowforgeHome
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'flowforge' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@flowforge/session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@flowforge/session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@flowforge/session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('flowforge-session-telemetry-otel real-load-path guard', () => {
