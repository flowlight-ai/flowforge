/**
 * OpenTelemetry Service Provider for the FlowForge telemetry capability.
 *
...
} from '@flowforge/session-telemetry'
import { APP_IDENTITY } from '@flowforge/llm'
...
// version (same pattern as flowforge-llm's attribution identity).
 ...
 * and one FlowForge-owned shutdown bound. Uploading modes validate their endpoint
...
export class OpenTelemetrySessionBackend extends SessionTelemetryBackend {
  ...
  constructor(ctx: Context, config: Config) {
    ...
    const ledger = this.provider.getLogger('@flowforge/session-telemetry-otel', version)
    const ops = this.provider.getLogger('@flowforge/session-telemetry-otel/ops', version)
