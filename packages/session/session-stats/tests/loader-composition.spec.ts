...
import * as SessionStatsPlugin from '@flowforge/session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'flowforge-session-stats-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@flowforge/session-projection', SessionProjectionRegistry],
    ['@flowforge/session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@flowforge/session-projection'",
      "- name: '@flowforge/session-stats'",
    ])
