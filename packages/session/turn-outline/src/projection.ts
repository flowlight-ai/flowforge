 ...
 *
 * @module @flowforge/session-turn-outline/projection
 */
...
export function preview(content: MessageContent, limit: number): string {
  let text = ''
  let unread = false
...
export const turnOutlineProjectionDefinition = {
  key: 'turnOutline',
  stateVersion: 2,
  schema: turnOutlineEntriesSchema,
  init: () => EMPTY_OUTLINE,
  apply: (state: TurnOutlineState, event: SessionEvent): TurnOutlineState => {
  ...
  view: (state: TurnOutlineState) => state.turns,
} satisfies ProjectionDefinition<'turnOutline', TurnOutlineState>

export type { TurnOutlineEntry, TurnOutlineState } from './types.ts'