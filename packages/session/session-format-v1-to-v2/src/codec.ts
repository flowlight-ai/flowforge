/**
 * Frozen physical JSON codec for released v2. Event rows carry ordinary events,
 * packed-free embedded assistant streams, and `sourceEventSeqs` provenance with
 * scalar/pair/long-run range compression. The physical header drops `seedLength`
 * in favor of an explicit `isSeeded` boolean; the inherited cut is derived from
 * the last tagged `session/end-seed{inherited:true}` marker.
 * @module @flowforge/session-format-v1-to-v2/codec
 */
