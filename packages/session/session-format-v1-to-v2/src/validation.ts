/**
 * Validation for frozen released-v2 Sessions. Covers the exact logical header,
 * physical envelopes, payload semantics, cross-event relationships, and the
 * inherited cut (last tagged `session/end-seed{inherited:true}` marker).
 * Ported from dsh `dsh-session-format-v1-to-v2`; the Assistant stream assembly
 * uses the local {@link BlockAssembler}/{@link expandAssistantStream} so there is
 * no runtime dependency on any `@deepseek-ai/*` package.
 * @module @flowforge/session-format-v1-to-v2/validation
 */

import { isAbsolute } from 'node:path'
import { BlockAssembler, expandAssistantStream } from './pure/assistant-stream.ts'
import { deepEqualJson } from '@flowforge/session-format'
import {
  SessionFormatError,
