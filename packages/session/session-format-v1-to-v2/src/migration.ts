/**
 * Adjacent v1->v2 Session migration that folds released-v1 top-level Assistant
 * chunks into released-v2 `assistant/attempt` + `assistant/message` embedded
 * streams, reorders events, remaps every sequence reference, and recomputes the
 * inherited cut. Ported from dsh `dsh-session-format-v1-to-v2`; Assistant stream
 * compaction uses the local {@link AssistantStreamAccumulator}.
 * @module @flowforge/session-format-v1-to-v2/migration
 */

import { AssistantStreamAccumulator } from './pure/assistant-stream.ts'
import {
...
export const sessionFormatV1ToV2 = defineSessionFormatMigration({
  name: '@flowforge/session-format-v1-to-v2',
  fromVersion: 1,
  toVersion: 2,
