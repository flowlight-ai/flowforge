...
import { releasedV2SessionFormatCodec } from '@flowforge/session-format-v1-to-v2'
...
describe('releasedV2SessionFormatCodec rows', () => {
  ...
  it('keeps non-monotonic provenance scalar-only and round-trips it losslessly', () => {
    ...
    const encoded = releasedV2SessionFormatCodec.encodeArtifact(source)
    expect(encoded.rows[6]?.['sourceEventSeqs']).toStrictEqual(sourceEventSeqs)
    expect(releasedV2SessionFormatCodec.decodeArtifact(encoded.header, encoded.rows)).toStrictEqual(source)
  })
