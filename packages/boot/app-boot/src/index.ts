/**
 * Shared boot glue for the app bins (`flowforge`, `flowforge-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the FlowForge home (`~/.flowforge`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @flowforge/app-boot
 */
...
import { Context, type FiberState } from '@flowforge/cordis'
import Loader, { type Entry, type EntryOptions } from '@flowforge/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@flowforge/cordis-plugin-include'
import Group from '@flowforge/cordis-plugin-group'
import { flowforgeHomePath, resolveFlowforgeHome } from '@flowforge/home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@flowforge/launch-environment'
import type {} from '@flowforge/cordis-plugin-hmr'
// Side-effect type import: resolves `ctx.get('systemPrompt')` to the service.
import type {} from '@flowforge/system-prompt'

declare module '@flowforge/cordis' {
  interface Context {
    /** FlowForge-home path resolver available to Loader `!!js` config expressions. */
    flowforgeHomePath?: typeof flowforgeHomePath
  }
}
...
export {
  ...
  type FlowforgeBundleManifest,
  type FlowforgeManifestSection,
  type FlowforgeProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$FF_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['FF_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
...
/**
 * Load the product CLI's inherited > invoking-directory `.env` > FlowForge-home
 * `.env` snapshot. The FlowForge home resolves before either file; both files
...
export function loadLayeredEnv(
  ...
  const home = resolveFlowforgeHome()
 ...
 * entries (`@flowforge/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@flowforge/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@flowforge/cordis-plugin-group`
...
export async function boot(
  ...
  try {
    ...
    ctx.provide('flowforgeHomePath', flowforgeHomePath)
...
/**
 * Add a global prompt section naming the on-disk FlowForge source checkout while
 * explicitly distinguishing it from the task workspace and current working
 * directory. The self-referential `tool-cordis` toolset reads and edits this
 * checkout. Call once on the settled boot context ({@link boot}); the section
 * orders just after the FlowForge identity opener (`-100`) and before the deployment
 ...
 * @param sourceRoot - the absolute path to the FlowForge checkout root.
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The FlowForge implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend FlowForge itself.`,
  })
}
