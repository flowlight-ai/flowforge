 ...
 * `flowforge --profile` launcher family.
 *
 * A profile is a directory under `$FF_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `flowforge.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"flowforge": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `flowforge.profile.bundles` order over
 ...
 * first from the flowforge installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$FF_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @flowforge/app-boot/profile
 */
...
import { applyEntryPatches, type PatchOptions } from '@flowforge/cordis-plugin-include'
import { resolveFlowforgeHome } from '@flowforge/home-paths'
import { loadOverlayPatches } from './index.ts'

/** Directory under the FlowForge home holding every profile. */
export const PROFILES_DIR = 'profiles'

/** The user patch layer inside a profile directory (hot-reloaded on long-lived surfaces). */
export const PROFILE_PATCH_FILENAME = 'cordis.patch.yml'

/** The bundle half of the `flowforge` manifest section: what a bundle package exports. */
export interface FlowforgeBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `flowforge` manifest section: what a profile directory composes. */
export interface FlowforgeProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `flowforge`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface FlowforgeManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: FlowforgeBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: FlowforgeProfileManifest
}
...
export interface ProfileManifest {
  ...
  flowforge?: FlowforgeManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `flowforge.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `flowforge.profile.bundles` order. */
  layers: ProfileLayer[]
...
/**
 * Resolve a profile's directory under the FlowForge home.
 * @param name - the profile name (`flowforge --profile <name>`).
 ...
 */
export function resolveProfileDir(name: string, home: string = resolveFlowforgeHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`flowforge: invalid profile name ${JSON.stringify(name)}`)
  }
...
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@flowforge/base', '@flowforge/web-app', '@flowforge/headless'],
}

/** The bundle list a `flowforge plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@flowforge/base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this flowforge profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `flowforge.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `flowforge-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      flowforge: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`flowforge: ${link} exists and is not a symlink; remove it so flowforge can manage the installation fallback`)
    }
 ...
 * Definition packages (`@flowforge/compaction`, `@flowforge/invariants`, ...) that the app
 ...
 * @param home - the FlowForge home; defaults to {@link resolveFlowforgeHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveFlowforgeHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (@flowforge/subprocess,
    // @flowforge/compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.flowforge?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    flowforge: {
      ...manifest.flowforge,
      profile: { ...manifest.flowforge?.profile, bundles: [...current] },
    },
  }
 ...
 * `@flowforge/base` (and every other in-box bundle) always comes from
 * the same installation as the running flowforge, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `flowforge.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the flowforge app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the flowforge installation or ${profileDir}; `
    + `run 'flowforge plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `flowforge.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `flowforge.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the flowforge app's package.json (first resolution anchor).
 * @param home - the FlowForge home; defaults to {@link resolveFlowforgeHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveFlowforgeHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'flowforge plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the flowforge section entirely.
  const bundles = manifest.flowforge?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.flowforge?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no flowforge.bundle in its package.json`)
    }
