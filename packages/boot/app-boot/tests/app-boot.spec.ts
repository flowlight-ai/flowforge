...
const NAME = 'flowforge-test-bin'

const tmp = (): string => mkdtempSync(join(tmpdir(), 'flowforge-app-boot-'))
...
describe('loadEnv', () => {
  it('loads variables from .env in the given dir', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'FF_APP_BOOT_SPEC_VAR=loaded\n')
    const warn = vi.fn()
    loadEnv(NAME, dir, warn)
    expect(process.env['FF_APP_BOOT_SPEC_VAR']).toBe('loaded')
    expect(warn).not.toHaveBeenCalled()
    delete process.env['FF_APP_BOOT_SPEC_VAR']
  })
  ...
  it('defaults dir to the process cwd and warn to a stderr write', () => {
    const dir = tmp()
    writeFileSync(join(dir, '.env'), 'FF_APP_BOOT_SPEC_DEFAULTS=yes\n')
    ...
    expect(process.env['FF_APP_BOOT_SPEC_DEFAULTS']).toBe('yes')
    delete process.env['FF_APP_BOOT_SPEC_DEFAULTS']
...
describe('loadLayeredEnv', () => {
  ...
  it.each([
    ['a flowforge switch', 'FF_PERMISSION_MODE=danger-full-access\n'],
    ['the executable search path', 'PATH=/tmp/evil\n'],
    ['a module preload', 'NODE_OPTIONS=--require /tmp/evil.js\n'],
    ['a skill root', 'FF_AGENTS_HOME=/tmp/injected\n'],
  ...
  it('resolves the flowforge home from the inherited environment, never from a file', () => {
    const home = tmp()
    const project = tmp()
  ...
  it('reads a flowforge home that is also the invocation directory exactly once', () => {
    const both = tmp()
    writeFileSync(join(both, '.env'), `${NAMES[2]}=one-file\n`)
    clear()
    vi.stubEnv('FF_HOME', both)
    try {
...
describe('boot', () => {
  ...
  it('can resolve bare plugins from the harness when the config project shadows their package name', async () => {
    ...
    const shadow = join(dir, 'node_modules', '@flowforge', 'system-prompt')
    const harnessPlugin = join(harness, 'node_modules', '@flowforge', 'system-prompt')
    mkdirSync(shadow, { recursive: true })
    mkdirSync(harnessPlugin, { recursive: true })
    writeFileSync(join(shadow, 'package.json'), JSON.stringify({
      name: '@flowforge/system-prompt',
      type: 'module',
      exports: './index.mjs',
    }))
  ...
  it('exposes flowforgeHomePath to Loader config expressions', async () => {
    const dir = tmp()
    const ffHome = join(dir, 'home')
    vi.stubEnv('FF_HOME', ffHome)
    ...
    writeFileSync(join(dir, 'cordis.yml'), [
      ...
      '  name: ./capture.mjs',
      '  config:',
      "    path: !!js flowforgeHomePath('sessions')",
      '',
    ].join('\n'))
    let ctx: Context | undefined
    try {
      ctx = await boot(NAME, join(dir, 'cordis.yml'))
      expect(ctx.get('capturedPath')).toBe(join(ffHome, 'sessions'))
    } finally {
      await ctx?.fiber.dispose()
      vi.unstubAllEnvs()
    }
...
describe('addHarnessSourceSection', () => {
  ...
  it('distinguishes the source path from the current workdir between identity and persona', async () => {
    ...
    try {
      ...
      const identityAt = rendered.indexOf('You are an AI agent powered by FlowForge.')
