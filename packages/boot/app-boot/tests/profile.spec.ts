/**
 * Profile machinery of `flowforge-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'flowforge-profile-'))
...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { flowforge: { bundle: { patch: './cordis.patch.yml' } } },
    }))
    if (spec.patch !== undefined) writeFileSync(join(dir, 'cordis.patch.yml'), spec.patch)
  }
  writeFileSync(join(appDir, 'package.json'), JSON.stringify({ name: 'flowforge-app', dependencies: appDeps }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@flowforge/base'])
    ...
    expect(readProfileManifest('t', dir).flowforge?.profile?.bundles).toEqual(['@flowforge/base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', flowforge: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).flowforge?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each flowforge.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or flowforge section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @flowforge/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@flowforge/base')
    try {
  ...
  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@flowforge/base': { patch: '[]\n' },
      '@flowforge/web-app': { patch: '[]\n' },
      '@flowforge/headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@flowforge/base', '@flowforge/web-app', '@flowforge/headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).flowforge?.profile?.bundles)
      .toEqual(['@flowforge/base', '@flowforge/headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@flowforge/base', '@flowforge/web-app', '@flowforge/headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).flowforge?.profile?.bundles).toEqual([
      '@flowforge/base', '@flowforge/web-app', '@flowforge/headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no flowforge.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no flowforge.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', () => {
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'flowforge-app']) {
  ...
  it('throws when a fallback entry is a real directory', () => {
    const anchor = stageInstallation({})
    const home = tmp()
    mkdirSync(join(home, 'profiles', 'node_modules', 'flowforge-app'), { recursive: true })
  ...
  it('replaces a wrong symlink', () => {
    ...
    symlinkSync(tmp(), join(fallback, 'flowforge-app'), 'junction')
    healProfilesModuleFallback(anchor, home)
    expect(readlinkSync(join(fallback, 'flowforge-app'))).toContain('app')
  })
  ...
  it('tolerates losing the concurrent-heal race to an identical link and rejects a different one', () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    expect(lstatSync(join(fallback, 'flowforge-app')).isSymbolicLink()).toBe(true)
  })
})
