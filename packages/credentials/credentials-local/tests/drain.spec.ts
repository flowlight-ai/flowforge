...
vi.mock('@flowforge/atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@flowforge/atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@flowforge/atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
