/**
 * File-backed credentials provider over `$FF_HOME/.credentials.yaml`, layered
 * against the environment by how much each layer is trusted:
 *
 * ```text
 * inherited process environment      (read-only, wins)
 * > $FF_HOME/.credentials.yaml      (provider-managed, writable)
 * > <invocation cwd>/.env            (read-only fallback)
 * > $FF_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `DEEPSEEK_API_KEY=… flowforge`, a CI
 ...
 * @module @flowforge/credentials-local
 */
...
import { withFileLock, writeFileAtomic } from '@flowforge/atomic-write'
...
import { CredentialProvider, credentialRef } from '@flowforge/credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@flowforge/credentials'
import type { LaunchEnvironmentEntry } from '@flowforge/launch-environment'
...
export interface Config {
  ...
  path?: string
  /** Harness home used when `path` is omitted; defaults to `$FF_HOME` or `~/.flowforge`. */
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveFlowforgeHome(config.flowforgeHome), CREDENTIALS_FILENAME)),
...
/** File-backed credentials provider (`$FF_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start flowforge from instead',
      )
    }
  }
