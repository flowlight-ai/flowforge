...
} from '@flowforge/compaction'
import * as commandCompact from '@flowforge/command-compact'
...
describe('command-compact real Loader composition', () => {
  it('discovers and executes /compact through the assembled command plane', async () => {
    root = await mkdtemp(join(tmpdir(), 'flowforge-command-compact-loader-'))
    const configPath = join(root, 'cordis.yml')
    await writeFile(configPath, [
      "- name: '@flowforge/commands'",
      "- name: '@test/compact-backend'",
      "- name: '@flowforge/command-compact'",
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@flowforge/command-compact', commandCompact],
    ])
