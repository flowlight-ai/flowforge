...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'flowforge-token-meter-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@flowforge/token-meter', TokenMeter],
    ['@flowforge/compaction-tool-result-pruner', ToolResultPruner],
    ['@flowforge/compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@flowforge/token-meter'",
      "- name: '@flowforge/compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
