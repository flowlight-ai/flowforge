...
describe('compaction-tool-result-pruner real Loader composition', () => {
  it('loads and resolves the flat YAML plugin shape', async () => {
    root = await mkdtemp(join(tmpdir(), 'flowforge-compact-tool-result-prune-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@flowforge/token-meter') return TokenMeter
        if (specifier === '@flowforge/compaction-tool-result-pruner') return ToolResultPruner
