 ...
 * harness home resolved by {@link resolveFlowforgeHome} (`$FF_HOME` > `~/.flowforge`),
 ...
 * machine: every process sharing one `$FF_HOME` reports the same id, and
 ...
 *
 * @module @flowforge/anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `FF_HOME`; defaults to `process.env`. */
...
export function getOrCreateAnonymousUserId(options: AnonymousUserIdOptions = {}): AnonymousUserId {
  const file = join(resolveFlowforgeHome(undefined, options.env ?? process.env), ANONYMOUS_USER_ID_FILE_NAME)
