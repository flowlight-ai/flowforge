# @flowforge/anonymous-user-id

English | [中文](README.zh.md)

Shared anonymous identity for session telemetry, direct feedback acknowledgement, and DeepSeek provider requests. `getOrCreateAnonymousUserId()` returns a random UUID v4 scoped to one harness home, persisted as the bare line `$FF_HOME/.anonymous-user-id` (`~/.flowforge/.anonymous-user-id` when `FF_HOME` is unset). The OpenTelemetry backend reports it as Resource `user.id`; `/feedback` includes the same value in its acknowledgement; and `flowforge-llm-deepseek` sends it as `x-flowforge-user-id`, allowing the receiving systems to correlate records without independently generated identities.
...
- **No cross-home identity** — different `$FF_HOME` values cannot be correlated.
- **Configured DeepSeek gateways receive the id** — `flowforge-llm-deepseek` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
