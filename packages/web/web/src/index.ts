...
export class WebRuntime extends Service {
   ...
   * `$FF_WEB_SEARCH_PROVIDER` / `$FF_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.FF_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.FF_WEB_FETCH_PROVIDER
  }
