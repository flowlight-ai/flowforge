 ...
 * a query; `maxResults` is a `flowforge-tool-web`-layer bound passed through unchanged
...
export interface WebSearchRequest {
   ...
   * bound. `flowforge-tool-web` always sets it. A provider whose API supports a
 ...
 * URL-only). `flowforge-tool-web` renders `title ?? hostname(url)` for display.
 */
 ...
 * `flowforge-web`: the provider decodes the kind and `flowforge-tool-web` renders it, so a
