/**
 * Integration: the real fetch backend (`flowforge-web-fetch-http`) + a real search provider
 * (`flowforge-web-search-exa`) + the real seam (`flowforge-web`) + the model tool (`flowforge-tool-web`) + the
 * tool-call timeout policy (`flowforge-tool-call-timeout-policy`), exercised through `ctx.tools.execute()` —
...
import ToolRuntime, { type ToolExecutionResult } from '@flowforge/tools'
...
import * as TimeoutPolicy from '@flowforge/tool-call-timeout-policy'
...
describe('tool-call timeout returns TOOL_TIMEOUT (deadline wins over a slow fetch)', () => {
  ...
  it('returns a structured TOOL_TIMEOUT (not the provider WEB_FETCH_TIMEOUT) when the tool-call budget wins', async () => {
    ...
    expect(out.isError).toBe(true)
    // The outer tool-call deadline won: TOOL_TIMEOUT, owned by flowforge-tool-call-timeout-policy,
