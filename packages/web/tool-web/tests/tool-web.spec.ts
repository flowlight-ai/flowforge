...
import type { WebSearchProvider, WebSearchResult } from '@flowforge/web'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@flowforge/web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by flowforge-tool-call-timeout-policy over exec.signal.
