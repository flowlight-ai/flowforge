...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@flowforge/tools'
import type { WebSearchResult, WebSearchSource } from '@flowforge/web'
import type {} from '@flowforge/system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `flowforge-tool-fs`'s
 ...
 *   `ToolDefinition.timeoutMs` for `@flowforge/tool-call-timeout-policy` to enforce.
