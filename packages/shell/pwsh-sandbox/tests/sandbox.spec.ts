...
import { SandboxProvider, SandboxUnavailableError } from '@flowforge/sandbox'
import type { ConfinedArgv, RunnerFailureRule, SandboxExecutionPolicy, SandboxPolicy } from '@flowforge/sandbox'
import { resolvePwshPath } from '@flowforge/pwsh-local'
...
// Windows hosts without PowerShell 7 resolve to powershell.exe 5.1, which
// runs every command here but cannot satisfy the pwsh argv-shape assertion;
// skip that single test instead of the whole integration block.
const resolvedPathIsPwsh = /pwsh(\.exe)?$/u.test(resolvePwshPath())

const spillDir = mkdtempSync(join(tmpdir(), 'flowforge-pwsh-sandbox-spec-'))
...
describe('helpers (pure)', () => {
  const workdir = mkdtempSync(join(tmpdir(), 'flowforge-pwsh-sandbox-helpers-'))
  afterAll(() => {
...
describe.skipIf(!pwshAvailable())('SandboxPwshExecutor', () => {
  ...
  const readOnlyDir = mkdtempSync(join(tmpdir(), 'flowforge-pwsh-sandbox-ro-'))
  ...
  it.skipIf(!resolvedPathIsPwsh)('wraps the exact pwsh argv through ctx.sandbox with the per-call policy', async () => {
