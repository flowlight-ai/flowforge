 ...
 * @module @flowforge/tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSendResult, TerminalSessionId } from '@flowforge/terminal'
...
const SHELL_PROMPT = '__FF_PERSISTENT_BASH_PROMPT__ '
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__FF_PERSISTENT_BASH_START_${nonce}__`,
    end: `__FF_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __ff_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__ff_persistent_bash_status"`
}
