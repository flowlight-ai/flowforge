 ...
 * managed `FF_*` collection, abort translation, canonical result projection,
...
import SystemPrompt, { renderPrompt } from '@flowforge/system-prompt'
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@flowforge/shell'
...
import type { ShellProcessRead } from '@flowforge/shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, flowforgeHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, flowforgeHome === undefined ? {} : { flowforgeHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, flowforgeHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed FF_* environment', async () => {
    const flowforgeHome = mkdtempSync(join(tmpdir(), 'flowforge-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, flowforgeHome)
    ...
    expect(request?.dshEnv).toEqual({
      FF_HOME: flowforgeHome,
      FF_SHELL: '1',
      FF_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    expect(dshEnv?.['FF_SHELL']).toBe('1')
    expect(dshEnv?.['FF_HOME']).toEqual(expect.any(String))
    expect(dshEnv).not.toHaveProperty('FF_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'flowforge-tool-pwsh-policy-'))
