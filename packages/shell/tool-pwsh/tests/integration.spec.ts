/**
 * Integration tests: the REAL `@flowforge/pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@flowforge/tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@flowforge/pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'flowforge-tool-pwsh-'))
