 ...
 *
 * @module @flowforge/pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors flowforge-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@flowforge/shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@flowforge/shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@flowforge/subprocess'
import { installSettingsSection } from '@flowforge/settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@flowforge/timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of flowforge-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `flowforge-bash-local`'s `runArgv`/`startArgv` hooks; see
   * `@flowforge/pwsh-sandbox`).
   */
   ...
   * `flowforge-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@flowforge/pwsh-sandbox`.
