 ...
 * to `@flowforge/jobs`; this seam exposes only process handles. The
 ...
 * root.
 * @module flowforge-shell/types
 */

import type { SandboxEnforcement, SandboxExecutionPolicy, SandboxMode } from '@flowforge/sandbox'
import type { CollectedOutput, FlowforgeEnvironment } from '@flowforge/subprocess'

export { FF_ENV_PREFIX } from '@flowforge/subprocess'
export type { CollectedOutput, FlowforgeEnvironment, FlowforgeEnvironmentKey } from '@flowforge/subprocess'
...
export interface ShellExecRequest {
  ...
  /**
   * Harness-owned `FF_*` variables for this execution (typed to managed
   * keys). Executors discard ambient `FF_*` entries before merging this
   ...
   */
  dshEnv?: FlowforgeEnvironment | undefined
...
export interface ShellExecSpec {
   ...
   */
  env?: Record<string, string> | undefined
  /** Managed `FF_*` snapshot (typed to managed keys); merges after {@link env}. */
  dshEnv?: FlowforgeEnvironment | undefined
