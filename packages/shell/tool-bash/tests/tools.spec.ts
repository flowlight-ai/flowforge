...
import { ShellExecutor } from '@flowforge/shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult } from '@flowforge/shell'
import SystemPrompt from '@flowforge/system-prompt'
import ToolRuntime, { TOOL_ABORTED, TOOL_ABORTED_BEFORE_DISPATCH } from '@flowforge/tools'
...
import JsonlSessionPersistence from '@flowforge/session-persistence-jsonl'
import LocalJobRegistry from '@flowforge/jobs-local'
import * as ToolTasks from '@flowforge/tool-jobs'
import ApprovalService from '@flowforge/user-approval'
import type { ApprovalOutcome } from '@flowforge/user-approval'
import { LocalBashExecutor } from '@flowforge/bash-local'
import LocalSubprocessRuntime from '@flowforge/subprocess-local'
import SandboxPolicyService from '@flowforge/sandbox-policy'
...
const spillDir = mkdtempSync(join(tmpdir(), 'flowforge-tool-bash-spec-'))
...
describe('bash tool', () => {
  ...
  it('surfaces spawn failures as isError', async () => {
    const ctx = await setup()
    const result = await call(ctx, 'bash', { command: 'true', description: 'test command', workdir: '/nonexistent-flowforge' })
...
describe('background execution through the job runtime', () => {
  ...
  it('fails loud when the job runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @flowforge/jobs and @flowforge/tool-jobs')
  })
...
describe('the model-facing bash tool builds its request from named args only (no {...args} forward)', () => {
  const recordingFlowforgeHome = join(spillDir, 'flowforge-home')
   ...
   * (the credential scrub in flowforge-bash-local is the security control; see the
  ...
  async function setupRecording(withJsonl = false) {
    ...
    await ctx.plugin(LocalJobRegistry)
    await ctx.plugin(ToolTasks)
    await ctx.plugin(BashEnvPlugin, { flowforgeHome: recordingFlowforgeHome })
  ...
  it('describes the managed harness environment namespace to the model', async () => {
    ...
    expect(description).toContain('$FF_*')
    expect(description).not.toContain('FF_SESSION_JSONL')
  })
  ...
  it('injects the session id and JSONL target path into a foreground request', async () => {
    ...
    expect(bash.requests[0]?.dshEnv).toEqual({
      FF_HOME: recordingFlowforgeHome,
      FF_SESSION_ID: 'request-fg',
      FF_SESSION_JSONL: path,
      FF_SHELL: '1',
    })
  })
  ...
  it('injects the same trusted variables into a background request without forwarding model env', async () => {
    ...
    await ctx.tools.execute({
      ...
      arguments: {
        ...
        env: { FF_SESSION_ID: 'spoofed', FF_SESSION_JSONL: '/tmp/spoofed' },
      },
      agent,
    })
  ...
  it('injects built-ins and the stable session id when no JSONL locator is available', async () => {
    ...
    const ambient = process.env.FF_SESSION_ID
    ...
    expect(bash.requests[0]?.dshEnv).toEqual({
      FF_HOME: recordingFlowforgeHome,
      FF_SESSION_ID: 'request-id-only',
      FF_SHELL: '1',
    })
    expect(process.env.FF_SESSION_ID).toBe(ambient)
  })
  ...
  it('keeps parent and child agent session environments isolated', async () => {
    ...
    expect(bash.requests.map(request => request.dshEnv)).toEqual([
      {
        FF_HOME: recordingFlowforgeHome,
        FF_SESSION_ID: 'request-parent',
        FF_SESSION_JSONL: ctx.sessionPersistence.locate(parent.session.header)?.path,
        FF_SHELL: '1',
      },
      {
        FF_HOME: recordingFlowforgeHome,
        FF_SESSION_ID: 'request-child',
        FF_SESSION_JSONL: ctx.sessionPersistence.locate(child.session.header)?.path,
        FF_SHELL: '1',
      },
    ])
    expect(bash.requests[0]?.dshEnv?.FF_SESSION_JSONL).not.toBe(bash.requests[1]?.dshEnv?.FF_SESSION_JSONL)
  })
