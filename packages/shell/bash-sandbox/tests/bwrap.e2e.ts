...
import { bwrapProfileArgs } from '@flowforge/sandbox-local/src/profiles.ts'
 ...
 * backend-only confinement is covered by `@flowforge/sandbox-local`.
 *
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'flowforge-bwrap-e2e-'))
  tempDirs.push(dir)
  return dir
}
