...
import type { ShellRunResult, CollectedOutput } from '@flowforge/shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@flowforge/sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@flowforge/sandbox'
import { SandboxPolicyService } from '@flowforge/sandbox-policy'
import { SandboxBashExecutor } from '@flowforge/bash-sandbox'
...
import type { Config } from '@flowforge/bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'flowforge-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'FF_WRAP=1', 'bash', '-c', 'printf "%s" "$FF_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$FF_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'flowforge-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.FF_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'flowforge-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'flowforge-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
