 ...
 * confinement is covered by `@flowforge/sandbox-local`. Skips off macOS or when
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'flowforge-seatbelt-e2e-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(!seatbeltUsable)('bash-sandbox: real Seatbelt confinement through ctx.shell', () => {
  ...
  it('evaluates BASH_ENV only after Seatbelt confines the inner Bash', async () => {
    ...
    await writeFile(hook, [
      'printf hook > "$FF_BASH_ENV_INSIDE"',
      'printf escaped > "$FF_BASH_ENV_OUTSIDE"',
      '',
    ].join('\n'))
    const bash = await sandboxedBash(workdir, 'workspace-write')
    ...
    await bash.run(bash.resolve({
      ...
      env: { BASH_ENV: hook },
      dshEnv: {
        FF_BASH_ENV_INSIDE: insideProbe,
        FF_BASH_ENV_OUTSIDE: outsideProbe,
      },
    }))
