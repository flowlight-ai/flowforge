...
import { CompactionId, compactCheckpointSource } from '@flowforge/compaction'
import { createUserMessage, CallId , createMessage, createToolResultMessage } from '@flowforge/llm'
import SessionStore, { Session, SessionId } from '@flowforge/session'
import SessionQueryEngine from '@flowforge/session-query'
...
} from '@flowforge/session-reference'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    ...
    expect(parseSessionReferenceText('what is a flowforge-session: URI?')).toEqual({
      text: 'what is a flowforge-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see flowforge-session:%%%')).toEqual({
      text: 'see flowforge-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see flowforge-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](flowforge-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `flowforge-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('flowforge-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
