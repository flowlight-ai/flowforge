/**
 * Host-workspace discovery for `@file` completion（dsh 移植，D39）。
 * 索引仅含路径：选中值保持为普通提示文本，文件内容留在模型侧 read 工具后面。
 *
 * @module @flowforge/context-file-reference-local/search
 */

import { lstat, readdir } from 'node:fs/promises'
import { isAbsolute, join, relative, resolve, sep } from 'node:path'
import type { FileReferenceCandidate } from '@flowforge/context-file-reference'
