 ...
 * mount it as a no-op.
 *
 * @module @flowforge/agent-instructions
 */
...
import type { ToolExecution, ToolExecutionResult, ToolExecutionToken } from '@flowforge/tools'
  ...
  const compose = async (
    ...
    if (!baselinePresent || !keepVisibleBaseline || excludedBaselineScopes === undefined) {
      ...
      const instructions = await loadBaselineInstructionSet({
        cwd,
        flowforgeHome: resolved.flowforgeHome,
