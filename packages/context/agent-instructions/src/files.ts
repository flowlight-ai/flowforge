 ...
 *
 * @module @flowforge/agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@flowforge/fs'
import { assertNever } from '@flowforge/llm'
import { flowforgeHomeDisplay } from '@flowforge/home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.flowforgeHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.flowforgeHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.flowforgeHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(flowforgeHome: string): string {
  return `${flowforgeHomeDisplay(flowforgeHome)}/AGENTS.md`
}
