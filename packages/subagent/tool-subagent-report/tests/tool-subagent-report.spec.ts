...
import * as control from '@flowforge/tool-subagent-control'
...
async function setup(options: { load?: boolean; config?: tool.Config } = {}) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'flowforge-tool-subagent-report-'))
...
describe('flowforge-tool-subagent-report', () => {
...
describe('flowforge-tool-subagent-report result independence', () => {
