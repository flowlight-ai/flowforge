 ...
 * @module @flowforge/subagent-ff-sdk
 */

import type { Context } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@flowforge/subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@flowforge/subagent'
import {
...
export const name = 'subagent-ff-sdk'
export const inject = ['subagents']

/** Config: how to spawn and drive the child SDK runtime process. */
export interface Config {
  /** Provider name on `ctx.subagents` (default `flowforge-sdk`). */
  providerName: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `FF_CORDIS_CONFIG` naming its
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('flowforge-sdk'),
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    const spec: SdkRunSpec = {
      ...
      cwd: resolveChildCwd('subagent-ff-sdk', this.config.cwd, request.parent.session.header.cwd),
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-ff-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-ff-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-ff-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-ff-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-ff-sdk maxTokens must be a positive safe integer')
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-ff-sdk', resolved.cwd)
