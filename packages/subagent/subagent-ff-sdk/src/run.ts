/**
 * Fresh-process SDK subagent client. Drives one child FlowForge
 * runtime over stdio JSON-RPC through `@flowforge/sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@flowforge/subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @flowforge/subagent-ff-sdk/run
 */
...
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@flowforge/subagent'
...
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `flowforge-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `DEEPSEEK_API_KEY`, or `FF_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `FF_*` fact survives while ambient namesakes never leak.
   */
