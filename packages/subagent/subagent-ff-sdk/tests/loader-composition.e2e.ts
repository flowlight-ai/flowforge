...
import { resolveExampleLaunch, runLoaderSmoke } from '@flowforge/loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-ff-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'flowforge-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'flowforge-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        FF_TEST_CHILD_COMMAND: childLaunch.command,
        FF_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        FF_TEST_CHILD_ENV: JSON.stringify({
