 ...
 * @module @flowforge/subagent-acp/run
 */
...
import { AssistantOutputFold } from '@flowforge/subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `FF_*` entries are deployment-owned facts for the child harness
   * (e.g. `FF_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and FF_* names
