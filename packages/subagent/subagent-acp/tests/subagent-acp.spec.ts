...
import type { SubprocessOutcome } from '@flowforge/subprocess'
...
import { spawnSubprocess } from '@flowforge/subprocess-local/src/spawn.ts'
...
describe('child env layering (through the subprocess seam)', () => {
  ...
  it('forwards explicit FF_* config entries to the child', async () => {
    // A deployment sets child-harness facts like FF_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'FF_ACP_TEST_FACT', FF_ACP_TEST_FACT: 'managed' })
...
describe('disposeAcpChild (the backend-owned teardown ladder over seam verbs)', () => {
  ...
  it('observes a spawn-level rejection and returns without a process to reap', async () => {
    const child = spawnSubprocess({
      ...
      cwd: '/nonexistent-dir-flowforge-acp-ladder-test',
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1000 }, stderr: { maxBytes: 1000 } },
      graceMs: 200,
    })
...
describe('flowforge-subagent-acp', () => {
