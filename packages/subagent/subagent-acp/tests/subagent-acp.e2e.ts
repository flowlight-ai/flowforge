...
import { resolveExampleLaunch } from '@flowforge/loader-smoke'
...
// How to launch the child acp-agent (src via tsx / lib via plain node, per FF_EXAMPLE_MODE).
...
const childLaunch = resolveExampleLaunch({
  ...
  env: {
    ...
    FF_PERMISSION_MODE: 'danger-full-access',
  },
})
...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('ACP backend with-key e2e (drive our own acp-agent)', () => {
  it('drives the real acp-agent example process to answer a prompt', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'flowforge-subagent-acp-e2e-'))
    ctx = new Context()
