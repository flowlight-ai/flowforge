...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'flowforge-tool-subagent-control-'))
  roots.push(root)
...
describe('flowforge-tool-subagent-control', () => {
...
describe('flowforge-tool-subagent-control interrupt_agent', () => {
