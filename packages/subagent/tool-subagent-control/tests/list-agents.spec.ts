...
import type { SubagentListEntry } from '@flowforge/subagent'
...
import { LlmAdapter } from '@flowforge/llm'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'flowforge-tool-list-agents-'))
  roots.push(root)
...
describe('flowforge-tool-subagent-control/list-agents', () => {
