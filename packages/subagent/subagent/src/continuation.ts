...
import { boundContextSummary, createUserMessage, errorChain } from '@flowforge/llm'
...
import type { ToolRestriction } from '@flowforge/tools'
...
export class SubagentContinuationManager {
   ...
   */
  // keep rejection semantics without yielding during admission
  async reportFrom(
    child: Agent,
    content: ContentBlock[],
  ...
  private requirePersistence(): SessionPersistence {
    ...
    if (persistence === undefined) {
      throw new SubagentError(
        'continuable subagents require session persistence (load a flowforge-session-persistence backend)',
