...
import type { SessionProjectionRegistry } from '@flowforge/session-projection'
import type { SessionProjectionCache } from '@flowforge/session-projection-cache'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @flowforge/session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @flowforge/session)',
