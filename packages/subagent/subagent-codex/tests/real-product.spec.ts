...
import type { SubprocessHandle } from '@flowforge/subprocess'
...
async function realHarness(script: readonly ResponsesBehavior[]): Promise<{
  ...
  const root = mkdtempSync(join(tmpdir(), 'flowforge-codex-real-'))
  roots.push(root)
  ...
  const env = {
    OPENAI_API_KEY: 'flowforge-fake-openai-key',
...
describe('real @openai/codex 0.147.0 product', () => {
  it('passes the exact task and fake authentication to local Responses and returns exact text', async () => {
    ...
    expect(recorded.headers.authorization).toBe('Bearer flowforge-fake-openai-key')
  ...
  it('cancels a real app-server command approval without executing the command', async () => {
    ...
    expect(fixture.requests.every(requestEntry =>
      requestEntry.headers.authorization === 'Bearer flowforge-fake-openai-key',
    )).toBe(true)
    await expectQuiescent(harness.handles)
  }, 60_000)
