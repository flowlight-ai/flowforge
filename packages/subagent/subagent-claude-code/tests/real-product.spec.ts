...
import type { SubprocessHandle, SubprocessSpawnSpec } from '@flowforge/subprocess'
...
const settingsModel = 'flowforge-settings-inheritance-marker'
const fakeKey = 'flowforge-fake-anthropic-key'
...
async function realHarness(behavior: MessagesBehavior): Promise<{
  ...
  const root = mkdtempSync(join(tmpdir(), 'flowforge-claude-code-real-'))
  roots.push(root)
  ...
  it('inherits host settings and sends the exact task and fake key to local Messages', async () => {
    ...
    if (process.platform === 'win32') {
      expect(harness.spawnSpecs[0]?.argv.slice(0, 6)).toEqual([
        'cmd.exe', '/d', '/v:off', '/s', '/c', '%FF_CLAUDE_CODE_EXECUTABLE%',
      ])
      const batchExecutable = harness.spawnSpecs[0]?.env?.FF_CLAUDE_CODE_EXECUTABLE
