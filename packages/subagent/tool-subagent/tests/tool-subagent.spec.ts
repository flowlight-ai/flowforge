...
import ToolRuntime, { TOOL_ABORTED_BEFORE_DISPATCH } from '@flowforge/tools'
import { assembleContextFor, type Agent } from '@flowforge/agent'
...
import type { SubagentStartRequest } from '@flowforge/subagent'
import LocalJobRegistry from '@flowforge/jobs-local'
import * as SubagentSpawn from '@flowforge/subagent-spawn-in-process'
...
/**
 * Drives the REAL plugin body: mounts `flowforge-tool-subagent` on a real
...
describe('flowforge-tool-subagent', () => {
...
describe('flowforge-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @flowforge/jobs')
  })
...
describe('flowforge-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'flowforge-tool-subagent-continuable-'))
    roots.push(root)
