...
import {
  ...
  createUserMessage,
  isTokenDelta,
} from '@flowforge/llm/message'
import { CallId } from '@flowforge/llm/brand'
import type {
  AssistantMessage,
  ContentBlock,
  MessageSource,
...
import type { AttachmentIdType, ImageAttachmentRef } from '@flowforge/attachment'
import type {
  SessionEvent,
  SessionId,
  TodoItem,
} from '@flowforge/session/types'
// Type-only: the brand constructor is host-side; the fixture casts at its
// wire-fabrication boundary (the schema layer's one-cast-point posture).
import type { CommandId } from '@flowforge/commands/brand'
import type { CommandDescriptor, CommandExecution, CommandResult } from '@flowforge/commands/types'
import { deriveEventMessage, foldSurface } from '@flowforge/session/surface'
import type {
  ApiProxy, ClientRequest, ClientResponse, HistoryEntry, HostFrame, MuxFrame, RpcReceipt,
  ModelProviderGroup, ModelSelection, RpcRequest, RpcResponse, RpcResult, ServerRequest, ServerResponse, SessionSummary,
  ToolCallView, ToolEventView, ToolResultView, WorkspaceId, WorkspaceView,
} from './api.ts'
import type { RequestPayload, ResponseValue, RpcMethodMap } from '@flowforge/host-apiproxy/api'
 ...
 * presenter's shape (see formatGrepOutput in tool-fs-search): a
 ...
 * formatGlobOutput in tool-fs-search).
 */
...
const WEB_SEARCH_RESULT: Omit<Extract<ToolResultView, { card: 'web'; kind: 'search' }>, 'card' | 'kind'> = {
  answer: 'FlowForge is a plugin-based agent harness on vendored Cordis where **every capability is a plugin**.',
  sources: [
    {
      url: 'https://gitee.com/flowlight-ai/flowforge',
      title: 'FlowForge — plugin-based agent harness',
      snippet: 'Everything is a plugin: session, tools, agent-loop, and LLM adapters all mount on the same Cordis context.',
      publishedAt: '2026-07-01',
    },
    {
      url: 'https://gitee.com/flowlight-ai/flowforge/wikis/capability-seams',
      snippet: 'The capability-seam pattern splits each capability into interface, implementation, and consumer packages.',
    },
    {
      url: 'https://gitee.com/flowlight-ai/flowforge/wikis/writing-plugins',
...
function buildAlphaLog(): SessionEvent[] {
  ...
  toolTurn(70, 'web_search', '{"query":"flowforge plugin architecture"}', 'Search results for flowforge plugin architecture.')
  toolTurn(71, 'web_fetch', '{"url":"https://gitee.com/flowlight-ai/flowforge/wikis/capability-seams"}', '# Harness architecture\n\nEverything is a plugin.')
...
/** Fixture-local mirror of the goal projection value (goal's GoalProjection shape). */
...
function createFixtureWorld(options: FixtureOptions): FixtureWorld {
  ...
  const fixturePresets = new Map<string, { trust: 'system' | 'user'; content: string }>([
    ['standard', { trust: 'system', content: "- id: tool-bash\n  name: '@flowforge/tool-bash'\n" }],
    ['minimal', { trust: 'system', content: "- id: tool-web-search\n  name: '@flowforge/tool-web-search'\n" }],
    ['my-agent', { trust: 'user', content: "- id: tool-read\n  name: '@flowforge/tool-read'\n" }],
  ])
  ...
  const directoryTree = new Map<string, string[]>([
    ...
    [`${FIXTURE_HOME}/Documents`, [
      'project', 'flowforge-web', 'flowforge-platform', 'flowforge-app',
      'flowforge-docs', 'flowforge-plugin-lab', 'flowforge-blog', 'flowforge-demos',
    ]],
  ])
  ...
  const api: ApiProxy = {
    ...
    env: {
      summary: request => ok(request, { variables: [], categories: {} }),
    },
    goals: {
...
export class FixtureApiClient extends AbstractApiClient {
  ...
  private dispatch(
    ...
    switch (method) {
      ...
      case 'env.summary': return this.api.env.summary(request)
