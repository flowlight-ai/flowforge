import { isKnownCat, type CallbackPrincipal, type CatId } from './runtime-session-types.ts'
import type { ISessionChainStore, IThreadStore } from './runtime-session-ports.ts'
import { DEFAULT_THREAD_ID } from './runtime-session-ports.ts'
import {
  appendRuntimeIdentity,
  type RuntimeSessionExternalRegistrationBinding,
  type RuntimeSessionExternalRegistrationState,
  type RuntimeSessionMetadata,
  type RuntimeSessionRuntime,
} from './runtime-session-metadata.ts'
import type { IRuntimeSessionStore } from './runtime-session-store.ts'
...
export function normalizeExternalRuntimeSessionRegistration(
  ...
  if (principal.kind !== 'agent_key') throw new Error('external runtime registration requires agent-key principal')
  ...
  if (catId !== principal.catId) throw new Error('payload catId must match agent-key principal')
  if (!isKnownCat(catId)) throw new Error(`invalid catId: ${catId}`)

  const runtime = requireRuntime(record.runtime)
  const startedAt = requireFiniteNumber(record.startedAt, 'startedAt')
  const lastObservedAt = record.lastObservedAt === undefined ? startedAt : requireFiniteNumber(record.lastObservedAt, 'lastObservedAt')
  if (lastObservedAt < startedAt) throw new Error('lastObservedAt must not precede startedAt')
...
async function registerExternalRuntimeSessionLocked(
  ...
  const existing = await deps.runtimeSessionStore.getByRuntimeSession(registration.runtime, registration.runtimeSessionId)
  ...
  if (existing) {
    ...
    if (existing.catId !== registration.catId || (existingRecord && existingRecord.catId !== registration.catId)) {
      throw new ExternalRuntimeSessionRegistrationError('external_runtime_cat_immutable', 409, 'external_runtime_cat_immutable')
    }
    if (existing.userId !== principal.userId || (existingRecord && existingRecord.userId !== principal.userId)) {
      throw new ExternalRuntimeSessionRegistrationError('external_runtime_user_immutable', 409, 'external_runtime_user_immutable')
    }
    if (existingThreadId !== binding.threadId || (existing.threadId && existing.threadId !== binding.threadId)) {
      throw new ExternalRuntimeSessionRegistrationError('external_runtime_binding_immutable', 409, 'external_runtime_binding_immutable')
    }
  ...
  const claimedMetadata = await deps.runtimeSessionStore.getByRuntimeSession(registration.runtime, registration.runtimeSessionId)
  if (claimedMetadata) {
    assertRuntimeMetadataCompatible(claimedMetadata, sessionRecord.id, registration, binding.threadId, principal.userId)
  }
...
function resolveExistingBinding(metadata: RuntimeSessionMetadata, threadId: string): ResolvedExternalRuntimeBinding {
  ...
  return { threadId, binding: { mode: 'thread', threadId, requestedBy: 'agent_key' } }
}
...
async function reopenFailedExternalRegistrationSessionRecord(
  ...
  if (sessionRecord.sealReason !== 'external_registration_failed') {
    throw new ExternalRuntimeSessionRegistrationError('external_runtime_session_record_inactive', 409, 'external_runtime_session_record_inactive')
  }
  const reopened = await sessionChainStore.update(sessionRecord.id, { status: 'active', sealReason: null, sealedAt: null })
...
async function resolveBinding(
  ...
  if (registration.binding.mode === 'orphan') {
    ...
    return { threadId: anchor.id, binding: { mode: 'orphan_anchor', anchorThreadId: anchor.id } }
  }
  ...
  if (!thread || !canBindExternalRuntimeSessionToThread(thread, principal.userId)) {
    throw new ExternalRuntimeSessionRegistrationError('external_runtime_thread_forbidden', 403, 'external_runtime_thread_forbidden')
  }
  return { threadId: thread.id, binding: { mode: 'thread', threadId: thread.id, requestedBy: 'agent_key' } }
}
...
function buildNewMetadata(
  ...
  return {
    ...
    surface: 'ide-direct',
    identityHistory: [
      { catId: registration.catId, model: registration.model, from: registration.startedAt, source: 'external_registration' },
    ],
    lifecycle: {
      state: 'active',
...
function buildUpdatedMetadata(
  ...
  const metadata: RuntimeSessionMetadata = {
    ...
    externalRegistration: buildExternalRegistrationField(existing, registration, binding, registrationIsCurrent),
  }
  if (!registrationIsCurrent) return metadata
  const currentIdentity = metadata.identityHistory.at(-1)
  if (!currentIdentity || currentIdentity.catId !== registration.catId || currentIdentity.model !== registration.model) {
...
function buildExternalRegistrationField(
  existing: RuntimeSessionMetadata,
  registration: ExternalRuntimeSessionRegistrationInput,
  binding: RuntimeSessionExternalRegistrationBinding,
  registrationIsCurrent: boolean,
): RuntimeSessionExternalRegistrationState {
  if (!registrationIsCurrent && existing.externalRegistration) {
    return existing.externalRegistration
  }
  return buildUpdatedExternalRegistration(existing, registration, binding)
}
...
function normalizeBinding(input: unknown): ExternalRuntimeSessionBindingInput {
  ...
  if (mode === 'thread') return { mode, threadId: requireNonEmptyString(record.threadId, 'binding.threadId') }
  throw new Error('invalid binding.mode')
}

function normalizeProvenance(input: unknown, agentKeyId: string, registeredAt: number): ExternalRuntimeSessionProvenance {
  if (input === undefined) {
    return { source: 'antigravity-ide-direct', agentKeyId, registeredAt }
  }
  const record = requireRecord(input, 'provenance')
  const source = record.source === undefined ? 'antigravity-ide-direct' : requireNonEmptyString(record.source, 'source')
  if (source !== 'antigravity-ide-direct') throw new Error('invalid provenance.source')
  return {
    source,
    agentKeyId,
    registeredAt,
...
function requireRuntime(value: unknown): ExternalRuntimeRegistrationRuntime {
  if (typeof value !== 'string' || !EXTERNAL_RUNTIME_REGISTRATION_RUNTIMES.includes(value as ExternalRuntimeRegistrationRuntime)) {
    throw new Error('invalid external runtime')
  }
  return value as ExternalRuntimeRegistrationRuntime
}
