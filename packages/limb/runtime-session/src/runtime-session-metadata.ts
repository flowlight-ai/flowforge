...
export const RUNTIME_SESSION_LIFECYCLE_STATES = ['active', 'runtime_seal_pending', 'runtime_conflict_pending', 'sealed'] as const
export type RuntimeSessionLifecycleState = (typeof RUNTIME_SESSION_LIFECYCLE_STATES)[number]

export const RUNTIME_SESSION_DRAIN_RESULTS = ['complete', 'best_effort_quiet_window', 'skipped_runtime_unreachable'] as const
...
export const RUNTIME_IDENTITY_SOURCES = ['session_init', 'trajectory', 'external_registration', 'legacy_json_import'] as const
...
export function normalizeRuntimeSessionMetadata(input: unknown): RuntimeSessionMetadata {
  ...
  const lifecycle: RuntimeSessionLifecycle = normalizeLifecycle(requireRecord(record.lifecycle, 'runtime session lifecycle'))
...
export function appendRuntimeIdentity(metadata: RuntimeSessionMetadata, entry: RuntimeIdentityHistoryEntry): RuntimeSessionMetadata {
  const normalized = normalizeRuntimeSessionMetadata(metadata)
  const nextEntry = normalizeIdentityHistoryEntry(entry)
  const history = normalized.identityHistory.map((identity) => ({ ...identity }))
  ...
  return {
    ...
    lifecycle: {
      ...
      lastObservedAt: Math.max(normalized.lifecycle.lastObservedAt, nextEntry.from),
    },
  }
}
...
function normalizeRetryFragment(input: unknown): RuntimeSessionRetryFragment {
  ...
  if (kind !== 'retry') throw new Error('invalid retryFragment.kind')
  return {
    kind,
...
function normalizeUnexpectedRuntimeSessionSwitch(input: unknown): RuntimeSessionUnexpectedRuntimeSessionSwitch {
  ...
  return {
    ...
    previousSessionId: requireNonEmptyString(record.previousSessionId, 'unexpectedRuntimeSessionSwitch.previousSessionId'),
    ...
    reason: requireOneOf(record.reason, RUNTIME_SESSION_UNEXPECTED_SWITCH_REASONS, 'unexpectedRuntimeSessionSwitch.reason'),
  }
}
...
function normalizeIdentityHistoryEntry(input: unknown): RuntimeIdentityHistoryEntry {
  ...
  return {
    ...
    ...optional(Boolean, record, 'identity.modelVerified', 'modelVerified'),
...
function normalizeExternalRegistrationBinding(input: unknown): RuntimeSessionExternalRegistrationBinding {
  ...
  if (mode === 'orphan_anchor') {
    return { mode, anchorThreadId: requireNonEmptyString(record.anchorThreadId, 'externalRegistration.binding.anchorThreadId') }
  }
  if (mode === 'thread') {
    return { mode, threadId: requireNonEmptyString(record.threadId, 'externalRegistration.binding.threadId'), requestedBy: 'agent_key' }
  }
...
function normalizeExternalRegistrationProvenance(input: unknown): RuntimeSessionExternalRegistrationProvenance {
  ...
  if (source !== 'antigravity-ide-direct') throw new Error('invalid externalRegistration.provenance.source')
  return {
    source,
...
function requireOneOf<const T extends readonly string[]>(value: unknown, allowed: T, name: string): T[number] {
  if (typeof value !== 'string' || !allowed.includes(value as T[number])) throw new Error(`invalid ${name}`)
...
function optional(
  ...
  if (value !== undefined) {
    if (!pred(value)) throw new Error(`${out} must be a boolean`)
    return { [out]: value as boolean }
  }
  return {}
}
...
function optionalOneOfField<const T extends readonly string[]>(value: unknown, allowed: T, name: string): Record<string, T[number]> {
  if (value === undefined) return {}
...
function optionalUnexpectedRuntimeSessionSwitch(value: unknown): Record<string, RuntimeSessionUnexpectedRuntimeSessionSwitch> {
  if (value === undefined) return {}
