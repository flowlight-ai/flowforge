/**
 * PluginLimbAdapter — 插件四肢适配器（T6.2）
 *
 * 本地化自 clowder-ai `src/domains/limb/PluginLimbAdapter.ts`（F126）：
 * 由 YAML 声明驱动的通用插件四肢节点——命令路由到 REST 执行器或
 * invoke handler（builtin:health_check / plugin-id:handler_name）。
 *
 * @module @flowforge/limb-node/plugin-limb-adapter
 */
...
import type {
  ...
  LimbInvokeResult,
  LimbNodeStatus,
} from '@flowforge/limb-core';
import type { LimbDeclaration } from '@flowforge/limb-embodiment';
import { PluginRestExecutor } from './plugin-rest-executor.js';
import { PluginTokenManager, RedisTokenLike } from './plugin-token-manager.js';
