/**
 * PluginTokenManager — 插件通用令牌管理器（T6.2）
 *
 * 本地化自 clowder-ai `src/domains/limb/PluginTokenManager.ts`（F126）：
 * 由 YAML auth 配置驱动 client_credentials 流程，带 Redis + 内存缓存，
 * single-flight 防并发刷新（微信类限流：最新令牌使旧令牌失效）。
 *
 * @module @flowforge/limb-node/plugin-token-manager
 */
...
/** Redis 缓存后端最小接口（组合根注入真实 redis 客户端） */
export interface RedisTokenLike {
...
export class PluginTokenManager {
  ...
  constructor(
    ...
    private readonly redis: RedisTokenLike | undefined,
  ) {}
