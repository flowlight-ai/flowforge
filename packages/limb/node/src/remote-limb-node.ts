/**
 * RemoteLimbNode — 远端四肢节点代理（T6.2）
 *
 * 本地化自 clowder-ai `src/domains/limb/RemoteLimbNode.ts`（F126 Phase C）：
 * ILimbNode 代理，将 invoke/healthCheck 转发到远端 HTTP 端点。
 * 远端节点经 /api/limb/register 注册后，由组合根创建 RemoteLimbNode
 * 代表它进入 LimbRegistry。
 *
 * @module @flowforge/limb-node/remote-limb-node
 */
