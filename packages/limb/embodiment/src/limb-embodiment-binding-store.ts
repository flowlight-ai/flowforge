/**
 * LimbEmbodimentBindingStore — 具身绑定存储（T6.4）
 *
 * 本地化自 clowder-ai `src/domains/limb/LimbEmbodimentBindingStore.ts`（F126）：
 * 将四肢节点绑定到（userId, threadId, catId）与表达/音色配置。
 * 绑定是用户可见状态（Iron Law #5），持久化后端不设 TTL。
 *
 * @module @flowforge/limb-embodiment/limb-embodiment-binding-store
 */
...
export interface RedisBindingLike {
...
export class RedisLimbEmbodimentBindingStore implements LimbEmbodimentBindingStore {
  constructor(private readonly redis: RedisBindingLike) {}
