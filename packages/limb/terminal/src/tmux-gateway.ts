/**
 * TmuxGateway — 每 worktree 一个 tmux server 的网关。
 * 本地化自 clowder-ai `src/domains/terminal/tmux-gateway.ts`（T6.5）。
 *
 * 本地化点：
 * - `CAT_CAFE_TMUX_PATH` → `FF_TMUX_PATH`（二进制路径环境变量）
 * - socket 前缀 `catcafe-` → `flowforge-`
 * - RUNTIME_ONLY_ENV_KEYS：clowder-ai 原为 CONNECTOR_GATEWAY_AUTOSTART /
 *   CAT_CAFE_PROVISION_GLOBAL_SIDECAR（catcafe runtime-only 生命周期变量）。
 *   FlowForge 当前无同名 runtime-only env，机制保留（防未来变量泄漏到
 *   tmux server 进程），列表留空待组合根按需补充。
 *
 * @module @flowforge/limb-terminal/tmux-gateway
 */
...
// Runtime-only env keys never inherited by the tmux server process. clowder-ai 原始列表
// 为 ['CONNECTOR_GATEWAY_AUTOSTART', 'CAT_CAFE_PROVISION_GLOBAL_SIDECAR']；FlowForge 当前
// 无同名变量，机制保留以阻止未来 runtime-only 生命周期 env 泄漏到子进程。
const RUNTIME_ONLY_ENV_KEYS: readonly string[] = [];
 ...
 * Priority: FF_TMUX_PATH env → well-known Homebrew/system paths → PATH lookup.
 */
function resolveTmuxBin(): string {
  ...
  const envPath = process.env['FF_TMUX_PATH'];
  ...
  try {
    ...
    throw new Error('tmux not found. Install tmux or set FF_TMUX_PATH to its absolute path.');
  }
}
