/**
 * TmuxAgentCarrierSession — F254 D2 duplex JSON carrier 的 tmux 实现。
 * 本地化自 clowder-ai `src/domains/terminal/tmux-agent-carrier-session.ts`（T6.5）。
 *
 * 双 FIFO（input/output）+ stderr/exit 哨兵文件；会话挂在 tmux pane 上，
 * 通过 `setPaneReadOnly` 保持人类可观察、机器只写。
 *
 * @module @flowforge/limb-terminal/tmux-agent-carrier-session
 */
...
import { isParseError, parseNDJSON } from './cli/ndjson-parser.js';
import type {
  AgentCarrierSession,
...
export function createTmuxAgentCarrierSessionFactory(input: {
  ...
  return async (options) => {
    const tmpDir = await mkdtemp(join(tmpdir(), `flowforge-agent-duplex-${options.invocationId}-`));
