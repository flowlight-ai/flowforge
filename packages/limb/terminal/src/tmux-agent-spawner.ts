 ...
 * 本地化自 clowder-ai `src/domains/terminal/tmux-agent-spawner.ts`（T6.5）。
 *
 ...
 *
 * 本地化点：
 * - `createModuleLogger('tmux-spawner')`（infrastructure/logger，pino）→ 可注入
 *   `TmuxSpawnerLogger`（测试友好，默认 no-op）
 * - `CliSpawnOptions`（cli-types，含 OTel Span/livenessProbe 等重型字段）→ 本地
 *   `TmuxCliSpawnOptions`（types.ts 精简版）
 * - `SpawnCliOverride`（cats/services/types）→ 本地 types.ts 同名契约
 * - F212 诊断依赖 → `./cli/*`（cli-diagnostics / cli-spawn-helpers / cli-timeout）
 *
 * @module @flowforge/limb-terminal/tmux-agent-spawner
 */
...
import { buildCliDiagnostics } from './cli/cli-diagnostics.js';
import { maybeCollectStreamError } from './cli/cli-spawn-helpers.js';
import { resolveCliTimeoutMs } from './cli/cli-timeout.js';
import type { SpawnCliOverride, TerminalGatewayLike, TmuxCliSpawnOptions } from './types.js';
import type { AgentPaneRegistry } from './agent-pane-registry.js';

const execAsync = promisify(execFile);

/** 可注入 logger（本地化自 createModuleLogger；组合根可注入 pino/winston 实现） */
export interface TmuxSpawnerLogger {
  error(obj: Record<string, unknown>, msg: string): void;
}

const NOOP_LOGGER: TmuxSpawnerLogger = { error: () => {} };

/** Default timeout for first valid NDJSON event after pane spawn (30s). */
const DEFAULT_FIRST_EVENT_TIMEOUT_MS = 30_000;

export interface TmuxSpawnOptions extends TmuxCliSpawnOptions {
  worktreeId: string;
  invocationId: string;
...
export interface TmuxSpawnDeps {
  tmuxGateway: TerminalGatewayLike;
  logger?: TmuxSpawnerLogger;
}
...
export async function* spawnCliInTmux(
  ...
  const tmpDir = await mkdtemp(join(tmpdir(), `flowforge-agent-${options.invocationId}-`));
  ...
  const resetIdleTimeout = (): void => {
    ...
    timeoutTimer = setTimeout(() => {
      logger.error({ invocationId: options.invocationId, paneId, idleTimeoutMs }, 'Idle timeout fired');
      timedOut = true;
  ...
  try {
      ...
      if (options.outputMode === 'plainText') {
        ...
        for await (const line of rl) {
          ...
          try {
            ...
            logger.error({ line: trimmed }, 'JSON parse error');
          ...
          // locally on the events we see and use it (below at the finalSemanticDone
          // predicate) instead of the sticky signal. Codex normal path: turn.completed →
          // localFinalTerminal=completed → suppress exit=1 quirk. Multi-turn failure path:
          // turn.completed → turn.failed → localFinalTerminal=failed → surface __cliError.
          // Symmetric fix lands in cli-spawn too — same sticky-signal class.
    ...
    // predicate mirroring cli-spawn. Handles all four cells:
 ...
 * Called per-invocation by the limb adapters when tmux is available.
 */
export function createTmuxSpawnOverride(
  ...
  return async function* tmuxOverride(cliOpts: TmuxCliSpawnOptions) {
