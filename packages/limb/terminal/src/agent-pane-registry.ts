/**
 * AgentPaneRegistry — 追踪哪些 invocation 运行在 tmux pane 中，
 * 以及（F198 Phase C AC-C1）每个 thread 的 bg carrier daemon 会话。
 * 内存存储；供 terminal 域让前端发现 agent panes。
 *
 * 本地化自 clowder-ai `src/domains/terminal/agent-pane-registry.ts`（T6.5）。
 *
 * @module @flowforge/limb-terminal/agent-pane-registry
 */
...
export class AgentPaneRegistry {
  ...
  /** P1-2: Set of all daemonShortIds known (running + done). Used to scope agent-sessions reads. */
