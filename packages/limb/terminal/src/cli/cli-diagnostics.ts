 ...
 * 本地化自 clowder-ai `src/utils/cli-diagnostics.ts`（F212 诊断体系全量移植）。
 *
 * Public API:
 ...
 *
 * @module @flowforge/limb-terminal/cli/cli-diagnostics
 */

import type { CliDiagnostics, CliErrorReasonCode } from '../types.js';
import { CLASSIFIER_PATTERNS } from './cli-error-patterns.js';
import { sanitizeCliStderr } from './sanitize-cli-stderr.js';

// F212 Phase B: CliDiagnostics + CliErrorReasonCode hoisted to @cat-cafe/shared in
// clowder-ai so the web folded panel can import the same contract. FlowForge 本地化
// 后契约内联于 ../types.ts（@flowforge/limb-terminal/types），此处 re-export 保持调用方兼容。
...
export function buildCliDiagnostics(args: {
  ...
  const classifierSafeExcerpt = reasonCode
    ? maybeExtractSafeExcerpt({
        ...
        reasonCode,
        ...(additionalHomePaths ? { additionalHomePaths } : {}),
  ...
  return {
    ...
    debugRef: args.debugRef,
    ...(safeExcerpt && excerptSource ? { safeExcerpt, excerptSource } : {}),
  };
}
