...
import type { LimbAccessPolicy } from './limb-access-policy.js';
import type { LimbActionLog } from './limb-action-log.js';
import type { LimbLeaseManager } from './limb-lease-manager.js';
  ...
  async invoke(
    ...
    try {
      ...
      if (requestId && this.deps.actionLog) {
        if (result.success) {
          this.deps.actionLog.complete(
            requestId,
            result.artifactUri ? { artifactUri: result.artifactUri } : undefined,
          );
        } else {
