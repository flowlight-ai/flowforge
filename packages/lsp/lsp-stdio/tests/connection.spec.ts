...
import { LspConnection } from '@flowforge/lsp-stdio'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit FF_* env entries to the child', async () => {
    // A configured FF_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'FF_LSP_TEST_FACT', FF_LSP_TEST_FACT: 'managed' })
