...
```
@flowforge/cordis-plugin-timer  timer service (writes nothing to stdout)
@flowforge/llm              abstract LLM service + content-block vocabulary
@flowforge/session          event-sourced session log + store
@flowforge/session-title    log-backed title service + deterministic fallback
@flowforge/system-prompt    prompt-section + tool-schema assembly
@flowforge/tools            registry + guarded pre/around/post/final-result pipeline
@flowforge/skill            skill provider registry
@flowforge/skill-filesystem      local filesystem skill provider
@flowforge/agent            agent registry + initiator scope + agent/* events
@flowforge/goal             optional persisted same-session goal domain
@flowforge/tool-goal        optional model-facing goal controls
@flowforge/goal-round-driver     optional same-session goal-round driver
@flowforge/llm-retry        provider-routed request retry policy
@flowforge/jobs-local      generic background-job registry
@flowforge/invariants       configurable invariant registry service
@flowforge/session/invariant
@flowforge/agent/invariant
@flowforge/scope/invariant
@flowforge/agent-loop/invariant
                                  package-owned relational checks
@flowforge/tool-bash        the model-facing bash schema (unless toolBash=false)
@flowforge/agent-instructions  AGENTS.md/CLAUDE.md workspace context loader
@flowforge/tool-skill       session-prefix skill catalog + model-facing loader schema
@flowforge/tool-jobs       job_output/job_list/job_kill schemas + completion notices
@flowforge/agent-loop       THE concrete loop (gets the forwarded `agents`)
                                  (flowforge-system-prompt gets the forwarded `persona`)
```
...
```ts
import type { Config } from '@flowforge/agent-spine-demo'
// { agents?, maxParallelToolCalls?, includeHarnessIdentity?, includeRuntimeContext?, persona?, toolOrder?, tools?, flowforgeHome?, sessionTitle?, skills?, workspaceContext, toolBash?, jobs?, toolJobs?, goals?, invariants? }
// workspaceContext requires { maxBytes } or false; the other owner schemas supply defaults.
```

The bundle forwards each field to the child that owns it. App packages supply any pre-created agents: headless and JSON-RPC compositions create `main`, while the ACP app creates agents on demand at `session/new`. `includeRuntimeContext: false` is forwarded to `flowforge-system-prompt` and suppresses all dynamic context snapshots for fresh sessions without disabling their policy services. Prompt, tool, title, skill, agent-instructions, invariant, goal, and task settings retain the schemas and defaults documented by their owning packages; `jobs.maxConcurrentJobsPerOwner` configures the local provider independently of the model-facing `toolJobs` controls. `pickSpineConfig()` copies only fields owned by this bundle, and conflicting `flowforgeHome` values fail during composition.

For example, `{ invariants: { enabled: true, package_allowlist: ['^@flowforge/'], package_blocklist: ['agent-loop$'] } }` keeps the package-owned companions mounted but suppresses the blocked owner. Blocklist matches override allowlist matches; see [`flowforge-invariants`](../../runtime-diagnostics/invariants/README.md) for regex and lifecycle rules.
...
Indirectly, through `flowforge-system-prompt`, `flowforge-tool-skill`, `flowforge-tool-bash`, `flowforge-tools`, and `flowforge-llm-retry`, plus `flowforge-tool-goal` and goal-round prompts when `goals` is enabled. The bundle adds no model-bound wrapper content of its own.
