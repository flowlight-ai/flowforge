...
import { seatbeltProfileArgs } from '@flowforge/sandbox-local/src/profiles.ts'
...
import type { ToolResult } from '@flowforge/tools'
...
async function projectDir(label: string): Promise<string> {
  const dir = await mkdtemp(join(homedir(), `ff-${label}-`))
  tempDirs.push(dir)
  return dir
}
