...
import { renderPrompt, TOOL_ORDER_REST } from '@flowforge/system-prompt'
import * as agentCore from '../src/index.ts'
import { agentEvents, type Agent } from '@flowforge/agent'
...
/**
 * Unit coverage for the @flowforge/agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  const oldFlowforgeHome = process.env.FF_HOME
  const oldAgentsHome = process.env.FF_AGENTS_HOME
  process.env.FF_HOME = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-home-'))
  process.env.FF_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-agents-'))
  ...
  try {
    ...
    if (oldFlowforgeHome === undefined) {
      delete process.env.FF_HOME
    } else {
      process.env.FF_HOME = oldFlowforgeHome
    }
    if (oldAgentsHome === undefined) {
      delete process.env.FF_AGENTS_HOME
    } else {
      process.env.FF_AGENTS_HOME = oldAgentsHome
    }
  }
}
...
describe('flowforge-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@flowforge/agent$'] },
      { package_blocklist: ['^@flowforge/session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent powered by FlowForge.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-skill-custom-'))
    ...
    const ctx = await mount({
      ...
      skills: {
        ...
        filesystem: {
          flowforgeHome: join(home, '.flowforge'),
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-skill-refresh-'))
    const home = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-skill-refresh-home-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('shares top-level flowforgeHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'flowforge-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'flowforge-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-flowforge-home-test') as ToolExecution['token'],
      callId: CallId('agent-core-flowforge-home'),
      rootCallId: CallId('agent-core-flowforge-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ FF_HOME: home, FF_SHELL: '1' })
    await ctx.fiber.dispose()
  })
  ...
  it('rejects conflicting global and nested DSH home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        flowforgeHome: '/global-flowforge-home',
        workspaceContext: false,
        skills: { filesystem: { flowforgeHome: '/nested-flowforge-home' } },
      })
    }).toThrow('agent-spine-demo: flowforgeHome and skills.filesystem.flowforgeHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('picks shared spine config without leaking entry-point fields', () => {
    const appConfig = {
      ...
      flowforgeHome: '/tmp/flowforge-home',
    ...
    expect(agentCore.pickSpineConfig(appConfig)).toEqual({
      ...
      flowforgeHome: appConfig.flowforgeHome,
