 ...
 * @module @flowforge/agent-spine-demo
 */

import type { Context } from '@flowforge/cordis'
import Timer from '@flowforge/cordis-plugin-timer'
import z from '@flowforge/schemastery'
import LlmRuntime from '@flowforge/llm'
import SessionStore from '@flowforge/session'
import SessionTitleService, { type Config as SessionTitleConfig } from '@flowforge/session-title'
import SystemPrompt, { type Config as SystemPromptConfig } from '@flowforge/system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@flowforge/tools'
import SkillRegistry, { type Config as SkillRegistryConfig } from '@flowforge/skill'
import * as SkillFileSystem from '@flowforge/skill-filesystem'
import AgentRegistry from '@flowforge/agent'
import GoalService, { type Config as GoalDomainConfig } from '@flowforge/goal'
import * as goalSession from '@flowforge/goal-round-driver'
import * as toolGoal from '@flowforge/tool-goal'
import LocalJobRegistry, { type Config as JobsConfig } from '@flowforge/jobs-local'
import InvariantRegistry, { type Config as InvariantConfig } from '@flowforge/invariants'
import * as sessionInvariant from '@flowforge/session/invariant'
import * as agentInvariant from '@flowforge/agent/invariant'
import * as scopeInvariant from '@flowforge/scope/invariant'
import * as agentLoopInvariant from '@flowforge/agent-loop/invariant'
import * as toolBash from '@flowforge/tool-bash'
import * as bashEnv from '@flowforge/shell-env'
import * as workspaceContext from '@flowforge/agent-instructions'
import * as toolSkill from '@flowforge/tool-skill'
import * as toolJobs from '@flowforge/tool-jobs'
import AgentLoop, { type Config as AgentLoopConfig } from '@flowforge/agent-loop'
import * as llmRetry from '@flowforge/llm-retry'
 ...
 * `flowforgeHome` to bash environment and local skill discovery, `sessionTitle` to
...
export interface Config {
  /** The agent-loop `agents` list (see flowforge-agent-loop's `Config`). */
  ...
  /** The deployment persona (see flowforge-system-prompt's `Config`). */
  persona?: SystemPromptConfig['persona']
  /** The explicit model-facing tool order (see flowforge-system-prompt's `Config`). */
  toolOrder?: SystemPromptConfig['toolOrder']
  /** The tool registry's config — its presentation `mode` (see flowforge-tools' `Config`). */
  tools?: ToolsConfig
  /** FlowForge home directory shared by shell context and local skill discovery. */
  flowforgeHome?: string
...
export const Config = z.intersect([
  ...
  }) as unknown as z<Pick<Config, 'tools' | 'flowforgeHome' | 'sessionTitle' | 'skills' | 'workspaceContext' | 'toolBash' | 'jobs' | 'toolJobs' | 'invariants' | 'goals'>>,
...
export function pickSpineConfig(config: Omit<Config, 'agents'>): Omit<Config, 'agents'> {
  return {
    ...
    ...config.flowforgeHome !== undefined ? { flowforgeHome: config.flowforgeHome } : {},
...
export function apply(ctx: Context, config: Config): void {
  const nestedFlowforgeHome = config.skills?.filesystem?.flowforgeHome
  if (config.flowforgeHome !== undefined && nestedFlowforgeHome !== undefined
    && resolveFlowforgeHome(config.flowforgeHome) !== resolveFlowforgeHome(nestedFlowforgeHome)) {
    throw new Error('agent-spine-demo: flowforgeHome and skills.filesystem.flowforgeHome must resolve to the same directory')
  }
  const flowforgeHome = resolveFlowforgeHome(config.flowforgeHome ?? nestedFlowforgeHome)
  ...
  if (skillsEnabled) {
    ...
    ctx.plugin(SkillFileSystem, Object.assign({}, config.skills?.filesystem, { flowforgeHome }))
  }
  ...
  if (config.toolBash !== false) {
    ctx.plugin(bashEnv, { flowforgeHome })
