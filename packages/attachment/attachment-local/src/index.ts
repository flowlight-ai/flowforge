/** Local durable attachment backend rooted below `FF_HOME`. @module @flowforge/attachment-local */
...
export interface Config {
  /** Explicit harness home; omitted follows `FF_HOME`, then `~/.flowforge`. */
...
export class LocalAttachmentStore extends AttachmentStore {
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx)
    this.root = resolve(join(resolveFlowforgeHome(config.flowforgeHome), 'attachments', 'v1'))
