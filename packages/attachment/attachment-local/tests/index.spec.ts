...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const flowforgeHome = await mkdtemp(join(tmpdir(), 'flowforge-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { flowforgeHome })
      ...
      await rm(flowforgeHome, { recursive: true, force: true })
    }
  })

  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const flowforgeHome = await mkdtemp(join(tmpdir(), 'flowforge-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { flowforgeHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { flowforgeHome, maxImageBytes: 1 })
