...
|---|---|---|---|
| `file` | string | `$FF_SNAPSHOT_FILE` | Path to the primary (parent) `session.jsonl` fixture. Required (config or env). |
| `overrideFile` | string | `$FF_SNAPSHOT_OVERRIDE` | Optional `ReplayOverrideDoc` sidecar for the primary session: a bare `ReplayEntry[]` replaces its derived script, while `{ patches }` augments it by call index. |
| `childFiles` | string[] | `$FF_SNAPSHOT_CHILD_FILES` (path-delimited) | Recorded subagent child-session logs for a nested scenario; empty for a single-session scenario. |
    ...
    providers:
      - id: deepseek-official
        ...
        models:
          ...
          - id: deepseek-v4-pro
  # file/overrideFile/childFiles default to $FF_SNAPSHOT_FILE /
  # $FF_SNAPSHOT_OVERRIDE / $FF_SNAPSHOT_CHILD_FILES, set by the snapshot
