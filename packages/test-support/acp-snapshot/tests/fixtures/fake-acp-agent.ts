/**
 * Scripted fake ACP agent bin for `flowforge-acp-snapshot`'s unit specs. Speaks
 * newline-delimited JSON-RPC on stdio like the real `flowforge-acp-agent` bin, but
 ...
 * `behavior.json` sitting NEXT to the `$FF_SNAPSHOT_FILE` fixture, so a spec
...
interface ScriptedLog {
  /** Path relative to `$FF_SNAPSHOT_SESSIONS_ROOT`, e.g. `project/session/session.jsonl`. */
  file: string
  /**
...
interface Behavior {
  ...
  /** Echo the `FF_SNAPSHOT_*` env the harness set as a chunk (spec-side env-plumbing assertions). */
  echoEnv?: boolean
...
const sessionsRoot = process.env.FF_SNAPSHOT_SESSIONS_ROOT ?? ''
const fixtureFile = process.env.FF_SNAPSHOT_FILE ?? ''
...
async function handlePrompt(id: number | string): Promise<void> {
  ...
  if (behavior.echoEnv === true) {
    chunk(`env:${JSON.stringify({
      mode: process.env.FF_SNAPSHOT,
      override: process.env.FF_SNAPSHOT_OVERRIDE ?? null,
      childFiles: process.env.FF_SNAPSHOT_CHILD_FILES ?? null,
      spillRoot: process.env.FF_SNAPSHOT_SPILL_ROOT ?? null,
      // Scenario-supplied deployment env (the `Scenario.env` layering hook).
      permissionMode: process.env.FF_PERMISSION_MODE ?? null,
    })}`)
  }
