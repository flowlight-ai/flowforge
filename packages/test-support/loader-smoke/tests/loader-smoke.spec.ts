...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, runLoaderSmoke } from '@flowforge/loader-smoke'
...
describe('runLoaderSmoke', () => {
  it('isolates the process, closes stdin, captures output, and removes the cwd', async () => {
    ...
    expect(canonicalTempPath(output.flowforgeHome)).toBe(canonicalTempPath(join(output.cwd, '.flowforge')))
