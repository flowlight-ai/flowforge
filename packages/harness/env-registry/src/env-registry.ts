...
export const ENV_VARS: EnvDefinition[] = [
  ...
  // --- proxy（OpenRoute 本地网关，批次50 llm-openroute 登记）---
  {
    name: 'FLOWFORGE_LLM_ROUTE_OPENROUTE_BASE_URL',
    defaultValue: '(未设置 — 回落 OPENROUTE_BASE_URL，再回落 http://localhost:13001)',
    description: 'OpenRoute 网关基础 URL（ForgekinBase 兜底 LLM 客户端与三协议代理的上游）',
    category: 'proxy',
    sensitive: false,
    maskMode: 'url',
  ...
  {
    name: 'OPENROUTE_BASE_URL',
    defaultValue: '(未设置 — 回落 http://localhost:13001)',
    description: 'OpenRoute 网关基础 URL（通用别名；trae-bridge operator 与各协议代理共用）',
    category: 'proxy',
    sensitive: false,
    maskMode: 'url',
    runtimeEditable: true,
  ...
  {
    name: 'FLOWFORGE_LLM_ROUTE_OPENROUTE_API_KEY',
    defaultValue: '(未设置 — 回落 OPENROUTE_API_KEY)',
    description: 'OpenRoute 网关 API key（Bearer）',
    category: 'proxy',
    sensitive: true,
    runtimeEditable: true,
  },
  {
    ...
    defaultValue: '(未设置)',
    description: 'OpenRoute 网关 API key（通用别名）',
    category: 'proxy',
    sensitive: true,
    runtimeEditable: true,
  },
  {
    name: 'FLOWFORGE_FORGEMIND_OPENROUTE_MODEL',
    defaultValue: 'openai/gpt-4o-mini',
    description: 'ForgekinBase 兜底 LLM 客户端的默认模型名',
    category: 'proxy',
    sensitive: false,
    runtimeEditable: true,
  },
