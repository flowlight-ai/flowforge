 ...
 *
 * Defines the managed block content for the hub itself and for external
 * project CLAUDE.md/AGENTS.md/GEMINI.md/KIMI.md files.
 *
 * Port values are read from environment variables at runtime,
 * falling back to hub defaults (3003/3004/6399/6398).
 * Fixes clowder-ai #601 (Redis port hardcoding) and #602 (frontend/API
 * port hardcoding).
 *
 * 移植自 clowder-ai `config/governance/governance-pack.ts`。
 */
 ...
 *  External caches assuming "same hub version → same governance block"
...
function getPortConstraint(context: GovernanceManagedBlockContext, ports: RuntimePorts): string {
  if (context === 'external') {
    return `- **Hub runtime ports**: frontend ${ports.frontendPort} and API ${ports.apiPort} are reserved by the hub runtime. Avoid using these ports for this project's dev servers.`;
  }
...
function getHardConstraints(context: GovernanceManagedBlockContext = 'self'): string {
  ...
  return `## Governance Rules (Auto-managed)

### Hard Constraints (immutable)
${getPortConstraint(context, ports)}
- **Redis port ${ports.redisPort}** is the hub's production Redis. Never connect to it from external projects. Use ${ports.redisDevPort} for dev/test.
