...
export interface McpServiceOpts {
  /** Monorepo/binary root for topology healing. */
...
export async function installMcpCapability(
  ...
  return io.withLock(async () => {
    ...
    const existingEntry = existingIdx >= 0 ? (config.capabilities[existingIdx] ?? null) : null;
    const before = existingEntry ? structuredClone(existingEntry) : null;

    // Plugin ownership validation (only when the incoming entry has a pluginId)
    if (entry.pluginId && existingEntry) {
      if (existingEntry.pluginId !== undefined && existingEntry.pluginId !== entry.pluginId) {
        throw new Error(`Capability '${entry.id}' is already owned by plugin '${existingEntry.pluginId}'`);
      }
      if (existingEntry.pluginId === undefined) {
    ...
    if (!afterSource) {
      throw new Error(`Capability '${entry.id}' vanished after install mutation`);
    }
    const afterEntry = structuredClone(afterSource);
...
export async function removeMcpCapability(
  ...
  return io.withLock(async () => {
    ...
    const target = nextConfig.capabilities[idx];
    if (!target) return { before: null };
