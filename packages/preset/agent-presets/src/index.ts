 ...
 * @module @flowforge/agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context, Service } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@flowforge/scope'
// Type-only: resolves the `agent/created` lifecycle event this service watches.
import type {} from '@flowforge/agent'
import { settingsNamespace, type SettingsScope, type default as SettingsService } from '@flowforge/settings'
import { flowforgeHomePath } from '@flowforge/home-paths'
...
export class AgentPresets extends Service {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = config.includeUserRoot
      ? [...config.roots, { path: flowforgeHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...config.roots]
   ...
   * scope key. The binding is flowforge-scope's only re-link capability; holding it
   ...
   * from the agent's mount — flowforge-scope's only re-link authority. An agent
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `flowforge-skill-filesystem` watches its
