 ...
 * @module @flowforge/agent-presets/mount
 */
...
import type { EntryTree } from '@flowforge/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@flowforge/scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@flowforge/*` row would fail to import. The
 ...
 * invariant companion's service listener, and `flowforge-invariants` is a
