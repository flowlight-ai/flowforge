...
import { COMPOSITION_FILE, discoverPresets, scanRoot } from '@flowforge/agent-presets'
...
describe('display order', () => {
  it('puts declared order first, then everything else by id', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-order-'))
  ...
  it('breaks a tie between equal declared orders by id', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-order-tie-'))
...
describe('preset discovery', () => {
  ...
  it('skips a directory whose name no preset id could ever claim', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-presets-oddname-'))
  ...
  it('ignores a plain file sitting beside the preset directories', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-presets-'))
  ...
  it('expands a leading tilde in a root path', async () => {
    ...
    const found = await scanRoot({ path: '~/.flowforge-agent-presets-absent', trust: 'user' })
...
describe('composition health', () => {
  ...
  async function scanned(composition: string): Promise<string | undefined> {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-presets-health-'))
  ...
  it('reports a composition that stats but cannot be read', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-presets-unreadable-'))
