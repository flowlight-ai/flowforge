/**
 * cat-catalog.json 引导/迁移/读写（C37）。
 *
 * TS 移植自 clowder-ai `config/cat-catalog-store.ts`（666 行）：
 *   - migrateCatalogVariants：P5 一次性变体迁移（provider→clientId、
 *     ocProviderName→provider、providerProfileId→accountRef、旧 gemini 消费 carrier、
 *     降级 standalone breed 的遗留变体）— 重写文件后不再执行
 *   - stripLegacySourceField / ensureOwnerInRoster：幂等一次迁移
 *   - persistMissingTemplateBreeds/Variants：模板新增成员白名单回填（墓碑阻止复活）
 *   - bootstrapCatCatalog：首次引导（#948 种子 breed + roster 裁剪）与升级修复
 *   - readCatCatalogRaw/readCatCatalog/writeCatCatalog：读写入口
 *
 * 插件化改造决策：
 *   - `resolveBuiltinClientForProvider` → 内部 `isBuiltinClientId` 纯判断（仅用存在性）
 *   - `inheritFullyBlockedMcpCapabilitiesForNewCatsSync` → `catalogPorts.mcpCapabilitiesInheritor`
 *     端口注入（缺省 noop，capabilities 域由宿主接入）
 *   - 测试沙箱写保护 env 改名 FF_* 系（R17）
 */

import { existsSync, mkdirSync, readFileSync } from 'node:fs';
import { dirname } from 'node:path';
import type { CatCafeConfig, ClientId, RosterEntry } from '@flowforge/cats-shared';

import { pickSeedBreed, pruneRosterToRuntimeBreeds, type RuntimeBreedWithCatIds } from './cat-catalog-bootstrap-roster.js';
import {
...
import { isRecord, resolveCafeFilePath, safePath, writeFileAtomic } from './fs-utils.js';
import { catalogPorts, type RecordOf } from './types.js';
...
/** 存在性判断：provider/clientId 是否内置 client（原 resolveBuiltinClientForProvider 简化）。 */
export function isBuiltinClientId(clientId: string | undefined): boolean {
  return clientId !== undefined && CLIENT_ID_VALUES.has(clientId);
}
...
function migrateCatalogVariants(
  ...
  for (const breed of next.breeds as unknown as RecordOf[]) {
    if (typeof breed.id === 'string') standaloneBreedIds.add(breed.id);
  }

  for (const breed of next.breeds as unknown as RecordOf[]) {
    const variants = Array.isArray(breed.variants) ? (breed.variants as RecordOf[]) : [];
    for (const variant of variants) {
      ...
      const clientId = (variant.clientId ?? variant.provider) as ClientId | undefined;
      if (!isBuiltinClientId(clientId)) continue;
...
function resolveVariantCatId(breed: RecordOf, variant: RecordOf): string | undefined {
...
function collectRuntimeIdentityOccupancy(breeds: RecordOf[]): RuntimeIdentityOccupancy {
...
function persistMissingTemplateBreeds(projectRoot: string, catalogPath: string, templatePath: string): boolean {
  ...
  const next = structuredClone(catalog) as CatCafeConfig;
  const nextBreeds = next.breeds as unknown as RecordOf[];
  const existingBreedIds = new Set(
    nextBreeds.map((breed) => breed.id).filter((id): id is string => typeof id === 'string'),
  );
  const templateRoster =
    template.version === 2 ? (template.roster as unknown as RecordOf) : ({} as RecordOf);
  const nextRoster =
    next.version === 2 ? (next.roster as unknown as RecordOf) : ({} as RecordOf);
  ...
  let dirty = false;
  for (const templateBreed of template.breeds as unknown as RecordOf[]) {
    ...
    if (hasTombstonedTemplateBreedVariant(catalog as unknown as RecordOf, templateBreed)) continue;
  ...
  writeFileAtomic(catalogPath, `${JSON.stringify(next, null, 2)}\n`);
  catalogPorts.mcpCapabilitiesInheritor(projectRoot, backfilledCatIds, existingCatIds);
  return true;
}
...
function persistMissingTemplateVariants(projectRoot: string, catalogPath: string, templatePath: string): boolean {
  ...
  const template = JSON.parse(templateRaw) as CatCafeConfig;
  const next = structuredClone(catalog) as CatCafeConfig;
  const templateBreeds = new Map<string, RecordOf>();
  for (const breed of template.breeds as unknown as RecordOf[]) {
  ...
  const nextRoster =
    next.version === 2 ? (next.roster as unknown as RecordOf) : ({} as RecordOf);
  const occupancy = collectRuntimeIdentityOccupancy(next.breeds as unknown as RecordOf[]);
  const existingCatIds = new Set(occupancy.catIds);
  const backfilledCatIds: string[] = [];
  ...
  for (const breed of next.breeds as unknown as RecordOf[]) {
    ...
    const existingVariantIds = new Set(
      variants
        .filter((variant): variant is RecordOf => isRecord(variant))
    ...
    for (const templateVariantUnknown of templateVariants) {
      ...
      if (
        isTemplateVariantTombstoned(catalog as unknown as RecordOf, {
          breedId: breed.id,
          variantId: templateVariantUnknown.id,
          catId,
        })
...
export function resolveCatCatalogPath(projectRoot: string): string {
  return resolveCafeFilePath(projectRoot, 'cat-catalog.json');
}

/**
 * Best-effort read of breed.id values from a sibling cat-template.json.
 * Returns an empty set if the template is missing or unreadable.
 */
...
export function bootstrapCatCatalog(projectRoot: string, templatePath: string): string {
  ...
  // one usable member. In dev environments (template has no breeds), start empty.
