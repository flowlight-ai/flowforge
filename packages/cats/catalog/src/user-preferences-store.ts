/**
 * user-preferences.json 共享文件存储（C38）。
 *
 * TS 移植自 clowder-ai `config/user-preferences-store.ts`（137 行）：所有
 * 特性级偏好写入必须经此模块，防止一个设置覆盖另一个；写作为崩溃安全
 * temp+rename；偏好无 TTL。messageDisposition 支持 global/thread/onboarding
 * 三作用域解析（product 缺省 `next_work`）。
 */
