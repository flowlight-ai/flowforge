/**
 * Relationship-persona primer write + provenance (KD-12/KD-15/KD-18).
 *
 * Ported from clowder-ai `writeProfileUpdate.ts`
 * (api/src/domains/cats/services/profile/) as pure functions + injectable
 * file ops — no Cordis dependency, unit-testable in isolation.
 *
 * P1-1 (codex review): the two file side-effects are SPLIT into separate
 * functions so the approval pipeline can checkpoint `writtenPath` AFTER the
 * primer write but BEFORE provenance starts. A single combined call left an
 * unrecoverable window — if provenance failed or the process died between
 * writes, the pipeline never persisted that the primer had already changed,
 * and retry hit a stale-hash mismatch. Now: writeProfilePrimer →
 * recordCheckpoint(writtenPath) → writeProfileProvenance →
 * recordCheckpoint(provenancePath) → finalize. Crash recovery reads the
 * checkpoint and skips the already-done step (no primer re-hash, no
 * provenance dup).
 *
 * P1-2 (codex review): targetPath is validated as profile-dir-relative
 * `relationship/{relationshipKey}-primer.md` — no absolute paths, no `..`
 * escape.
 *
 * P1-2 optimistic lock: writeProfilePrimer re-reads the current primer and
 * compares its hash to the proposal's `baseContentHash`. The CALLER
 * (ProfileApprovalService) MUST hold the per-target lock so this
 * re-read→compare→write is atomic against concurrent same-primer approves.
 *
 * @module @flowforge/cats-profile/write
 */
...
import { relationshipPrimerRelativePath } from '@flowforge/cats-shared/profile-contract'
...
export interface WriteProfilePrimerOptions {
  /**
   * Crash recovery only: proposal is already `approving` and the primer may
   * have been written before `writtenPath` was checkpointed. Pending
   * proposals must not use this shortcut because it would bypass the
   * optimistic lock for independent proposals with identical afterContent.
   */
 ...
 * `relationship/{relationshipKey}-primer.md`, profile-dir-relative, no
 * `..`/absolute escape.
 */
 ...
 * Returns the writtenPath; the caller checkpoints it BEFORE calling writeProfileProvenance.
 */
...
/**
 * P1-1 step 2: write deterministic-path provenance. Uses `proposal.beforeContent`
 * (pinned at propose), NOT the current primer — so crash recovery (primer
 * already overwritten) still records the correct before/after. Idempotent:
 * same proposalId overwrites the same file.
 */
