/**
 * @flowforge/cats-agent-hooks — C31 sync-targets（agent hook 同步目标）
 *
 * TS 移植：clowder-ai `agent-hooks/sync-targets.ts`（纯函数直移）。
 * 4 个 SyncTarget：session-start / session-stop 可执行脚本 + codex-hooks /
 * gemini-hooks JSON；canonical JSON 漂移检测 + applySync（dryRun + chmod）。
 */
...
export function checkDrift(target: SyncTarget): DriftResult {
  ...
  return {
    ...
    drifted,
    targetPath: target.targetPath,
    ...(drifted ? { reason: 'content differs from rendered shards' } : {}),
  };
}
