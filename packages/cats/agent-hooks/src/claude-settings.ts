/**
 * @flowforge/cats-agent-hooks — C31 claude-settings（Claude settings.json 钩子管理）
 *
 * TS 移植：clowder-ai `agent-hooks/claude-settings.ts`（纯函数直移）。
 * MANAGED_HOOKS = SessionStart: session-start-recall.sh / Stop: session-stop-check.sh；
 * 四态健康（configured/missing/stale/error）+ 保留用户 hooks 的同步（bash 前缀跨平台）。
 */
