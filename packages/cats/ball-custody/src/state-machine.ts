/**
 * @flowforge/cats-ball-custody — C24 Ball Custody State Machine（transition 纯函数）
 *
 * TS 移植自 clowder-ai `packages/api/src/domains/ball-custody/ball-custody-state-machine.ts`
 * （F233 Phase B）。纯函数 — 零 IO、零副作用，照 community-state-machine 的**表驱动**
 * 模式（TRANSITION_TABLE 而非 if-chain，降单函数 cognitive complexity）。
 ...
 *   - invocation.heartbeat from dead：迟到心跳 grace（died.at < hb.at ≤ died.at+GRACE）
 *
 * @module @flowforge/cats-ball-custody/state-machine
 */

import type { BallCustodyEvent, BallCustodyProjection, BallIntent, BallState } from './models.js';

/** 死球迟到心跳 grace（复用 clowder DEAD_BALL_ZOMBIE_GRACE_MS=600s）。 */
export const DEAD_BALL_ZOMBIE_GRACE_MS = 600_000;

/** 全 8 态（穷举测试遍历用）。 */
export const ALL_BALL_STATES: readonly BallState[] = [
  'new',
  'active',
  'blocked',
  'parked',
...
/** 全 17 种事件 kind（穷举测试遍历用，INV-10）。 */
export const ALL_BALL_EVENT_KINDS: readonly BallCustodyEvent['kind'][] = [
  'ball.handed',
  'ball.handed_cvo',
  'ball.void_pass',
  'ball.held',
...
function resolveHandedCvo(event: BallCustodyEvent, current: BallState): BallTransitionResult {
  ...
  if (typeof intent !== 'string' || !VALID_INTENTS.includes(intent as BallIntent)) {
    return reject('bad_payload');
  }
  if (!set('new', 'active', 'blocked', 'parked', 'void', 'zombie').has(current)) {
...
const STATIC_TABLE: Partial<Record<BallCustodyEvent['kind'], StaticRule>> = {
  ...
  'task.blocked': { from: set('new', 'active', 'void', 'zombie', 'parked'), to: 'blocked' },
  ...
  'ball.wake_condition_met': { from: set('active'), to: 'active' }, // wakeWhen 命令完成，球保持 active
  // ─── Phase C 安乐死（KD-C2 三独立 kind 共享转移行为：7 非-resolved → resolved）───
