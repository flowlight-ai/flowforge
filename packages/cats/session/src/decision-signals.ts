 ...
 *
 * 移植自 clowder-ai
 * `packages/api/src/domains/cats/services/session/extractDecisionSignals.ts`
 * （全量移植；`ThreadMemorySourceRef` 依赖改从 `./thread-memory.ts` 导入，
 * 该类型在 flowforge 中随批次 6.4 落地于本包）。
 *
 * @module @flowforge/cats-session/decision-signals
 */

import type { ThreadMemorySourceRef } from './thread-memory.ts';
...
export function extractDecisionSignals(input: DecisionSignalsInput): DecisionSignals {
  ...
  for (const match of input.transcriptText.matchAll(ARTIFACT_PATTERN)) {
    if (match[1]) artifactMatches.add(match[1]);
  }
