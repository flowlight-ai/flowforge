 ...
 * （全量移植；使用 raw fetch 无 SDK 依赖，`fetchFn` 参数注入保留，
 * 模块自身零 clowder/flowforge 依赖 —— 可独立单测）。
 *
 ...
 *
 * @module @flowforge/cats-session/handoff
 */

import type { HandoffInvocationSummary } from './transcript-formatter.ts';
