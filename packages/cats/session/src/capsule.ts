/**
 * CollaborationContinuityCapsule — 协作连续性胶囊（纯函数模块）。
 *
 * 移植自 clowder-ai
 * `packages/api/src/domains/cats/services/agents/invocation/CollaborationContinuityCapsule.ts`
 * （全量移植：类型 + 8 个导出函数 + 私有守卫；零依赖，可独立单测）。
 *
 * 胶囊是 seal 边界上捕获的最新结构化协作控制流状态（mode / chain /
 * a2a / ball state），随 system_info 事件写入转录，并在续接时由
 * `formatContinuationPrompt` 渲染为续接指令。
 *
 * @module @flowforge/cats-session/capsule
 */
