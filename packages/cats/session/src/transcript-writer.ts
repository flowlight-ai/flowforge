/**
 * TranscriptWriterService — F24 Phase C 会话转录写入 Cordis 服务。
 *
 * 移植自 clowder-ai
 * `packages/api/src/domains/cats/services/session/TranscriptWriter.ts`
 * （全量移植；R13 一切皆插件改造，模块级类改为 Cordis 生命周期，
 * `Context` 扩展挂载点：`ctx.catsTranscriptWriter`）。
 *
 * 内存缓冲调用事件，seal 时落盘 JSONL：
 * - `events.jsonl` — 带 envelope 的规范 NDJSON（seal 时写入）
 * - `events.live.jsonl` — 增量崩溃恢复副本（仅活跃会话）
 * - `index.json` — 分页用稀疏字节偏移索引
 * - `digest.extractive.json` — 规则式抽取摘要（无 LLM、确定性、零成本）
 *
 * events.jsonl envelope:
 *   { v:1, t:number, threadId, catId, sessionId, cliSessionId, invocationId?, eventNo, event }
 *
 * @module @flowforge/cats-session/writer
 */

import { appendFile, mkdir, readFile, unlink, writeFile } from 'node:fs/promises'
import { join, resolve } from 'node:path'
import { Context, Service } from '@flowforge/cordis'
import type { CollaborationContinuityCapsuleV1 } from './capsule.ts'
import { extractContinuityCapsuleFromSystemInfo } from './capsule.ts'
import { DEFAULT_TRANSCRIPT_DIR, TRANSCRIPT_INDEX_STRIDE } from './invariant.ts'
import { stripLeakedToolCallPayload } from './text-sanitize.ts'
...
export interface TranscriptWriterOptions {
  /** 转录根目录；默认 `data/transcripts`（受 CATS_TRANSCRIPT_DIR 环境变量覆盖）。 */
  dataDir?: string;
  /** 稀疏索引步长（默认 100）。 */
  indexStride?: number;
}
...
declare module '@flowforge/cordis' {
  interface Context {
    /**
     * Forgekin (cats) 会话转录写入器 — mounted by `@flowforge/cats-session`.
     * 内存缓冲调用事件，seal 时落盘 events.jsonl + index + 抽取摘要。
     */
    catsTranscriptWriter: TranscriptWriterService
  }
...
/**
 * Cordis service exposing the session transcript writer at `ctx.catsTranscriptWriter`.
 */
export class TranscriptWriterService extends Service {
  ...
  constructor(ctx: Context, options: TranscriptWriterOptions = {}) {
    super(ctx, 'catsTranscriptWriter')
    this.dataDir = resolve(options.dataDir ?? process.env.CATS_TRANSCRIPT_DIR ?? DEFAULT_TRANSCRIPT_DIR)
    this.indexStride = options.indexStride ?? TRANSCRIPT_INDEX_STRIDE
  }
  ...
  async flush(
    ...
    if (liveEvents.length > 0) {
      ...
      for (let i = 0; i < buf.length; i++) {
        const entry = buf[i];
        if (entry) buf[i] = { ...entry, eventNo: i };
      }
      this.buffers.set(session.sessionId, buf);
    }
