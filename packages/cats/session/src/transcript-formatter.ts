 ...
 *
 * 移植自 clowder-ai
 * `packages/api/src/domains/cats/services/session/TranscriptFormatter.ts`
 * （全量移植；`TranscriptEvent` 依赖改从 `./transcript-reader.ts` 导入）。
 *
 * Views:
 * - chat: role/content pairs (filters out tool events)
 * - handoff: per-invocation summaries (meeting-minutes style)
 *
 * @module @flowforge/cats-session/formatter
 */

import type { TranscriptEvent } from './transcript-reader.ts';
