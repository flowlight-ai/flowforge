/**
 * @flowforge/cats-packs — C30 PackLoader（F129 校验 + 安装编排）
 *
 * TS 移植：clowder-ai `domains/packs/PackLoader.ts`。
 * 高层编排：source → 安全校验 → 增长边界 → 存储。
 *
 * AC-A4: cafe pack add <git-url>
 * AC-A5: cafe pack list / remove
 * Phase A：仅本地目录路径（Git URL 拒绝，减少攻击面）。
 */

import { readFile, stat } from 'node:fs/promises';
import { join } from 'node:path';
import type { PackManifest } from '@flowforge/cats-shared';
import { PackManifestSchema } from '@flowforge/cats-shared';
import { parse } from 'yaml';
import { checkGrowthBoundary } from './growth-boundary.js';
import type { PackSecurityGuard, SecurityResult } from './pack-security-guard.js';
