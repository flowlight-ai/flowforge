/**
 * @flowforge/cats-packs — C30 PackSecurityGuard（F129 恶意内容检测）
 *
 * TS 移植：clowder-ai `domains/packs/PackSecurityGuard.ts`。
 * 对 pack 目录做 9 步安全校验：
 * - Prompt injection 模式（AC-A7）
 * - Schema fail-closed（AC-A8）
 * - capabilities/ 拒绝（AC-A9）
 * - 身份覆盖阻止（KD-3）
 * - 约束方向强制（KD-9）
 */
