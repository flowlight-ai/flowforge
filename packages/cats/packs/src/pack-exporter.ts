/**
 * @flowforge/cats-packs — C30 PackExporter（F129 cat-config + shared-rules + skills → Pack 目录）
 *
 * TS 移植：clowder-ai `domains/packs/PackExporter.ts`。
 * Phase B-α：Dogfood 导出 "Coding World" pack（AC-B1）。
 *
 * Mapping:
...
export class PackExporter {
  ...
  exportMasks(catConfig: CatConfigLike): PackMask[] {
    ...
    for (const breed of catConfig.breeds) {
      ...
      const variantRole = variant.roleDescription ?? `${primaryRole} role`;
      masks.push({
        id: `${breed.id}-${primaryRole}`,
        name: `${breed.displayName} ${primaryRole}`,
        roleOverlay: variantRole,
        ...(variant.personality ? { personalityOverlay: variant.personality } : {}),
...
function extractSection(content: string, heading: string): string {
  ...
  for (const line of lines) {
    ...
    if (capturing) {
      if (headingMatch && headingLevel <= depth) break;
      result.push(line);
    }
  }
  return result.join('\n');
