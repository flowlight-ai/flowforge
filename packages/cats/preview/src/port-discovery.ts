/**
 * @flowforge/cats-preview — 端口发现服务（F120 Phase B）。
 *
 * TS 移植自 clowder-ai `domains/preview/port-discovery.ts`：
 * tmux pane stdout 监听 dev server 端口公告 → 解析 framework 提示 →
 * probe 可达性 → onDiscovered 事件（socket.io 投递剥离为监听器端口）。
 * 插件化改造：`fetch` 探测剥离为 `probeFn` 注入（缺省全局 fetch）。
 *
 * @module @flowforge/cats-preview/port-discovery
 */
 ...
 * （socket.io 投递剥离为 onDiscovered 监听器端口。）
 */
