/**
 * @flowforge/cats-preview — Bridge 注入脚本（F120 Phase C）。
 *
 * TS 移植自 clowder-ai `domains/preview/bridge-script.ts`（原样保留，
 * 纯前端字符串常量，无运行时依赖）：
 * 注入 preview 页面 → patch console.* 转发 postMessage 到父窗口（Hub）；
 * screenshot-request → SVG foreignObject + canvas 截屏回传。
 *
 * @module @flowforge/cats-preview/bridge-script
 */
