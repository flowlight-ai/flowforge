/**
 * F222: 富文本卡片构建器（迁移自 clowder-ai 同名文件）。
 */

import type { FrustrationIssue, RichCardBlock, RichInteractiveBlock } from './frustration-types.ts'
...
function buildSignalFields(issue: FrustrationIssue): Array<{ label: string; value: string }> {
  ...
  if (issue.signalType === 'cli_error') {
    if (issue.signalDetail.reasonCode) fields.push({ label: '错误类型', value: String(issue.signalDetail.reasonCode) })
    if (issue.signalDetail.publicHint) fields.push({ label: '建议', value: String(issue.signalDetail.publicHint) })
...
function buildBodyMarkdown(issue: FrustrationIssue): string {
  ...
  if (issue.signalType === 'cli_error' && issue.signalDetail.publicSummary) {
    ...
    parts.push('**你主动发起了问题反馈**，已采集当前上下文。')
  }
