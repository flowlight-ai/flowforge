/**
 * MemorySessionChainStore — in-memory ISessionChainStore（F24 会话链）。
 *
 * 移植自 clowder-ai `SessionChainStore`（`SessionChainStore.ts` 内的 Memory
 * 实现，批次 6.2a，语义全量保留）。单线程 Node.js → 同步 Map 操作：
 * randomUUID 主键、seq 自增、activeIndex / cliIndex / chainKeyIndex 三索引。
 *
 * 容量语义（源码照搬）：MAX_RECORDS=1000，三级驱逐 sealed → non-active →
 * superseded；全部为真 active 时拒绝驱逐——回滚刚创建的记录并抛错。
 * update 保留 sealReason / sealedAt 的 null 删除语义。
 *
...
import type {
  ...
  SessionRecordPatch,
  StoreReadOptions,
} from '../ports/session-chain-store.ts'
import { throwIfStoreReadAborted } from '../ports/session-chain-store.ts'
