/**
 * @flowforge/cats-projects — slice store（F076 Stage 4 切片规划）。
 *
 * TS 移植自 clowder-ai `domains/projects/slice-store.ts`：
 * per-project order 计数器 + reorder 交换 + 按类型过滤。
 *
 * @module @flowforge/cats-projects/slice-store
 */

import type { CreateSliceInput, Slice, SliceType, UpdateSliceInput } from './types.js';
import { generateSortableId } from './types.js';

/** F076 Stage 4 切片计划内存存储。 */
