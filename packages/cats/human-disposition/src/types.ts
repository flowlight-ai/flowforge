/**
 * @flowforge/cats-human-disposition — F281 类型 + 纯函数（zod 验证）。
 *
 * TS 移植自 clowder-ai `packages/shared/src/types/human-disposition-feedback.ts`：
 * reason codes 6 类、feedback discriminatedUnion、scope/expiry/invalidator 三 discriminatedUnion、
 * server binding / decision episode / envelope / ledger entry（superRefine 身份一致性）/ receipt，
 * 以及 build / classify / isEligible 纯函数。
 *
 * @module @flowforge/cats-human-disposition/types
 */
