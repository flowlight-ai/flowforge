/**
 * @flowforge/cats-human-disposition — F281 CAS append lua 脚本（宿主 Redis 集成）。
 *
 * TS 移植自 clowder-ai `domains/human-disposition/human-disposition-lua.ts`：
 * preflight（类型/字段/值三重校验）+ NEW/REPLAY/CONFLICT 判定 + 原子写入。
 * 本包默认 Memory KV 已在 `./ledger.js` 内置等价 CAS；此 lua 常量供 Redis 宿主直接加载。
 *
 * @module @flowforge/cats-human-disposition/lua
 */
