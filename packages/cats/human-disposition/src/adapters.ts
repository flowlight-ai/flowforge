/**
 * @flowforge/cats-human-disposition — F281 disposition adapters（3 场景 → ledger entry）。
 *
 * TS 移植自 clowder-ai `domains/human-disposition/human-disposition-adapters.ts`：
 * session-handoff 拒绝 / person-memory proposal 拒绝（opaque proof）/ wait-cancel 取消，
 * 统一产出 canonical ledger entry（zod 验证）。
 *
 * @module @flowforge/cats-human-disposition/adapters
 */

import { randomBytes } from 'node:crypto';
import { z } from 'zod';
import { buildHumanDispositionLedgerEntry } from './types.js';
import type { HumanDispositionFeedbackInput, HumanDispositionLedgerEntry } from './types.js';
import { userCancelWaitTerminationEventSchema } from './wait-cancel-types.js';
...
export interface WaitCancellationDispositionAdapterInput {
  event: z.input<typeof userCancelWaitTerminationEventSchema>;
  feedback?: HumanDispositionFeedbackInput;
}
