/**
 * @flowforge/cats-taste — In-memory TasteProposalStore（F221 Phase B）。
 *
 * TS 移植自 clowder-ai `domains/taste/stores/InMemoryTasteProposalStore.ts`（原样保留）：
 * 状态机 `pending → approving → approved`（claim 后 finalize）、
 * `pending → rejected`（one-shot）、`approving → pending`（writer 失败回滚）、
 * `approving → approved`（fresh-session resume after durable checkpoint）。
 *
 * 插件化改造：`@cat-cafe/shared` 替换为 `@flowforge/cats-shared`（generateProposalId /
 * assertApprovalEnvelopeIdentity / commitApprovalEnvelope），`ApprovalPublicationStore`
 * 端口来自本地 `./types.js`。
 *
 * @module @flowforge/cats-taste/store
 */

import type { ApprovalEnvelope, ApprovalPublication, TasteProposal } from '@flowforge/cats-shared';
import { assertApprovalEnvelopeIdentity, commitApprovalEnvelope, generateProposalId } from '@flowforge/cats-shared';
import type { ApprovalPublicationStore, CreateTasteProposalInput, ITasteProposalStore, TasteWriteCheckpoint } from './types.js';
...
export class InMemoryTasteProposalStore implements ITasteProposalStore, ApprovalPublicationStore {
  ...
  create(input: CreateTasteProposalInput): TasteProposal {
    ...
    const proposal: TasteProposal = {
      ...
      ...(input.approvalOriginRef !== undefined ? { approvalOriginRef: input.approvalOriginRef } : {}),
