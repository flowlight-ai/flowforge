/**
 * @flowforge/cats-plugin-inventory — C30 snapshot 解析（P15）
 *
 * TS 移植：clowder-ai `domains/plugin/host-inventory/snapshot.ts`。
 * fail-closed 快照解析：任何结构/不变量违规抛 CORRUPT_SNAPSHOT，
 * 未知 schema 版本抛 UNSUPPORTED_SCHEMA。
 */

import { PLUGIN_CONTRACT_VERSION, requestedCapabilitiesForManifest } from './contract-policy.js';
import { validateEffectiveGrants, validateManifest } from './contract.js';
import type {
  ActivationState,
  ConfigReadiness,
