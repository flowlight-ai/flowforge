/**
 * @flowforge/cats-plugin-inventory — C30 HostInventoryControlPlane（P15 控制面）
 *
 * TS 移植：clowder-ai `domains/plugin/host-inventory/control-plane.ts`。
 * install / upgrade / reinstall / revokeGrant / recoverAfterRestart，
 * now / createInstanceId 注入（插件化：宿主可控时钟与 ID 生成）。
 */
