/**
 * @flowforge/cats-plugin-inventory — C30 inventory 存储实现（P15）
 *
 * TS 移植：clowder-ai `domains/plugin/host-inventory/stores.ts`。
 * Memory 实现（事务队列串行化）+ File 实现（原子 temp+rename，
 * 每路径独立事务队列，fileOps 可注入以便测试）。
 */
