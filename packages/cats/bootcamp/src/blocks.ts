/**
 * Bootcamp 交互块定义（C8，clowder cats/services/bootcamp/bootcamp-blocks.ts 移植）。
 *
 * F087：bootcamp 各阶段的预置富交互块配置，供 bootcamp 引导技能渲染选择 UI：
 * 阶段 0 引导猫选择（card-grid）+ 阶段 4 任务选择（分层 card-grid）。
 */

export interface BootcampInteractiveBlockOption {
  id: string;
  label: string;
  emoji?: string;
  icon?: string;
...
export interface BootcampInteractiveBlock {
  ...
  title: string;
  description?: string;
  options: BootcampInteractiveBlockOption[];
  allowRandom?: boolean;
...
/** 全部 bootcamp 交互块索引。 */
export const BOOTCAMP_BLOCKS: Readonly<Record<string, BootcampInteractiveBlock>> = {
