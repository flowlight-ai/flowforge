/**
 * 来源访问租约：为会议来源的受控读取颁发/消费一次性 grant。
 * 忠实移植 clowder-ai `domains/signal-intake/SourceAccessLeaseService.ts`。
 *
...
export class SourceAccessLeaseService {
  ...
  async issue(input: {
    ...
    readonly principalId: string
    readonly purpose: SourceAccessPurpose
  }): Promise<{ readonly grant: string; readonly expiresAt: number }> {
  ...
  async resolve(
    ...
    signal: AbortSignal,
  ): Promise<SourceArtifact & { readonly provenance: { readonly sourceHandle: string; readonly trust: 'untrusted_external'; readonly instructionPolicy: 'data_only' } }> {
