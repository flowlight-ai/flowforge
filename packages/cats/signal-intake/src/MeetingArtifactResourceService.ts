/**
 * 会议产物资源服务：owner/thread/cat 授权 + revision 校验 + 分页游标。
 * 忠实移植 clowder-ai `domains/signal-intake/MeetingArtifactResourceService.ts`，
 * 交付载体读取改为经 `MeetingArtifactCarrierReader` 端口注入（宿主 EP2 接线）。
 *
...
import { pageWithinMeetingArtifactBudgets } from './meeting-artifact-read-budget.ts'
import {
...
} from './meeting-artifact-resource-contract.ts'
import { parsePrivateThreadHandle } from './ThreadDestinationAuthority.ts'
...
function filterHash(input: MeetingArtifactReadInput): string {
  return createHash('sha256').update(JSON.stringify(normalizedFilters(input))).digest('hex')
}
...
function parseLine(text: string, start: number, end: number): ParsedLine {
  ...
  const timeMs = structural?.[1] ? parseClock(structural[1].split(':')) : undefined
  const speaker = structural?.[2] ? structural[2].trim() : speakerOnly?.[1] ? speakerOnly[1].trim() : undefined
  return {
    start,
    end,
    text,
...
export class MeetingArtifactResourceService {
  ...
  async read(input: MeetingArtifactReadInput) {
    ...
    const carrier = message?.meetingArtifact
    if (
      !message ||
      message.connectorFromSource !== 'feishu' ||
    ...
    if (
      ...
      throw new MeetingArtifactResourceError('SOURCE_REVISION_CHANGED', 'source bytes changed after this task was bound')
    }
