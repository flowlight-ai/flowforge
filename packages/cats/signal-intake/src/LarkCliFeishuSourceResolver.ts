/**
 * LarkCli 飞书来源解析器：把 canonical `feishu://meeting-artifacts/…` 句柄
 * 解析为来源制品；`run`/`fs` 均注入（默认回退为发起子进程，宿主 EP2 接凭据）。
 * 忠实移植 clowder-ai `domains/signal-intake/LarkCliFeishuSourceResolver.ts`。
 *
...
import {
  ...
  isValidFeishuArtifactId,
  isValidFeishuRevision,
} from './extract/minutes-reference.ts'
import type { SourceArtifact, SourceResolver } from './SourceAccessLeaseService.ts'

const execFileAsync = promisify(execFile)
const MAX_TRANSCRIPT_BYTES = 2_000_000
const TEMP_DIR_PREFIX = 'cat-cafe-f292-'
...
function parseSourceHandle(value: string): FeishuLocator {
  ...
  if (
    ...
    !revision ||
    !isValidFeishuArtifactId(artifactId) ||
    !isValidFeishuRevision(revision)
  ) {
    throw new TypeError('Feishu source handle must be canonical')
  }
...
export class LarkCliFeishuSourceResolver implements SourceResolver {
  ...
  constructor(options: LarkCliFeishuSourceResolverOptions = {}) {
    ...
    this.makeTempDirectory = options.makeTempDirectory ?? (() => mkdtemp(join(tmpdir(), TEMP_DIR_PREFIX)))
  ...
  async resolve(
    ...
    try {
      ...
      if (!verbatimToken || !isValidFeishuArtifactId(verbatimToken)) {
