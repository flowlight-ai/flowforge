/**
 * 会议入站服务：confirm/dismiss/markRepair/clearRepair 的状态机写操作（CAS）。
 * 忠实移植 clowder-ai `domains/signal-intake/MeetingIntakeService.ts`。
 * `MeetingIntake`/`MeetingIntakeChoices`/`MeetingIntakeRepair` 见 `contract/signals.ts`。
 *
 * @flowforge/cats-signal-intake
 */

import type { DestinationAuthority } from './DestinationAuthority.ts'
import type { MeetingIntake, MeetingIntakeChoices, MeetingIntakeRepair } from './contract/signals.ts'
