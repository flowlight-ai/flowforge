/**
 * SkillConsumptionReceiptService — ported from clowder-ai. Issues verifiable
 * one-shot skill "consumption receipts" bound to a specific scope + revision,
 * so a downstream (e.g. workspace navigator) can prove it was authorized before
 * mutating. Skill package revisions are computed by hashing the package tree;
 * audit events are written to an injected audit-log port.
 */
...
export class SkillConsumptionReceiptService {
  ...
  private decrypt(handle: string): SkillConsumptionCoordinate | null {
    ...
    try {
      ...
      const ciphertext = Buffer.from(part2, 'base64url')
      const tag = Buffer.from(part3, 'base64url')
