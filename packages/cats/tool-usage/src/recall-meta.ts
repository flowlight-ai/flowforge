/**
 * Recall meta sidecar parsing — inlined from clowder `@cat-cafe/shared` so the
 * tool-usage package stays dependency-free. When a memory tool (search_evidence,
 * graph_resolve, list_recent) appends a `<recall-meta>…</recall-meta>` sidecar
 * block to its text output, this module parses the structured summary the
 * event-log aggregator needs.
 */
...
const RECALL_MATCH_RANKS = ['high', 'mid', 'low'] as const
export type RecallMatchRank = (typeof RECALL_MATCH_RANKS)[number]

const RECALL_MATCH_TYPES = ['direct', 'alias', 'source-thread', 'convention'] as const
...
// biome-ignore lint/correctness/noUnusedVariables: shared compatibility shim.
...
function isRecallMatchRank(value: unknown): value is RecallMatchRank {
  return typeof value === 'string' && MATCH_RANK_SET.has(value)
}

function isRecallMatchType(value: unknown): value is RecallMatchType {
...
function parseConventionEdgeFunnel(raw: unknown): {
  ...
  return { attempted: o['attempted'], added, deduped, staleSkipped }
}
...
function parseExpansionHints(raw: unknown): ExpansionHintRef[] | null {
  ...
  for (const value of raw) {
    ...
    seen.add(key)
    hints.push({ anchor: item['anchor'], targetRef })
  }
  return hints
}
