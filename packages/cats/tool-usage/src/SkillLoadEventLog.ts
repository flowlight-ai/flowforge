 ...
 * skill triggered) — carries loadTrigger context that Skill `tool_use` count
 * doesn't (and avoids dedup).
 */

import { skillLoadLogKey, TOOL_EVENT_LOG_TTL_SECONDS } from './redis-keys.ts'
import { createModuleLogger } from './logger.ts'
import type { SkillLoadedEvent } from './event-log-types.ts'
import type { SortedSetStore } from './store.ts'

const log = createModuleLogger('skill-load-event-log')

export class SkillLoadEventLog {
  constructor(private readonly store: SortedSetStore) {}
