 ...
 * - `cat_cafe_*`            — bare cat-cafe MCP tool
 * - `signal_*`              — bare signal MCP tool
 *
 * Used by span-emission helpers to decide whether to create a child
 * `cat_cafe.tool_use` span.
 *
 * NOTE: intentionally wider than `classifyTool()`; this helper does not change
 * F150 behavior (classifyTool returns `native` for bare prefixes).
 */
