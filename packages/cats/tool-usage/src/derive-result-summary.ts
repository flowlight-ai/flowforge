 ...
 * Used by route-serial / route-parallel tool_result handlers to merge
 ...
 * machine-readable result markers in their text output; this parser extracts
 * the structured summary the aggregator needs. Ported from clowder-ai with the
 * `@cat-cafe/shared` recall-meta helpers inlined locally.
 */

import { parseRecallMetaFromText, stripAppendedRecallMetaBlock } from './recall-meta.ts'
...
function deriveSearchEvidence(text: string): ResultSummary {
  ...
  for (let i = 0; i < anchorMatches.length; i++) {
    ...
    const cand: { anchor: string; rank: number; docKind?: string; sourcePath?: string } = {
      anchor,
      rank: f200Cands.length,
    }
    if (docKind) cand.docKind = docKind
    if (sourcePath) cand.sourcePath = sourcePath
    f200Cands.push(cand)
  }
...
function deriveGraphResolve(text: string): ResultSummary {
  ...
  if (candidates?.[1]) {
    ...
    while (cm !== null) {
      const cAnchor = cm[2]!
      const cKind = cm[3]
      const cand: { anchor: string; rank: number; docKind?: string } = {
        anchor: cAnchor,
        rank: Number.parseInt(cm[1]!, 10),
      }
      if (cKind) cand.docKind = cKind
      f200Cands.push(cand)
      cm = candDetailRe.exec(text)
    }
    if (f200Cands.length > 0) summary._f200Candidates = f200Cands
...
function deriveListRecent(text: string): ResultSummary {
  ...
  rm = itemRe.exec(text)
  while (rm !== null) {
    const itemAnchor = rm[1]!
    const itemKind = rm[2]
    const cand: { anchor: string; rank: number; docKind?: string } = { anchor: itemAnchor, rank: f200Cands.length }
    if (itemKind) cand.docKind = itemKind
    f200Cands.push(cand)
    rm = itemRe.exec(text)
  }
  if (f200Cands.length > 0) summary._f200Candidates = f200Cands
