/**
 * @flowforge/cats-guides — GuideRoutingInterceptor（B-2，clowder GuideRoutingInterceptor.ts 直译）。
 *
 ...
 *
 * 插件化改造：clowder 模块级 registry 全局单例（动态 import getRegistryEntries）
 * → registryLoader 可选注入（缺省不注入 → name 回退 guideId，estimatedTime 为空）；
 * dismissTracker / guideSessionStore / threadStore 均为注入依赖。
 *
 * @module @flowforge/cats-guides/routing-interceptor
 */

import type { GuideStateV1 } from './models.js';
import type { GuideThreadAccess } from './ports.js';
import type { GuideRegistryLoader } from './registry-loader.js';
import type { GuideStateBridge, IGuideSessionStore } from './session-repository.js';
import { sessionToLegacyState } from './session-repository.js';
import { canAccessGuideState, hasHiddenForeignNonTerminalGuideState } from './state-access.js';
...
export interface GuideRouteThread extends GuideThreadAccess {
...
export interface GuideRoutingContext {
  ...
  /** string | undefined：exactOptionalPropertyTypes 下允许显式 undefined（clowder 同型）。 */
  offerOwner?: string | undefined;
  offerSelectionFallback?: string | undefined;
...
function resolveRegistryEntry(
  guideId: string,
  registryLoader?: GuideRegistryLoader,
): { name: string; estimatedTime: string } | undefined {
  if (!registryLoader) return undefined;
  try {
    const entry = registryLoader.getRegistryEntries().find((e) => e.id === guideId);
...
async function resolveExistingCandidate(
  ...
  const entry = resolveRegistryEntry(gs.guideId, registryLoader);
...
async function resolveBootcampGuide(
  thread: GuideRouteThread | null | undefined,
  targetCats: readonly string[],
  ctx: GuideRoutingContext,
  registryLoader?: GuideRegistryLoader,
  ...
  const entry = resolveRegistryEntry(guideId, registryLoader);
  if (!entry) return;
...
export async function prepareGuideContext(params: {
  ...
  const { thread, guideSessionStore, targetCats, message, userId, threadId, dismissTracker, registryLoader } = params;
  ...
  if (guideState) {
    await resolveExistingCandidate(guideState, message, targetCats, targetCatIds, ctx, registryLoader);
  }

  // F171: Bootcamp phase → guide bridge (auto-offer before keyword matching)
  if (!ctx.candidate && !ctx.hiddenForeign) {
    await resolveBootcampGuide(thread, targetCats, ctx, registryLoader, dismissTracker);
  }
...
export async function ackGuideCompletion(params: {
  ...
  threadStore: { get(id: string): GuideRouteThread | null | Promise<GuideRouteThread | null> };
}): Promise<void> {
