/**
 * @flowforge/cats-guides — GuidePromptSection（B-3，clowder GuidePromptSection.ts 直译）。
 *
 * Builds guide-related prompt lines for SystemPromptBuilder.
 *
 ...
 *
 * 插件化改造：clowder 模块级 loadGuideFlow(id) → registryLoader 可选注入
 * （缺省不注入 → 步骤概览加载失败文案）。
 *
 * @module @flowforge/cats-guides/prompt-section
 */
...
function buildPreviewLines(
  ...
  if (registryLoader) {
    try {
      const flow = loadGuideFlowFrom(registryLoader, id);
 ...
 */
export function buildGuidePromptLines(
  candidate: GuidePromptInput,
  threadId: string | undefined,
  registryLoader?: GuideRegistryLoader,
): string[] {
  ...
  if ((status === 'offered' || status === 'awaiting_choice') && userSelection?.includes('步骤概览')) {
    return buildPreviewLines(id, name, threadId, status, registryLoader);
  }
