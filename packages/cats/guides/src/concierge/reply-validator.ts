/**
 * @flowforge/cats-guides — ConciergeReplyValidator（F229 KD-17 → KD-27）。
 *
 ...
 *
 * @module @flowforge/cats-guides/concierge/reply-validator
 */

import { randomUUID } from 'node:crypto';
import type { HandleAnchor, HandleEntry, TriagePlanIntent, TriagePlanTarget } from '../models.js';
import {
  computeConciergeHandleDigest,
  formatConciergeHandleBindingTitle,
  normalizeConciergeHandleTitle,
} from './search-context.js';
import { resolveTargetCats, type TargetCatsResolverDeps } from './target-cats-resolver.js';
...
function parseBoundHandleReference(
  ...
  if (!handle || !title || !digest) return undefined;
  return { handle: handle.toUpperCase(), title, digest: digest.toLowerCase() };
}
