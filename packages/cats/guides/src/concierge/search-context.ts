/**
 * @flowforge/cats-guides — ConciergeSearchContext（F229 KD-17 → KD-23）。
 *
 ...
 * The reply validator (concierge/reply-validator.ts) post-processes these
 * markers into CardBlock actions using the same handle table passed at call time.
 *
 * 插件化改造：evidenceStore 为注入接口（ConciergeEvidenceStore，见 models.ts），
 * 缺省不注入 → fail-open 空上下文。
 *
 * @module @flowforge/cats-guides/concierge/search-context
 */

import { createHash } from 'node:crypto';
import type { ConciergeEvidenceItem, ConciergeEvidenceStore, HandleAnchor, HandleEntry } from '../models.js';
...
// Re-export model types for consumers that import from this module (clowder-compatible).
export type { ConciergeEvidenceItem, ConciergeEvidenceStore, HandleAnchor, HandleEntry };
