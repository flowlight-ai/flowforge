/**
 * @flowforge/cats-guides — ConciergeTargetCatsResolver（F229 Phase B）。
 *
 ...
 *
 * 插件化改造：clowder catRegistry 模块单例 → RosterResolver 注入（缺省空 roster，
 * 显式猫不校验）；messageStore 依赖裁剪掉（clowder 实际未使用，见原实现）。
 *
 * @module @flowforge/cats-guides/concierge/target-cats-resolver
 */

import type { IThreadStore } from '../ports.js';
import type { RosterResolver } from './config-store.js';
...
export interface TargetCatsResolverDeps {
  ...
  /** Roster for explicit-cat validation（缺省空 roster → 不校验，跳过 filter）。 */
...
export async function resolveTargetCats(
  ...
  // 1. User explicitly specified — validate against roster (cloud P2 fix).
  //    Filter out hallucinated/unknown catIds so they don't reach confirm card.
  //    Guard: skip validation when roster is empty (not yet initialized at startup,
  //    or running in unit tests). Same pattern as cat-config-loader.ts:720.
  if (explicitCats && explicitCats.length > 0) {
    const roster = deps.roster?.() ?? [];
    const registryPopulated = roster.length > 0;
    const validCats = registryPopulated ? explicitCats.filter((id) => roster.includes(id)) : explicitCats;
 ...
 * Returns unique catIds from the thread participant index.
 */
async function resolveFromParticipants(threadId: string, deps: TargetCatsResolverDeps): Promise<string[]> {
  try {
    ...
    return (participants ?? []).filter((id) => id && id !== 'system');
  } catch {
    return [];
  }
}
