/**
 * service-manifest — 服务清单 / 状态解析 / 端点解析 / 健康探测。
 * Ported from clowder-ai `domains/services/service-manifest.ts`（F195）。
 *
 * 适配：maskUrlCredentials 由 `config/env-registry` 改为包内 env-mask 模块（自包含）。
 */

import { maskUrlCredentials } from './env-mask.ts'
import { normalizeLoopbackUrl } from './loopback-url.ts'
import { getServiceConfig } from './service-config.ts'
...
function buildClientServiceManifest(service: ServiceManifest) {
  return {
    ...
    ...(service.portFallback === undefined ? {} : { portFallback: service.portFallback }),
...
export async function resolveServiceStates(options: {
  ...
  return Promise.all(
    SERVICE_MANIFESTS.map((service) => {
      ...
      const lifecycleAction = options.getLifecycleAction?.(service.id)
      return resolveServiceState(service, {
        ...(options.env === undefined ? {} : { env: options.env }),
        ...(options.fetchHealth === undefined ? {} : { fetchHealth: options.fetchHealth }),
