/**
 * service-config — 服务持久化配置（services.json）。
 * Ported from clowder-ai `domains/services/service-config.ts`（F195）。
 *
 * 适配：默认配置根从 clowder 仓库根改为 flowforge 仓库根（../.. 深度同步调整）；
 * 存储路径仍可用 CAT_CAFE_HOME / CAT_CAFE_SERVICES_CONFIG 覆盖（保留 clowder 契约）。
 */
...
import type { ServiceConfig } from './service-manifest.ts'
