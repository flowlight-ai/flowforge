/**
 * environment-detector — 环境画像探测（OS / Arch / GPU / Python / 内存 / 磁盘）。
 * Ported from clowder-ai `domains/services/environment-detector.ts`（F195）。
 *
 * 适配：CAT_CAFE_HOME 默认根从 clowder 仓库根改为 flowforge 仓库根；
 * sysctl/exec 探测保持可注入（resolveArch.sysctlProbe 契约测试注入内存实现）。
 */
...
import type { EnvArch, EnvGpu, EnvironmentProfile, EnvOs, PythonArch } from './recommendation-types.ts'
...
function detectPython(os: EnvOs, arch: EnvArch): { pythonArch: PythonArch; pythonVersion?: string } {
  ...
  if (acceptableNative) {
    return acceptableNative.version === null || acceptableNative.version === undefined
      ? { pythonArch: 'native' }
      : { pythonArch: 'native', pythonVersion: acceptableNative.version }
  }
  ...
  if (emulated) {
    return emulated.version === null || emulated.version === undefined
      ? { pythonArch: 'x86-emulated' }
      : { pythonArch: 'x86-emulated', pythonVersion: emulated.version }
  }
...
function resolveCatCafeHome(): string {
  ...
  // ../../../.. from packages/cats/services-panel/src/ → repo root
...
export function detectEnvironmentSync(): EnvironmentProfile {
  ...
  return {
    os,
    arch,
    ...(arch === 'unsupported' ? { archRaw: process.arch } : {}),
    gpu,
    ...(gpuDetail === undefined ? {} : { gpuDetail }),
    pythonArch,
