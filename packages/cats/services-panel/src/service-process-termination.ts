/**
 * service-process-termination — 服务进程优雅终止（SIGTERM 宽限 → SIGKILL 升级）。
 * Ported from clowder-ai `domains/services/service-process-termination.ts`（F195）。
 *
 * kill/isAlive 均为可注入端口，默认落到 node:process；契约测试注入内存实现。
 */
