/**
 * recommendation-matrix — 环境画像 → 服务安装推荐（模型列表 / 不支持原因）。
 * Ported from clowder-ai `domains/services/recommendation-matrix.ts`（F195）。
 */

import { SERVICE_MATRIX } from './recommendation-matrix-data.ts'
import type { EnvironmentProfile, MatchCriteria, MatrixEntry, ServiceRecommendation } from './recommendation-types.ts'
...
export function buildRecommendation(serviceId: string, profile: EnvironmentProfile): ServiceRecommendation {
  ...
  return {
    ...
    notes: entry.notes ?? [],
    ...(entry.customModelHint === undefined ? {} : { customModelHint: entry.customModelHint }),
  }
}
