/**
 * service-lifecycle — 服务脚本运行 / 进程识别 / 端口探测 / 日志尾读。
 * Ported from clowder-ai `domains/services/service-lifecycle.ts`（F195）。
 *
 * 适配：REPO_ROOT 从 clowder 仓库根改为 flowforge 仓库根（../.. 深度同步调整）；
 * execFile/ps/netstat 探测均为可注入端口（ProbeOptions.execFile），契约测试注入内存实现。
 */
