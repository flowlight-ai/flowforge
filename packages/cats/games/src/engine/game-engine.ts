/**
 * Game engine core (S5-2c).
 *
 * Faithful port of clowder-ai `game/GameEngine.ts`. Manages the event log, action
 * validation and phase state. Game-specific resolution and phase transitions are
 * implemented by subclasses (see {@link WerewolfEngine}). Pure and deterministic —
 * all runtime mutation is confined to the passed-in `GameRuntime`.
 * @module @flowforge/cats-games/engine/game-engine
 */

import type { EventScope, GameAction, GameEvent, GameRuntime, PendingAction, SeatId } from '@flowforge/cats-shared'

type PartialEvent = Omit<GameEvent, 'eventId' | 'timestamp'>

/** Base game engine: event log + action validation + phase state. */
export class GameEngine {
  ...
  /** Get events visible to a specific viewer (`SeatId` or `'god'`). */
  ...
  /** Check if all expected actions for the current phase have been collected. */
