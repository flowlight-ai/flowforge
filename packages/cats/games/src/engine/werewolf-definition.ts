/**
 * Werewolf game definition factory (S5-2c).
 *
 * Faithful port of clowder-ai `werewolf/WerewolfDefinition.ts`. Builds a
 * complete pure-data `GameDefinition` (roles / phases / actions / win conditions)
 * for a given player count using `WEREWOLF_PRESETS`. No state, no I/O — a single
 * pure factory consumed by {@link WerewolfLobby}.
 * @module @flowforge/cats-games/engine/werewolf-definition
 */

import type { ActionDefinition, GameDefinition, PhaseDefinition, RoleDefinition, WinCondition } from '@flowforge/cats-shared'
import { WEREWOLF_ROLES } from './werewolf-roles.ts'

/** Role distribution for a specific board player count. */
...
/** Standard phase sequence — phases with no acting role are auto-skippable. */
...
/** Create a full pure-data `GameDefinition` for a given board player count. */
