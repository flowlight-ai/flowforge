/**
 * Werewolf engine (S5-2c).
 *
 * Faithful port of clowder-ai `werewolf/WerewolfEngine.ts`. Extends {@link GameEngine}
 * with werewolf-specific night/day resolution and win conditions.
 * Night: 同守同救, witch self-heal rule, hunter can-shoot, guard consecutive restriction.
 * Day: vote, PK, exile, hunter shoot, idiot reveal, speeches, last words.
 * Pure and deterministic — mutates only the captured `GameRuntime`.
 * @module @flowforge/cats-games/engine/werewolf-engine
 */

import type { Ballot, Resolution } from '@flowforge/cats-shared'
...
/** Werewolf engine: night/day resolution, ballots and win conditions. */
export class WerewolfEngine extends GameEngine {
  ...
  resolveNight(): NightResult {
    ...
    if (this.nightBallots.size > 0) {
      knifed = this.resolveNightBallots().winningChoice ?? undefined
    } else {
    ...
    const guard = this.nightActions.guard
    if (guard) {
      const guardSeat = runtime.seats.find((s) => s.seatId === guard.by)
      if (guardSeat) {
        guardSeat.properties.lastGuardTarget = guard.target
      }
    }
  ...
  /** Ballot-based day vote with revision tracking. */
  ...
  resolveDayVotes(): VoteResult {
    ...
    const exiled = topCandidates[0] ?? null
    if (exiled) this.applyExile(exiled)
