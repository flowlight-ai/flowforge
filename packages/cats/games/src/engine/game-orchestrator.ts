/**
 * Game orchestrator (S5-2c).
 *
 * Faithful port of clowder-ai `game/GameOrchestrator.ts`. System-driven game
 * lifecycle: start → tick → action → advance → end. Coordinates a game engine
 * (logic) with injected persistence + broadcast ports. All external concerns
 * (game store, message store, socket, logger) are injected seams, so the whole
 * lifecycle is exercisable offline with fakes.
 * @module @flowforge/cats-games/engine/game-orchestrator
 */

import type { GameAction, GameConfig, GameDefinition, GameRuntime, PendingAction, Seat, SeatId } from '@flowforge/cats-shared'
import { GameEngine } from './game-engine.ts'
import { GameViewBuilder } from './game-view-builder.ts'
import { WerewolfEngine } from './werewolf-engine.ts'
import { appendGameSystemMessage, noopAppLogger, type AppLogger, type IGameStore, type IMessageStore, type SocketLike } from './engine-ports.ts'
...
/** Coordinates engine + store + socket for the full game lifecycle. */
export class GameOrchestrator {
  ...
  /** Create and persist a new game, broadcast to the thread. */
  async startGame(input: StartGameInput): Promise<GameRuntime> {
    ...
    // Auto-skip initial phase(s) with no actors for the acting role.
  ...
  async handlePlayerAction(gameId: string, seatId: string, action: GameAction): Promise<void> {
    ...
    // Record speech + dual-write to messageStore when a speak action has text.
    ...
    // Record night action reasoning (wolf → faction:wolf; others → god-only).
    if (action.actionName !== 'speak' && action.actionName !== 'vote' && speechText) {
      ...
      const scope: 'faction:wolf' | 'god' = faction === 'wolf' ? 'faction:wolf' : 'god'
      engine.appendEvent({
        round: runtime.round,
        phase: runtime.currentPhase,
        type: 'night_thought',
    ...
    // Emit real-time ballot.updated for day votes (live transparency).
  ...
  async broadcastGameState(gameId: string): Promise<void> {
    ...
    for (const seat of runtime.seats) {
      const view = GameViewBuilder.buildView(runtime, seat.seatId as SeatId)
  ...
  /** God action: skip the current phase. */
  ...
  /** Write a cat speech message to messageStore (dual-write). */
  private writeSpeech(runtime: GameRuntime, catId: string, content: string): void {
    ...
    Promise.resolve(
      ...
      this.logger.error({ err }, '[GameOrchestrator] Failed to write speech to messageStore')
    })
  }
  ...
  private writeAnnounce(runtime: GameRuntime, content: string): void {
    ...
    Promise.resolve(
      ...
      this.logger.error({ err }, '[GameOrchestrator] Failed to write announce to messageStore')
    })
  }
  ...
  private applyFallbacks(engine: GameEngine): void {
    ...
    for (const seat of expectedSeats) {
      ...
      // Non-wolf night roles (seer/witch/guard) skip on timeout — random fallback
      // would be game-breaking (e.g. witch randomly healing/poisoning).
      ...
      const randomTarget =
        validTargets.length > 0 ? validTargets[Math.floor(Math.random() * validTargets.length)]! : (aliveSeatIds.find((id) => id !== seat.seatId) ?? seat.seatId)
      ...
      const actionForSeat = phaseActions.find((a) => a.allowedRole === seat.role) ?? phaseActions.find((a) => a.allowedRole === '*')
      const fallbackActionName = actionForSeat?.name ?? 'vote'

      const fallbackAction: PendingAction = {
        seatId: seat.seatId,
        actionName: fallbackActionName,
        targetSeat: randomTarget as SeatId,
        submittedAt: Date.now(),
        status: 'fallback',
        requestedAt: runtime.phaseStartedAt ?? runtime.updatedAt,
  ...
  private advancePhase(engine: GameEngine): void {
    ...
    // Write round announce to messageStore.
    if (isNewRound) {
    ...
    // Auto-skip if the new phase has no actors.
    this.skipEmptyPhases(runtime)

    // Generate last words immediately on ENTERING day_last_words.
    if (runtime.currentPhase === 'day_last_words' && engine instanceof WerewolfEngine) {
      this.resolveLastWords(engine)
    }
  }

  /** Resolve current phase's actions into game state via the Werewolf engine. */
  ...
  /** Reconstruct night actions from the event log and resolve. */
  private resolveNightFromEvents(werewolf: WerewolfEngine, runtime: GameRuntime): void {
    ...
    // Write public dawn announce so players can see who died.
    const dawnText =
      result.deaths.length > 0 ? `☀️ 天亮了。昨夜 ${this.formatDeaths(result.deaths, runtime)} 被袭击。` : '☀️ 天亮了。昨夜是平安夜。'
  ...
  /** Feed pending day votes into the engine and resolve. */
  private resolveDayVoteFromPending(werewolf: WerewolfEngine, runtime: GameRuntime): void {
    for (const [seatId, pending] of Object.entries(runtime.pendingActions)) {
      ...
      if (pa.actionName === 'vote' && pa.targetSeat) {
        // Skip revealed idiots — they lose voting rights.
        const seat = runtime.seats.find((s) => s.seatId === seatId)
    ...
    // Write public exile announce with vote tally.
  ...
  /** Generate template last words for the recently exiled player. */
  private resolveLastWords(werewolf: WerewolfEngine): void {
    const runtime = werewolf.getRuntime()
  ...
  private skipEmptyPhases(runtime: GameRuntime): void {
    ...
    while (safety-- > 0) {
      ...
      // day_hunter is deferred (v1) — always skip, death-trigger needs a special resolve phase.
