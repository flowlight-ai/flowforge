/**
 * @flowforge/cats-workspace — 导航投递回执聚合（纯函数）。
 *
 * TS 移植自 clowder-ai `domains/workspace/workspace-navigation-delivery.ts`：
 * ack 回执按 status/reason 优先级聚合 → deliveryStatus，emitter 注入（socket.io
 * 剥离为 `WorkspaceNavigationEmitter` 端口，host 提供 socketEmit / socketEmitWithAck）。
 *
 * @module @flowforge/cats-workspace/navigation-delivery
 */
