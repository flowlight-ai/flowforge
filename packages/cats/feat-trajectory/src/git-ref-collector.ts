/**
 * @flowforge/cats-feat-trajectory — git-ref-collector（C26 GitRefSnapshotCollector，F233）。
 *
 * TS 移植自 clowder-ai `domains/feat-trajectory/GitRefSnapshotCollector.ts`：
 *   - 砚砚 P2-2 collector/projector 分层：所有 git/gh IO 住在 collector，输出
 *     immutable `GitRefSnapshot` DTO 喂 projector（纯函数 / rebuild-safe）
 *   - **外部 IO 全部接口化注入**（GitRunner / GhClient / FeatIndexLookup /
 *     ThreadSearch）——宿主按 dhs 插件范式提供 Real 实现，本包只定义契约
 *   - heuristic feat join（feat_index / branch_name_F# / commit_message_F#）
 *     证据累加 → high/medium/low 置信度
 *   - multiCandidatePolicy default `'skip-low-confidence'`（production-safe）：
 *     0 candidates / low confidence / multi-candidate → skip emit
 *   - collectAll 单次 lsRemote scan + per-branch failure isolation + prefetch
 *
 ...
 *
 * @module @flowforge/cats-feat-trajectory/git-ref-collector
 */

import type { FeatThreadJoinMethod, GitRefSnapshot } from '@flowforge/cats-shared';
...
/**
 * Mock-friendly IO interfaces — production impls 由宿主注入（git CLI runner /
 * gh REST client / feat_index / thread store）。
 */
...
/**
 * 启发式 join：方法累加证据 → confidence 由证据强度决定。
 *
...
export function heuristicFeatJoin(branchName: string, commitMessages: string[]): HeuristicJoinResult {
  ...
  // Cloud P2 fix: use matchAll to collect ALL F# from branch name.
  // branchName.match() only captures first match, so
...
export class GitRefSnapshotCollector implements IGitRefSnapshotCollector {
  ...
  private async collectBranchRef(branchRef: GitBranchRef, now: number): Promise<GitRefSnapshot | null> {
    ...
    // Step C+D+E: Resolve feat join + apply policy decision
