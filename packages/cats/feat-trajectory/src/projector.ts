/**
 * @flowforge/cats-feat-trajectory — projector（C26 FeatTrajectoryProjector，F233）。
 *
 * TS 移植自 clowder-ai `domains/feat-trajectory/FeatTrajectoryProjector.ts`：
 ...
 *   - 三源 apply 路径：applyBallCustodyEvent / applyGitRefSnapshot /
 *     applyThreadSplit / applyCrossPost；applyStitchedEntry 保持 RED（未实现）
 *
 * **砚砚 P2-2 collector/projector 分层**：git/gh IO 在 collector，projector 只消费
 * DTO（`GitRefSnapshot` / `BallCustodyEvent` / split / merge snapshot）。
 *
 * **rebuild-safe invariant (INV-2)**：upsert 同 entryId 替换不增计数；新 entry
 * append + sort + counts++；updatedAt monotonic max（多源 out-of-order 防倒退）。
 *
 * @module @flowforge/cats-feat-trajectory/projector
 */
...
import type { CrossPostSnapshot } from './cross-post-collector.js';
...
import type { ThreadSplitSnapshot } from './thread-split-collector.js';
...
function upsertEntry(proj: FeatTrajectoryProjection, entry: FeatTrajectoryEntry): void {
  ...
  // INV-2 rebuild-safe：createdAt 收敛为最早 entry.at（顺序无关，重放同 entries 逐字段一致）
  proj.createdAt = Math.min(proj.createdAt, entry.at);
}
...
export class FeatTrajectoryProjector {
  ...
  async applyGitRefSnapshot(snapshot: GitRefSnapshot): Promise<void> {
    ...
    const featId = snapshot.featureCandidates[0]!; // collector enforces single-candidate via policy (length>0 已检查)
