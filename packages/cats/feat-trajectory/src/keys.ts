/**
 * @flowforge/cats-feat-trajectory — keys（C26 FeatTrajectory F233）。
 *
 * TS 移植自 clowder-ai `domains/feat-trajectory/feat-trajectory-keys.ts`：
 *   - KD-1：subjectKey 派生自现有痕迹（featId / branchName），不引入新原语
 *   - stale bucket 阈值 + 分配（largest crossed；<24h → null 不 emit）
 *   - 三源 entry id 派生：event-stream `evt:{sourceEventId}` / historical-stitched
 *     `stitch:{featId}:{at}:{stitchType}` / git-ref-snapshot makeGitRefEntryId 公式
 *   - Redis key namespace 保留（供持久 store 实现分桶；本包内置 InMemory）
 *
 * @module @flowforge/cats-feat-trajectory/keys
 */
...
/** Re-export shared pure formula for collector / projector / tests. */
export { makeGitRefEntryId };
export type { GitRefEntryIdParts };

// ============================================================================
// Stale bucket thresholds + assignment（F188 提包球 4 段护栏支持）
// ============================================================================

/**
 * Stale bucket → 阈值 ms 映射。Projector 用此算
 ...
 * - git-ref-snapshot: makeGitRefEntryId(parts) 公式（per-kind stable id）
 */
...
/**
 * Store key namespace（照 Phase B BallCustodyKeys 模式）。
