/**
 * Message Bundle source projection (self-contained port of clowder
 * `MessageBundleSourceProjection.ts`).
 *
 * Turns stored rows into the readable / raw planes used for quote validation and
 * carrier re-resolution. CLI segment projection, CLI v2 readable stdout, rich
 * block read fallbacks, message/group readable content and v1/v2/v3 quote source
 * planes. Depends only on cats-shared types + local pure modules.
 */

import type { MessageContent, RichBlock } from '@flowforge/cats-shared';
import { cleanCliToolLabel, projectCliToolUseLabel } from '../pure/cli-tool-label.ts';
import { projectMarkdownReadableText } from '../pure/markdown-readable.ts';
import type { StoredMessage, StoredToolEvent } from '../ports/message-store.ts';
...
import {
  ...
  isTimelinePublished,
} from '../ports/visibility.ts';
import type { MessageSelectionAuth } from './message-selection-types.ts';
...
export function sanitizeRichBlock(block: RichBlock): RichBlock {
  ...
  if (block.kind === 'interactive') {
    return {
      ...
      fields: block.options.map((option) => ({
        label: option.label,
        value: option.description?.trim() || '选项',
      })),
    };
  }
...
/** v3 quote plane: readable characters of the one canonical bubble the browser rendered. */
