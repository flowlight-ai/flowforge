/**
 * Message Bundle prompt resolver (self-contained port of clowder
 * `MessageBundlePromptResolver.ts`).
 *
 * Resolves a refs-only carrier into the ephemeral, model-facing content string
 * immediately before consumption. The durable target body stays a safe summary;
 * this projection is not persisted.
 */
...
import { MessageSelectionResolver } from './message-selection-resolver.ts';
import type {
...
export interface MessageBundlePromptResult {
  status: 'ready' | 'unavailable';
  items: MessageSelectionProjectedItem[];
  content?: string;
  reason?: 'invalid_carrier' | 'all_unavailable' | 'prompt_too_large';
}
...
export async function resolveMessageBundlePrompt(
  ...
  if (resolved.items.length === 0 || !resolved.items.some((item) => item.status === 'available')) {
