/**
 * Canonical source group resolution (self-contained port of clowder
 * `MessageBundleSourceGroup.ts`).
 *
 * Mirrors the Web canonical bubble projector's grouping identity on persisted
 * records so whole-message / quote / rich-block selection resolves the same
 * bubble the human sees in the browser.
 */

import type { IMessageStore, StoredMessage } from '../ports/message-store.ts';
import { isSystemUserMessage } from '../ports/visibility.ts';
import {
  isAccessibleSourceRecord,
  isSourceGroupTerminal,
  sortSourceRecords,
