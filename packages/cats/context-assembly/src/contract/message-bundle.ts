/**
 * Message Bundle contract — self-contained重建 of clowder `@cat-cafe/shared`
 * `schemas/message-bundle.schema.ts` + context-attachment length limits (B12).
 *
 * Only depends on `zod`. Digest domain constants, projection version constants,
 * carrier schema, selection schema and their inferred types are reproduced
 * verbatim so MessageSelectionResolver / MessageBundleCarrierResolver /
 * MessageBundleProjectionDigest can run with zero `@cat-cafe/*` imports.
 */

import { z } from 'zod';
import type { RichBlock } from '@flowforge/cats-shared';

// ── Context-attachment length limits (clowder `types/context-attachment.ts`) ──
...
// ── Re-exported type bridge (resolved to @flowforge/cats-shared) ──
