/**
 * Rich block usage rules (progressive disclosure) — self-contained port of
 * clowder `rich-block-rules.ts`. Extracted from SystemPromptBuilder's MCP tools
 * section to avoid ~950 duplicated chars per invocation prompt.
 */
...
- **interactive**（用户可交互选择/确认）
  ...
  - option 优先用 \`icon\`（café SVG 图标名）而不是 \`emoji\`
  - option 可加 \`customInput: true\` + \`customInputPlaceholder\`，选中后展开文本输入框
  - 发多个 interactive block 时用相同 \`groupId\` 实现批量提交
  - 用户选择后 block 自动 disabled，选择结果持久化（刷新不丢）
- **html_widget**（内嵌交互 HTML 小组件）
  - 数据可视化（图表、仪表盘）、交互 demo、mini 工具
  - 定时任务中动态生成数据面板
  - \`html\` 必填，\`title\`/\`height\`(50-2000) 可选；沙盒隔离（allow-scripts 但无 allow-same-origin）
...
- interactive: \`interactiveType\` + \`options\` (id+label) 必填，多块用 \`groupId\` 批量提交
- html_widget: \`html\` 必填（完整 HTML），\`title\`/\`height\`(50-2000) 可选`;

/**
 * Condensed rich block reference for injection into system prompts.
 * Full rules: load rich-messaging skill (primary) / MCP tool / HTTP endpoint.
 */
...
interactive: 用户可交互选择（select/multi-select/card-grid/confirm）。`;