/**
 * Intent parser (self-contained port of clowder `IntentParser.ts`).
 *
 * Parses intent (`ideate | execute`) and prompt tags (`#critique` etc.) from a
 * message. Rules: explicit `#ideate` → ideate; explicit `#execute` → execute;
 * ≥2 cats without explicit tag → ideate; 1 cat → execute. `#critique` becomes a
 * prompt tag (changes thinking, not routing). Strong custody signals wake the
 * `skill:custody-recognition` prompt tag (policy only, no Task/time ownership).
 */

import { containsEntrustedWorkTimeSignal } from '../pure/entrusted-work-signals.ts';
