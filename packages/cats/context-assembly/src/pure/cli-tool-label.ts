/**
 * CLI tool label projection (self-contained, ported from clowder `cli-tool-label.ts`).
 *
 * `cleanCliToolLabel` strips the ` → ` prefix that appears in callback tool labels;
 * `projectCliToolUseLabel` appends a single primary argument for readability.
 */
