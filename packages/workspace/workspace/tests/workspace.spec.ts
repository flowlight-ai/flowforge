...
import type { StorageBackend } from '@flowforge/storage'
import { DomainFacility } from '@flowforge/storage-domain'
import type { DomainChanged } from '@flowforge/storage-domain'
import SessionStore, { SessionId } from '@flowforge/session'
import type { SessionHeader } from '@flowforge/session'
...
async function makeDir(name: string): Promise<string> {
  base ??= await realpath(await mkdtemp(join(tmpdir(), 'flowforge-workspace-')))
