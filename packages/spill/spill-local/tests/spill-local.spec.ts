...
import type { SaveTextSpill } from '@flowforge/spill'
import LocalSpillStore, { encodeSegment, privateRoot, saveTextFile, sessionDir } from '@flowforge/spill-local'

let root: string

beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), 'flowforge-spill-test-'))
})
afterEach(() => {
