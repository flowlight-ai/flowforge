...
import * as GoalSessionInvariant from '@flowforge/goal-round-driver/invariant'
import { renderGoalRoundPrompt } from '@flowforge/goal-round-driver'
import InvariantRegistry, { InvariantError } from '@flowforge/invariants'
import SessionStore, { SessionId, type Session } from '@flowforge/session'
...
describe('goal-round-driver prompt invariants', () => {
  ...
  it('rejects a continuation whose content differs from the package renderer', async () => {
    ...
    expect(() => {
      ...
      code: 'INVARIANT',
      packageName: '@flowforge/goal-round-driver',
    }))
  })
