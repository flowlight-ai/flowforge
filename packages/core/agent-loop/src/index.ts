...
import { SessionId, SessionPreparation } from '@flowforge/session'
import type { Session, SessionHeader } from '@flowforge/session'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    ...
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a flowforge-session-persistence backend)')
    }
