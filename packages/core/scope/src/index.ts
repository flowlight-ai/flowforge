 ...
 *
 * @module @flowforge/scope
 */

import type { Context, Fiber } from '@flowforge/cordis'
import { Context as CordisContext } from '@flowforge/cordis'
...
const kScope = Symbol('flowforge.scope')
...
function linkScopeParent(key: ScopeKey, parent: ScopeKey): void {
  for (let cursor: ScopeKey | undefined = parent; cursor !== undefined; cursor = scopeParents.get(cursor)) {
    if (cursor === key) throw new Error('flowforge-scope: scope parent link would form a cycle')
  }
...
export function bindScopeParent(key: ScopeKey, parent: ScopeKey): ScopeParentBinding {
  if (scopeParents.has(key)) {
    throw new Error('flowforge-scope: scope key is already bound to a parent; re-linking requires the binding returned by the original bind')
  }
