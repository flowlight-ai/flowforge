...
import { createUserMessage, CallId  } from '@flowforge/llm'
import { createScope } from '@flowforge/scope'
import type { Scope } from '@flowforge/scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@flowforge/tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@flowforge/tools'
import type { Agent } from '@flowforge/agent'
import { Session, SessionId } from '@flowforge/session'
import type { JsonValue, SessionEventMap } from '@flowforge/session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@flowforge/scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `flowforge-agent-tool-presentation` produces.
