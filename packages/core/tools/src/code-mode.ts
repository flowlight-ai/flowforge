 ...
 * @module @flowforge/tools/src/code-mode
 */

import { CallId, HarnessError } from '@flowforge/llm'
import type { ContentBlock } from '@flowforge/llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@flowforge/code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`flowforge-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
