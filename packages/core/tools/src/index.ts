 ...
 * execution pipeline.
 * @module @flowforge/tools
 */

import { Context, Service } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@flowforge/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@flowforge/scope'
import type { CallId, ContentBlock, ToolSchema } from '@flowforge/llm'
import { assertNever, deepFreeze, HarnessError } from '@flowforge/llm'
import type { Agent } from '@flowforge/agent'
import { snapshotJsonValue } from '@flowforge/session'
import type { JsonValue, UserMessage } from '@flowforge/session'
import type { ToolProviderResult } from '@flowforge/system-prompt'
import type { CodeRuntime } from '@flowforge/code-runtime'
 ...
 * instead of deriving them: the seam's `flowforge-code-runtime` README pair, its
...
export type { JsonValue } from '@flowforge/session'
...
// lives in its own UI-facing module; re-export it so `@flowforge/tools`
...
declare module '@flowforge/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@flowforge/scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@flowforge/tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@flowforge/tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `flowforge-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`flowforge-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`flowforge-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @flowforge/code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`flowforge-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `flowforge-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('flowforge.tool.execution') as ToolExecutionToken
}
