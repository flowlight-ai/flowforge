 ...
 *
 * @module @flowforge/system-prompt
 */

import { Context, Service } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@flowforge/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@flowforge/scope'
import type { ContextSnapshotSection, ToolSchema } from '@flowforge/llm'
...
declare module '@flowforge/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@flowforge/scope`): scoped listeners
...
export interface Config {
  /** Include the fixed FlowForge identity before the deployment persona (default true). */
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by FlowForge.',
      })
    }
    this.section({
