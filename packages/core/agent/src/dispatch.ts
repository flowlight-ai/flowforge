 ...
 * @module @flowforge/agent/dispatch
 */

import type { Context, Events } from '@flowforge/cordis'
import { scopeTarget } from '@flowforge/scope'
import type { Scoped } from '@flowforge/scope'
import type { AssembleContext } from '@flowforge/system-prompt'
...
export function agentEvents(ctx: Context, agent: Agent, carrier: Scoped<Agent> = agentCarrier(agent)): AgentEventDispatch {
  ...
  return {
    ...
    async serial(name, payload) {
      // the events mixin accessor returns a pre-bound function
