/**
 * CodexMarketplaceAdapter — codex ecosystem marketplace adapter (T5.8.1).
 *
 * Ported from clowder-ai `marketplace/adapters/codex-adapter.ts`
  ...
  async buildInstallPlan(artifactId: string): Promise<InstallPlan> {
    ...
    return {
      ...
      mcpEntry: {
        ...
        ...(entry.serverUrl !== undefined ? { url: entry.serverUrl } : {}),
        ...(entry.serverUrl !== undefined ? { transport: 'streamableHttp' } : {}),
        ...(entry.env_vars !== undefined ? { env: entry.env_vars } : {}),
        ...(entry.env_http_headers !== undefined ? { headers: entry.env_http_headers } : {}),
      },
      metadata: { ...(entry.versionRef ? { versionRef: entry.versionRef } : {}), publisherIdentity: entry.publisher },
    }
  }
