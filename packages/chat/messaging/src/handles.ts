 ...
 *
 * Ported from clowder-ai `domains/messaging/handles.ts`.
 */

import { randomUUID } from 'node:crypto'
import type { MessageAddress, MessageHandle } from '@flowforge/plugin-contract'
  ...
  async ensureMessageHandle(parent: AddressHandleRecord, messageId: string): Promise<MessageHandleRecord> {
    ...
    try {
      ...
      if (err instanceof Error && !(err instanceof MessagingError)) {
        ...
        if (errMsg.startsWith('handle index corruption') || errMsg.startsWith('handle binding violation')) {
          throw new MessagingError('CONFLICT', errMsg)
        }
      }
      throw err
    }
