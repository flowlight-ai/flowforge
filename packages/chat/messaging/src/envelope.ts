 ...
 *
 * MessageEnvelope is a PROJECTION of a stored message — the message store stays
 ...
 *
 * Ported from clowder-ai `domains/messaging/envelope.ts`.
 */
...
import { isWireUInt53, MESSAGING_BOUNDS, validateMessagingRowResult } from '@flowforge/plugin-contract'
import type { StoredMessage } from '@flowforge/cats-stores/ports'
import { isBoundedScalarString } from './contract/source-admission.js'

/**
 * Message-store row widened with the plugin-message envelope channel.
 * FlowForge's cats-stores rows expose `metadata` today; the strict
 * `extra.pluginMessage` channel is additive and fail-closed (absent → plain row).
 */
export type EnvelopeStoredMessage = StoredMessage & {
  readonly extra?: { readonly pluginMessage?: unknown }
}
...
function isAppendOp(value: unknown): boolean {
  ...
  return (
    ...
    (typeof value.baseRevision === 'number' && isWireUInt53(value.baseRevision, 1))
  )
}
...
/**
 * Reuse the published result validator as the canonical JSON-scalar
...
export function readPluginMessageExtra(msg: EnvelopeStoredMessage): PluginMessageExtra | null {
  return parsePluginMessageExtra(msg.extra?.pluginMessage)
}

function audienceOf(msg: EnvelopeStoredMessage): CanonicalAudience {
...
function hostRelayedEpistemic(msg: EnvelopeStoredMessage): EpistemicStatus {
 ...
 */
export function projectEnvelope(msg: EnvelopeStoredMessage): MessageEnvelope | null {
...
export function renderElementsText(elements: ReadonlyArray<MessageElement>): string {
  const parts: string[] = []
