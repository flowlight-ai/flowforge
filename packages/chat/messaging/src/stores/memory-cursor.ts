 ...
 *
 * Ported from clowder-ai `domains/messaging/stores/memory-cursor.ts`.
 */
...
export class MemoryCursorStore implements CursorStore {
  ...
  private readonly snapshotItems = new Map<string, ReadonlyArray<MessageEnvelope>>()
  private readonly snapshotCaptures = new Map<string, MemorySnapshotCapture>()
  private readonly snapshotCaptureItems = new Map<string, ReadonlyArray<MessageEnvelope>>()
  ...
  async appendSnapshotCapture(
    ...
    items: ReadonlyArray<MessageEnvelope>,
  ): Promise<boolean> {
    const key = MemoryCursorStore.key(pluginInstanceId, subscriptionId)
    const record = this.subs.get(key)
  ...
  async commitSnapshotCapture(
    ...
    const published: SubscriptionRecord = {
  ...
  async readSnapshotPage(
    ...
    offset: number,
    limit: number,
  ): Promise<ReadonlyArray<MessageEnvelope> | null> {
    const key = MemoryCursorStore.key(pluginInstanceId, subscriptionId)
    const record = this.subs.get(key)
    if (
      !record ||
  ...
  async consumeSnapshotPage(
    ...
    const view: SnapshotViewRecord = {
      ...
      ...(next.tokenId !== undefined ? { nextPageTokenId: next.tokenId } : {}),
    }
    this.subs.set(key, { ...record, snapshotView: view })
    return true
  }
  ...
  async ackSnapshot(
    ...
    const acknowledged: SubscriptionRecord = {
      subscriptionId: record.subscriptionId,
      pluginInstanceId: record.pluginInstanceId,
      handleId: record.handleId,
      threadId: record.threadId,
    ...
    this.subs.set(key, acknowledged)
    ...
    for (const [key, record] of this.subs.entries()) {
      if (record.handleId === handleId && record.revokedAt === undefined) {
        const revoked: SubscriptionRecord = {
          ...
          replayFloorSequence: record.replayFloorSequence,
          revokedAt,
          ...(record.lastSnapshotCompletion !== undefined
            ? { lastSnapshotCompletion: record.lastSnapshotCompletion }
            : {}),
        }
        this.subs.set(key, revoked)
        this.snapshotItems.delete(key)
        this.snapshotCaptures.delete(key)
        this.snapshotCaptureItems.delete(key)
        count += 1
