...
import type { ShellExecRequest, ShellExecSpec, ShellExecutor, ShellRunResult } from '@flowforge/shell'
import { DEFAULT_HOOK_TIMEOUT_MS, runHook } from '@flowforge/hook-protocol'
import type { RunHookOptions } from '@flowforge/hook-protocol'
 ...
 * executor (flowforge-bash-local) is exercised end-to-end by the hook-bridge plugins
...
function recordingBash(run: (spec: ShellExecSpec) => Promise<ShellRunResult>): {
  ...
  const bash = {
    resolve(request: ShellExecRequest): ShellExecSpec {
      ...
      // exactly what flowforge-bash-local's resolve does for the fields runHook sets.
      return {
