...
import type { Agent, AgentStatus } from '@flowforge/agent'
import CommandRuntime from '@flowforge/commands'
import SessionStore, { SessionId } from '@flowforge/session'
import * as CommandFeedback from '@flowforge/command-feedback'
...
describe('/feedback real Loader composition through cordis.yml', () => {
  it('boots cordis.yml and records feedback without model-visible output', async () => {
    root = await mkdtemp(join(tmpdir(), 'flowforge-command-feedback-loader-'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@flowforge/commands', CommandRuntime],
      ['@flowforge/command-feedback', CommandFeedback],
    ])
    ...
    const userId = getOrCreateAnonymousUserId({ env: { FF_HOME: root } })
