...
import Storage from '@flowforge/storage'
import * as StorageDomain from '@flowforge/storage-domain'
import * as StorageJson from '@flowforge/storage-json'
import { remoteMethods } from '@flowforge/typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ['@flowforge/session', SessionStore],
    ['@flowforge/session-persistence-jsonl', JsonlSessionPersistence],
    ['@flowforge/storage', Storage],
    ['@flowforge/storage-json', StorageJson],
    ['@flowforge/storage-domain', StorageDomain],
    ['@flowforge/message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'flowforge-message-feedback-loader-'))
    const configPath = join(root, 'cordis.yml')
    await writeFile(configPath, [
      "- name: '@flowforge/session'",
      "- name: '@flowforge/session-persistence-jsonl'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'sessions'))}`,
      '    compression: none',
      '    writeBatchMaxDelayMs: 1',
      "- name: '@flowforge/storage'",
      "- name: '@flowforge/storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@flowforge/storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@flowforge/message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
