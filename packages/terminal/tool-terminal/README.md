...
The six generated schemas are listed in the [`flowforge-tool-terminal` catalog section](../../../docs/tool-catalog.md#deepseek-aidsh-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `@flowforge/jobs` and its model-facing controller.
