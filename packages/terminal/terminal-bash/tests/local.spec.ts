...
import type { TerminalSendOperation } from '@flowforge/terminal'
...
import * as ptyLocal from '@flowforge/terminal-bash'
...
async function harness(
  ...
  const root = mkdtempSync(join(tmpdir(), 'flowforge-pty-local-'))
  roots.push(root)
...
describe('terminal-bash real shell', () => {
  it('persists cwd and environment across sends, scrubs secrets, and closes', async () => {
    const previous = process.env.FF_TEST_SECRET
    process.env.FF_TEST_SECRET = 'must-not-leak'
    try {
      const { ctx, root, agent } = await harness('danger-full-access')
      const created = await ctx.terminals.spawn(agent, { type: 'shell', name: 'main', cwd: root })
      expect(created.motd).toContain('flowforge> ')
      ...
      if (previous === undefined) delete process.env.FF_TEST_SECRET
      else process.env.FF_TEST_SECRET = previous
    }
  }, 10_000)
  ...
  it('quiesces a disowned same-session descendant after the shell exits naturally', async () => {
    ...
    try {
      const background = ctx.terminals.startSend(agent, created.sessionId, {
        text: `sh -c 'trap "" TERM; printf "%s" "$$" > "$1"; sleep 60' flowforge "${pidFile}" & disown`,
        submit: true,
      })
