...
import AgentRegistry, { Inbox, type Agent } from '@flowforge/agent'
import SandboxProvider from '@flowforge/sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@flowforge/sandbox'
import SandboxPolicyService, { setSandboxMode } from '@flowforge/sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@flowforge/terminal'
import { BashTerminalBackend } from '@flowforge/terminal-bash'
import * as ptyLocal from '@flowforge/terminal-bash'
import type { ResolvedConfig } from '@flowforge/terminal-bash/src/config.ts'
import type { LocalPtySession } from '@flowforge/terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'flowforge> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        FF_SHELL: '1', FF_SESSION_ID: 'agent', FF_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    expect(session.motd).toBe('flowforge> ')
