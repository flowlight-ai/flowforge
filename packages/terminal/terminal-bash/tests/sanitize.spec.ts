...
import { normalizeTerminalText, TerminalSanitizer } from '@flowforge/terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07dsh> ')).toEqual({ text: 'flowforge> ', prompt: true, promptTail: 'flowforge> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('flowforge> ')).toEqual({ text: 'flowforge> ', prompt: false, promptTail: 'flowforge> ' })
  })
