 ...
 * @module @flowforge/sandbox-windows-acl/acl
 */
...
/**
 * One lock file per protected path: `<GetTempPathW()>\flowforge-acl-locks\<first 16
 * hex of sha256(lowercased path)>.lock`. The lock root derives from
 * GetTempPathW (never from runner argv or FF_HOME), and the lowercasing
...
export function lockFilePath(api: Win32Bindings, path: string): string {
  ...
  return join(getTempPath(api), 'flowforge-acl-locks', `${digest}.lock`)
}
