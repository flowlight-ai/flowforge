 ...
 * the argv shape flowforge-sandbox-local's confine() builds), with piped stdio
...
// pwsh invocation's exit status is truth. The suite confines the LITERAL
// `pwsh` command, so probe exactly that binary: probing resolvePwshPath()
// would falsely pass on hosts where the resolver falls back to Windows
// PowerShell 5.1 (PowerShell 7 absent) while the confined spawn still fails.
...
describe.skipIf(!isWin32 || !pwshAvailable())('windows-acl runner', () => {
  ...
  beforeAll(() => {
    scratchRoot = mkdtempSync(join(tmpdir(), 'flowforge-acl-runner-'))
    writableDir = join(scratchRoot, 'writable')
    mkdirSync(writableDir)
    isolatedTemp = mkdtempSync(join(tmpdir(), 'flowforge-acl-runner-temp-'))
    ...
    try {
      publicProbeDir = mkdtempSync(join(process.env.PUBLIC ?? 'C:\\Users\\Public', 'flowforge-acl-public-'))
    } catch {
