...
import SandboxPolicyService, { SANDBOX_MODES, effectiveSandboxMode, setSandboxMode } from '@flowforge/sandbox-policy'
import SystemPrompt, { renderContextSnapshot, renderPrompt } from '@flowforge/system-prompt'
...
describe('SandboxPolicyService', () => {
  ...
  it.skipIf(process.platform === 'win32')('resolves a symlink-sensitive session cwd with POSIX component semantics', async () => {
    const root = mkdtempSync(join(tmpdir(), 'flowforge-policy-cwd-'))
    try {
