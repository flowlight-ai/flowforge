# @flowforge/sandbox-local

English | [中文](README.zh.md)

Local implementation of the [`flowforge-sandbox`](../sandbox/) seam. It selects and caches one platform runner: Linux prefers a working `bwrap` then Landlock; macOS uses Seatbelt; Windows uses the ACL restricted-token runner. Multiple candidates are probed in order, while a sole candidate is selected directly.
...
Consumers: [`@flowforge/bash-sandbox`](../../shell/bash-sandbox/); see [the acp-agent example](../../../examples/acp-agent/) for the runnable default composition.

## Model Experience

Indirectly, through [`flowforge-bash-sandbox`](../../shell/bash-sandbox/README.md) and [`flowforge-tool-bash`](../../shell/tool-bash/README.md), which render this provider's enforcement and denial facts while the [`flowforge-sandbox`](../sandbox/README.md) seam owns the `SANDBOX_UNAVAILABLE` text and runner selection and profiles stay outside context.
