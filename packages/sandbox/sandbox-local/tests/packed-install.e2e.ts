...
const platformPackageName = `@flowforge/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @flowforge peer (koffi arrives
  // from the registry).
  ...
  // rescoped into @flowforge, so the consumer installs this repository's
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'flowforge-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'flowforge-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'flowforge-packed-work-'))
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'flowforge-packed-consumer', private: true, type: 'module' }))
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@flowforge/node-addon-landlock-run'
