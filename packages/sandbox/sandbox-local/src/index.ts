 ...
 * @module @flowforge/sandbox-local
 */
...
} from '@flowforge/node-addon-landlock-run'
import { Context } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import { assertNever } from '@flowforge/llm'
import { SandboxProvider, SandboxUnavailableError } from '@flowforge/sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@flowforge/sandbox'
import type { SessionId } from '@flowforge/session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@flowforge/sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@flowforge/sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@flowforge/sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@flowforge/sandbox-windows-acl/src/runner.ts'))
