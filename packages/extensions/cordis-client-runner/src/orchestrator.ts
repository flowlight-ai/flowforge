 ...
 * (seam port of the dsh `orchestrator.ts`). Host activation always precedes
 * Client loading. The same Plugin-keyed state drives every surface, so
 * remounting a panel never loses an open approval or an in-flight transition.
 *
 * The dsh Remote transport is folded into a package-local {@link CordisRunHostSeam};
 * the page-local loader is {@link DynamicCordisPackageRunner}.
 *
 * @module @flowforge/cordis-client-runner/orchestrator
 */
...
import type {
  ...
  CordisDynamicPluginRunId, CordisDynamicRunMode, DynamicCordisClientSource,
  DynamicCordisHostHalfResult, DynamicCordisInventoryRow, DynamicCordisResolveAck,
  DynamicCordisRunResolution, DynamicCordisRunResponse, SessionId,
} from './types.ts'
...
export class CordisRunOrchestrator {
  ...
  /** Register a Client activation request, starting it immediately when already authorized. */
  ...
  private async settleDirect(plan: RunPlan, resolution: DynamicCordisRunResolution): Promise<void> {
    try {
      ...
      if (!response.ok) {
        this.fail(plan, 'client-half-failed', {
          message: typeof response.message === 'string' && response.message !== ''
            ? response.message
            : 'Host did not accept the Client run settlement',
        })
      }
    } catch (error) {
