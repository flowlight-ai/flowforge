/**
 * Browser registry for read-only Cordis capability providers (seam port of the
 * dsh `inspect-registry.ts`). The dsh transport types and the `Context`
 * augmentation are replaced by package-local {@link ClientCordisInspectHost}
 * and a `ServiceHostPort` provision.
 *
 * @module @flowforge/cordis-client-runner/inspect-registry
 */

import type { ServiceHostPort } from './ports/service-host.ts'
import type { JsonValue } from './values.ts'
import type { CordisInspectRequestId, SessionId } from './types.ts'

/** Inspect provider manifest and query directory (mirrors the wire shape). */
export interface CordisInspectProviderManifest {
  readonly id: string
  readonly description: string
  readonly methods: readonly CordisInspectProviderMethod[]
}

/** One declared read-only method. */
export interface CordisInspectProviderMethod {
...
/** A Host-broadcast query this page must execute and answer. */
export interface CordisInspectQueryRequest {
  readonly requestId: CordisInspectRequestId
...
/** How a query result is settled back to the Host. */
export type CordisInspectQueryResolution =
  | { ok: true; data: JsonValue }
  | { ok: false; reason: 'provider-missing' | 'method-missing' | 'provider-error' | 'cancelled'; message: string }
 ...
 * @param host - package-local service host receiving the service.
 * @param registry - page-local inspect registry to publish.
 * @returns disposer removing the service.
 */
export function provideClientCordisInspect(host: ServiceHostPort, registry: ClientCordisInspectRegistry): () => void {
  return host.provide('cordisInspect', registry)
}