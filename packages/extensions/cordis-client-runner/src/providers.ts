/**
 * Built-in Client inspect providers over live Client-owned services (seam port
 * of the dsh `providers.ts`). `Context` is the package-local `ServiceHostPort`;
 * the slot snapshot is the `ClientSlotsPort`; the catalogs the compact
 * self-contained tables in this package.
 *
 * @module @flowforge/cordis-client-runner/providers
 */
...
import type { ClientSlotsPort, LiveSlotNode } from './ports/slots.ts'
...
const SERVICE_OUTPUT = { description: 'Compact Service directory, or one exact Service contract.' } as const
const EVENT_OUTPUT = { description: 'Compact Event directory, or one exact Event contract.' } as const
/* jscpd:ignore-end */
...
const SUBTREE_INPUT = {
  type: 'object',
  properties: {
    root: { type: 'string', description: 'Exact live Slot key. When supplied, selected contains the full contract.' },
  },
...
export const CLIENT_BUILTIN_INSPECTION: readonly JsonValue[] = [
  ...
  },
  {
    ...
    signatures: ['React.createElement(type, props, ...children)', 'React.useState(initial)', 'React.useEffect(effect, deps)'],
  },
  {
    name: 'host',
    description: 'Package-private JSON RPC from Client to this Package\'s Host half.',
    signatures: ['host.call(method: string, args?): Promise'],
  },
  {
    name: 'styles',
 ...
 * @param host - package-local service host used for live Service-backed queries.
 * @returns registrations for static catalogs and live Client capabilities.
 */
export function clientInspectProviders(host: ServiceHostPort): ClientCordisInspectProviderRegistration[] {
  return [
    registration(
      'Service',
    ...
    })),
    {
      ...
      query(method, input) {
        ...
        const slots = host.get<ClientSlotsPort>('slots')
        if (slots === undefined) throw new Error('Client Slots service is not running')
        const root = jsonStringField(input, 'root')
    ...
    registration('Theme', 'Current theme token names and light/dark override requirements.', 'listTokens', () => {
      const theme = host.get('theme')
      if (theme === undefined) throw new Error('Client Theme service is not running')
      const tokens = typeof theme === 'object' && theme !== null
        && typeof (theme as { exportInspectTokens?: unknown }).exportInspectTokens === 'function'
        ? (theme as { exportInspectTokens(): unknown }).exportInspectTokens()
        : []
...
function registration(
  ...
  return {
    manifest: {
      id,
      description,
      methods: [{ name: method, description, inputSchema, outputSchema }],
    },
...
function readExact(input: JsonValue | undefined, field: string): string | undefined {
  return jsonStringField(input, field)
}

/** Read a string field off an opaque JSON object (undefined for non-objects or missing). */
function jsonStringField(input: JsonValue | undefined, field: string): string | undefined {
...
function inspectLiveSlot(node: LiveSlotNode): JsonValue {
  ...
  return {
    ...
    occupants: node.occupants.map(occupant => ({
      ...
      component: componentLabel(occupant.component),
    })),
    ...catalog === undefined ? {} : { catalog: inspectSlotCatalog(catalog) },
  }
}

/** A stable JSON-safe label for an arbitrary seated component. */
