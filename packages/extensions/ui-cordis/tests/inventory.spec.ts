...
// behind, and why a read is single-flight (pure-logic port of the dsh
// inventory.client.spec.ts).

import { describe, expect, it, vi } from 'vitest'
import { createCordisInventory } from '../src/inventory.ts'
import type { CordisDynamicPort, CordisInventoryRow } from '../src/dynamic-port.ts'

const ROW: CordisInventoryRow = {
  agentId: 'sess-1', pluginId: 'dyn-1',
  packages: [{ packageId: 'pkg-1', name: 'clock', purpose: '顶栏时钟' }],
}
...
describe('reading the registry', () => {
  ...
  it('records an explicit remove and drops the live row, then lets it be re-read', async () => {
    const seam = port(() => Promise.resolve([ROW]))
    const inventory = createCordisInventory(seam.port, vi.fn())
    inventory.refresh()
    await vi.waitFor(() => { expect(inventory.getSnapshot().read).toBe(true) })

    inventory.retire('dyn-1')
    expect(inventory.getSnapshot().rows).toEqual([])
    expect(inventory.getSnapshot().removed.has('dyn-1')).toBe(true)
  })
})
