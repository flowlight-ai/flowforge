/**
 * Replay-stable view models for Cordis lifecycle Tool calls (pure-logic port of
 * the dsh `card-model.ts`). The dsh `Block` shape is a package-local
 * {@link ToolCallViewModelBlock}; everything else derives from it with no UI
 * framework in the module.
 *
 * @module @flowforge/ui-cordis/card-model
 */

import type { ToolCallViewModelBlock } from './block.ts'
import type {
...
function resultText(block: Extract<ToolCallViewModelBlock, { kind: string }>): string | null {
...
function stateOf(block: ToolCallViewModelBlock): CordisToolState {
  if (!('kind' in block)) return 'running'
  if (block.error?.code === 'interrupted') return 'stopped'
  return block.isError === true ? 'error' : 'ok'
}

function metaObject(block: ToolCallViewModelBlock): Record<string, unknown> | null {
  if (!('kind' in block) || block.isError === true || typeof block.meta !== 'object' || block.meta === null) return null
 ...
 */
export function cordisDefineCard(block: ToolCallViewModelBlock): CordisDefineCard {
 ...
 */
export function cordisRunCard(block: ToolCallViewModelBlock): CordisRunCard {
  const settled = 'kind' in block
  const argsRaw = (settled ? block.call?.argsRaw : block.argsRaw) ?? ''
  const args = parseArgs(argsRaw)
  const meta = metaObject(block)
  ...
  return {
    ...
    seq: settled ? block.seq ?? null : null,
    output,
    errorSummary: state === 'error' && output !== null ? firstLine(output) : null,
    state,
  }
 ...
 */
export function cordisActionCard(block: ToolCallViewModelBlock): CordisActionCard {
  const settled = 'kind' in block
  const argsRaw = (settled ? block.call?.argsRaw : block.argsRaw) ?? ''
  const args = parseArgs(argsRaw)
  const state = stateOf(block)
