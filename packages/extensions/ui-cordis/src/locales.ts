/**
 * Cordis dynamic-plugin UI dictionaries — pure-logic port of the dsh
 * `locales.ts`. The dsh `declare module '@deepseek-ai/dsh-client-ui-slots'`
 * augmentation is deliberately dropped; this package owns no locale registry,
 * so consumers bind the dictionaries (and the `CordisKey` key space) to their
 * own ICU/i18n slot. User-facing strings stay externalized here instead of
 * scattered through the presentation logic (rule 3).
 *
 * @module @flowforge/ui-cordis/locales
 */

/** The locale namespace Cordis UI copy lives under. */
...
/** The shipped UIT8 translations, keyed by locale. */
