...
export const SERVICE_API: readonly ServiceApiEntry[] = [
  ...
  },
  {
    ...
    description: 'Agent service (`ctx.agents`): tracks live agents and carries the initiating Agent through one process-local asynchronous driver chain. Agent *creation* is provided by whichever plugin implements the AgentFactory (`@flowforge/agent-loop`), registered via setFactory.\n\nInitiator methods provide same-process causal attribution only. Ambient presence is neither liveness proof nor authorization; subjects and owners remain explicit, as does identity at worker, process, persistence, and wire boundaries. Returned Promise boundaries drain during teardown, except a nested lineage that starts an owning-fiber unload is excluded from its own drain.',
    methods: [
      {
  ...
  {
    key: 'clientModules',
    summary: 'The web plugin table service: incremental `flowforge.client` scan + wire composition + bundle route + index tap.',
    description: 'The web plugin table service: incremental `flowforge.client` scan + wire composition + bundle route + index tap. Construction runs the activation scan synchronously — a malformed declaration or missing bundle among the already-loaded entries aggregates into one loud throw (FAILED fiber; the boot activation audit reports it).',
    methods: [
      {
        signature: 'graph(): WebBootGraph',
        description: 'Current composed entry graph (stable object between changes).',
        parameters: [],
        returns: 'the graph served as `window.__FF_BOOT__`.',
      },
      {
  ...
  {
    key: 'shellEnv',
    summary: 'Registry (`ctx.shellEnv`) for trusted, per-execution `FF_*` variables.',
    description: 'Registry (`ctx.shellEnv`) for trusted, per-execution `FF_*` variables. The namespace is rebuilt for every model shell call: ambient `FF_*` values are discarded by the executor, then the registry\'s current snapshot is injected. Built-in shell facts remain owned by the registry itself while plugins can register additional, enumerable facts with effect-scoped disposal.',
    methods: [
      {
      ...
      {
        signature: 'collect(execution: ToolExecution): FlowforgeEnvironment',
        description: 'Build the trusted `FF_*` snapshot for one shell tool execution.',
...
export const EVENT_API: readonly EventApiEntry[] = [
  ...
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.agent - the newly registered agent with its live session and completed setup. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/disposed',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.agent - the exact agent removed from the registry. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/error',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.error - the failure, verbatim. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.turn - the owning turn. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the discarded message. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.message - the inserted message. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/pre-step',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s cancellation signal. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/request',
    mode: 'waterfall',
    ...
    parameters: [{ name: 'payload', description: '.signal - the current turn\'s explicit abort signal. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.signal - the turn abort signal. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    ...
    parameters: [{ name: 'payload', description: '.source - why the session started (fresh startup, resume, …). Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
    name: 'agent/status',
    mode: 'emit',
    ...
    parameters: [{ name: 'payload', description: '.status - the status just entered (the transition\'s destination). Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Ask composed answerers for one decision. Return an outcome to claim the request or call `next()`; failure yields the fail-closed default. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.',
    parameters: [{ name: 'req', description: 'the pending decision (agent, tool identity, reason, signal).' }],
  },
  {
  ...
  },
  {
    name: 'goal/changed',
    mode: 'emit',
    signature: '\'goal/changed\'(this: import(\'@flowforge/scope\').Scoped<Agent>, payload: { agent: Agent; change: GoalChanged }): void',
    summary: 'Goal mutation accepted by one live agent.',
    description: 'Goal mutation accepted by one live agent. The matching `goal/change` session event has already committed. Listener failures are contained. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent.',
  ...
  },
  {
    ...
    description: 'Creation announcement during session publication. A synchronous throw vetoes and rolls back with a paired disposal; detach requested during dispatch is deferred. A returned-promise rejection is logged but cannot retroactively veto this synchronous boundary. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only sessions entered through that agent\'s context.',
  ...
  },
  {
    ...
    description: 'Emitted once when an announced session leaves the store, including publication rollback, but never for an entry whose creation announcement did not begin. Listener failures are logged and contained. Scope-filtered dispatch (`@flowforge/scope`) reuses the owner scope.',
  ...
  },
  {
    ...
    description: 'Awaited parallel durability checkpoint: every listener runs and the caller awaits all of them, with no waterfall veto. Scope-filtered dispatch (`@flowforge/scope`) reuses the session\'s owner scope.',
    parameters: [{ name: 'session', description: 'the session whose buffered events must reach durable storage.' }],
  },
  {
  ...
  },
  {
    ...
    description: 'Accept, replace, enrich, or block a normalized dispatch result. `next()` accepts it unchanged; thrown tools still reach this waterfall as errors. Async listeners must observe `exec.signal`; after they settle, caller cancellation replaces only a successful accepted outcome with the code selected by whether the tool body was invoked. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent\'s calls.',
  ...
  },
  {
    ...
    description: 'Allow, deny, or ask before dispatch. `next()` delegates to allow; missing approval support turns `ask` into denial. Async gates must observe `exec.signal`; the registry rechecks cancellation after they settle but never abandons their promise. Scope-filtered dispatch (`@flowforge/scope`): agent-scoped listeners receive only that agent\'s calls.',
  ...
  },
  {
    ...
    description: 'Observe the frozen, lossless-JSON final outcome. Listener failures are contained. Scope-filtered dispatch (`@flowforge/scope`): keyed by `exec.agent`.',
...
export const TYPE_API: readonly TypeApiEntry[] = [
  ...
  {
    name: 'FlowforgeEnvironment',
    declaration: 'export type FlowforgeEnvironment = Readonly<Record<FlowforgeEnvironmentKey, string>>;',
  },
  {
    name: 'FlowforgeEnvironmentKey',
    declaration: 'export type FlowforgeEnvironmentKey = `${typeof FF_ENV_PREFIX}${string}`;',
  },
  {
  ...
  {
    name: 'SkillSource',
    declaration: 'export type SkillSource = \'project-flowforge\' | \'project-agents\' | \'runtime\' | \'user-flowforge\' | \'user-agents\' | \'custom\' | \'bundled\' | (string & {});',
  },
  {
    name: 'SkillSummary',
