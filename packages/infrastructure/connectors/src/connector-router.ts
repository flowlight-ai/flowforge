/**
 * Connector Router
 * Routes inbound messages from external platforms to agent threads.
 *
 * Flow:
...
import type { CatId, ConnectorSource, MessageContent } from '@flowforge/cats-shared';
import { getConnectorDefinition } from '@flowforge/cats-shared';
import type { Logger } from './connector-redis-client';
import type { ConnectorCommandLayer } from './connector-command-layer';
import { ConnectorMessageFormatter, DEFAULT_QUICK_ACTIONS } from './connector-message-formatter';
import type { IConnectorPermissionStore } from './connector-permission-store';
import type { IConnectorThreadBindingStore } from './connector-thread-binding-store';
import type { InboundMessageDedup } from './inbound-message-dedup';
import { parseMentions } from './mention-parser';
import type { IOutboundAdapter } from './outbound-delivery-hook';

/** Abstract injected socket broadcast seam (web layer provides the impl). */
export interface SocketManager {
  broadcastToRoom(room: string, event: string, data: unknown): unknown;
}

/** Emit a connector_message socket event using the canonical protocol. */
function emitConnectorMessage(
  socketManager: SocketManager | null | undefined,
  threadId: string,
...
function connectorSourceIcon(def: { icon: { type: string; src?: string; iconId?: string } } | undefined): string {
  if (!def) return 'message';
  if ('src' in def.icon && def.icon.src) return def.icon.src;
  return def.icon.type === 'png' ? (def.icon.src ?? 'message') : (def.icon.iconId ?? 'message');
}
...
export interface ConnectorRouterOptions {
  ...
  /** Media service for downloading uploads into local storage. */
  ...
  readonly mentionPatterns?: Map<string, string[]> | undefined;
  /** Repository/project path backfilled onto created threads. */
...
export class ConnectorRouter {
  ...
  /** Request the injected @-mention pattern map. Injected to avoid CatRegistry coupling. */
  private getMentionPatterns(): Map<string, string[]> {
    return this.opts.mentionPatterns ?? new Map<string, string[]>();
  }
  ...
  async route(
    ...
    if (this.opts.commandLayer && trimmedText.startsWith('/')) {
      ...
      if (cmdResult.kind !== 'not-command' && cmdResult.response) {
        ...
        if (cmdResult.forwardContent && cmdResult.newActiveThreadId) {
          ...
          // F151 P1: If the target queue was full, close the task here.
    ...
    if (!binding) {
      ...
      const thread = await threadStore.create(this.opts.defaultUserId, title, this.defaultProjectPath());
      ...
      if (existing && (!existing.projectPath || existing.projectPath === 'default')) {
        await threadStore.updateProjectPath(binding.threadId, this.defaultProjectPath());
      }
    }
  ...
  private async resolveHubThreadOnce(
    ...
    const hubThread = await threadStore.create(this.opts.defaultUserId, hubTitle, this.defaultProjectPath());
