 ...
 * Extracted to keep the main class file focused.
 */

import type { SlashCommandDefinition } from '@flowforge/cats-shared';
import type { CommandResult } from './connector-command-layer';

/**
 * Local narrowed command registry view the command layer + helpers need
 * (mirrors the flowforge `@flowforge/commands` surface). Optional/injected.
 */
export interface ConnectorCommandRegistry {
  getAll(): readonly SlashCommandDefinition[];
  listBySurface(surface: string): readonly SlashCommandDefinition[];
  get?(name: string): { source?: string } | undefined;
}
...
export function auditSlashCommand(trimmed: string, duration: number, registry?: ConnectorCommandRegistry): void {
  const cmd = trimmed.split(/\s+/)[0]?.toLowerCase();
  const src = (cmd && registry?.get?.(cmd)?.source) ?? ('core' as const);
  console.log(
    JSON.stringify({
      event: 'slash_command',
      command: cmd,
...
export function buildCommandsList(registry?: ConnectorCommandRegistry): CommandResult {
