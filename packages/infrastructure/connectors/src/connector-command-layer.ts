import { type CatRegistry, normalizeCatId, parseCommand } from '@flowforge/cats-shared';
...
export interface CommandResult {
  ...
  /** one-shot target cat for /ask routing */
...
export interface ConnectorCommandLayerDeps {
  ...
  readonly catRegistry?: CatRegistry;
...
export class ConnectorCommandLayer {
  ...
  private async handleFocus(
    ...
    const resolved = this.deps.catRegistry
      ? normalizeCatId(trimmed, this.deps.catRegistry)
      : { ok: false as const, reason: 'not-found' as const, input: trimmed };
    if (!resolved.ok) {
  ...
  private async handleAsk(
    ...
    const resolved = this.deps.catRegistry
      ? normalizeCatId(catInput, this.deps.catRegistry)
      : { ok: false as const, reason: 'not-found' as const, input: catInput };
    if (!resolved.ok) {
      if (resolved.reason === 'ambiguous') {
        return {
          kind: 'ask',
