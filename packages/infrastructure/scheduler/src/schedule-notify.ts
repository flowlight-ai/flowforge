/**
 * #415 Phase 2: 任务生命周期通知（TS 移植自 clowder-ai `schedule-notify.ts`）。
 *
 * fire-and-forget 通知投递到 delivery 线程：registered/paused/resumed/deleted/
 * failed/missed_window。
 */

import { getNextCronMs } from './cron-utils.ts';
import type { DynamicTaskDef, ScheduleLifecycleNotifier, SchedulerLifecycleEvent, SchedulerToastPayload, TriggerSpec } from './types.ts';
...
export function notifyTaskSucceeded(notify: ScheduleLifecycleNotifier | undefined, def: DynamicTaskDef): void {
  // Reminder runs already create a hidden trigger + accented reply. Adding a success toast
