 ...
 *
 * 注入式 roster getter（与 cat-config-loader 单例解耦）。
 * 移植自 clowder-ai `infrastructure/scheduler/ActorResolver.ts`。
 */

import type { ActorRole, CostTier } from './types.ts';
...
export type RosterGetter = () => Record<string, RosterEntry>;
