/**
 * F202 Phase 2D: IssueCommentRouter — 格式化并投递 GitHub issue 评论通知。
 *
 * AC-D-security：外部评论 body 包裹 [UNTRUSTED EXTERNAL CONTENT]。
 * AC-C2：Phase-C issue 延续备注存在时附加。
 *
 * TS 移植自 clowder-ai `infrastructure/email/IssueCommentRouter.ts`；
 * selectIssueFixReadiness 自包含移植于 `./issue-fix-evidence.ts`。
 */

import type { ConnectorSource } from '@flowforge/cats-shared';

import { deliverConnectorMessage, type ConnectorDeliveryDeps } from './deliver-connector-message.ts';
import { selectIssueFixReadiness } from './issue-fix-evidence.ts';
...
export class IssueCommentRouter {
  ...
  async route(
    ...
    this.opts.log.info(
      `[IssueCommentRouter] ${signal.repoFullName}#${signal.issueNumber} → ${tracking.catId} (${signal.newComments.length} comments)`,
    );
...
/** 格式化 issue 评论通知（外部内容包裹 + fix readiness 标注）。 */
export function buildIssueCommentContent(signal: IssueCommentSignal): string {
  ...
  if (fixReadiness.kind === 'ready') {
    ...
    lines.push('', '⏳ **Fix claim detected — evidence missing**', '- Keep awaiting evidence; do not mark re-review ready.');
  }
