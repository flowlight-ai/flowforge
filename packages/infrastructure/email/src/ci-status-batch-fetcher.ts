/**
 * F304: 批量 GitHub GraphQL 读取 — 单次 cicd-check tick 用一次 gh GraphQL
 * 进程读取所有已跟踪 PR 的状态与 rollup。
 *
 * TS 移植自 clowder-ai `infrastructure/email/ci-status-batch-fetcher.ts`。
 * 失败检查仍走既有 typed REST enrich；普通 pass/pending 轮询不再为每个
 * 已跟踪 PR 派生 1-2 个 gh 进程。
 */

import type { CiCheckDetail, CiPollResult } from './index.ts';
import {
...
/**
 * 单次 GraphQL 进程读取所有已跟踪 PR 的状态与 status rollup。
 * 失败检查仍使用既有 typed REST enrich；普通 pass/pending 轮询不再为每个
 * PR 派生 1-2 个 gh 进程。
 */
