/**
 * @flowforge/infrastructure-github-signals — GitHub PR wait baseline reader
 *
 * Self-contained port of the clowder-ai `domains/github-signals/GitHubWaitBaselineReader.ts`.
 * Builds the PR capture-time baseline + collector state from lazy fetch seams,
 * conditionally fetching only the surfaces required by the predicate set.
 */
...
import type { GitHubPrWaitPredicate } from './contract/predicate.ts'

export interface GithubIdItem {
  readonly id?: unknown
}

export interface GithubReviewItem extends GithubIdItem {
...
export interface InitialPrWaitSnapshot {
  ...
  readonly collectorState: PrAutomationStateLike
}

/** Minimal collector-state projection returned alongside the baseline. */
export interface PrAutomationStateLike {
  readonly review?: {
