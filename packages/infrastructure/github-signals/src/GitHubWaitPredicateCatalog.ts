/**
 * @flowforge/infrastructure-github-signals — GitHub wait predicate catalog
 *
 * Self-contained port of the clowder-ai `domains/github-signals/GitHubWaitPredicateCatalog.ts`
 * (zod `discriminatedUnion` validation + deduped predicate-list canonicalization
 * + `matchGitHubWaitPredicates` pure matching function).
 */
  ...
  return z
    ...
    .superRefine((predicates, ctx) => {
      ...
      for (const [index, raw] of predicates.entries()) {
        const predicate = raw as { kind: string; reviewThreadIds?: readonly string[] }
