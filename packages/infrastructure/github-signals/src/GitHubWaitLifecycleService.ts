/**
 * @flowforge/infrastructure-github-signals — GitHub wait lifecycle service
 *
 * Self-contained port of the clowder-ai `domains/github-signals/GitHubWaitLifecycleService.ts`.
 * Observes GitHub wait facts against an active await, terminalizes via the wait
 * state machine, and publishes pending outcomes through an injectable connector
 * delivery port (with owner-fence quarantine + optimistic generation CAS retry).
 */
...
import { createWaitContinuationCarrier, parseWaitOwnerFence } from './contract/owner-fence.ts'
import type { ConnectorDeliveryDeps, ConnectorDeliveryInput } from './ports/ConnectorDelivery.ts'
import type { IWaitLifecycleEventLog } from './ports/IWaitLifecycleEventLog.ts'
import type { ITaskStore } from './ports/ITaskStore.ts'
import {
  markWaitOutcomeDelivered,
  markWaitOutcomeLegacyUnfenced,
  transitionWaitState,
  type WaitTransitionEvent,
} from './wait-state-machine.ts'
import { type GitHubWaitFacts, matchGitHubWaitPredicates } from './GitHubWaitPredicateCatalog.ts'
import {
  type GitHubReviewLoopBrake,
  REVIEW_LOOP_BRAKE_NEXT_STEP,
  REVIEW_LOOP_HISTORY_WARN_NEXT_STEP,
  renderGitHubWaitOutcome,
} from './github-wait-renderer.ts'
...
export class GitHubWaitLifecycleService {
  ...
  private async publishPending(
    ...
    const result = await this.opts.deliveryDeps.deliver({
      threadId: task.threadId,
      userId: task.userId ?? '',
