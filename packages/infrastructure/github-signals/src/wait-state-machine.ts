/**
 * @flowforge/infrastructure-github-signals — wait runtime state machine (pure)
 *
 * Self-contained port of the clowder-ai `../ball-custody/wait-state-machine.ts`
 * pure functions: `transitionWaitState`, `markWaitOutcomeDelivered` and
 * `markWaitOutcomeLegacyUnfenced`. Terminalizes an active await into a
 * `waitOutcome`, copying the owner fence from the await (never reconstructed).
 */
...
import type { AwaitStateV1 } from './contract/predicate.ts'
import type { GitHubWaitMatchedDelta } from './contract/predicate.ts'
import { parseWaitOwnerFence } from './contract/owner-fence.ts'
...
function terminalize(
  ...
  // Faithful to the upstream `await: undefined` terminalize: strip the active
  // await from the outgoing state. Destructure avoids an exactOptionalPropertyTypes
  // error while still removing the optional `await` key.
  const { await: _removedAwait, ...rest } = current
  return {
    applied: true,
    state: { ...rest, waitOutcome },
  }
}
