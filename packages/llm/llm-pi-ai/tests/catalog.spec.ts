...
import LlmRuntime, { createUserMessage } from '@flowforge/llm'
import type { StreamChunk } from '@flowforge/llm'
...
import { PiAiAdapter } from '@flowforge/llm-pi-ai'
...
/** A throwaway $FF_HOME with an empty settings document. */
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'flowforge-pi-catalog-'))
  homes.push(dir)
