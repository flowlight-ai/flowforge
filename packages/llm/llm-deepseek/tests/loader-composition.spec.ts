...
import LocalCredentialProvider from '@flowforge/credentials-local'
import { settingsNamespace } from '@flowforge/settings'
import FileSettingsProvider from '@flowforge/settings-file'
...
async function loadComposition(
  ...
  root = options.reuseRoot ?? await mkdtemp(join(tmpdir(), 'flowforge-llm-composition-'))
  vi.stubEnv('FF_HOME', root)
  ...
  await writeFile(configPath, [
    ...
    ...options.withDynamic
      ? [
        '- id: settings',
        "  name: '@flowforge/settings-file'",
        '  config:',
        `    path: ${JSON.stringify(settingsPath)}`,
        '    debounceMs: 10',
        '- id: credentials',
        "  name: '@flowforge/credentials-local'",
        '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@flowforge/settings-file', FileSettingsProvider],
    ['@flowforge/credentials-local', LocalCredentialProvider],
    ['@flowforge/llm-deepseek', LlmDeepSeek],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('llm-deepseek real dynamic composition', () => {
  it('boots from cordis.yml and routes the next request after external settings and credential edits', async () => {
    ...
    expect(serverA.headers[0]?.['x-flowforge-user-id']).toBe(getOrCreateAnonymousUserId())
