/**
 * Skill content scanning: line-by-line pattern scan of untrusted skill
 * content (SKILL.md bodies, injected instructions) for prompt-injection,
 * data-exfiltration, credential-access and safety-disable attempts, in
 * both English and Chinese phrasings. Mapped from the upstream skill-security
 * reference `content-scanner` (C36); findings feed {@link SkillSecurityStore}
 * quarantine decisions.
 *
 * @module @flowforge/skill-security/scanner
 */

/** One scan hit: which pattern, how severe, where, and the offending slice. */
...
/** The closed pattern table (English + Chinese phrasings). */
...
/** Context slice length kept per finding (keeps findings loggable). */
const CONTEXT_LIMIT = 120

/**
 * Scan untrusted skill content. Every line is tested against every
 * pattern; a line may yield several findings. Pure and synchronous —
 * hosts run it at install time, before any skill content reaches an
 * agent prompt.
 */
    ...
    for (const pattern of SCAN_PATTERNS) {
      if (pattern.regex.test(line)) {
        findings.push({
          ...
          line: index + 1,
          context: line.slice(0, CONTEXT_LIMIT),
        })
      }
    }
  }
