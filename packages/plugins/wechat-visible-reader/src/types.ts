/**
 * WeChat Visible Reader 类型与解析器（C35，自包含移植）。
 *
 * zod 严格校验原生读取结果：NormalizedRect 归一化边界、VisibleMessageUnit
 * （完整/部分文本/非文本三态）、WeChatVisibleReadResult 成功+失败判别联合；
 * parse* 解析器校验 limits（maxBlocks/maxChars）与文本字符数一致性。
 *
 * @module @flowforge/plugins-wechat-visible-reader/types
 */
import { z } from 'zod';
...
export const normalizedRectSchema = z
  ...
  .superRefine((rect, context) => {
    if (rect.x + rect.width > 1.000_001) {
      context.addIssue({ code: 'custom', message: 'rect exceeds horizontal bounds' });
    }
    if (rect.y + rect.height > 1.000_001) {
      context.addIssue({ code: 'custom', message: 'rect exceeds vertical bounds' });
    }
  });
