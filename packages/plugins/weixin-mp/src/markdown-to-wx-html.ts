/**
 * Markdown → 微信公众号 HTML 转换器（C35 weixin-mp 插件，自包含移植）。
 *
 * 支持：代码块/表格/标题/引用/列表/分割线/链接/图片（URL 白名单 http(s)/#），
 * 内联加粗/斜体/行内代码，HTML 转义（& < > " '），微信内联样式表。
 *
 * @module @flowforge/plugins-weixin-mp/markdown-to-wx-html
 */
const S = {
