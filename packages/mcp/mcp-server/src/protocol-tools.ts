...
/**
 * Minimal local callback POST using the injectable callback-config seam.
 * Returns a ToolResult-shaped response or null when the request fails.
 */
async function localCallbackPost(path: string, payload: unknown): Promise<ToolResult> {
  ...
  if (!response.ok) return errorResult(`callback ${path} failed: HTTP ${response.status}`);
...
function createSubmitTool(config: ProtocolToolConfig, capabilities: string[], taskVars: TaskVars): ToolDef {
  return {
    ...
    inputSchema: {
      ...
      vars: z.record(z.string(), z.string()).describe('Template variables (prompt, imageUrl, etc.)'),
    },
...
/**
 * Best-effort: create a RichFileBlock via the callback API so the media renders
 * inline in chat. Returns true if the block was emitted, false otherwise.
 * When no callback config is present this returns false (skip). Poll result is
 * returned regardless of emission success.
 */
async function emitMediaRichBlock(url: string, prefix: string, taskId: string, capability?: string): Promise<boolean> {
  ...
  // The server expects `block` as an object, not a JSON string.
  const result = await localCallbackPost('/api/callbacks/create-rich-block', { block });
...
function createExecuteTool(config: ProtocolToolConfig, capabilities: string[]): ToolDef {
  return {
    ...
    inputSchema: {
      capability: z.enum(capabilities as [string, ...string[]]).describe('Capability to invoke'),
      vars: z.record(z.string(), z.string()).describe('Template variables (videoUrl, prompt, etc.)'),
    },
    handler: (async (input: { capability: string; vars: Record<string, string> }, extra?: ToolExtra) => {
      try {
...
export function buildProviderFromEnv(
  ...
  if (model) providerInstance.model = model;
