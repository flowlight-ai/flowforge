...
async function executeRequest(
  ...
  const signingRequest: { method: string; url: string; body?: string } = { method: endpoint.method, url };
  if (body !== undefined) signingRequest.body = body;
  const authResult = auth.sign(credentials, signingRequest);
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    let resp: Response;
    try {
      const fetchInit: RequestInit = { method: endpoint.method, headers, signal: requestSignal };
      if (body !== undefined && endpoint.method !== 'GET') fetchInit.body = body;
      resp = await fetch(finalUrl, fetchInit);
    } catch (err) {
...
export async function poll(
  ...
  const scrubbedUrl = resultUrl ? scrubSecrets(resultUrl, secrets) : undefined;
  const scrubbedCover = coverUrl ? scrubSecrets(coverUrl, secrets) : undefined;
  const scrubbedError = error ? scrubSecrets(error, secrets) : undefined;

  const pollResult: PollResult = { status };
  if (scrubbedUrl !== undefined) pollResult.resultUrl = scrubbedUrl;
  if (scrubbedCover !== undefined) pollResult.coverUrl = scrubbedCover;
  if (scrubbedError !== undefined) pollResult.error = scrubbedError;
  return pollResult;
}
...
function resolvePoll(template: ProtocolTemplate, cap: Capability, capName: string): PollEndpoint {
  ...
  if (cap.inherit) {
    ...
    const parentName = ref[0] ?? '';
    const parentCap = template.capabilities[parentName];
