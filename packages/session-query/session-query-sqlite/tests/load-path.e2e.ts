 ...
 *
 * @module @flowforge/session-query-sqlite/tests/load-path
 */

import { afterEach, describe, expect, it } from 'vitest'
import { Context } from '@flowforge/cordis'
import Loader from '@flowforge/cordis-plugin-loader'
import { SESSION_FORMAT_VERSION, SessionId } from '@flowforge/session'
import SessionStore from '@flowforge/session'
import SqliteSessionPersistence from '@flowforge/session-persistence-sqlite'
import SqliteSessionQueryEngine, * as queryModule from '@flowforge/session-query-sqlite'
...
async function temporaryPath(name: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'flowforge-session-search-loader-'))
  temporaryDirectories.push(directory)
  return join(directory, name)
}

describe('flowforge-session-query-sqlite real Loader path', () => {
