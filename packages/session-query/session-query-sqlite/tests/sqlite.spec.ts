...
import { Context, type Fiber } from '@flowforge/cordis'
import { DatabaseSync } from 'node:sqlite'
import { chmod, mkdtemp, rm, stat, writeFile } from 'node:fs/promises'
import { tmpdir } from 'node:os'
import { dirname, join } from 'node:path'
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@flowforge/session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@flowforge/session'
import SessionPersistence, { SessionPersistenceRevision } from '@flowforge/session-persistence'
import type { SessionPersistenceSnapshot } from '@flowforge/session-persistence'
import SqliteSessionPersistence from '@flowforge/session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@flowforge/session-query-sqlite'
import {
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'flowforge-session-search-'))
