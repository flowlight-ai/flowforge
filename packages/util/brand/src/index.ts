 ...
 *
 * Policy: a package brands the ids it owns — `CallId` in flowforge-llm (tool-call
 * correlation), the shared agent/session `SessionId` in flowforge-session, and
 * `JobId` in flowforge-jobs. Branding is for ids that cross package boundaries and
