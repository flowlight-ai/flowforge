 ...
 *
 * Nothing here imports a runtime transport, so the module stays loadable in any runtime.
 * @module @flowforge/http-proxy/policy
 */
 ...
 *
 * `::1` and `[::1]` are both listed because the resolved string is also handed to a consumer whose
...
/**
 * The environment names each policy field owns, lowercase first — a consumer reads the lowercase
 * name first, so both casings are always written or cleared together.
 */
...
/**
 * Read one environment name in a consumer's precedence order — lowercase first, uppercase as the
 * fallback — treating a blank value as unset. Blank matters: a `??` chain lets an empty
 ...
 * a bracketed literal takes its port from after the bracket. Getting this wrong is how a consumer
 ...
 * Node's proxy rules so this function and the installed environment never disagree about one URL.
 *
...
export function resolveProxyPolicy(env: EnvLookup): ProxyResolution {
  ...
  // HTTPS falls back to the HTTP proxy last, matching Node — but never past a value the user named
 ...
 *
 * This is the single answer both the published environment and every routing consumer consult, so a URL
