/**
 * External URL safety — SSRF-style validation and pinned fetching.
 *
 * Ported from clowder-ai `api/src/utils/url-safety.ts`.
 * Only depends on Node built-ins (`node:dns/promises`, `node:http`, `node:https`,
 * `node:net`, `node:perf_hooks`).
 *
 * `validateExternalUrl` rejects non-http(s) protocols, blocked hostnames
 * (localhost / cloud metadata endpoints) and private/reserved IP ranges so
 * that server-side fetches cannot be redirected at internal resources.
 */
...
/**
 * Validate a URL for external (server-side) fetch safety. Protocol must be
 * http/https and the hostname must not be blocked or resolve to a private or
 * reserved IP range. Returns the parsed URL when allowed.
 */
...
/**
 * Validate a URL and resolve its hostname to a pinned public address.
 * DNS records that point back into private/reserved ranges are rejected.
 */
...
/** Validate and resolve a URL; throws when resolution is not possible. */
...
/** Build a `RequestOptions` that connect to the pinned address while sending
 * the original Host header (and sni servername for https). */
...
/**
 * Fetch an external URL with SSRF pinning: the hostname is resolved up front,
 * every resolved address must be public, and the request connects to the pinned
 * address (not to whatever DNS says later). Redirects and oversized bodies are
 * rejected; the whole round trip is bounded by `timeoutMs`.
 */
export async function fetchExternalUrlPinned(url: string, options: PinnedFetchOptions): Promise<PinnedFetchResult> {
  ...
  return new Promise((resolve, reject) => {
    ...
    // The outer wall-clock timeout and the socket timeout are both idempotent
    // through `settle`; only the first one to fire publishes a settlement.
    setTimeout(() => {
