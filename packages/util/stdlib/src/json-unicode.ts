/**
 * Unicode scalar-value normalization for JSON-shaped values.
 *
 * Ported from clowder-ai `api/src/utils/json-unicode.ts`.
 * Converts every string leaf of a JSON-shaped value to well-formed Unicode:
 * valid surrogate pairs are preserved, isolated UTF-16 surrogates become
 * U+FFFD. Well-formed inputs are returned unchanged; inputs are never mutated.
 */
