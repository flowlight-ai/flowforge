...
import {
  createLaunchEnvironmentSnapshot, FF_LAUNCH_ENVIRONMENT_KEY, launchEnvironmentOf,
} from '../src/index.ts'
...
const layered = createLaunchEnvironmentSnapshot([
  ...
  { source: 'user-env', path: '/home/.flowforge/.env', values: { SHARED: 'from-user', ONLY_USER: 'u' } },
])
...
describe('createLaunchEnvironmentSnapshot', () => {
  it('resolves across every layer, most trusted first, and reports the winning source', () => {
    ...
    expect(layered.get('ONLY_USER')).toEqual({ value: 'u', source: 'user-env', path: '/home/.flowforge/.env' })
...
describe('launchEnvironmentOf', () => {
  it('returns the launcher snapshot when the product CLI provided one', () => {
    ...
    ctx.provide(FF_LAUNCH_ENVIRONMENT_KEY, layered)
    expect(launchEnvironmentOf(ctx)).toBe(layered)
  })

  it('falls back to the inherited environment as the only layer', () => {
    vi.stubEnv('FF_ENV_SPEC_FALLBACK', 'ambient')
    try {
      const snapshot = launchEnvironmentOf(new Context())
      expect(snapshot.get('FF_ENV_SPEC_FALLBACK')).toEqual({ value: 'ambient', source: 'process' })
    } finally {
      vi.unstubAllEnvs()
    }
  })
