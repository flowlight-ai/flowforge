...
import { FsVersion } from '@flowforge/fs'
...
import SandboxedFileSystem from '@flowforge/fs-sandbox'
import SandboxPolicy from '@flowforge/sandbox-policy'
...
async function setup(
  ...
  const root = await mkdtemp(join(tmpdir(), 'flowforge-tool-str-replace-editor-'))
  roots.push(root)
...
describe('tool-str-replace-editor', () => {
  ...
  it('reports missing sandbox-policy composition during plugin startup', async () => {
    const root = await mkdtemp(join(tmpdir(), 'flowforge-tool-str-replace-editor-missing-policy-'))
    roots.push(root)
    const ctx = new Context()
    contexts.push(ctx)
    await ctx.plugin(SystemPrompt)
