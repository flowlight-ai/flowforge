 ...
 * `@flowforge/fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@flowforge/bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@flowforge/tool-fs`),
 * exactly as bash's does.
 *
 * @module @flowforge/fs-sandbox
 */

import { Context } from '@flowforge/cordis'
import { LocalFileSystem } from '@flowforge/fs-local'
import type { Config as LocalConfig } from '@flowforge/fs-local'
import { FsError } from '@flowforge/fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@flowforge/fs'
import { writableRoots } from '@flowforge/sandbox'
import type { SandboxExecutionPolicy, SandboxMode } from '@flowforge/sandbox'
import type {} from '@flowforge/sandbox-policy'
 ...
 * INSTEAD OF `flowforge-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `flowforge-tool-fs` resolves
