...
import { FsError, FsTargetKey } from '@flowforge/fs'
import type { FsTarget } from '@flowforge/fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.flowforge-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'flowforge-fssbx-tmp-')), 'temp.txt')
