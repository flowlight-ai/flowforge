...
import { CodeRuntime } from '@flowforge/code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@flowforge/code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@flowforge/llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, StreamChunk } from '@flowforge/llm'
import SystemPrompt from '@flowforge/system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@flowforge/tools'
import type { Config as ToolConfig } from '@flowforge/tools'
import LocalFileSystem from '@flowforge/fs-local'
import * as FsPolicy from '@flowforge/fs-observation-policy'
import LocalAttachmentStore from '@flowforge/attachment-local'
import { AttachmentId, AttachmentStore } from '@flowforge/attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'flowforge-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'flowforge-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { flowforgeHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { flowforgeHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { flowforgeHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
