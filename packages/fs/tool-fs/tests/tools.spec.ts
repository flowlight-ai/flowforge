...
import ToolRuntime, { type ToolResult } from '@flowforge/tools'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@flowforge/fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'flowforge-tool-fs-session-cwd-'))
