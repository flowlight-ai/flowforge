 ...
 *
 * @module @flowforge/tool-fs-search/search-core
 */

import { isAbsolute, relative, sep } from 'node:path'
import type { Context } from '@flowforge/cordis'
import { HarnessError } from '@flowforge/llm'
import { ItemRetainer, TextRetainer } from '@flowforge/output-retention'
import type { RetainedItems } from '@flowforge/output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@flowforge/subprocess'
import type { SaveTextSpill, SpillRef } from '@flowforge/spill'
 ...
 * `@flowforge/tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`flowforge-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@flowforge/tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@flowforge/tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@flowforge/tool-fs-search/presentation} `grepSearchMeta`)
