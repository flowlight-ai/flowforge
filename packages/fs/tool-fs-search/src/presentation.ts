 ...
 * model-facing render consumes ({@link module:@flowforge/tool-fs-search/search-core}
 ...
 * final output budget (`flowforge-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @flowforge/tool-fs-search/presentation
 */
 ...
 * ({@link module:@flowforge/tool-fs-search/grep} `formatGrepMatches`), so
