 ...
 *
 * @module @flowforge/skill-filesystem
 */
...
import type Schema from '@flowforge/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@flowforge/fs'
import { canonicalizeWatchPath, resolveFlowforgeHome } from '@flowforge/home-paths'
import {
  BUNDLED_SKILL_RANK,
  isSkillName,
  type SkillCandidate,
...
export interface Config {
  ...
  /** FlowForge config root. Defaults to `$FF_HOME` or `~/.flowforge`. */
  flowforgeHome?: string
  /** Shared agent config root. Defaults to `$FF_AGENTS_HOME` or `~/.agents`. */
  agentsHome?: string
  ...
  /** Bundled skill root; defaults to `$FF_BUNDLED_SKILL_DIR` when default roots are included, otherwise mounts none. */
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.FF_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.FF_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.flowforge/skills'), source: 'project-flowforge', rank: PROJECT_FF_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.flowforgeHome, 'skills'), source: 'user-flowforge', rank: USER_FF_RANK, skipSystem: true },
...
class SkillWatchManager {
  ...
  private async replaceWatcher(state: RootWatchState): Promise<void> {
    ...
    try {
      ...
      // The teardown race is documented by the caller; only healthy paths proceed.
      if (!this.closing) {
