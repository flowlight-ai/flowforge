...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@flowforge/fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `ff-${name}-`)))
}
...
describe('flowforge-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.flowforge/skills'), 'same', 'user flowforge skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.flowforge/skills'), 'same', 'project flowforge skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.flowforge/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project flowforge skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-flowforge')
    ...
    await writeSkill(join(noGit, '.flowforge/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.flowforge/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.flowforge/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.flowforge/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.flowforge/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.flowforge/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.flowforge/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.flowforge/skills/broken-link'))
    await symlink('/dev/null', join(home, '.flowforge/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { flowforgeHome: join(home, '.flowforge'), agentsHome: join(home, '.agents'), watch: false })

    expect((await ctx.skills.list({ cwd: nestedCwd })).map(skill => [skill.name, skill.source])).toEqual([
      ['backend-root', 'project-agents'],
      ['text-skill', 'user-flowforge'],
    ])
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.flowforge/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.flowforge/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    const previousFlowforgeHome = process.env.FF_HOME
    const previousAgentsHome = process.env.FF_AGENTS_HOME
    const previousBundledSkillDir = process.env.FF_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.FF_HOME = join(envHome, '.flowforge')
      process.env.FF_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.FF_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.flowforge/skills'), 'env-skill', 'Env skill')
      ...
      process.env.FF_HOME = join(envHome, 'empty-flowforge')
      delete process.env.FF_BUNDLED_SKILL_DIR
      process.env.FF_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { flowforgeHome: join(envHome, 'empty-flowforge') }).name).toBe('filesystem')
    } finally {
      if (previousFlowforgeHome === undefined) {
        delete process.env.FF_HOME
      } else {
        process.env.FF_HOME = previousFlowforgeHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.FF_AGENTS_HOME
      } else {
        process.env.FF_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.FF_BUNDLED_SKILL_DIR
      } else {
        process.env.FF_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
