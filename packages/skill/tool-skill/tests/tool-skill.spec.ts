...
import { createUserMessage, CallId, type Message } from '@flowforge/llm'
import { createScope, type Scope } from '@flowforge/scope'
import { Session, SessionId, type SessionEvent, type UserMessage } from '@flowforge/session'
...
import AgentRegistry, { agentEvents, Inbox, type Agent, type PreStepDecision } from '@flowforge/agent'
...
describe('flowforge-tool-skill', () => {
  ...
  it('resumes from the durable entries of the latest visible catalog', async () => {
    ...
    session.append('user/message', createUserMessage({
      ...
      source: { kind: 'plugin', plugin: 'flowforge-tool-skill' },
    }), { surfaceOp: 'append' })
  ...
  it('loads a skill for the calling agent cwd', async () => {
    ...
    await writeSkill(join(project, '.flowforge/skills'), 'project-skill', 'Project skill', 'Project instructions.')
    const ctx = await setup(home)
    ...
    expect(result.value).toEqual({
      name: 'project-skill',
      provider: 'filesystem',
      resourceBase: { kind: 'directory', path: join(project, '.flowforge/skills/project-skill') },
    ...
    expect(block.text).toBe([
      ...
      '<skill_resources>',
      `Base directory for this skill: ${join(project, '.flowforge/skills/project-skill')}`,
  ...
  it('returns isError for unknown, invalid, and model-disabled skills', async () => {
    ...
    await writeSkill(join(home, '.flowforge/skills'), 'hidden-skill', 'Hidden skill', 'Hidden instructions.')
    await writeFile(join(home, '.flowforge/skills/hidden-skill/SKILL.md'), '---\nname: hidden-skill\ndescription: Hidden skill\ndisable-model-invocation: true\n---\n\nHidden instructions.\n')
    const ctx = await setup(home)
    ctx.skills.register({
      name: 'model-only-skill',
      description: 'Model-only skill',
