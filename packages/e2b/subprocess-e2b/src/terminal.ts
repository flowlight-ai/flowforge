...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'ff_state=$1',
  'mapfile -d \'\' -t ff_env < "$ff_state/environment"',
  'mapfile -d \'\' -t ff_argv < "$ff_state/argv"',
  'ff_output_marker=$(<"$ff_state/output-marker")',
  'rm -f -- "$ff_state/environment" "$ff_state/argv" "$ff_state/output-marker" "$ff_state/runner.bash"',
  'if (( ${#ff_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$ff_output_marker"',
  'exec env -i -- "${ff_env[@]}" "${ff_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`flowforge-e2b-bootstrap:${randomUUID()}`)
