...
import type { CommandHandle, CommandResult, Sandbox } from '@flowforge/e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$ff_e2b_env_bin" -i "$ff_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$ff_e2b_tee" --output-error=warn-nopipe >("$ff_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$ff_e2b_tee" --output-error=warn-nopipe >("$ff_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'ff_e2b_rm=$7',
    'shift 7',
    'ff_e2b_pgid="$("$ff_e2b_ps" -o pgid= -p "$$" | "$ff_e2b_tr" -d " ")"',
    `printf '%s\\n' "$ff_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t ff_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$ff_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$ff_e2b_env_bin" -i -- "\${ff_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'ff_e2b_status=$?',
    `printf '%s\\n' "$ff_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$ff_e2b_status"',
  ].join('\n')
  ...
  const bootstrap = [
    `mapfile -d '' -t ff_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'ff_e2b_env_bin="$(command -v env)"',
    'ff_e2b_setsid="$(command -v setsid)"',
    'ff_e2b_bash="$(command -v bash)"',
    'ff_e2b_node="$(command -v node)"',
    'ff_e2b_ps="$(command -v ps)"',
    'ff_e2b_tr="$(command -v tr)"',
    'ff_e2b_tee="$(command -v tee)"',
    'ff_e2b_head="$(command -v head)"',
    'ff_e2b_rm="$(command -v rm)"',
    'for ff_e2b_tool in "$ff_e2b_env_bin" "$ff_e2b_setsid" "$ff_e2b_bash" "$ff_e2b_node" "$ff_e2b_ps" "$ff_e2b_tr" "$ff_e2b_tee" "$ff_e2b_head" "$ff_e2b_rm"; do',
    '  [[ "$ff_e2b_tool" == /* && -x "$ff_e2b_tool" ]] || exit 125',
    'done',
    `exec "$ff_e2b_env_bin" -i -- "\${ff_e2b_env[@]}" "$ff_e2b_setsid" --wait -- "$ff_e2b_bash" -c ${quoteE2BShellArg(inner)} flowforge-e2b "$ff_e2b_env_bin" "$ff_e2b_node" "$ff_e2b_ps" "$ff_e2b_tr" "$ff_e2b_tee" "$ff_e2b_head" "$ff_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
