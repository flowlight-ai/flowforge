...
import type E2BRuntime from '@flowforge/e2b'
import type { SubprocessSpawnSpec } from '@flowforge/subprocess'
import E2BSubprocessRuntime from '@flowforge/subprocess-e2b'
...
class FakeSandbox {
  ...
  alive = true
  zombieOnly = false
  ambient = 'PATH=/ambient/bin\0KEEP=safe\0UNICODE=你好\0NPM_TOKEN=secret\0FF_STALE=old\0BROKEN\0=bad\0'
...
function runtime(fake: FakeSandbox, getSandbox: () => Promise<Sandbox> = async () => fake.sandbox): E2BRuntime {
  return {
    cwd: '/workspace',
    runtimeRoot: '/workspace/.flowforge-e2b',
    getSandbox,
...
describe('E2BSubprocessHandle', () => {
  it('starts asynchronously, keeps secrets out of the command, and supports deferred piped stdin/output', async () => {
    ...
    }), '/workspace/.flowforge-e2b/processes/one')
    ...
    expect(controlEnvs?.HOME).toMatch(/^\/\.flowforge-e2b-control-/)
    ...
    const command = fake.commandsSeen.find(value => value.includes('exec "$ff_e2b_env_bin" -i'))!
    expect(command).toContain('"$ff_e2b_setsid" --wait -- "$ff_e2b_bash" -c')
    expect(command).not.toContain('DEEPSEEK_API_KEY')
    expect(command).not.toContain('FF_MODE')
    ...
    expect(command).not.toContain('${!ff_e2b_name}')
    ...
    expect(environmentProbe).toContain('test -n "$ff_e2b_home" -a -d "$ff_e2b_home"')
    expect(environmentProbe).not.toContain('"$PWD"')
    expect(command).toContain('mapfile -d')
    expect(command).toContain('ff_e2b_node="$(command -v node)"')
    expect(command).toContain('"$ff_e2b_env_bin" -i "$ff_e2b_node" -e')
    expect(command).toContain('"$ff_e2b_env_bin" -i -- "${ff_e2b_env[@]}" "$@"')
    expect(command).toContain('exec "$ff_e2b_env_bin" -i -- "${ff_e2b_env[@]}"')
    ...
    expect(fake.writtenFiles[0]).toEqual([
      '/workspace/.flowforge-e2b/processes/one/pid',
      '/workspace/.flowforge-e2b/processes/one/exit-code',
      '/workspace/.flowforge-e2b/processes/one/environment',
      '/workspace/.flowforge-e2b/processes/one/stderr.log',
    ])
    expect(fake.writtenFileData.get('/workspace/.flowforge-e2b/processes/one/environment')).toBe(
      'PATH=/bin\0UNICODE=你好\0HOME=/home/user\0FOO-BAR=hyphen-value\0--split-string=literal-value\0DEEPSEEK_API_KEY=explicit-secret\0FF_MODE=test\0',
    )
    ...
    expect(fake.removed).toContain('/workspace/.flowforge-e2b/processes/one/stderr.log')
  ...
  it('removes a spill once the complete stream exceeds its cap', async () => {
    ...
    const command = fake.commandsSeen.find(value => value.includes('ff_e2b_tee='))!
    expect(command).toContain('"$ff_e2b_head" -c 3')
    expect(command).toContain('/runtime/oversize/stdout.log')
    expect(command).toContain('"$ff_e2b_tee" --output-error=warn-nopipe')
