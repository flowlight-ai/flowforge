...
import { FsTargetKey, FsVersion } from '@flowforge/fs'
import E2BFileSystem from '@flowforge/fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.flowforge-e2b-control-/)
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['flowforge-version']).toBeDefined()
