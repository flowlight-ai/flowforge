...
import { Inbox } from '@flowforge/agent'
import type { Agent } from '@flowforge/agent'
import { runLoaderSmoke } from '@flowforge/loader-smoke'
import {
  FileNotFoundError,
  Sandbox,
  SandboxNotFoundError,
} from '@flowforge/e2b'
import TerminalSessionService, { TerminalSessionId } from '@flowforge/terminal'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', FF_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/flowforge-e2b-bootstrap-profile-leak'
      ...
      ctx.provide('e2b', {
        cwd: '/home/user',
        runtimeRoot: '/home/user/.flowforge-e2b',
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'ff_leak=0',
          'for ff_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$ff_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$ff_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then ff_leak=1; fi',
          'done',
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'flowforge-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.flowforge-e2b/terminals/')
