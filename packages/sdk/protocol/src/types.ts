/**
 * Named wire types for the FlowForge SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@flowforge/sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `flowforge-sdk-runtime`.
 *
 * @module @flowforge/sdk-protocol/types
 */

import type { ContentBlock } from '@flowforge/llm'
import type { SessionEvent } from '@flowforge/session'
import type { SubagentStopReason } from '@flowforge/subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`flowforge-sdk-runtime`) and version. */
