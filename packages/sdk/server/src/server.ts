 ...
 *
 * @module @flowforge/sdk-jsonrpc-server/server
 */

import type { Context } from '@flowforge/cordis'
import { resolve } from 'node:path'
import type { Agent, AgentHandle } from '@flowforge/agent'
import { createUserMessage } from '@flowforge/llm'
import { carrierKeyOf, type Scoped } from '@flowforge/scope'
import { SessionId } from '@flowforge/session'
import type SubagentRuntime from '@flowforge/subagent'
import type { SubagentRunEndInfo } from '@flowforge/subagent'
...
export class HarnessSdkJsonRpcServer {
  ...
  async initialize(params: InitializeParams): Promise<InitializeResult> {
    ...
    return { serverInfo: { name: 'flowforge-sdk-runtime', version: '0.0.1' } }
  }
  ...
  async handleRequest(method: string, params: Record<string, unknown> | undefined): Promise<unknown> {
    switch (method) {
      ...
      default:
        throw new Error(`unknown FlowForge SDK runtime method: ${method}`)
    }
  }
