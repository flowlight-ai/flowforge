...
import HttpServer from '@flowforge/host-webserver'
...
async function loadComposition(): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'frontend-static-'))
  ...
  await writeFile(configPath, [
    ...
    '    port: 0',
    '- id: frontend',
    "  name: '@flowforge/host-frontend-static'",
    '  config:',
    `    distIndex: '${distIndex}'`,
    '',
  ].join('\n'))
  ...
  const modules = new Map<string, unknown>([
    ['@flowforge/host-webserver', HttpServer],
    ['@flowforge/host-frontend-static', FrontendStatic],
  ])
