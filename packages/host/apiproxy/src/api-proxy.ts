...
import { installModelSelection } from '@flowforge/agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@flowforge/agent'
import type {} from '@flowforge/agent-presets/types'
import { AttachmentError } from '@flowforge/attachment'
import type { ImageAttachmentRef } from '@flowforge/attachment'
import { contentHasImage, createUserMessage, freezeMessage, ReasoningEffortId } from '@flowforge/llm'
import { errorChain } from '@flowforge/llm'
import type { ContentBlock, MessageSource } from '@flowforge/llm'
import { isAppendSurfaceEvent, isJsonValue } from '@flowforge/session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@flowforge/session'
import type { SessionPersistence } from '@flowforge/session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@flowforge/session-query'
import { SubagentError } from '@flowforge/subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@flowforge/subagent'
import { isUserInvocable } from '@flowforge/skill'
import type { Workspace, WorkspaceRecord } from '@flowforge/workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@flowforge/workspace'
...
} from '@flowforge/agent-presets'
import type { PresetBearingSession } from '@flowforge/agent-presets'
import type {} from '@flowforge/tools'
import type {
...
import type { SessionRawArtifact } from '@flowforge/session-persistence'
import {
...
import type {} from '@flowforge/session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@flowforge/jobs'
import type { JobSnapshot } from '@flowforge/jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@flowforge/session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@flowforge/goal'
import type { GoalRef as CoreGoalRef } from '@flowforge/goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@flowforge/commands'
...
import type {} from '@flowforge/cordis-host-runner/types'
import type {} from '@flowforge/skill'
...
import type { ScopeKey } from '@flowforge/scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@flowforge/user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@flowforge/user-approval'
...
} from '@flowforge/user-questions'
import { UserQuestionError } from '@flowforge/user-questions'
import { DirectoryPickerError } from '@flowforge/host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @flowforge/session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: llm depends on attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @flowforge/goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @flowforge/settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @flowforge/credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @flowforge/session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @flowforge/skill', details: {} })
        }
