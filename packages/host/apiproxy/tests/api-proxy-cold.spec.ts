...
import { TypertLookupFailure } from '@flowforge/typert-protocol'
import TypertRegistry from '@flowforge/typert-registry'
import { createUserMessage, MessageId } from '@flowforge/llm'
import type { Agent } from '@flowforge/agent'
import UserQuestionService from '@flowforge/user-questions'
import type { SessionEvent, SessionHeader, SessionId } from '@flowforge/session'
import {
  PersistenceCoordinator,
  SessionPersistenceRevision,
  type PersistenceBackend,
  type StoredPrefix,
} from '@flowforge/session-persistence'
import type { RpcRequest } from '@flowforge/host-apiproxy/api/rpc'
...
describe('sessions.list cold merge', () => {
  it('verifies only small possibly-blank artifacts and treats every unavailable probe as visible', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'cold-'))
  ...
  it('replaces a probed cold row with the live Session that attached during the read', async () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'cold-race-'))
...
describe('Remote Agent and Session lookup policy', () => {
  it('deduplicates a cold resume across Agent and Session parameters', async () => {
    ...
    const resumedSession = { id: sessionId, header: meta, events: [] } as unknown as import('@flowforge/session').Session
