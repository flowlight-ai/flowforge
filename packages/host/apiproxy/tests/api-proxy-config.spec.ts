...
import LlmRuntime, { LlmAdapter } from '@flowforge/llm'
import type { GenerateOptions, LlmModelInfo, LlmProviderInfo, StreamChunk } from '@flowforge/llm'
import { SettingsProvider, settingsNamespace } from '@flowforge/settings'
import type { SettingsNamespace } from '@flowforge/settings'
import { CredentialProvider } from '@flowforge/credentials'
import type { CredentialInfo, CredentialRef, ResolvedCredential } from '@flowforge/credentials'
import type { HostFrame } from '../src/api/index.ts'
import type { RpcRequest, RpcResponse } from '../src/api/rpc.ts'
import { RpcId } from '../src/api/rpc.ts'
import { AGENT_DEFAULT_MODEL_SETTINGS_NAMESPACE } from '@flowforge/agent-default-model'
...
describe('settings domain', () => {
  it('reports an actionable error when no settings provider is mounted', async () => {
    ...
    expect(error.message).toContain('@flowforge/settings-file')
  })
...
describe('credentials domain', () => {
  it('reports an actionable error when no credential provider is mounted', async () => {
    ...
    expect(error.code).toBe('internal')
    expect(error.message).toContain('@flowforge/credentials-local')
  })
