 ...
 * @module @flowforge/directory-picker-auto/resolve
 */

/** Loopback-only bind host selector used by the resolver. */
export const LOOPBACK_BIND_HOST = '127.0.0.1' as const
...
export interface DirectoryPickerHostFacts {
  /** Effective webserver bind host (a loopback-only or all-interfaces union). */
  bindHost: string
...
/** An env value counts only when set and non-blank (an empty export is "unset"). */
 ...
 * the operator can see a display the native backend can serve: a loopback-only
 ...
 * darwin/win32, requiring `DISPLAY`/`WAYLAND_DISPLAY` plus a chooser binary on
 * linux, and never true elsewhere. Anything ambiguous resolves to `browse`,
...
export function resolveDirectoryPickerBackend(facts: DirectoryPickerHostFacts): DirectoryPickerBackendKind {
  if (facts.bindHost !== LOOPBACK_BIND_HOST) return 'browse'
