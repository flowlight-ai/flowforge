 ...
 * @module @flowforge/sdk-app
 */

import { Command } from 'commander'
import type { Context } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import { exitOnStdinEnd, parseCmdline } from '@flowforge/cmdline'
...
function sdkCommand(profile: string): Command {
  return new Command()
    .name(`flowforge --profile ${profile}`)
    .description('Serve flowforge SDK clients over stdio JSON-RPC.')
    .helpOption('-h, --help', 'show this help')
    .addHelpText('after', `
Example:
  flowforge --profile ${profile}     serve one SDK runtime until its client disconnects
`)
}
