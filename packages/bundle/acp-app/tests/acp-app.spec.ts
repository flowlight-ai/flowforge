...
describe('@flowforge/acp-app bundle', () => {
  it('declares startup-gated ACP serving without overriding base HMR policy', () => {
    ...
    expect(manifest.peerDependencies).toHaveProperty('@flowforge/acp')
    ...
    expect(rows.find(row => row.id === 'acp-app-startup')?.name).toBe('@flowforge/acp-app')
