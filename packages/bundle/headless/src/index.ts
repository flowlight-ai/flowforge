/**
 * @flowforge/headless — one-shot direct Agent driver. The bundle patch
 * rides over base without Host, HTTP, or browser plugins; this runner
...
import type { ModelSelectionRef } from '@flowforge/agent'
import type {} from '@flowforge/agent-default-model'
...
import type {} from '@flowforge/cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`flowforge: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  // (@flowforge/agent-presets README, "Composing a child agent").
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`flowforge: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
