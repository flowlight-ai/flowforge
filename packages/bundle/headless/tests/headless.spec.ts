...
import AgentDefaultModelConfig from '@flowforge/agent-default-model'
import { createAssistantMessage } from '@flowforge/llm'
import SessionStore from '@flowforge/session'
import type { Session, UserMessage } from '@flowforge/session'
...
describe('headless runner', () => {
  ...
  it('prints the durable model failure when the final turn ends in error', async () => {
    ...
    expect(await test.run()).toMatchObject({
      code: 1,
      out: '\n',
      err: 'flowforge: SERVER: provider unavailable\n',
    })
    await test.ctx.fiber.dispose()
  })
  ...
  it('reports a direct Agent creation failure', async () => {
    ...
    expect(err).toBe('flowforge: factory exploded\n')
