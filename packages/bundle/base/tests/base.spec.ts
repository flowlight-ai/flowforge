/**
 * The bundle's substance is its patch file: the `flowforge.bundle.patch` manifest
...
import { evaluate } from '@flowforge/cordis-plugin-loader'

describe('ff-base bundle', () => {
  it('declares a parseable patch list through the flowforge.bundle.patch manifest field', () => {
    ...
    expect(rows.find(row => row.id === 'session-telemetry-otel')?.config?.['mode']).toEqual({
      __jsExpr: "process.env.FF_TELEMETRY_MODE || 'DISABLED'",
    })
    expect(rows.filter(row => row.id === 'subagent-codex')).toHaveLength(0)
    expect(rows.filter(row => row.id === 'subagent-claude-code')).toHaveLength(0)
    expect(manifest.dependencies).not.toHaveProperty('@flowforge/subagent-codex')
    expect(manifest.dependencies).not.toHaveProperty('@flowforge/subagent-claude-code')
  })
