 ...
 * process-local registry lives in `@flowforge/jobs-local`.
 * @module @flowforge/jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@flowforge/jobs is the abstract job registry seam; load an implementation such as @flowforge/jobs-local instead')
    }
    super(ctx, 'jobs')
  }
