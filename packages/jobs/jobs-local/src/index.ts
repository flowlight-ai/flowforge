 ...
 * @module @flowforge/jobs-local
 */

import { Context } from '@flowforge/cordis'
import z from '@flowforge/schemastery'
import type { Agent } from '@flowforge/agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@flowforge/scope'
import type { ScopeLayer } from '@flowforge/scope'
import { deadline, timeoutOf } from '@flowforge/timeout'
import { JobRegistry, JobId } from '@flowforge/jobs'
import type {
 ...
 * `@flowforge/jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @flowforge/tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @flowforge/agent)')
    }
