...
import { JobId } from '@flowforge/jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@flowforge/jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @flowforge/tool-jobs in its composition)')
  })
