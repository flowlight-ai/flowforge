...
import { scrubbedParentEnv, SubprocessRuntime } from '@flowforge/subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and FF_ names (case-insensitively) but keeps PATH', () => {
    process.env.FF_SCRUB_PROBE = 'stale'
    process.env.ff_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.FF_SCRUB_PROBE).toBeUndefined()
      expect(env.ff_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.FF_SCRUB_PROBE
      delete process.env.ff_scrub_probe_lower
