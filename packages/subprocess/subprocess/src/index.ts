 ...
 * `@flowforge/subprocess-local`.
 * @module @flowforge/subprocess
 */

import { Context, Service } from '@flowforge/cordis'
import { FF_ENV_PREFIX } from './types.ts'
import type { SubprocessHandle, SubprocessSpawnSpec } from './types.ts'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from './types.ts'

export { FF_ENV_PREFIX } from './types.ts'
export type {
  CollectedOutput,
  FlowforgeEnvironment,
  FlowforgeEnvironmentKey,
  SubprocessCollect,
 ...
 * `FF_*` names — the canonical base every harness child starts from. `PATH`,
 ...
 * credential or current `FF_*` fact goes through the spec's explicit `env`,
 * which merges after this scrub). Both scrubs match case-insensitively:
 * Windows environment names are case-insensitive, so a parent `ff_*` entry
 * would otherwise survive and read back as `$env:FF_*` in the child;
 * deliberate lowercase `ff_*` names on POSIX are implausible. Exported as a plain function so spawners
...
export function scrubbedParentEnv(): Record<string, string> {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (value !== undefined && !SENSITIVE_ENV_PATTERN.test(key) && !key.toUpperCase().startsWith(FF_ENV_PREFIX)) env[key] = value
  }
  return env
}
