 ...
 * @module flowforge-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for FlowForge-managed child environment facts. */
export const FF_ENV_PREFIX = 'FF_' as const

/** One environment key inside the managed {@link FF_ENV_PREFIX} namespace. */
export type FlowforgeEnvironmentKey = `${typeof FF_ENV_PREFIX}${string}`

/** Trusted FlowForge variables for one child-process execution. */
export type FlowforgeEnvironment = Readonly<Record<FlowforgeEnvironmentKey, string>>
 ...
 * not a hidden subprocess-service default — decides them (the `flowforge-shell`
...
export interface SubprocessSpawnSpec {
   ...
   * entry or current `FF_*` fact survives the scrub; `undefined` is a
 ...
 * to `TerminalSignal` in `@flowforge/terminal` without a cross-seam dependency;
