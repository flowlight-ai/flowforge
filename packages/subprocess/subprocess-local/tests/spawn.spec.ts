...
import type { SubprocessHandle, SubprocessOutputReader } from '@flowforge/subprocess'
...
const spillDir = mkdtempSync(join(tmpdir(), 'flowforge-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-flowforge-test' })).done)
...
describe('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@flowforge/cordis')
    const { default: LocalSubprocessRuntime } = await import('@flowforge/subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-flowforge-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-flowforge-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient DSH env vars from child processes', async () => {
    process.env.FF_TEST_API_KEY = 'super-secret'
    process.env.FF_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.FF_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${FF_TEST_API_KEY:-absent}|${FF_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${FF_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.FF_TEST_API_KEY
      delete process.env.FF_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.FF_TEST_PLAIN
    }
  })

  it('forwards explicit FF_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient FF_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.FF_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${FF_STALE:-absent}|$FF_SHELL|$FF_SESSION_ID]"', {
        env: { FF_SHELL: '1', FF_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.FF_STALE
    }
  })
  ...
  it('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/flowforge-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/flowforge-subprocess-/)
