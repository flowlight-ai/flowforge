"""
FlowForge Solo UI E2E 测试 (v10.1)
对应 test.md 第十七章：9个Solo UI意图类型测试 + 负向测试
...
class E2EMetricsCollector:
    ...
    def _process_event(self, data: dict):
        ...
        # LLM事件
        if "llm.start" in event_type or event_type == "solo.llm.start":
        ...
        elif "llm.end" in event_type or event_type == "solo.llm.end":
            if self.llm_calls and "end" not in self.llm_calls[-1]:
                self.llm_calls[-1]["end"] = now
                self.llm_calls[-1]["tokens"] = payload.get("usage", {})

        # Tool事件
        elif "tool.start" in event_type or event_type == "solo.tool.start":
            self._tool_start_times[seq] = now
            self.tool_calls.append({
                "start": now,
                "tool": payload.get("tool_name"),
            })
        elif "tool.end" in event_type or event_type == "solo.tool.end":
...
class SoloUITestBase:
    """Solo UI 测试基类 — 通过HTTP API模拟Solo对话 + WebSocket事件采集"""
    ...
    def send_solo_message(self, message: str, persona: str = "default") -> tuple:
        """发送Solo消息，返回(task_data, collector)"""
        ...
        with httpx.Client(timeout=180.0) as client:
            ...
            assert resp.status_code in [200, 201], \
                f"创建Solo任务失败: {resp.status_code} {resp.text[:500]}"
            data = resp.json()["data"]
    ...
    def wait_for_completion(self, task_id: str, timeout: int = 120) -> dict:
        ...
        while time.time() - start < timeout:
            try:
                with httpx.Client(timeout=30.0) as client:
                    ...
                    if resp.status_code == 200:
                        ...
                        if status == "completed":
                            ...
                            pytest.fail(
                                f"Solo任务 {task_id} 终止于非成功状态 '{status}'，"
                ...
                continue
            time.sleep(2)
        pytest.fail(f"Solo任务 {task_id} 超时({timeout}s)")
...
class TestSimpleGreeting(SoloUITestBase):
    """IT-SOLO-01: 简单问候（Fast-path）— LLM只调用1次"""
    ...
    def test_simple_greeting_fastpath(self):
        ...
        4. 响应内容非空且有意义
        """
        result, collector = self.send_solo_message("你好", persona="e2e_greet")
        task_id = result["task_id"]

        final = self.wait_for_completion(task_id, timeout=60)
        report = self.finalize_collector(collector, "IT-SOLO-01")
...
class TestWritingIntent(SoloUITestBase):
    """IT-SOLO-02: 写作意图（Planning路径）"""
    ...
    def test_writing_intent_planning(self):
        ...
        report = self.finalize_collector(collector, "IT-SOLO-02")
...
class TestSearchIntent(SoloUITestBase):
    """IT-SOLO-03: 搜索意图（Planning路径）"""
    ...
    def test_search_intent(self):
        ...
        final = self.wait_for_completion(task_id, timeout=600)
        report = self.finalize_collector(collector, "IT-SOLO-03")
...
class TestResearchIntent(SoloUITestBase):
    """IT-SOLO-04: 研究意图（Planning路径）"""
    ...
    def test_research_intent(self):
        ...
        final = self.wait_for_completion(task_id, timeout=600)
        report = self.finalize_collector(collector, "IT-SOLO-04")
        assert final.get("status") == "completed", \
...
class TestTranslateIntent(SoloUITestBase):
    """IT-SOLO-05: 翻译意图（Planning路径，非Fast-path）"""
    ...
    def test_translate_intent(self):
        ...
        final = self.wait_for_completion(task_id, timeout=600)
        report = self.finalize_collector(collector, "IT-SOLO-05")
...
class TestCodeIntent(SoloUITestBase):
    """IT-SOLO-06: 代码意图（Planning路径）"""
    ...
    def test_code_intent(self):
        ...
        final = self.wait_for_completion(task_id, timeout=600)
        report = self.finalize_collector(collector, "IT-SOLO-06")
...
class TestPlanDegradation(SoloUITestBase):
    """IT-SOLO-07: Plan降级场景"""
    ...
    def test_plan_degradation(self):
        ...
        3. 输出内容非空
        """
        result, collector = self.send_solo_message("帮我分析一下这个数据", persona="e2e_plan")
        task_id = result["task_id"]

        final = self.wait_for_completion(task_id, timeout=900)
        report = self.finalize_collector(collector, "IT-SOLO-07")
...
class TestComplexMultiStep(SoloUITestBase):
    """IT-SOLO-08: 复杂多步意图"""
    ...
    def test_complex_multi_step(self):
        ...
        final = self.wait_for_completion(task_id, timeout=900)
        report = self.finalize_collector(collector, "IT-SOLO-08")
...
class TestFastpathNegative(SoloUITestBase):
    """IT-SOLO-09: Fast-path负面测试"""
    ...
    def test_complex_input_not_fastpath(self):
        ...
        final = self.wait_for_completion(task_id, timeout=600)
        report = self.finalize_collector(collector, "IT-SOLO-09")
...
# ---------------------------------------------------------------------------
# IT-SOLO-NEG: Solo UI负向测试
# ---------------------------------------------------------------------------

class TestSoloNegative(SoloUITestBase):
    """IT-SOLO-NEG: Solo UI负向/异常路径测试"""

    def test_special_characters_only(self):
        """IT-SOLO-NEG-01: 仅特殊字符输入不应崩溃
        ...
        3. 输出内容非空
        """
        result, collector = self.send_solo_message("!@#$%^&*()", persona="e2e_neg")
        task_id = result["task_id"]
        final = self.wait_for_completion(task_id, timeout=120)
        report = self.finalize_collector(collector, "IT-SOLO-NEG-01")
